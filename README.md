...
Everything runs on infrastructure you control. No telemetry leaves your network.
...
enterprise SSO (SAML / OIDC / LDAP) · mobile device management · compliance
automation and scorecards (NIST CSF, ISO 27001, SOC 2) · CSPM and cloud runtime
security · XDR and SIEM connectors · AI-assisted triage and investigation ·
automatic SigmaHQ community rule synchronisation · managed update delivery ·
commercial support
...
- **すべて自分の環境で動く。** テレメトリが外部に送信されることはありません
- **Linux は eBPF、Windows は ETW、macOS は ESF** によるカーネル/OS ネイティブな収集
...
サポートされているのは検知と対応の経路です。

## 含まれないもの

企業向け SSO（SAML/OIDC/LDAP）、MDM、コンプライアンス自動評価、CSPM・クラウドランタイム、
XDR・SIEM 連携、AI 支援トリアージ、SigmaHQ ルールの自動同期、アップデート配信、商用サポート。
これらは別途提供されます。
...
プルリクエストの前に [`CLA.md`](CLA.md) をご確認ください。**著作権は譲渡されません**
（あなたのものです）が、オープンソース版と商用版の二本立てを可能にするための
ライセンス付与に同意していただく必要があります。

...
CASES = [
    ...
    (W, '!names[pkg+"|"+fn.Name.Name]', '!names[pkg+"/"+fn.Name.Name]',
     ...
     '0 件を検査して緑になります**）'),
    (W, 'const minMatchedWorkerDecls = 80', 'const minMatchedWorkerDecls = 0',
     '鍵が合っているかの床を 0 に落とす'),
    (W, 'const trackedCallDepth = 3', 'const trackedCallDepth = 1',
     '呼ばれる先をたどらなくなる（**`upsertCase` のように、渡した関数から'
     '呼ばれる先が漏れます**）'),
    (W, 'const minTrackedWorkerNames = 80', 'const minTrackedWorkerNames = 0',
     '到達した関数の床を 0 に落とす'),

    (T, 'backgroundFailedCount = 83', 'backgroundFailedCount = 500',
     '箇所の数を留めなくなる'),
    (T, 'backgroundFailedFuncs = 28', 'backgroundFailedFuncs = 500',
     '宛先の数を留めなくなる'),
    (T, '\treturn m[key] == ""\n', '\treturn false\n',

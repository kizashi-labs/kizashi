...
CASES = [
    ...
    # ── 理由つきで外した件数 ─────────────────────────────────────────────
    (A, '\tconst returnExceptionCount = 41', '\tconst returnExceptionCount = 100',
     '理由つきで外した件数を実測から引き離す'),
...
# ここに並ぶ名前は、すべて木にあるものです。
#
# 公開版（#92）はここから 8 本を削っていました。**あちらの木に無いから**で、
# 判断としては正しい。ですが本流にはあります —— 8 本は
# `api/handlers/table_probe_test.go` にあり、あのファイルは公開版で
# 落とされているだけです。**削った側を本流に持ち帰ると、本流で 8 本が
# 黙って走らなくなります**（公開版が直そうとしたのと同じ形を、こちら側に
# 作ることになる）。
#
# なので本流は和集合を持ちます。**この木に無い名前を落とすのは生成器の
# 仕事**で、`scripts/public-snapshot/trim-mutation-runs.py` が行います。
RUN = ('TestTableProbeFailureIsNotAbsence|TestTableProbeTimeoutIsNotAbsence|'
       ...
       'TestNoNewHandlerAsksInformationSchemaOnItsOwn|TestAProbeFailureIsNotAnAbsence')

...
CASES = [
    ...
    (SK, '\t// `cloud/poller.go:poll` は直しました (2026-08-12) —— 記録だけでなく\n',
     ...
     '生えたときに黙って通ります**）'),
    (AW, '\tconst returnExceptionCount = 41', '\tconst returnExceptionCount = 43',
     '理由つきで外した件数を実測から引き離す'),

    # ── 走査そのもの ─────────────────────────────────────────────────────
    (W, 'const reachableSlogErrorSites = 1', 'const reachableSlogErrorSites = 100',
     '件数を留めなくなる'),
    (W, 'const reachableSlogWarnSites = 10', 'const reachableSlogWarnSites = 100',
     '`Warn` の件数を留めなくなる'),
    (W, 'const silentErrorBranchSites = 19', 'const silentErrorBranchSites = 100',

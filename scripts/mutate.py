...
# ── `-run` の名前を、この木に合わせる ────────────────────────────────────────
NOT_SHIPPED_TXT = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                               'mutations', 'NOT_SHIPPED.txt')


def not_shipped_runs() -> set[str]:
    """`NOT_SHIPPED.txt` の `run:` 行。この配置に同梱していないテスト名。"""
    try:
        with open(NOT_SHIPPED_TXT, encoding='utf-8') as f:
            lines = f.read().splitlines()
    except FileNotFoundError:
        return set()
    return {ln.strip()[len('run:'):] for ln in lines
            if ln.strip().startswith('run:')}


def _run_names(cmd: list[str]) -> list[str]:
    """`-run` に並んでいるテスト名。無ければ空。"""
    if '-run' not in cmd:
        return []
    i = cmd.index('-run') + 1
    return cmd[i].split('|') if i < len(cmd) else []


def _drop_absent_runs(cmd: list[str], root: str) -> tuple[list[str], list[str]]:
    """`-run` の値から、同梱していないテスト名を落とす。

    落とすのは `NOT_SHIPPED.txt` の `run:` に載っているものだけ。**推測しない。**
    """
    absent = not_shipped_runs()
    if not absent or '-run' not in cmd:
        return cmd, []
    out = list(cmd)
    i = out.index('-run') + 1
    if i >= len(out):
        return cmd, []
    names = out[i].split('|')
    keep = [n for n in names if n not in absent]
    dropped = [n for n in names if n in absent]
    if not dropped:
        return cmd, []
    # **全部落ちたら、それは「走らせない」と同じ。** 呼び出し側が気づけるよう
    # `-run` ごと外さず、当たらない名前を 1 つ残す —— go test が 0 件で緑を
    # 返し、SURVIVED として出る。黙って通るより落ちるほうがよい。
    out[i] = '|'.join(keep) if keep else names[0]
    return out, dropped
...
class Harness:
    def __init__(self, root: str, cmd: list[str], cwd: str = '.',
        ...
        # この配置に無いテスト名を `-run` から落とす。**落とさないと、
        # `go test -run` は当たらない名前を黙って無視するので、書いた検査が
        # 1 本も走っていなくても緑になる。** 公開版 #92 がこれを見つけた。
        #
        # 落としてよい名前は `NOT_SHIPPED.txt` の `run:` 行に書いてあるものだけ。
        # **一覧に無いのに木にも無い名前は、名前が変わったか消えたかで、
        # 落とすのではなく人が読むべきもの** —— そちらは run_mutations.py の
        # `--check` が挙げる。ここは黙って落とさず、落とした名前を必ず出す。
        self.requested_runs = _run_names(cmd)
        self.cmd, self.dropped_runs = _drop_absent_runs(cmd, self.root)

...
# ── 5-b. 渡す形の道具 ─────────────────────────────────────────────
# **`timeout-minutes` 49 件の「値」が書いてあるのは、この表だけです。**
# 上の検査は「無い」ことは言えますが、「**いくつだったか**」は言えません。
# 表と道具が一緒に消えると、次の同期で消されたとき**戻す先が分かりません。**
#
# 表は手で書いた散文ではなく、`handover_timeouts.py --apply` の出力です。
# 道具が消えると、表は書いた瞬間だけ正しい散文に戻ります。
HANDOVER = [
...
def main() -> int:
    ...
    sections = [
        ...
        ('渡す形の道具', files_exist(
            tree, HANDOVER,
            '**49 件の「値」が書いてあるのはこの表だけです。** 他の検査は'
            '「無い」とは言えますが、「いくつだったか」は言えません —— '
            '一緒に消えると、戻す先が分からなくなります')),

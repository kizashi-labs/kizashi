...
# 足し引きしたら、ここも合わせて直すこと。** 片方だけ変わると「ローカルで緑なのに CI で落ちる」、あるいは
...
# 読めます。** 領域判定に任せると、workflows/ を触っていない PR では
# 走らないことになりますが、この検査は速いので毎回でよいものです。
section "workflows"
...
for path in sorted(glob.glob(".github/workflows/*.yml") +
    ...
    for job_name, job in jobs.items():
        ...
        if "timeout-minutes" not in job:
            ...
            problems.append("%s / %s: timeout-minutes が %d 分です"

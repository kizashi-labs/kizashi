...
#
# ── 追従の義務 ──────────────────────────────────────────────────
# ここは .github/workflows/ の ci.yml / coverage.yml / merge-gate.yml を
# 1:1 で写したもの。**あちらのジョブやステップを足し引きしたら、ここも
# 合わせて直すこと。** 片方だけ変わると「ローカルで緑なのに CI で落ちる」、
# あるいはもっと悪い「ローカルで緑だが、CI にしか無い検査を通していない」
# 状態になる。ci.yml 側の changes ジョブにも同じ注意書きを置いてある。
#

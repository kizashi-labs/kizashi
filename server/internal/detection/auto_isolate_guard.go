...
const (
	// defaultIsolationCooldown は同じ端末を再び自動隔離するまでの最短間隔。
	// 周期的な誤検知は同じ端末で繰り返し鳴る。1 度目は止められないが、
	// 2 度目以降を抑えれば「解除しても数分で再隔離される」状態は避けられる。
	defaultIsolationCooldown = 30 * time.Minute

	// defaultIsolationBudget は 1 時間あたりに自動隔離を許す台数。
	// 誤検知が広範囲に及んだとき、業務停止の規模をここで頭打ちにする。
	// 本物の攻撃が同時に多数の端末へ及ぶ場合は上限に当たるが、そのときは
	// 人が介入すべき事象なので、黙って全台止めるより通知して止まるほうがよい。
	defaultIsolationBudget = 3

	isolationBudgetWindow = time.Hour
)

// isolationGuard decides whether an auto-isolation may proceed.
...
func newIsolationGuard(cooldown time.Duration, budget int, dryRun bool) *isolationGuard {
	if cooldown <= 0 {
		cooldown = defaultIsolationCooldown
	}
	if budget <= 0 {
		budget = defaultIsolationBudget
	}
	return &isolationGuard{
		cooldown:    cooldown,
		budget:      budget,
		window:      isolationBudgetWindow,
		dryRun:      dryRun,
...
// isolationVerdict is why an auto-isolation was allowed or refused.
type isolationVerdict struct {
	allow  bool
	reason string
}

// allow reports whether agentID may be auto-isolated now, and records the
// decision when it is allowed.
//
// 呼び出し側は allow が false でも黙ってはいけない。抑止したこと自体が
// 「誤検知が続いている」か「本物の広範囲攻撃が起きている」かの手がかりになる。
func (g *isolationGuard) allow(agentID string) isolationVerdict {
	g.mu.Lock()
	defer g.mu.Unlock()
	...
	if last, ok := g.lastByAgent[agentID]; ok {
		if elapsed := now.Sub(last); elapsed < g.cooldown {
			return isolationVerdict{false, "同じ端末を " +
	...
	if len(g.recent) >= g.budget {
		return isolationVerdict{false, "直近 " + g.window.String() +
			" の自動隔離が上限に達しました（サーキットブレーカー作動）"}
	}

	g.lastByAgent[agentID] = now
	g.recent = append(g.recent, now)
	return isolationVerdict{true, ""}
}

// isDryRun reports whether isolation should be logged instead of executed.
func (g *isolationGuard) isDryRun() bool { return g.dryRun }

// logRefusal records a refused isolation. 抑止は沈黙させない。
func (g *isolationGuard) logRefusal(agentID, ruleLabel, reason string) {
	slog.Warn("自動隔離を抑止しました",
		"agent", agentID, "rule", ruleLabel, "理由", reason)
}

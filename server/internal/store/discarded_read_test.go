...
func TestNoDiscardedRowReadOutsideTheTwoCoveredPackages(t *testing.T) {
	...
	if n := len(sites); n != discardedReadSites {
		if n < discardedReadSites {
			t.Errorf("捨てている1行読みが %d か所です（留めているのは %d）。"+
				"discardedReadSites を %d に下げてください",
				n, discardedReadSites, n)
		} else {
			t.Errorf("捨てている1行読みが %d か所です（留めているのは %d）。"+
				"**増えたなら、その 0 が何を決めているかを読んでください**",
				n, discardedReadSites)
		}
	}

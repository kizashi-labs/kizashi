...
// ErrUpdateNotAvailable is returned by Approve when no row matched — either the
// id does not exist, or its status is not 'available'. Callers map this to 409;
// anything else from Approve is a real failure and must not be reported as a
// state mismatch.
var ErrUpdateNotAvailable = errors.New("update not in 'available' state or not found")
...
func (s *SystemUpdatesStore) Approve(ctx context.Context, id, approvedBy string) (SystemUpdate, error) {
	...
	u, err := scanSystemUpdate(row)
	if err != nil {
		// **「行が返らなかった」と「SQL が落ちた」を混ぜない。**
		//
		// 以前はどちらも "update not in 'available' state or not found" になって
		// いました。実際には approved_by (UUID REFERENCES users(id)) に UUID で
		// ない値が渡って 22P02 で落ちており、画面には「承認できる状態にない」と
		// **嘘の理由**が出ていました。承認が通らない理由を運用者が特定できません。
		if errors.Is(err, pgx.ErrNoRows) {
			return SystemUpdate{}, ErrUpdateNotAvailable
		}
		return SystemUpdate{}, fmt.Errorf("承認の書き込みに失敗しました: %w", err)
	}
	return u, nil
}

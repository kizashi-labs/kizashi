...
//
// ログインの env フォールバック（ADMIN_PASSWORD による管理者アカウント）が
// 「まだ誰も居ない初期セットアップ」に限られていることを確かめるために使います。
// SeedAdminUser がシードを行うかどうかの判定と同じ条件なので、両者が食い違わない
// ようにここに置いています。
func (s *UserStore) CountUsers(ctx context.Context) (int, error) {
	...
	if err := s.pool.QueryRow(ctx, "SELECT COUNT(*) FROM users").Scan(&count); err != nil {
		return 0, fmt.Errorf("ユーザー数の取得に失敗しました: %w", err)
	}
	return count, nil
}
...
// ResetAdminPassword rewrites admin@example.com's password hash from
// ADMIN_PASSWORD, and clears any lockout. Returns the number of rows updated.
//
// **既定では呼ばれません**（cmd/api が ADMIN_PASSWORD_RESET=true のときだけ呼ぶ）。
//
// SeedAdminUser は users が空のときしか動かないので、一度でも運用が始まった環境では
// `.env` の ADMIN_PASSWORD を変えても DB のハッシュは変わりません。以前はログインの
// env フォールバックがその食い違いを覆い隠していましたが、フォールバックを初期
// セットアップ限定に戻したので、**DB のパスワードを知らない運用者が締め出されます**。
// その復旧手段がこれです。
//
// 毎回無条件に上書きしないのは、それをやると `.env` が DB より強くなり、画面から
// 変更したパスワードが再起動のたびに巻き戻るためです。明示的に頼まれたときだけ動きます。
func ResetAdminPassword(ctx context.Context, pool *pgxpool.Pool, password string) (int64, error) {
	if password == "" {
		return 0, fmt.Errorf("ADMIN_PASSWORD が空です")
	}
	hash, err := bcrypt.GenerateFromPassword([]byte(password), bcrypt.DefaultCost)
	if err != nil {
		return 0, fmt.Errorf("パスワードのハッシュ化に失敗しました: %w", err)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("管理者パスワードの再設定に失敗しました: %w", err)
	}

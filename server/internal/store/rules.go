...
//
// actor は「誰がこの隔離を起こしたか」で、Gatekeeper が response_actions に
// 書くのと同じ値である（isolation.actorOf）。
//
// 以前はここが "ai_agent" を決め打ちしていた。全ての無人隔離はこの1箇所を通る
// ので、**経路にかかわらず agents.isolated_by が常に ai_agent** になっていた。
// 手動隔離も例外ではない——ハンドラが操作者のユーザーIDを書いた直後にこの
// 更新が走り、押した人を消していた。誤隔離の調査で最初に見る列がそれでは、
// 無実の AI トリアージが犯人に見え、本当の経路が視界から外れる。
func (s *CommandStore) IsolateEndpoint(ctx context.Context, agentID, reason, alertID, commandID, actor string) error {
	// Update database
	agentStore := &AgentStore{pool: s.pool}
	if err := agentStore.IsolateAgent(ctx, agentID, reason, actor); err != nil {
		return err
	}

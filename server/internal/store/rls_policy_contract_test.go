...
//
// **agents (451) と alerts (453) はもう抜け道を持ちません。** 残る 2 表
// (incidents / users) には、まだこの形が残っています:
//
...
// 4 表を一度に落とすと切り分けられません。agents を先にしたのは検知
// パイプラインが最も強く依存する表だからで、alerts を 2 番目にしたのは
// **系が絶えず書き込む表**だからです —— 読みが通ることを agents で
// 確かめたあと、書きがいちばん重いところで確かめます。
//
...
//
// **agents (451) と alerts (453) は落としました。** 残る 2 表を落とすときも、
// 同じ手順を踏んでください:
//
...
//     後始末が抜け道を復活させます**
//
// 落ちたのは agents で台帳 2 件、alerts で 1 件でした。
//
...
var permissiveWhenUnset = map[string]string{
	// agents (451) と alerts (453) は **もう抜け道を持ちません**。
	// 取り込み・検知エンジン・アラートパイプラインは
	// `store.WithSystemAccess` で名乗るようになったので、テナント無しで
	// 繋ぐ経路はありません。**4 表のうち 2 表が済みました。**

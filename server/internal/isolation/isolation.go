...
type CommandSender interface {
	IsolateEndpoint(ctx context.Context, agentID, reason, alertID, commandID, actor string) error
...
func (g *Gatekeeper) Isolate(ctx context.Context, req Request) (Result, error) {
	...
	if err := g.commands.IsolateEndpoint(ctx, req.AgentID, req.Reason, req.AlertID, actionID, actorOf(req)); err != nil {
...
// actorOf は「誰がこの隔離を起こしたか」を一つに決める。
//
// response_actions.executed_by と agents.isolated_by は同じ値でなければならない。
// 別々に決めていた結果、CommandStore が agents.isolated_by へ "ai_agent" を
// 決め打ちで書き、**経路にかかわらず全ての隔離が AI トリアージの仕業に見えて
// いた**。手動隔離も例外ではなく、ハンドラが操作者のユーザーIDを書いた直後に
// 上書きされ、押した人が記録から消えていた。
//
// 誤隔離を追うとき最初に見るのがこの列なので、そこが常に同じ値を指していると、
// 無実の経路が犯人に見え、本当の経路（ルール・プレイブック・自動修復・API）は
// 視界に入らない。
func actorOf(req Request) string {
	if req.TriggeredBy != "" {
		return req.TriggeredBy
	}
	return string(req.Origin)
}
...
func (g *Gatekeeper) record(ctx context.Context, req Request, actionType, status string, extra map[string]string) string {
	...
	id, err := g.actions.Record(ctx, req.AgentID, actionType, status, actorOf(req), details)
	if err != nil {

...
func TestScheduler_Workers(t *testing.T) {
	...
	// per-row processing loops rather than returning early on empty tables. All
	...
	if agentID != "" {
		...
		t.Cleanup(func() { _, _ = pool.Exec(ctx, "DELETE FROM network_connections WHERE agent_id=$1", agentID) })
	}
	_, _ = pool.Exec(ctx,
	...
	t.Cleanup(func() { _, _ = pool.Exec(ctx, "DELETE FROM ioc_entries WHERE value='198.51.100.7'") })

	// pool-only workers
	NewAlertAggregator(pool).aggregate(ctx)
	...
	NewThreatHuntAutomator(pool, nc).huntFromIOCs(ctx)
	...
	NewIOCMatcher(pool, nc).match(ctx)
	...
	NewReportGenerator(pool, t.TempDir()).generate(ctx)
}

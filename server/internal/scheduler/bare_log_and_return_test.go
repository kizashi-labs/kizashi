...
const bareLogAndReturnSiteCount = 20
...
func TestEveryBareLogAndReturnHasAReason(t *testing.T) {
	...
	if n := len(sites); n != bareLogAndReturnSiteCount {
		// 減った側だけ定数名を出す（理由は answered_with_a_value_test.go の
		// ceilingComplaint）。増えた側に出すと自動昇格の入口になる。
		if n < bareLogAndReturnSiteCount {
			t.Errorf("ログを書いて戻るだけの箇所が %d か所です（留めているのは %d）。"+
				"bareLogAndReturnSiteCount を %d に下げてください —— "+
				"理由の鍵は関数までなので、同じ関数に増えた分は鍵の検査では見えません",
				n, bareLogAndReturnSiteCount, n)
		} else {
			t.Errorf("ログを書いて戻るだけの箇所が %d か所です（留めているのは %d）。"+
				"**増えたなら、それが誰かに届かなくてよいかを読んでください** —— "+
				"理由の鍵は関数までなので、同じ関数に増えた分は鍵の検査では見えません",
				n, bareLogAndReturnSiteCount)
		}
	}

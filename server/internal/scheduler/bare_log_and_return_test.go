...
var bareLogAndReturnReasons = map[string]string{
	// ── 運用者が明示的に止めた ─────────────────────────────────
	// どちらも外向き通信のキルスイッチ。止まっているのは失敗ではなく
	// 設定どおりなので、fail() で上げると「止めたのに毎周回エラーが出る」
	// ことになる。起動時に一度だけ出て、以後は静かなのが正しい。
	"feed_scheduler.go:Run":       "THREAT_FEED_SYNC_ENABLED=false。運用者が止めた状態で、失敗ではない。",
	"threat_feed_importer.go:Run": "THREAT_FEED_SYNC_ENABLED=false。運用者が止めた状態で、失敗ではない。",

...
const discardedReadSiteCount = 9
...
func TestEveryDiscardedRowReadHasAReason(t *testing.T) {
	...
	if n := len(sites); n != discardedReadSiteCount {
		if n < discardedReadSiteCount {
			t.Errorf("捨てている1行読みが %d か所です（留めているのは %d）。"+
				"discardedReadSiteCount を %d に下げてください —— 理由の鍵は"+
				"関数までなので、同じ関数に増えた分は鍵の検査では見えません",
				n, discardedReadSiteCount, n)
		} else {
			t.Errorf("捨てている1行読みが %d か所です（留めているのは %d）。"+
				"**増えたなら、その 0 が何を決めているかを読んでください** —— "+
				"理由の鍵は関数までなので、同じ関数に増えた分は鍵の検査では"+
				"見えません", n, discardedReadSiteCount)
		}
	}

...
//
// Migration 443 declares the table, so a failure here now means something else
// — no permission, no database — and that is worth saying out loud.
//
// The CREATE TABLE is gone (2026-08-17). Under the least-privilege runtime role
// it could only ever fail: `CREATE TABLE IF NOT EXISTS` checks the schema's
// CREATE privilege before it checks whether the table exists. The seeding
// INSERT stays, so a database that was unreachable at boot still recovers on a
// later tick.
...
func TestPreparingTheStateReportsItsFailure(t *testing.T) {
	...
	// 何をしようとして落ちたかが分かること。理由の無いエラーは、
	// 「DB が落ちている」と「権限が無い」を同じ形にします。
	if !strings.Contains(err.Error(), "状態行") {
		t.Errorf("落ちるのは状態行の seeding なのに、そう報告されていません: %v", err)
	}
}

// ensureState は DDL を発行しないこと。
//
// ここには `CREATE TABLE IF NOT EXISTS retro_rule_state` がありました。
// **最小権限ロール edr_app では、テーブルが既にあっても必ず失敗します** ——
// PostgreSQL は存在チェックより先に schema の CREATE 権限を見るためです。
// 失敗すれば ensureState が error を返し、hunt() は毎 tick 見送ります。
// 表側の症状は「遡及ハントが動かない」だけで、理由はどこにも出ません。
func TestPreparingTheStateIssuesNoDDL(t *testing.T) {
	_, src := parseFn(t, "retro_rule_hunter.go", "ensureState")
	if strings.Contains(strings.ToUpper(src), "CREATE TABLE") {
		t.Error("ensureState が CREATE TABLE を発行しています。テーブルは " +
			"migration 443 が宣言します —— ここでの CREATE は edr_app では" +
			"必ず permission denied になり、遡及ハントが黙って止まります")
	}
	if !strings.Contains(src, "状態行を用意できませんでした") {
		t.Error("seeding の失敗が報告されません。捨てると hunt() の SELECT が" +
			"行を見つけられず、「状態が無い」として毎回黙って見送られます")
	}
}

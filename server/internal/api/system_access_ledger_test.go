...
var dbFreeRoutes = map[string]string{
	...
	"/ws/cloud":             "同上",
	// **公開版には無い経路です。** 有償機能（課金）なので、あちらの木では
	// この行が要りませんでした。本流に取り込むにあたって読み、
	// `HandleBillingEvents` が billingClients に登録して押し出すだけで
	// DB を触らないことを確かめています。
	"/ws/billing":                           "notification.Hub の SSE。**DB に触りません**",
	"/health":                               "固定の応答",
	"/healthz":                              "固定の応答",
	"/readyz":                               "固定の応答",
...
// 値は「引数名 → 呼び出し側のグループ名」。registerNoncoreRoutes は api（公開・
// テナント無し）と protected（認証+テナント済み）の両方を受けるので、引数ごとに
// 素性が違う。
var routeHelpers = map[string]map[string]string{
	"registerPlatformUpgradeRoutes": {"protected": "protected"},
	"darkwebRoutes":                 {"rg": "protected"},
	"registerNoncoreRoutes":         {"api": "api", "protected": "protected"},
}
...
func scanRoutes(t *testing.T) []routeFact {
	t.Helper()
	// 経路登録は router.go と routes_noncore.go（Free 版が同梱しない側の継ぎ目）
	// の 2 ファイルに分かれている。片方だけ読むと、移した経路が全部
	// 「張ってない」に見える。
	var decls []ast.Decl
	for _, fname := range []string{"router.go", "routes_noncore.go"} {
		fset := token.NewFileSet()
		file, err := parser.ParseFile(fset, fname, nil, parser.ParseComments)
		if err != nil {
			t.Fatalf("%s を読めません: %v", fname, err)
		}
	...
	for _, decl := range decls {
		...
		if roots, viaHelper := routeHelpers[fn.Name.Name]; viaHelper {
			// 引数で受けたグループを、呼び出し側の名前に読み替えます。
			for _, p := range fn.Type.Params.List {
				for _, n := range p.Names {
					if root, ok := roots[n.Name]; ok {
						parent[n.Name] = root
						if root == "protected" {
...
// funcBody は router.go / routes_noncore.go の関数 1 本を、そのままの文字列で返します。
func funcBody(t *testing.T, name string) string {
	...
	t.Fatalf("router.go / routes_noncore.go に %s がありません。**この検査は何も見ていません**", name)
	return ""
}

...
var backgroundFailedSites = map[string]string{
	...
	// ── 要求ごと（利用者が待っていて、応答でも分かります） ───────────
	// #865 の監査ヘルパ 3 本: 要求処理の中から fire-and-forget で監査行を書く。
	// 書けなくても要求は成立し、次の要求がまた書く。
	"api/handlers/auth_handler.go:auditLoginFailed": catPerReq,
	"api/handlers/users_handler.go:auditRoleChange": catPerReq,
...
// → **抑制 UI の撤去で 75 / 60** (2026-08-18)。`internal/suppression` を
// 畳んだので `incrementHit` の報告が1つ減りました。**この表は消した分も
// 消さないと落ちます** —— 残しておくと「まだそこにある」と読めます。
...
const (
	backgroundFailedCount = 34
	backgroundFailedFuncs = 28
)

func TestEveryBackgroundFailedSiteIsClassified(t *testing.T) {
	sites := backgroundFailedCallSites(t)

	if n := len(sites); n != backgroundFailedCount {
		// 減った側は**定数名と新しい値を文言に入れる**。`recalibrate_ratchets.py`
		// はこの形（`<定数名> を <数> に下げてください`）しか自動で下げられず、
		// 名前が無いと「手で下げてください」に落ちる。公開版スナップショットは
		// 有償機能のファイルを落とすぶん必ず減る側に振れるので、名前が無いだけで
		// **生成のたびに人手が要る**（実測: 75 → 68）。
		if n < backgroundFailedCount {
			t.Errorf("`metrics.BackgroundFailed` が %d か所です（留めているのは %d）。"+
				"減った分は留め直してください: backgroundFailedCount を %d に下げてください",
				n, backgroundFailedCount, n)
		} else {
			t.Errorf("`metrics.BackgroundFailed` が %d か所です（留めているのは %d）。"+
				"**増えたなら分類してください**", n, backgroundFailedCount)
		}
	}
	...
	if n := len(seen); n != backgroundFailedFuncs {
		if n < backgroundFailedFuncs {
			t.Errorf("宛先が %d 個です（留めているのは %d）。"+
				"backgroundFailedFuncs を %d に下げてください",
				n, backgroundFailedFuncs, n)
		} else {
			t.Errorf("宛先が %d 個です（留めているのは %d）", n, backgroundFailedFuncs)
		}
	}

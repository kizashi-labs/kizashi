-- 456: 抑制ルールの入れ物を1つに寄せる（移送）。
--
-- **本流では 450 でした。受け側で 456 に振り直しています。**
-- 450 は受け側の `450_rls_system_tenant_claim.sql`（RLS の名乗り）が
-- 先に使っており、**そちらは既に適用済み**です。適用済みの側を改名すると
-- `schema_migrations` がファイル名で記録しているため再実行されるので、
-- **まだ一度も当たっていないこちらを動かしました。**
--
-- この表は `suppression_rules` しか触らず、450〜455（RLS の 4 表）とは
-- 依存がありません。順序が後ろになっても影響しません。
--
-- **次の同期でまた 450 で来ます。** 同じ判断をもう一度してください
-- （`TestMigrationNumbersAreUnique` が教えます）。
--
-- ── 何があったか ──────────────────────────────────────────────────────────
--

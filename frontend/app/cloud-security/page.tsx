...
function StatusBadge({ status }: { status: FindingStatus }) {
  const cfg: Record<FindingStatus, { cls: string; label: string }> = {
    ...
    suppressed: { cls: 'bg-[#1e2d42] text-[#7d92b0] border-[#2a3f5f]',   label: '抑制済み' },
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1e2d42]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[#7d92b0] text-xs font-mono">{finding.resource_id}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-xs text-[#7d92b0]">{finding.resource_type}</span>
            <span className="text-xs text-[#7d92b0]">—</span>
            <span className="text-xs text-[#7d92b0]">{finding.region}</span>
          </div>
            ...
            <div className="space-y-2">
              {finding.remediation_steps.map((step, i) => (
                <div key={i} className="flex items-start gap-3">
                  <span className="shrink-0 w-6 h-6 rounded-full bg-[#e8002d]/20 border border-[#e8002d]/40 flex items-center justify-center text-[11px] font-bold text-[#e8002d]">
                    {i + 1}
                  </span>
                  <p className="text-[#e2e8f4] text-sm pt-0.5">{step}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative bg-[#070d19] rounded-lg border border-[#1e2d42] p-3">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Score */}
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 flex items-center gap-5">
        ...
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
          <p className="text-[#7d92b0] text-xs font-medium mb-3">検出件数</p>
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <div key={f.label} className={`${f.bg} rounded-lg p-2 text-center`}>
                ...
                <p className="text-[#7d92b0] text-[10px]">{f.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Compliance */}
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
          <p className="text-[#7d92b0] text-xs font-medium mb-3">コンプライアンス</p>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-[#1e2d42] flex items-center gap-3 flex-wrap">
          ...
          <select value={severityFilter} onChange={e => setSeverityFilter(e.target.value as Severity | 'all')}
            className="bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-xs text-white focus:outline-hidden focus:border-[#e8002d]/60">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map(m => (
                <tr key={m.id} className={`border-b border-[#1e2d42]/50 hover:bg-[#1e2d42]/20 transition-colors ${m.status === 'suppressed' ? 'opacity-60' : ''}`}>
                  <td className="py-3 px-4 text-xs text-[#7d92b0]">{m.resource_type}</td>
                  <td className="py-3 px-4 font-mono text-xs text-white max-w-[160px]">
                    <span className="truncate block">{m.resource_id}</span>
                  </td>
                  <td className="py-3 px-4 text-xs text-[#7d92b0] max-w-[220px]">
                    <span className="truncate block">{m.finding}</span>
                  </td>
                  <td className="py-3 px-4"><SeverityBadge severity={m.severity} /></td>
                  <td className="py-3 px-4 text-xs text-[#7d92b0] whitespace-nowrap">{m.region}</td>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
        ...
        <div className="space-y-2">
          {posture.top_risky_resources.map((r, i) => (
            <div key={r.resource_id} className="flex items-center gap-3 p-3 bg-[#070d19] rounded-lg border border-[#1e2d42]">
              <span className="text-[#3d5068] text-xs w-4">{i + 1}</span>
              <div className="flex-1 min-w-0">
                <p className="font-mono text-xs text-white truncate">{r.resource_id}</p>
                <p className="text-[#7d92b0] text-[10px]">{r.resource_type} — {r.region}</p>
              </div>
              <div className="flex items-center gap-2">
                <SeverityBadge severity={r.highest_severity} />
                <span className="text-xs text-[#7d92b0]">{r.finding_count}件</span>
              </div>
            </div>
          ))}
        </div>
...
function CrossProviderSummaryPanel({ summary, postureMap }: { summary: CrossProviderSummary; postureMap: Record<Provider, CloudPosture> }) {
  ...
  return (
    <div className="space-y-6">
      {/* Total resources */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 md:col-span-1">
          <p className="text-[#7d92b0] text-xs mb-1">総監視リソース数</p>
          <p className="text-3xl font-bold text-white">{(summary.total_resources ?? 0).toLocaleString()}</p>
          <div className="flex items-center gap-3 mt-3 text-xs text-[#7d92b0]">
        ...
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 md:col-span-2">
          <p className="text-[#7d92b0] text-xs font-medium mb-3">コンプライアンストレンド (過去30日)</p>
          ...
          <div className="flex items-center justify-between mt-1 text-xs text-[#3d5068]">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {summary.common_misconfigs.map(m => {
                ...
                return (
                  <tr key={m.type} className="border-b border-[#1e2d42]/50 hover:bg-[#1e2d42]/20 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex gap-1">
                        {m.providers.map(p => (
                          <span key={p} className={`px-1.5 py-0.5 rounded-sm text-[9px] font-bold ${
...
export default function CloudSecurityPage() {
  ...
  // 進捗バーは消した。サーバ側スキャンの進捗を返す経路は無く、以前の
  // バーはタイマーで動いていただけなので、走っていないスキャンでも
  // 100% まで進んで「完了」と表示できてしまう。結果だけを出す。
  ...
  const { data: awsData } = useQuery<CloudPosture | null>({
    ...
    queryFn: () => apiFetch<CloudPosture>('/api/v1/cloud/posture?provider=aws'),
    staleTime: 120_000,
  })

  const { data: azureData } = useQuery<CloudPosture | null>({
    queryKey: ['cloud-posture-azure'],
    queryFn: () => apiFetch<CloudPosture>('/api/v1/cloud/posture?provider=azure'),
    staleTime: 120_000,
  })

  const { data: gcpData } = useQuery<CloudPosture | null>({
    queryKey: ['cloud-posture-gcp'],
    queryFn: () => apiFetch<CloudPosture>('/api/v1/cloud/posture?provider=gcp'),
    staleTime: 120_000,
  })
  ...
  const handleScan = async () => {
    setScanning(true)
    setScanResult(null)
    const ok = await persist('クラウドスキャンの開始', '/api/v1/cloud/scan', { method: 'POST' })
    setScanResult(ok
      ? { ok: true, message: 'スキャンを開始しました' }
      : { ok: false, message: 'スキャンを開始できませんでした' })
    setScanning(false)
  }
    ...
    <div className="min-h-screen bg-[#070d19] p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-[#7d92b0] text-sm mt-1">
            ...
            <span className="ml-3 text-[#3d5068] text-xs">最終スキャン: {fmtTs(activePosture.last_scanned)}</span>
          </p>
        </div>
        <button onClick={handleScan} disabled={scanning}
          className="flex items-center gap-2 px-4 py-2 bg-[#e8002d] hover:bg-[#c0001f] disabled:opacity-60 text-white rounded-lg text-sm font-medium transition-colors">
          ...
          <button key={p.key} onClick={() => setActiveProvider(p.key)}
            className={`px-5 py-2.5 rounded-lg text-sm font-semibold border transition-colors ${
              activeProvider === p.key
                ...
                : 'border-[#1e2d42] bg-[#0d1220] text-[#7d92b0] hover:text-white hover:border-[#7d92b0]/40'
            }`}>
      ...
      <div className="border-t border-[#1e2d42] pt-6">
        <div className="flex items-center gap-2 mb-4">
          <Cloud className="w-5 h-5 text-[#7d92b0]" />

...
function StatusBadge({ status }: { status: ImplementationStatus }) {
  ...
  const Icon = cfg.icon
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold border ${cfg.bg} ${cfg.border} ${cfg.color}`}>
                  ...
                  {filtered.map(c => (
                    <tr key={c.id} className="hover:bg-[#070d19]/50 transition-colors">
                      <td className="px-4 py-3">
                        <span className="font-mono text-xs text-[#e8002d] bg-[#e8002d]/5 border border-[#e8002d]/20 px-1.5 py-0.5 rounded">{c.article_id}</span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-0.5 rounded border ${c.applicable === 'yes' ? 'bg-green-500/10 text-green-400 border-green-500/30' : c.applicable === 'partial' ? 'bg-amber-500/10 text-amber-400 border-amber-500/30' : 'bg-[#1e2d42] text-[#3d5068] border-[#1e2d42]'}`}>
                  ...
                  {upcoming.filter(c => c.framework === activeFramework).slice(0, 5).map(c => {
                    ...
                    return (
                      <div key={c.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-[#070d19] border border-[#1e2d42]">
                        <span className={`text-xs font-bold w-16 text-right flex-shrink-0 ${diff <= 7 ? 'text-red-400' : 'text-amber-400'}`}>{diff}日後</span>
                        ...
                        {gapFrameworks.map(fw => {
                          ...
                          return (
                            <td key={fw} className="px-4 py-3 text-center">
                              <span className={`inline-flex items-center justify-center w-6 h-6 rounded ${MATRIX_CELL[status]}`}>
                ...
                {(Object.keys(STATUS_CONFIG) as ImplementationStatus[]).map(s => {
                  ...
                  return (
                    <div key={s} className="flex items-center gap-1">
                      <span className={`inline-flex items-center justify-center w-5 h-5 rounded ${MATRIX_CELL[s]}`}><Icon className="w-3 h-3" /></span>

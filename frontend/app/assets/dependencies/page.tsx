    ...
    <div className="min-h-screen bg-[#070d19] text-[#7d92b0] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <select value={zoomLevel} onChange={e => setZoomLevel(e.target.value as ZoomLevel)} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-[#7d92b0] outline-hidden">
        ...
        <div className="w-56 shrink-0 space-y-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder="アセットを検索..."
              className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg pl-8 pr-3 py-2 text-sm text-white outline-hidden"
            />
          </div>
                    ...
                    <div className="flex items-center flex-wrap gap-1">
                      {criticalPath.map((name, i) => (
                        <span key={i} className="flex items-center gap-1">
                          <span className="text-xs text-white bg-[#1e2d42] px-2 py-0.5 rounded-sm">{name}</span>
                    ...
                    return (
                      <tr key={dep.id} className="border-b border-[#1e2d42]/50 hover:bg-[#1e2d42]/20">
                        ...
                        <td className="px-3 py-2">
                          <button onClick={() => handleDeleteDep(dep.id)} className="p-1 rounded-sm hover:bg-red-900/30 text-[#3d5068] hover:text-red-400 transition-colors">
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={newDep.from_asset_id} onChange={e => setNewDep(p => ({ ...p, from_asset_id: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-hidden">
              ...
              <div>
                ...
                <select value={newDep.relationship_type} onChange={e => setNewDep(p => ({ ...p, relationship_type: e.target.value as RelationshipType }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-hidden">
              ...
              <div>
                ...
                <select value={newDep.to_asset_id} onChange={e => setNewDep(p => ({ ...p, to_asset_id: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-hidden">
                  <option value="">選択してください</option>
                  {assets.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                </select>
              </div>
              <div>
                ...
                <select value={newDep.criticality} onChange={e => setNewDep(p => ({ ...p, criticality: e.target.value as Criticality }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-hidden">

...
function QueueCard({ item }: { item: WorkQueueItem }) {
  ...
  return (
    <Link href={item.link} className="block group">
      <div className="bg-falcon-surface border border-falcon-border rounded-lg p-3.5 hover:border-[#2a3f60] hover:bg-falcon-card transition-all">
        <div className="flex items-start gap-2.5">
          <TypeIcon className="w-4 h-4 shrink-0 mt-0.5" style={{ color }} />
          <div className="flex-1 min-w-0">
            <p className="text-sm text-falcon-text font-medium leading-snug line-clamp-2 group-hover:text-white">
              {item.title}
            </p>
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              ...
              {item.hostname && (
                <span className="text-[10px] text-falcon-muted font-mono truncate max-w-[120px]">
                  {item.hostname}
                </span>
              )}
          ...
          <ExternalLink className="w-3.5 h-3.5 text-falcon-subtle shrink-0 opacity-0 group-hover:opacity-100 transition-opacity mt-0.5" />
        </div>
      </div>
    </Link>
  )
...
function Lane({
  ...
  return (
    <div className="flex flex-col min-h-0">
      {/* Header */}
      <div className="flex items-center gap-2 mb-3 pb-2 border-b border-falcon-border">
        <Icon className="w-4 h-4" style={{ color }} />
        <div>
          <p className="text-sm font-bold text-falcon-text">{title}</p>
          <p className="text-[10px] text-falcon-subtle">{subtitle}</p>
        </div>
        <span
    ...
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2.5">
            <Inbox className="w-6 h-6 text-falcon-red" />
            SOC ワークキュー
          </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => refetch()}
            className="flex items-center gap-1.5 px-3 py-2 bg-falcon-raised hover:bg-falcon-active rounded-lg text-sm transition-colors"
          >
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="bg-falcon-bg border border-falcon-red/20 rounded-xl p-4">
            <Lane
              icon={Zap}
              title="今すぐ対応"
          ...
          <div className="bg-falcon-bg border border-[#f97316]/20 rounded-xl p-4">
            <Lane
              icon={Clock}
              title="今日中に対応"
          ...
          <div className="bg-falcon-bg border border-[#3b82f6]/20 rounded-xl p-4">
            <Lane
              icon={CalendarDays}
              title="今週中に対応"
      ...
      {correlationGroups.length > 0 && (
        <div className="mt-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-bold text-falcon-text">アラートグルーピング（相関検知）</h2>
            ...
            <span className="text-xs text-falcon-subtle ml-2">同一エージェント・同一MITREテクニックで相関検知されたアラート群</span>
          </div>
          <div className="bg-falcon-bg border border-purple-400/20 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-falcon-muted text-xs border-b border-falcon-border bg-falcon-surface">
              ...
              <tbody className="divide-y divide-falcon-border">
                {correlationGroups.map((g) => (
                  <tr key={g.id} className="hover:bg-falcon-card transition-colors">
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-1.5">
                        <Target className="w-3.5 h-3.5 text-purple-400 shrink-0" />
                        <span className="text-xs text-falcon-text font-mono">{g.mitre_technique || '—'}</span>
                      </div>
                    </td>
                    <td className="px-4 py-2.5 text-xs text-falcon-muted font-mono truncate max-w-[140px]">
                      {g.agent_id.slice(0, 8)}…
                    </td>
                    <td className="px-4 py-2.5 text-right">
                      <span className="text-purple-400 font-bold text-sm">{g.alert_count}</span>
                      <span className="text-falcon-subtle text-xs">件</span>
                    </td>
                    <td className="px-4 py-2.5 hidden md:table-cell text-xs text-falcon-subtle">
                      {g.last_seen_at
                    ...
                    <td className="px-4 py-2.5 text-right">
                      {g.incident_id ? (
                        ...
                        <span className="text-xs text-falcon-subtle">未リンク</span>
                      )}
                    </td>
                  </tr>
                ))}
          ...
          <p className="text-xs text-falcon-subtle mt-2 text-right">
      ...
      {candidates.length > 0 && (
        <div className="mt-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-bold text-falcon-text">抑制候補（ノイズアラート）</h2>
            <span className="text-xs text-falcon-subtle bg-yellow-400/10 px-2 py-0.5 rounded-full border border-yellow-400/20 text-yellow-400">
              {candidates.length}件
            </span>
            <span className="text-xs text-falcon-subtle ml-2">過去7日間に5回以上繰り返し発生したアラート</span>
          </div>
          <div className="bg-falcon-bg border border-yellow-400/20 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-falcon-muted text-xs border-b border-falcon-border bg-falcon-surface">
              ...
              <tbody className="divide-y divide-falcon-border">
                {candidates.map((c, i) => (
                  <tr key={i} className="hover:bg-falcon-card transition-colors">
                    <td className="px-4 py-2.5 font-mono text-xs text-falcon-text max-w-[200px] truncate">
                      {c.rule_name || '—'}
                    </td>
                    <td className="px-4 py-2.5 text-xs text-falcon-muted font-mono">
                      {c.hostname || '—'}
                    </td>
                    <td className="px-4 py-2.5 text-right">

      ...
      >
        ...
        <div className="px-5 py-3 border-t border-falcon-border flex justify-end gap-2">
          ...
          <button
            ...
            className="px-4 py-1.5 text-sm bg-purple-700 hover:bg-purple-600 text-white rounded-lg
...
export default function VulnerabilityDashboard() {
  ...
  const { data: inventoryData, isLoading: invLoading, refetch } = useQuery<{ data: SoftwareInventoryItem[] } | SoftwareInventoryItem[]>({
  ...
  const bulkStatusMut = useMutation({
    mutationFn: ({ ids, status }: { ids: string[]; status: string }) =>
      apiFetch('/api/v1/software-inventory/bulk', {
        ...
        // Fallback: fire individual updates
        return Promise.allSettled(
          ids.map(id => apiFetch(`/api/v1/software-inventory/${id}`, {
            method: 'PATCH',
                      ...
                      <td className="px-4 py-2.5">
                        {row.isMock ? (
                          ...
                          <select
                            ...
                            className={`text-[11px] font-medium rounded border px-1.5 py-0.5 bg-transparent cursor-pointer

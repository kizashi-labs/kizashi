    ...
    <div className="min-h-screen bg-[#070d19] text-[#7d92b0]">
            ...
            <div className="flex items-center gap-1 bg-[#0d1220] border border-[#1e2d42] rounded-lg p-1">
              {(['30d', '90d', '180d'] as Period[]).map(p => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${period === p && !showCustom ? 'bg-[#1e2d42] text-white' : 'text-[#7d92b0] hover:text-white'}`}
                >
              ...
              <button
                ...
                className={`flex items-center gap-1 px-3 py-1.5 rounded text-xs transition-colors ${showCustom ? 'bg-[#1e2d42] text-white' : 'text-[#7d92b0] hover:text-white'}`}
              >
        ...
        {showCustom && (
          <div className="flex items-center gap-3 mt-3">
            ...
            <input type="date" value={customFrom} onChange={e => setCustomFrom(e.target.value)}
              className="px-3 py-1.5 text-xs bg-[#0d1220] border border-[#1e2d42] rounded text-white focus:outline-none focus:border-[#7d92b0]" />
            <label className="text-xs text-[#7d92b0]">終了日</label>
            <input type="date" value={customTo} onChange={e => setCustomTo(e.target.value)}
              className="px-3 py-1.5 text-xs bg-[#0d1220] border border-[#1e2d42] rounded text-white focus:outline-none focus:border-[#7d92b0]" />
            <button className="px-3 py-1.5 text-xs bg-[#e8002d] text-white rounded hover:bg-[#c0001f] transition-colors">適用</button>
          </div>
        )}
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        {period === '30d' && daily30.length > 0 && (
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
            ...
            <div className="flex items-center gap-4 mt-2">
              <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-[#e8002d]/60" /><span className="text-xs text-[#7d92b0]">新規</span></div>
              <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-green-500/60" /><span className="text-xs text-[#7d92b0]">修正済</span></div>
            </div>
          </div>
        )}
        ...
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
          ...
          <div className="flex items-center gap-4 mt-2">
            <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-[#e8002d]/70" /><span className="text-xs text-[#7d92b0]">新規</span></div>
            <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-green-500/70" /><span className="text-xs text-[#7d92b0]">修正済</span></div>
          </div>
        </div>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
            ...
            <div className="flex items-center gap-3 mt-3">
              {([['Critical', 'bg-red-500/80'], ['High', 'bg-orange-500/80'], ['Medium', 'bg-yellow-500/80'], ['Low', 'bg-blue-500/80']] as [string, string][]).map(([label, cls]) => (
                <div key={label} className="flex items-center gap-1"><div className={`w-2.5 h-2.5 rounded-sm ${cls}`} /><span className="text-[10px] text-[#7d92b0]">{label}</span></div>
              ))}
            </div>
          </div>
              ...
              <tbody className="divide-y divide-[#1e2d42]/50">
                {data.top_cves.map(cve => (
                  <tr key={cve.cve_id} className="hover:bg-[#0a1628] transition-colors">
                    ...
                    <td className="px-3 py-2.5">
                      <span className={`px-2 py-0.5 rounded border text-[10px] font-medium ${statusBadge(cve.status)}`}>{statusLabel(cve.status)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              {data.age_buckets.map(bucket => {
                ...
                return (
                  <div key={bucket.label} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-6 bg-[#070d19] rounded overflow-hidden relative">
                      <div
                        className={`h-full ${colors[idx]} transition-all rounded`}
                        style={{ width: `${pct}%` }}
                      />

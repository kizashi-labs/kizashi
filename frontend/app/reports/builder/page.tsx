    ...
    <div className="px-2 py-2 space-y-1.5">
      {threats.map(t => (
        <div key={t.name} className="flex items-center gap-2 text-xs">
          ...
          <div className="flex-1 h-3 bg-[#1e2d42] rounded-sm overflow-hidden">
    ...
    <div className="px-2 py-2 space-y-1.5">
      {items.map(item => (
        <div key={item.label} className="flex items-center gap-2 text-xs">
          ...
          <div className="flex-1 h-2.5 bg-[#1e2d42] rounded-sm overflow-hidden">
...
function FreeTextPreview({ widgetUid, texts, onTextChange }: { widgetUid: string; texts: Record<string, string>; onTextChange: (uid: string, v: string) => void }) {
  return (
    <textarea
      ...
      placeholder="テキストを入力..."
      rows={3}
      className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-[#c8d6ea] resize-none focus:outline-hidden focus:border-[#e8002d]"
    />
  )
}
...
export default function ReportBuilderPage() {
  ...
  return (
    <div className="min-h-screen bg-[#070d19]">
      <PageSaveFailed className="m-6 mb-0" />
      {/* Config Modal */}
      {configModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-6 w-80 space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-1.5 text-sm text-white focus:outline-hidden focus:border-[#e8002d]"
                />
              </div>
              <div>
                ...
                <div className="flex gap-2">
                  {(['half', 'full'] as const).map(s => (
                    <button
                      ...
                      className={`flex-1 py-1.5 rounded-sm text-xs font-medium border transition-colors ${configDraft.size === s ? 'bg-[#e8002d]/20 border-[#e8002d] text-[#e8002d]' : 'border-[#1e2d42] text-[#7d92b0]'}`}
                    >
                      {s === 'half' ? '1/2幅' : '全幅'}
                    </button>
                  ))}
            ...
            <button
              onClick={applyConfig}
              className="w-full py-2 bg-[#e8002d] hover:bg-[#c0001e] rounded-sm text-white text-sm font-medium transition-colors"
            >
              適用
            </button>
          </div>
          ...
          <div className="max-w-5xl mx-auto">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-[#1e2d42] hover:bg-[#2d3f58] rounded-sm text-white text-sm transition-colors"
              >
      ...
      <div className="flex h-screen">
        {/* Left Panel — Palette */}
        <div className="w-64 shrink-0 bg-[#0d1220] border-r border-[#1e2d42] flex flex-col">
                ...
                <div className="space-y-1">
                  {group.widgets.map(def => (
                    ...
                    >
                      ...
                      <Plus className="w-3.5 h-3.5 text-[#3d5068] group-hover:text-[#e8002d] shrink-0" />
                    </button>
                  ))}
                </div>
              </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="bg-[#0d1220] border-b border-[#1e2d42] px-5 py-3 space-y-2">
            <div className="flex items-center gap-3">
              <h1 className="text-sm font-semibold text-[#7d92b0] shrink-0">カスタムレポートビルダー</h1>
              <div className="flex items-center gap-2 flex-1">
                <input
                  value={reportTitle}
                  onChange={e => setReportTitle(e.target.value)}
                  placeholder="レポートタイトル"
                  className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-1.5 text-sm text-white focus:outline-hidden focus:border-[#e8002d] min-w-0"
                />
                <input
                  type="date"
                  value={dateFrom}
                  onChange={e => setDateFrom(e.target.value)}
                  className="bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-1.5 text-xs text-[#c8d6ea] focus:outline-hidden focus:border-[#e8002d] w-36"
                />
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1e2d42] hover:bg-[#2d3f58] rounded-sm text-xs text-white transition-colors"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-[#e8002d] hover:bg-[#c0001e] rounded-sm text-xs text-white transition-colors disabled:opacity-50"
                >
                  <Save className="w-3.5 h-3.5" /> テンプレート保存
                </button>
                <button
                  onClick={() => { setWidgets([]); setFreeTexts({}) }}
                  className="flex items-center gap-1.5 px-3 py-1.5 border border-[#1e2d42] hover:border-[#e8002d] rounded-sm text-xs text-[#7d92b0] hover:text-[#e8002d] transition-colors"
                >
            ...
            <textarea
              ...
              rows={1}
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-1.5 text-xs text-[#c8d6ea] resize-none focus:outline-hidden focus:border-[#e8002d]"
            />
                          ...
                          <div className="flex items-center justify-between px-4 py-2 bg-[#0a1020] border-b border-[#1e2d42]">
                            ...
                            <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                ...
                                className="p-1 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] disabled:opacity-30"
                              >
                              ...
                              <button
                                ...
                                className="p-1 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-white"
                              >
                                <Settings className="w-3.5 h-3.5" />
                              </button>
                              <button
                                onClick={() => removeWidget(w.uid)}
                                className="p-1 rounded-sm hover:bg-[#e8002d]/20 text-[#7d92b0] hover:text-[#e8002d]"
                              >

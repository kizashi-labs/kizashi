...
// 取得できなかったときのレポート。数字は空で、作りません。
// 以前はここが buildMockReport で、initialData にも入っていたので、
// この画面は取得の成否にかかわらず必ず作り物のレポートを描いていました。
function emptyReport(period: string): SecurityOpsReport {
  return {
    period,
    generated_at: '',
    kpi: {
      total_incidents: 0, mttr_minutes: 0, alert_volume: 0, false_positive_rate: 0,
      incidents_delta: 0, mttr_delta: 0, alert_delta: 0, fp_delta: 0,
    },
    severity_breakdown: [],
    threat_categories: [],
    analyst_performance: [],
            ...
            <div className="flex gap-2 mb-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg pl-9 pr-3 py-2 text-white text-sm placeholder-[#3d5275] focus:outline-hidden focus:border-[#1a6bff] transition-colors"
                />
              </div>
              <button
                onClick={addEmail}
...
function Toast({ message, onClose }: { message: string; onClose: () => void }) {
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 bg-[#0d1220] border border-[#1e2d42] rounded-xl px-4 py-3 shadow-2xl animate-fade-in">
      <div className="w-7 h-7 rounded-full bg-[#1a6bff]/15 flex items-center justify-center shrink-0">
...
export default function SecurityOpsReportPage() {
  ...
  const { data: report, isLoading, refetch } = useQuery<SecurityOpsReport>({
    ...
    staleTime: 60_000,
    initialData: () => mockOr(buildMockReport(period), emptyReport(period)),
  })
  ...
  async function handleScheduleSubmit(payload: SchedulePayload) {
    ...
    try {
      await persist('レポートの配信予約', '/api/v1/reports/security-ops/schedule', {
        method: 'POST',
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-[#e8002d]/15 border border-[#e8002d]/30 flex items-center justify-center">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm text-[#7d92b0] hover:text-white border border-[#1e2d42] hover:border-[#2d4060] rounded-lg transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[#e8002d] hover:bg-[#c20026] text-white text-sm font-medium rounded-lg transition-colors"
            >
          ...
          {period === 'custom' && (
            <div className="flex items-center gap-2">
              <input
                ...
                className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#1a6bff] transition-colors"
              />
        ...
        {isLoading ? (
          ...
          <>
                        ...
                        <tr key={row.email} className={`border-b border-[#1e2d42]/50 hover:bg-[#111827]/60 transition-colors ${i === (report.analyst_performance.length - 1) ? 'border-0' : ''}`}>
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-2.5">
                              <div className="w-7 h-7 rounded-full bg-linear-to-br from-[#1a6bff] to-[#0044cc] flex items-center justify-center text-xs font-bold text-white shrink-0">
                                {row.analyst[0]}
                              </div>
                              <div>
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl px-5 py-4 flex items-start gap-3">
              <Lock className="w-4 h-4 text-[#7d92b0] shrink-0 mt-0.5" />

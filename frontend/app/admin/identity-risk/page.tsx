...
function RiskProfileModal({ user, onClose, weights }: { user: RiskUser; onClose: () => void; weights: Record<RiskFactor, number> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-falcon-surface border-b border-falcon-border px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-falcon-muted text-xs">{user.email} · {user.department}</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#070d19] border border-falcon-border rounded-xl p-4 text-center">
              <p className="text-falcon-muted text-xs mb-2">リスクスコア</p>
            ...
            <div className="bg-[#070d19] border border-falcon-border rounded-xl p-4">
              <p className="text-falcon-muted text-xs mb-3">部門平均比較 ({user.department})</p>
              <div className="space-y-2">
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-falcon-muted">部門平均</span>
          ...
          {/* Factor breakdown */}
          <div className="bg-[#070d19] border border-falcon-border rounded-xl p-4">
            <p className="text-falcon-muted text-xs font-medium uppercase tracking-wider mb-3">スコア内訳</p>
                ...
                return (
                  <div key={f}>
                    <div className="flex items-center justify-between text-xs mb-1">
                      <span className={active ? 'text-white' : 'text-falcon-subtle'}>
                        {active && <span className="text-red-400 mr-1">●</span>}
                        {FACTOR_LABELS[f]}
                      </span>
                      <span className={active ? 'text-red-400' : 'text-falcon-subtle'}>+{contribution}</span>
                    </div>
                    <div className="h-1.5 bg-falcon-border rounded-full overflow-hidden">
                      <div className={`h-full ${active ? 'bg-red-500' : 'bg-falcon-border'} transition-all`}
          ...
          {/* Login history */}
          <div className="bg-[#070d19] border border-falcon-border rounded-xl p-4">
            <p className="text-falcon-muted text-xs font-medium uppercase tracking-wider mb-3">最近10件のログイン</p>
            <div className="space-y-2">
              {user.login_history.map((l, i) => (
                <div key={i} className="flex items-center gap-3 text-xs">
                  <span className="text-falcon-muted font-mono w-28 shrink-0">{fmt(l.timestamp)}</span>
                  <span className="text-falcon-text font-mono w-28 shrink-0">{l.ip}</span>
                  <span className="text-falcon-muted flex-1 truncate">{l.device}</span>
                  {l.success
          ...
          {user.risk_factors.length > 0 && (
            <div className="bg-[#070d19] border border-falcon-border rounded-xl p-4">
              <p className="text-falcon-muted text-xs font-medium uppercase tracking-wider mb-3">推奨アクション</p>
              <div className="space-y-2">
                {user.risk_factors.map(f => (
                  <div key={f} className="flex items-start gap-2">
                    <AlertTriangle className="w-3.5 h-3.5 text-yellow-400 shrink-0 mt-0.5" />
                    <p className="text-falcon-text text-xs">{FACTOR_RECOMMENDATIONS[f]}</p>
                  </div>
                ))}
              </div>
            </div>
    ...
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-falcon-muted text-sm">ユーザーのリスク評価と対応管理</p>
        </div>
      </div>
      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-xl mb-6">
        <button onClick={() => setShowConfig(p => !p)}
          ...
          リスクスコアリング設定
          {showConfig ? <ChevronUp className="w-4 h-4 text-falcon-muted" /> : <ChevronDown className="w-4 h-4 text-falcon-muted" />}
        </button>
        {showConfig && (
          <div className="px-5 pb-5 border-t border-falcon-border">
            <p className="text-falcon-muted text-xs mt-4 mb-4">各リスク要因のウェイトを調整します。スコア = Σ(要因の基本ウェイト × 0.8)</p>
            <div className="space-y-4">
              {(Object.keys(weights) as RiskFactor[]).map(f => (
                <div key={f} className="flex items-center gap-4">
                  <span className="text-sm text-white w-36 shrink-0">{FACTOR_LABELS[f]}</span>
                  ...
                  <span className="text-falcon-red font-bold text-sm w-8 text-right">{weights[f]}</span>
                </div>
              ))}
            </div>
            <div className="mt-4 p-3 bg-[#070d19] border border-falcon-border rounded-lg">
              <p className="text-xs text-falcon-muted">最大スコア (全要因該当時): <span className="text-white font-bold">{Math.round(Object.values(weights).reduce((a, b) => a + b, 0) * 0.8)}</span></p>
            </div>
            <button onClick={handleWeightSave}
              className="mt-4 px-6 py-2 bg-falcon-red text-white text-sm font-medium rounded-lg hover:bg-[#c8001e] transition-colors">
              設定保存
            </button>
          </div>
        )}
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id} onClick={() => setSelectedUser(u)}
                className="border-b border-falcon-border/50 hover:bg-[#070d19]/50 transition-colors cursor-pointer">
                ...
                <td className="px-4 py-3 text-xs text-falcon-muted">{u.role}</td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {u.risk_factors.length > 3 && <span className="text-[10px] text-falcon-muted">+{u.risk_factors.length - 3}</span>}
                  </div>
                </td>
                <td className="px-4 py-3 text-xs text-falcon-muted font-mono whitespace-nowrap">{fmt(u.last_login)}</td>
                <td className="px-4 py-3">{statusBadge(u.status)}</td>
                <td className="px-4 py-3">
                  <button onClick={e => { e.stopPropagation(); setSelectedUser(u) }}
                    className="text-xs text-falcon-muted hover:text-white transition-colors border border-falcon-border px-2 py-1 rounded-sm hover:border-falcon-muted/40">
                    詳細
                  </button>
                </td>
              </tr>

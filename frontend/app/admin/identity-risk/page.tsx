...
function RiskProfileModal({ user, onClose, weights }: { user: RiskUser; onClose: () => void; weights: Record<RiskFactor, number> }) {
  ...
  // MFA の強制は、リスクの高い利用者に対する実際の対処です。失敗を捨てて
  // 1.5秒後にボタンを戻すだけだったので、押した側からは実行されたように
  // 見えます。/api/v1/admin/identity-risk/* にはサーバ側のルートがありません。
  const doAction = async (action: string, label: string) => {
    setActing(action)
    const ok = await persist(label, `/api/v1/admin/identity-risk/users/${user.id}/enforce-mfa`, { method: 'POST' })
    setActing(null)
    if (!ok) return
  }
            ...
            <div className="space-y-2">
              {user.login_history.map((l, i) => (
                <div key={i} className="flex items-center gap-3 text-xs">
                  <span className="text-[#7d92b0] font-mono w-28 shrink-0">{fmt(l.timestamp)}</span>
                  <span className="text-[#e2e8f4] font-mono w-28 shrink-0">{l.ip}</span>
...
export default function IdentityRiskPage() {
  ...
  // リスク係数の重み付け。保存の失敗を捨ててダイアログを閉じていたので、
  // 調整したつもりで次に開くと元に戻っています。
  const handleWeightSave = async () => {
    if (await persist('リスク係数の重み', '/api/v1/admin/identity-risk/config', {
      method: 'PUT',
              ...
              <tr key={u.id} onClick={() => setSelectedUser(u)}
                ...
                <td className="px-4 py-3">
                  <button onClick={e => { e.stopPropagation(); setSelectedUser(u) }}
                    className="text-xs text-[#7d92b0] hover:text-white transition-colors border border-[#1e2d42] px-2 py-1 rounded-sm hover:border-[#7d92b0]/40">
                    詳細
                  </button>
                </td>
              </tr>

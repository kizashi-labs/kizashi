...
export default function CustomFieldsPage() {
  ...
  const reorderMutation = useMutation({
    mutationFn: ({ id, direction }: { id: string; direction: 'up' | 'down' }) => {
      ...
      // 2本とも .catch(() => null) で失敗が握りつぶされ、onSuccess が走って
      // 一覧が再取得されるので、並べ替えたつもりで順序は元のままになります。
      return Promise.all([
  ...
  return (
    <div className="min-h-screen bg-[#070d19] p-6">
      <PageDataUnavailable />
      <SaveFailed error={saveErrorOf('カスタムフィールド', reorderMutation)} />
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#7d92b0] text-sm ml-11">アラートおよびエンドポイントに付加するカスタムメタデータフィールドを管理します</p>
        </div>
        <button
          onClick={openCreate}
        ...
        {(['alert', 'endpoint'] as const).map(tab => (
          ...
          >
            ...
            <span className={`ml-2 text-xs px-1.5 py-0.5 rounded-full ${
              activeTab === tab ? 'bg-white/20 text-white' : 'bg-[#1e2d42] text-[#7d92b0]'
            }`}>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-[#1e2d42] flex items-center justify-between">
          ...
          <span className="text-xs text-[#7d92b0]">{sortedFields.length} 件</span>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            <Layers className="w-10 h-10 text-[#1e2d42] mx-auto mb-3" />
            <p className="text-[#7d92b0] text-sm">フィールドが登録されていません</p>
            <button
              onClick={openCreate}
              className="mt-3 text-xs text-[#e8002d] hover:text-[#ff2040] transition-colors"
            >
              最初のフィールドを追加する
            </button>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#1e2d42]">
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#7d92b0] uppercase tracking-wider w-8">#</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#7d92b0] uppercase tracking-wider">フィールド名</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#7d92b0] uppercase tracking-wider">表示名</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#7d92b0] uppercase tracking-wider">タイプ</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#7d92b0] uppercase tracking-wider">必須</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#7d92b0] uppercase tracking-wider">デフォルト値</th>
              ...
              <tbody className="divide-y divide-[#1e2d42]">
                {sortedFields.map((field, idx) => (
                  <tr key={field.id} className="hover:bg-[#0a1628] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-0.5">
                        <button
                          ...
                          className="p-0.5 text-[#3d5068] hover:text-[#7d92b0] disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
                          title="上へ"
                        >
                    ...
                    {/* field_name */}
                    <td className="px-4 py-3">
                      <span className="text-xs font-mono text-[#e2e8f4] bg-[#070d19] px-2 py-0.5 rounded-sm border border-[#1e2d42]">
                        {field.field_name}
                      </span>
                    </td>
                    ...
                    {/* field_type */}
                    <td className="px-4 py-3">
                      <span className="text-xs px-2 py-0.5 rounded-sm border bg-[#070d19] border-[#1e2d42] text-[#7d92b0]">
                        {FIELD_TYPE_LABELS[field.field_type]}
                        {field.field_type === 'select' && field.options && field.options.length > 0 && (
                          <span className="ml-1 text-[#3d5068]">({field.options.length})</span>
                        )}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {field.required ? (
                        <span className="text-xs px-2 py-0.5 rounded-sm bg-[#e8002d]/10 text-[#e8002d] border border-[#e8002d]/20">必須</span>
                      ) : (
                    ...
                    {/* default_value */}
                    <td className="px-4 py-3 text-xs text-[#7d92b0] font-mono max-w-[140px] truncate">
                      {field.default_value || <span className="text-[#3d5068]">—</span>}
                    </td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="px-5 py-4 space-y-4 overflow-y-auto">
              ...
              {/* entity_type hint */}
              <div className="flex items-center gap-2 px-3 py-2 rounded-sm bg-[#070d19] border border-[#1e2d42] text-xs text-[#7d92b0]">
                <Layers className="w-3.5 h-3.5 text-[#e8002d]" />
              ...
              <div>
                <label className="block text-xs font-medium text-[#7d92b0] mb-1.5">
                  フィールド名 <span className="text-[#e8002d]">*</span>
                  <span className="ml-2 text-[#3d5068] font-normal">(snake_case)</span>
                </label>
                <input
                  type="text"
                  ...
                  className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white font-mono placeholder-[#3d5068] focus:outline-hidden focus:border-[#e8002d]/50"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-medium text-[#7d92b0] mb-1.5">
                  表示名 <span className="text-[#e8002d]">*</span>
                </label>
                <input
                  type="text"
              ...
              {form.field_type !== 'boolean' && (
                <div>
                  <label className="block text-xs font-medium text-[#7d92b0] mb-1.5">デフォルト値</label>
                  <input
                    type="text"
              ...
              {form.field_type === 'select' && (
                <div>
                  <label className="block text-xs font-medium text-[#7d92b0] mb-1.5">
                    選択肢
                    <span className="ml-2 text-[#3d5068] font-normal">(1行1オプション)</span>
                  </label>
                  <textarea
                    ...
                    rows={4}
                    placeholder={"low\nmedium\nhigh\ncritical"}
                    className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white font-mono placeholder-[#3d5068] focus:outline-hidden focus:border-[#e8002d]/50 resize-none"
                  />
                </div>
              )}
              ...
              <div className="flex items-center justify-between">
                <label className="text-xs font-medium text-[#7d92b0]">必須フィールド</label>
                <button
                  type="button"
                  onClick={() => setForm(f => ({ ...f, required: !f.required }))}
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                    form.required ? 'bg-[#e8002d]' : 'bg-[#1e2d42]'
                  }`}
                >
            ...
            {/* Modal footer */}
            <div className="px-5 py-4 border-t border-[#1e2d42] flex justify-end gap-3 shrink-0">
              <button
                onClick={() => setModalOpen(false)}

      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={item.label} className="flex gap-2 text-sm">
                  <span className="text-[#7d92b0] w-28 shrink-0">{item.label}:</span>
          ...
          <div>
            ...
            <p className="text-[#e2e8f4] text-sm bg-[#070d19] rounded-sm border border-[#1e2d42] p-3">{tool.notes}</p>
          </div>
              ...
              : <ul className="space-y-1">
                  {tool.documentation_links.map(d => (
                    <li key={d} className="flex items-center gap-2 text-sm text-[#7d92b0]">
                      <ChevronRight className="w-3.5 h-3.5 text-[#e8002d] shrink-0" />{d}
                    </li>
                  ))}
                </ul>
            }
          ...
          <div className="flex gap-3 pt-2">
            {tool.license_expiry && (
              <button onClick={onRenew} className="flex items-center gap-2 px-4 py-2 bg-[#e8002d] rounded-sm text-white text-sm hover:bg-[#c0001f]">
                <RefreshCw className="w-4 h-4" />更新手続き
              </button>
            )}
            <button onClick={onClose} className="px-4 py-2 border border-[#1e2d42] rounded-sm text-[#7d92b0] hover:text-white text-sm">閉じる</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 p-5 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input type={showKey ? 'text' : 'password'} value={form.license_key} onChange={e => setForm(f => ({ ...f, license_key: e.target.value }))}
                  placeholder="••••••••••••"
                  className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 pr-10 text-white text-sm focus:border-[#e8002d] focus:outline-hidden" />
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" className="flex-1 px-4 py-2 bg-[#e8002d] rounded-sm text-white font-medium text-sm hover:bg-[#c0001f]">
              {tool ? '更新する' : '追加する'}
            </button>
          </div>
        </form>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleExportCSV}
            className="flex items-center gap-2 px-3 py-2 border border-[#1e2d42] rounded-sm text-[#7d92b0] hover:text-white text-sm">
              ...
              return (
                <div key={t.id} className="flex items-center justify-between bg-orange-900/10 rounded-sm px-4 py-2.5">
                  ...
                  <button onClick={() => showToast(`${t.tool_name}の更新手続きを開始します`)}
                    className="flex items-center gap-1 px-3 py-1.5 bg-[#e8002d] rounded-sm text-white text-xs hover:bg-[#c0001f]">
        ...
        <div className="px-5 py-4 border-b border-[#1e2d42] flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value as ToolCategory | 'all')}
              className="bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-1.5 text-white text-sm focus:outline-hidden">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                        <button onClick={() => { setFormTool(t); setShowForm(true) }}
                          className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-white"><Pencil className="w-3.5 h-3.5" /></button>
                        <button onClick={() => showToast(`${t.tool_name}を削除しました`)}
                          className="p-1.5 rounded-sm hover:bg-red-900/40 text-[#7d92b0] hover:text-red-300"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                )
        ...
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
          ...
          <div className="space-y-2">
            {CAPABILITY_MATRIX.map(row => {
              ...
              return (
                <div key={row.domain} className="flex items-center justify-between p-3 bg-[#070d19] rounded-sm border border-[#1e2d42]">

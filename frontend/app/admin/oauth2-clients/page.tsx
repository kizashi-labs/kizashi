        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2.5">
              ...
              <button
                onClick={copy}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-sm bg-[#e8002d] hover:bg-[#c8001d] text-white text-xs font-medium transition-colors"
              >
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <textarea
                ...
                rows={2}
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-hidden focus:border-[#1a6bff]/60 transition-colors resize-none"
              />
            </div>
          </div>
              ...
              {form.redirect_uris.map((uri, i) => (
                <div key={i} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-hidden focus:border-[#1a6bff]/60 transition-colors"
                  />
                  {form.redirect_uris.length > 1 && (
                    <button
                      type="button"
                      onClick={() => removeUri(i)}
                      className="p-2 rounded-sm border border-[#1e2d42] text-[#7d92b0] hover:text-[#e8002d] hover:border-[#e8002d]/30 transition-colors"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  )}
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="flex items-center gap-2 bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2">
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {client.redirect_uris.map((uri, i) => (
                  <div key={i} className="flex items-center gap-2 bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-1.5">
            ...
            <div className="space-y-2">
              {client.allowed_scopes.map(scope => (
                <div key={scope} className="flex items-start gap-2">
                  <Check className="w-3.5 h-3.5 text-[#00c853] mt-0.5 shrink-0" />
                  <div>
            ...
            <div className="space-y-2">
              {client.grant_types.map(grant => (
                <div key={grant} className="flex items-start gap-2">
                  <ChevronRight className="w-3.5 h-3.5 text-[#1a6bff] mt-0.5 shrink-0" />
                  <div>
...
export default function OAuth2ClientsPage() {
  ...
  const { data: displayClients = [], isLoading } = useQuery<OAuth2Client[]>({
    ...
    queryFn: () => apiFetchList<OAuth2Client>('/api/v1/admin/oauth2'),
    staleTime: 60_000,
  })
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      setShowCreate(false)
      // 以前はここが else setSecretValue(generateMockSecret()) でした。
      // 管理者はブラウザで作った cs_... を控えて連携先に設定します。
      // サーバが発行していない値なので、その連携は永久に認証できません。
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      apiFetch(`/api/v1/admin/oauth2/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['oauth2-clients'] })
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex items-start justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[#1a6bff] hover:bg-[#1558d6] text-white text-sm font-semibold transition-colors shadow-lg"
          >
                      ...
                      >
                        ...
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                          <label className="flex items-center gap-2 cursor-pointer select-none">
                            ...
                            >
                              <span className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-[#e2e8f4] shadow-sm transition-transform duration-200 ${client.enabled ? 'translate-x-4' : 'translate-x-0'}`} />
                            </div>

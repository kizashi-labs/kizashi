...
export default function ZTNAPage() {
  ...
  const { data: policies = [] } = useQuery<ZTNAPolicy[]>({
    ...
    queryFn: () => apiFetchList<ZTNAPolicy>('/api/v1/admin/ztna/policies'),
    staleTime: 30_000,
  })

  const { data: logs = [] } = useQuery<AccessLog[]>({
    queryKey: ['ztna-logs', refreshKey],
    queryFn: () => apiFetchList<AccessLog>('/api/v1/admin/ztna/logs'),
    staleTime: 15_000,
  })

  const { data: devices = [] } = useQuery<DevicePosture[]>({
    queryKey: ['ztna-devices'],
    queryFn: () => apiFetchList<DevicePosture>('/api/v1/admin/ztna/devices'),
    staleTime: 30_000,
  })

  const togglePolicy = useMutation({
    mutationFn: (id: string) =>
      // .catch(() => null) が失敗を成功に変えていました。ZTNA ポリシーの
      // 有効/無効はネットワークへの到達可否そのもので、しかも
      // /api/v1/admin/ztna/* にはサーバ側のルートが1本もありません。
      apiFetch(`/api/v1/admin/ztna/policies/${id}/toggle`, { method: 'POST' }),
  ...
  return (
    <div className="min-h-screen bg-[#070d19] text-white">
      <PageDataUnavailable />
      <SaveFailed error={saveErrorOf('ZTNAポリシー', togglePolicy)} />
              ...
              <div key={node.label} className="flex items-center">
                ...
                {i < arr.length - 1 && (
                  <div className="flex items-center w-12">
                    <div className="h-0.5 flex-1 bg-linear-to-r from-[#1e2d42] to-[#e8002d]/40" />

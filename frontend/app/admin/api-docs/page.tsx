...
const endpointGroups: EndpointGroup[] = [
  ...
  },
  {
    ...
    endpoints: [
      {
        method: 'GET',
        path: '/api/v1/admin/billing/dashboard',
        description: '課金ダッシュボード情報（顧客数・サブスクリプション・MRR）を返します（管理者のみ）。',
        parameters: [],
        response: JSON.stringify({
          summary: {
            total_customers: 28,
          ...
          customers: [
            { id: 'cus-uuid', email: 'billing@example.com', plan: 'professional' }
          ],
      ...
      },
      {
        ...
        description: 'Stripe Checkout セッションを作成します。返された URL にリダイレクトして支払いフローを完了させます。',
        parameters: [],
        requestBody: JSON.stringify({
          plan: 'professional',
          agent_count: 100
      ...
      },
      {
        ...
        description: 'Stripe Customer Portal セッションを作成します。支払い方法変更・請求履歴・プラン変更に使用。',
        parameters: [],
        requestBody: JSON.stringify({}, null, 2),
        response: JSON.stringify({
          portal_url: 'https://billing.stripe.com/session/bps_xxx'
        }, null, 2),
      },
      {
        method: 'POST',
        path: '/api/v1/webhooks/stripe',
        description: 'Stripe Webhook エンドポイント（JWT認証不要）。Stripe が支払い完了・失敗等のイベントを送信します。',
        parameters: [],
        requestBody: JSON.stringify({
          type: 'checkout.session.completed',
          data: { object: { id: 'cs_live_xxx', customer: 'cus_xxx' } }
        }, null, 2),
        response: JSON.stringify({ received: true }, null, 2),
      },
    ],
  },
]
  ...
  return (
    <button
      ...
      className="flex items-center gap-1.5 px-3 py-1.5 rounded text-xs
                 bg-[#1e2d42] hover:bg-[#253650] border border-[#2a3f5f]
                 text-[#7d92b0] hover:text-[#e2e8f4] transition-all duration-150"
    >
    ...
    <div className="mt-4 space-y-3">
      <div className="flex items-center gap-2">
        <input
          ...
          className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded px-3 py-1.5 text-xs text-[#e2e8f4] placeholder-[#3d5068] focus:outline-none focus:border-[#2a3f5a] font-mono"
        />
        <button
          onClick={run}
          disabled={loading}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-[#e8002d]/20 hover:bg-[#e8002d]/30 border border-[#e8002d]/40 text-[#e8002d] text-xs rounded transition-colors disabled:opacity-50"
        >
      ...
      {ep.method !== 'GET' && (
        <div>
          ...
          <textarea
            ...
            rows={4}
            className="w-full bg-[#070d19] border border-[#1e2d42] rounded p-2 text-xs text-[#e2e8f4] font-mono focus:outline-none focus:border-[#2a3f5a] resize-none"
          />
        </div>
      )}
      ...
      {response && (
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            {status !== null && (
              <span className={`text-[10px] px-1.5 py-0.5 rounded font-mono ${status < 300 ? 'bg-[#22c55e]/20 text-[#22c55e]' : 'bg-[#e8002d]/20 text-[#e8002d]'}`}>
                {status}
              </span>
            )}
          </div>
          <pre className="bg-[#070d19] border border-[#1e2d42] rounded p-3 text-xs font-mono text-[#e2e8f4] overflow-x-auto max-h-48 overflow-y-auto">
            {response}
          </pre>
        </div>
      )}
...
function EndpointCard({ ep }: { ep: Endpoint }) {
  ...
  return (
    <div className="border border-[#1e2d42] rounded-lg overflow-hidden bg-[#0d1220]">
      ...
      >
        <span className={`text-[11px] font-bold px-2 py-0.5 rounded font-mono w-16 text-center flex-shrink-0 ${METHOD_COLORS[ep.method] ?? ''}`}>
          {ep.method}
        </span>
        ...
        <ChevronRight className={`w-4 h-4 text-[#3d5068] flex-shrink-0 transition-transform ${open ? 'rotate-90' : ''}`} />
      </button>
        ...
        <div className="border-t border-[#1e2d42] px-4 py-4 space-y-5">
                  ...
                  <tbody>
                    {ep.parameters.map(p => (
                      <tr key={p.name} className="border-b border-[#1e2d42]/50">
                        ...
                        <td className="py-2 pr-4">
                          {p.required
                            ? <span className="text-xs px-1.5 py-0.5 rounded bg-[#e8002d]/20 text-[#e8002d] border border-[#e8002d]/30">必須</span>
          ...
          {ep.requestBody && (
            <div>
              ...
              <pre className="bg-[#070d19] border border-[#1e2d42] rounded p-3 text-[#e2e8f4] text-xs font-mono overflow-x-auto leading-relaxed">
                {ep.requestBody}
              </pre>
            </div>
          )}
          ...
          <div>
            ...
            <pre className="bg-[#070d19] border border-[#1e2d42] rounded p-3 text-[#7d92b0] text-xs font-mono overflow-x-auto leading-relaxed">
              {curl}
            </pre>
          </div>
...
export default function ApiDocsPage() {
  ...
  return (
    <div className="flex h-full bg-[#070d19] text-[#e2e8f4]">
      ...
      {/* ── Left Sidebar ─────────────────────────────────────────── */}
      <div className="w-52 flex-shrink-0 border-r border-[#1e2d42] flex flex-col sticky top-0 h-screen overflow-y-auto">
        ...
        <div className="border-b border-[#1e2d42] p-6 bg-[#0d1220]">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-2xl font-bold text-white flex items-center gap-3">
                API ドキュメント
                <span className="text-xs font-normal px-2 py-1 rounded bg-[#e8002d]/20 text-[#e8002d] border border-[#e8002d]/30 font-mono">
                  v1.0
                </span>
              </h1>
          ...
          <div className="mt-4 relative">
            ...
            <input
              ...
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg pl-9 pr-4 py-2 text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-none focus:border-[#2a3f5a]"
            />
            {searchQuery && (
          ...
          <div className="mt-4 p-4 rounded-lg bg-[#070d19] border border-[#1e2d42]">
            ...
            <div className="flex items-center gap-3">
              <pre className="flex-1 bg-[#0d1220] border border-[#1e2d42] rounded p-2.5 text-xs font-mono text-[#7d92b0] overflow-x-auto">

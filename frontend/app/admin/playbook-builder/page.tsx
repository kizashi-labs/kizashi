...
function StepCard({ step, index, total, onEdit, onDelete, onInsertAfter }: {
  ...
  return (
    <div className="flex flex-col items-center">
      <div className={`w-full max-w-lg bg-[#0d1220] border rounded-xl p-4 group hover:border-[#7d92b0]/30 transition-colors ${cfg.border}`}>
        <div className="flex items-start gap-3">
          {/* Drag handle */}
          <div className="flex-shrink-0 mt-1 text-[#3d5068] group-hover:text-[#7d92b0] transition-colors cursor-grab">
            <GripVertical className="w-4 h-4" />
          </div>
          {/* Step number */}
          <div className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border ${cfg.bg} ${cfg.border} ${cfg.color}`}>
            {step.step_number}
          </div>
          {/* Content */}
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-semibold border ${cfg.bg} ${cfg.border} ${cfg.color}`}>
                <Icon className="w-2.5 h-2.5" />
                {step.type === 'action' ? 'アクション' : step.type === 'condition' ? '条件' : step.type === 'notification' ? '通知' : step.type === 'wait' ? '待機' : '並列'}
              </span>
              <span className="text-[10px] text-[#7d92b0] bg-[#1e2d42] px-1.5 py-0.5 rounded">{ACTION_TYPE_LABELS[step.action_type]}</span>
            </div>
            <p className="text-sm font-semibold text-white">{step.title}</p>
            <p className="text-xs text-[#7d92b0] mt-0.5">{step.description}</p>
            {step.parameters.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1">
                {step.parameters.map((p, pi) => (
                  <span key={pi} className="text-[10px] font-mono bg-[#070d19] border border-[#1e2d42] px-1.5 py-0.5 rounded text-[#7d92b0]">
                    {p.key}: {p.value}
                  </span>
                ))}
              </div>
            )}
            <div className="flex items-center gap-3 mt-2 text-[10px] text-[#3d5068]">
          ...
          <div className="flex-shrink-0 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={onEdit} className="p-1.5 rounded hover:bg-[#1e2d42] text-[#7d92b0] hover:text-white transition-colors">
              <Pencil className="w-3.5 h-3.5" />
            </button>
            <button onClick={onDelete} className="p-1.5 rounded hover:bg-red-900/20 text-[#7d92b0] hover:text-red-400 transition-colors">
      ...
      {index < total - 1 && (
        <div className="flex flex-col items-center my-1 group/insert">
          ...
          <button onClick={onInsertAfter}
            className="w-6 h-6 rounded-full bg-[#070d19] border border-[#1e2d42] hover:border-[#e8002d]/50 hover:bg-[#e8002d]/10 flex items-center justify-center text-[#3d5068] hover:text-[#e8002d] transition-all opacity-0 group-hover/insert:opacity-100">
...
function StepModal({ step, onClose, onSave }: { step?: PlaybookStep; onClose: () => void; onSave: (d: Partial<PlaybookStep>) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg mx-4 shadow-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
          <div>
            <label className="block text-xs text-[#7d92b0] mb-1">ステップタイトル</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-[#7d92b0] mb-1">アクションタイプ</label>
          ...
          {/* Parameters */}
          <div>
            <label className="block text-xs text-[#7d92b0] mb-2">パラメータ</label>
            <div className="space-y-2">
              {form.parameters.map((p, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="font-mono text-xs bg-[#070d19] border border-[#1e2d42] rounded px-2 py-1 text-[#7d92b0] flex-1">{p.key}</span>
                  <span className="text-[#3d5068]">:</span>
                  <span className="font-mono text-xs bg-[#070d19] border border-[#1e2d42] rounded px-2 py-1 text-white flex-1">{p.value}</span>
                  <button onClick={() => removeParam(i)} className="p-1 rounded hover:bg-red-900/20 text-[#7d92b0] hover:text-red-400 transition-colors"><X className="w-3 h-3" /></button>
                </div>
              ))}
              <div className="flex items-center gap-2">
                <input value={newParamKey} onChange={e => setNewParamKey(e.target.value)} placeholder="キー"
                  className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded px-2 py-1 text-white text-xs font-mono focus:outline-none focus:border-[#e8002d]/50" />
                <span className="text-[#3d5068]">:</span>
                <input value={newParamVal} onChange={e => setNewParamVal(e.target.value)} placeholder="値"
                  className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded px-2 py-1 text-white text-xs font-mono focus:outline-none focus:border-[#e8002d]/50" />
                <button onClick={addParam} className="p-1.5 rounded bg-[#1e2d42] hover:bg-[#e8002d]/20 text-[#7d92b0] hover:text-[#e8002d] transition-colors"><Plus className="w-3 h-3" /></button>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-[#7d92b0] mb-1">タイムアウト (分)</label>
              <input type="number" value={form.timeout_minutes} onChange={e => setForm(p => ({ ...p, timeout_minutes: Number(e.target.value) }))}
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/50" />
            </div>
            <div>
              <label className="block text-xs text-[#7d92b0] mb-1">失敗時の動作</label>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-md mx-4 shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-xs text-[#7d92b0] mb-1">トリガータイプ</label>
          ...
          {form.trigger_type === 'alert' && (
            <>
              <div>
                <label className="block text-xs text-[#7d92b0] mb-1">アラート深刻度 ({'>'}=)</label>
              ...
              <div>
                <label className="block text-xs text-[#7d92b0] mb-1">アラートカテゴリ</label>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg mx-4 shadow-2xl">
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div key={s.id} className={`flex items-center gap-3 p-3 rounded-lg border transition-all ${
                ...
                state === 'running' ? 'bg-[#e8002d]/5 border-[#e8002d]/20 animate-pulse' :
                'bg-[#070d19] border-[#1e2d42]'}`}>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold transition-colors ${
                  ...
                  state === 'running' ? 'bg-[#e8002d] text-white' :
                  'bg-[#1e2d42] text-[#3d5068]'}`}>
                  {state === 'done' ? '✓' : s.step_number}
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium ${state === 'done' ? 'text-green-300' : state === 'running' ? 'text-white' : 'text-[#7d92b0]'}`}>{s.title}</p>
                  <p className="text-xs text-[#3d5068] truncate">{s.description}</p>
                </div>
                {state === 'running' && <RefreshCw className="w-3.5 h-3.5 text-[#e8002d] animate-spin flex-shrink-0" />}
                {state === 'done' && !done && <CheckCircle className="w-3.5 h-3.5 text-green-400 flex-shrink-0" />}
              </div>
            )
          })}
          {done && (
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-[#1e2d42]">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-sm text-[#7d92b0] hover:text-white border border-[#1e2d42] transition-colors">閉じる</button>
...
export default function PlaybookBuilderPage() {
  ...
  return (
    <div className="min-h-screen bg-[#070d19] text-[#7d92b0] flex flex-col">
      {/* Toast */}
      {savedToast && (
      ...
      {/* Header */}
      <div className="border-b border-[#1e2d42] px-8 py-6 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#7d92b0] mt-0.5">インシデント対応プレイブックの作成・編集</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-72 flex-shrink-0 border-r border-[#1e2d42] flex flex-col bg-[#0d1220]">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#1e2d42]">
            <span className="text-xs font-semibold text-[#7d92b0] uppercase tracking-wider">プレイブック</span>
            <button onClick={createNew}
              className="flex items-center gap-1 px-2 py-1 rounded text-xs bg-[#e8002d] hover:bg-[#c0001f] text-white font-medium transition-colors">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {playbooks.map(pb => (
              <button key={pb.id} onClick={() => setSelectedPB({ ...pb })}
                className={`w-full text-left p-3 rounded-lg border transition-all ${selectedPB?.id === pb.id ? 'bg-[#1d2f4a] border-[#e8002d]/30 text-white' : 'bg-[#070d19] border-[#1e2d42] text-[#7d92b0] hover:border-[#7d92b0]/30 hover:text-white'}`}>
                <div className="flex items-start justify-between gap-2">
                  <span className="text-sm font-medium truncate">{pb.name}</span>
                  <span className={`flex-shrink-0 px-1.5 py-0.5 rounded text-[9px] font-bold border ${pb.is_active ? 'bg-green-500/10 text-green-400 border-green-500/30' : 'bg-[#1e2d42] text-[#3d5068] border-[#1e2d42]'}`}>
                    {pb.is_active ? '有効' : '無効'}
                  </span>
                </div>
                <div className="flex items-center gap-2 mt-1.5">
                  <span className="text-[10px] text-[#3d5068]">{pb.steps.length}ステップ</span>
                  <span className="text-[10px] bg-[#1e2d42] text-[#7d92b0] px-1.5 py-0.5 rounded">{TRIGGER_TYPE_LABELS[pb.trigger.trigger_type]}</span>
                </div>
                <p className="text-[10px] text-[#3d5068] mt-1">{new Date(pb.last_updated).toLocaleDateString('ja-JP')}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        {selectedPB ? (
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* Toolbar */}
            <div className="flex items-center gap-3 px-6 py-3 border-b border-[#1e2d42] bg-[#070d19] flex-shrink-0">
              <div className="flex-1 flex items-center gap-2">
                {editingName ? (
                  <input value={selectedPB.name}
                    ...
                    className="text-white font-semibold bg-transparent border-b border-[#e8002d]/50 focus:outline-none text-sm" />
                ) : (
                  <button onClick={() => setEditingName(true)} className="text-white font-semibold hover:text-[#e8002d] transition-colors text-sm flex items-center gap-1">
                    {selectedPB.name} <Pencil className="w-3 h-3 opacity-60" />
                  </button>
                )}
                <button onClick={() => setShowTriggerModal(true)}
                  className="flex items-center gap-1 px-2 py-1 rounded text-[10px] bg-[#1e2d42] text-[#7d92b0] hover:text-white border border-[#1e2d42] transition-colors">
                  トリガー: {TRIGGER_TYPE_LABELS[selectedPB.trigger.trigger_type]}
                  {selectedPB.trigger.alert_severity && <span className="text-[#e8002d]"> {selectedPB.trigger.alert_severity}</span>}
                </button>
              </div>
              <button onClick={() => setSelectedPB(p => p ? { ...p, is_active: !p.is_active } : p)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs border transition-colors ${selectedPB.is_active ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-[#1e2d42] border-[#1e2d42] text-[#7d92b0]'}`}>
              ...
              <button onClick={handleSave}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-[#e8002d] hover:bg-[#c0001f] text-white transition-colors">
            ...
            <div className="flex-1 overflow-y-auto px-8 py-6">
              ...
              <div className="max-w-lg mx-auto mb-6">
                <textarea value={selectedPB.description}
                  ...
                  rows={2}
                  className="w-full bg-transparent border-b border-[#1e2d42] focus:border-[#7d92b0]/50 text-[#7d92b0] text-sm focus:outline-none resize-none placeholder-[#3d5068]" />
              </div>
              ...
              <div className="max-w-lg mx-auto">
                ...
                {selectedPB.steps.length === 0 && (
                  <div className="text-center py-12">
                    <GitBranch className="w-10 h-10 mx-auto mb-3 text-[#1e2d42]" />
                    <p className="text-[#3d5068] mb-4">ステップがありません</p>
                    <button onClick={() => { setEditingStep(undefined); setInsertAfterIndex(null); setShowStepModal(true) }}
                      className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm bg-[#e8002d] hover:bg-[#c0001f] text-white mx-auto transition-colors">
                ...
                {selectedPB.steps.length > 0 && (
                  <div className="flex flex-col items-center mt-1">
                    <div className="w-px h-4 bg-[#1e2d42]" />
                    <button onClick={() => { setEditingStep(undefined); setInsertAfterIndex(null); setShowStepModal(true) }}
                      className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm bg-[#0d1220] hover:bg-[#1e2d42] text-[#7d92b0] hover:text-white border border-dashed border-[#1e2d42] hover:border-[#7d92b0]/50 transition-colors mt-1">
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <GitBranch className="w-12 h-12 mx-auto mb-4 text-[#1e2d42]" />
              <p className="text-[#7d92b0] mb-2">プレイブックを選択するか新規作成してください</p>
              <button onClick={createNew}
                className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm bg-[#e8002d] hover:bg-[#c0001f] text-white mx-auto transition-colors">

    ...
    <div className="flex flex-col items-center">
      <div className={`w-full max-w-lg bg-[#0d1220] border rounded-xl p-4 group hover:border-[#7d92b0]/30 transition-colors ${cfg.border}`}>
        <div className="flex items-start gap-3">
          {/* Drag handle */}
          <div className="shrink-0 mt-1 text-[#3d5068] group-hover:text-[#7d92b0] transition-colors cursor-grab">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] text-[#7d92b0] bg-[#1e2d42] px-1.5 py-0.5 rounded-sm">{ACTION_TYPE_LABELS[step.action_type]}</span>
            </div>
            ...
            {step.parameters.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1">
                {step.parameters.map((p, pi) => (
                  <span key={pi} className="text-[10px] font-mono bg-[#070d19] border border-[#1e2d42] px-1.5 py-0.5 rounded-sm text-[#7d92b0]">
                    {p.key}: {p.value}
                  </span>
                ))}
              </div>
          ...
          <div className="shrink-0 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={onEdit} className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-white transition-colors">
              <Pencil className="w-3.5 h-3.5" />
            </button>
            <button onClick={onDelete} className="p-1.5 rounded-sm hover:bg-red-900/20 text-[#7d92b0] hover:text-red-400 transition-colors">
            ...
            <div className="space-y-2">
              {form.parameters.map((p, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="font-mono text-xs bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1 text-[#7d92b0] flex-1">{p.key}</span>
                  <span className="text-[#3d5068]">:</span>
                  <span className="font-mono text-xs bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1 text-white flex-1">{p.value}</span>
                  <button onClick={() => removeParam(i)} className="p-1 rounded-sm hover:bg-red-900/20 text-[#7d92b0] hover:text-red-400 transition-colors"><X className="w-3 h-3" /></button>
                </div>
              ))}
              <div className="flex items-center gap-2">
                <input value={newParamKey} onChange={e => setNewParamKey(e.target.value)} placeholder="キー"
                  className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1 text-white text-xs font-mono focus:outline-hidden focus:border-[#e8002d]/50" />
                <span className="text-[#3d5068]">:</span>
                <input value={newParamVal} onChange={e => setNewParamVal(e.target.value)} placeholder="値"
                  className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1 text-white text-xs font-mono focus:outline-hidden focus:border-[#e8002d]/50" />
                <button onClick={addParam} className="p-1.5 rounded-sm bg-[#1e2d42] hover:bg-[#e8002d]/20 text-[#7d92b0] hover:text-[#e8002d] transition-colors"><Plus className="w-3 h-3" /></button>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div key={s.id} className={`flex items-center gap-3 p-3 rounded-lg border transition-all ${
                ...
                {state === 'running' && <RefreshCw className="w-3.5 h-3.5 text-[#e8002d] animate-spin shrink-0" />}
...
export default function PlaybookBuilderPage() {
  ...
  return (
    <div className="min-h-screen bg-[#070d19] text-[#7d92b0] flex flex-col">
      ...
      {/* Header */}
      <div className="border-b border-[#1e2d42] px-8 py-6 shrink-0">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-72 shrink-0 border-r border-[#1e2d42] flex flex-col bg-[#0d1220]">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#1e2d42]">
            ...
            <button onClick={createNew}
              className="flex items-center gap-1 px-2 py-1 rounded-sm text-xs bg-[#e8002d] hover:bg-[#c0001f] text-white font-medium transition-colors">
            ...
            {playbooks.map(pb => (
              <button key={pb.id} onClick={() => setSelectedPB({ ...pb })}
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className={`shrink-0 px-1.5 py-0.5 rounded-sm text-[9px] font-bold border ${pb.is_active ? 'bg-green-500/10 text-green-400 border-green-500/30' : 'bg-[#1e2d42] text-[#3d5068] border-[#1e2d42]'}`}>
                    {pb.is_active ? '有効' : '無効'}
                  </span>
                </div>
                <div className="flex items-center gap-2 mt-1.5">
                  <span className="text-[10px] text-[#3d5068]">{pb.steps.length}ステップ</span>
                  <span className="text-[10px] bg-[#1e2d42] text-[#7d92b0] px-1.5 py-0.5 rounded-sm">{TRIGGER_TYPE_LABELS[pb.trigger.trigger_type]}</span>
                </div>
        ...
        {selectedPB ? (
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* Toolbar */}
            <div className="flex items-center gap-3 px-6 py-3 border-b border-[#1e2d42] bg-[#070d19] shrink-0">
              <div className="flex-1 flex items-center gap-2">
                {editingName ? (
                  <input value={selectedPB.name}
                    ...
                    className="text-white font-semibold bg-transparent border-b border-[#e8002d]/50 focus:outline-hidden text-sm" />
                ) : (
                ...
                <button onClick={() => setShowTriggerModal(true)}
                  className="flex items-center gap-1 px-2 py-1 rounded-sm text-[10px] bg-[#1e2d42] text-[#7d92b0] hover:text-white border border-[#1e2d42] transition-colors">
            ...
            <div className="flex-1 overflow-y-auto px-8 py-6">
              ...
              <div className="max-w-lg mx-auto mb-6">
                <textarea value={selectedPB.description}
                  ...
                  rows={2}
                  className="w-full bg-transparent border-b border-[#1e2d42] focus:border-[#7d92b0]/50 text-[#7d92b0] text-sm focus:outline-hidden resize-none placeholder-[#3d5068]" />
              </div>

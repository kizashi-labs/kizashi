        ...
        <div className="flex items-center justify-between p-5 border-b border-[#1e2d42]">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className={`px-2 py-0.5 rounded text-xs font-medium ${FEED_TYPE_STYLES[feed.feed_type].bg} ${FEED_TYPE_STYLES[feed.feed_type].text}`}>
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-5">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              <div key={m.label} className="bg-[#070d19] border border-[#1e2d42] rounded p-3 text-center">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-[#070d19] border border-[#1e2d42] rounded p-4">
              ...
              {([
                ...
                return (
                  <div key={key} className="flex items-center gap-3">
                    <Icon className="w-4 h-4 text-[#7d92b0] flex-shrink-0" />
                    <span className="text-[#7d92b0] text-sm w-28 flex-shrink-0">{label}</span>
              ...
              <ul className="space-y-2">
                {recommendations.map((r, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-[#7d92b0]">
                    <ChevronRight className="w-4 h-4 text-[#e8002d] flex-shrink-0 mt-0.5" />{r}
                  </li>
                ))}
              </ul>
            </div>
              ...
              {feeds.map(f => {
                ...
                return (
                  <tr key={f.id} onClick={() => setSelectedFeed(f)}
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${ft.bg} ${ft.text}`}>{ft.label}</span>
                    </td>
                    ...
                    <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          title="手動同期"
                          className="p-1.5 rounded bg-blue-900/40 hover:bg-blue-900/70 text-blue-300 disabled:opacity-40 transition-colors"
                        >
                        ...
                        <button
                          ...
                          className={`p-1.5 rounded transition-colors ${f.status === 'disabled' ? 'bg-green-900/40 hover:bg-green-900/70 text-green-300' : 'bg-red-900/30 hover:bg-red-900/60 text-red-400'}`}
                        >
                          <ShieldOff className="w-3.5 h-3.5" />
                        </button>
                      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
          ...
          <div className="space-y-3 mb-5">
            {IOC_OVERLAP.map(o => (
              <div key={o.label} className="flex items-center justify-between p-3 bg-[#070d19] border border-[#1e2d42] rounded">
        ...
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
          ...
          <div className="space-y-4">
            <div className="p-4 bg-[#070d19] border border-[#1e2d42] rounded">
              <p className="text-[#7d92b0] text-sm mb-1">年間総コスト</p>
              <p className="text-white text-2xl font-bold">{fmtYen(annualCost)}</p>
            </div>
            {potentialSavings > 0 && (
              <div className="p-4 bg-green-900/10 border border-green-800/30 rounded">
              ...
              {(() => {
                ...
                return roiFeeds.map(f => (
                  <div key={f.id} className="flex items-center gap-3">
                    <span className="text-[#7d92b0] text-xs w-32 truncate flex-shrink-0">{f.feed_name.split(' ')[0]}</span>

        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2.5 text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-hidden focus:border-[#1a6bff] transition-colors"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2.5 text-sm text-[#e2e8f4] focus:outline-hidden focus:border-[#1a6bff] transition-colors"
            />
          </div>
      ...
      <div className="bg-[#111827] border border-[#1e2d42] rounded-xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg pl-9 pr-3 py-2.5 text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-hidden focus:border-[#1a6bff] transition-colors"
          />
        </div>
...
export default function GeoBlockingPage() {
  ...
  const { data: apiConfig } = useQuery<GeoConfig | undefined>({
    ...
    queryFn: () => apiFetch<GeoConfig>('/api/v1/admin/geo-blocking/config'),
    staleTime: 30_000,
  })

  const { data: apiCountries } = useQuery<CountryRule[] | undefined>({
    queryKey: ['geo-blocking-countries'],
    queryFn: () => apiFetchList<CountryRule>('/api/v1/admin/geo-blocking/countries'),
    staleTime: 30_000,
  })
  ...
  // 30秒ごとにアクセスログを再取得します。
  //
  // 以前はここで、30秒ごとに4割の確率で遮断されたアクセスを1件
  // 「発生させて」いました。IP は乱数の4オクテット、国は設定済みの
  // 遮断国から無作為、パスは /api/v1/auth/login など4つから、
  // User-Agent は curl や python-requests から選ばれます。それが
  // 運用担当の読んでいる遮断ログの先頭に積まれていきます。
  //
  // API の失敗時ではなく常時動いていました。つまりこの画面の遮断ログは、
  // 実データの上に架空の攻撃者が混ざった状態で表示され続けていました。
  // 「103.44.201.18 から /api/v1/auth/login へ curl で」という行を
  // ブロックリストの根拠にはできません。
  useEffect(() => {
    const id = setInterval(() => setAutoRefreshCount(n => n + 1), 30_000)
    return () => clearInterval(id)
  }, [])

  // ── Handlers ─────────────────────────────────────────────────

  // 以下の4つは、ローカルの状態を先に変えてから保存し、保存の失敗を
  // .catch(() => {}) で捨てていました。/api/v1/admin/geo-blocking/* は
  // サーバに1本もルートが無いので、この保存は毎回失敗します。管理者は
  // 遮断対象の国を足したつもり、ジオブロッキングを有効にしたつもりで、
  // 画面を閉じればすべて消えます。保存できたときだけ画面を進めます。

  async function toggleEnabled() {
    const next = { ...config, enabled: !config.enabled, updated_at: new Date().toISOString() }
    setConfirmToggleOpen(false)
    if (await persist('ジオブロッキングの有効/無効', '/api/v1/admin/geo-blocking/config', {
      method: 'PUT',
  ...
  async function changeMode(mode: GeoConfig['mode']) {
    const next = { ...config, mode, updated_at: new Date().toISOString() }
    if (await persist('ジオブロッキングのモード', '/api/v1/admin/geo-blocking/config', {
      method: 'PUT',
      body: JSON.stringify(next),
    })) {
      setConfig(next)
  ...
  async function addCountry(code: string, name: string, action: 'block' | 'allow') {
    ...
    if (await persist(`${name}の遮断ルール`, '/api/v1/admin/geo-blocking/countries', {
      method: 'POST',
  ...
  async function removeCountry(id: string) {
    if (await persist('遮断ルールの削除', `/api/v1/admin/geo-blocking/countries/${id}`, {
      method: 'DELETE',
    })) {
            ...
            <div className="flex items-center justify-between p-4 border-b border-[#1e2d42]">
              <div className="relative w-72">
                ...
                <input
                  ...
                  placeholder="国名またはコードで検索..."
                  className="w-full bg-[#080e1a] border border-[#1e2d42] rounded-lg pl-9 pr-3 py-2 text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-hidden focus:border-[#1a6bff] transition-colors"
                />
              </div>
              <button
                ...
                {filteredCountries.map(c => (
                  <tr key={c.id} className="hover:bg-[#080e1a]/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs font-mono text-[#7d92b0] bg-[#080e1a] px-2 py-1 rounded-sm">{c.country_code}</span>
                    </td>
                    <td className="px-4 py-3">
                      <button

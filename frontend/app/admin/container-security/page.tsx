...
export default function ContainerSecurityPage() {
  ...
  const { data: images = [], isLoading: imagesLoading } = useQuery<ContainerImage[]>({
    ...
    queryFn: () => apiFetchList<ContainerImage>('/api/v1/admin/container-security/images'),
  })
  ...
  const scanMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      return await apiFetch(`/api/v1/admin/container-security/images/${id}/scan`, { method: 'POST' })
    },
  ...
  const { data: policies = [], isLoading: policiesLoading } = useQuery<ContainerPolicy[]>({
    ...
    queryFn: () => apiFetchList<ContainerPolicy>('/api/v1/admin/container-security/policies'),
  })

  const togglePolicyMutation = useMutation({
    mutationFn: async (id: string) => {
      return await apiFetch(`/api/v1/admin/container-security/policies/${id}/toggle`, { method: 'POST' })
    },
  ...
  const scanAllMutation = useMutation({
    mutationFn: async () => {
      ...
      // .catch(() => null) が1件ずつの失敗を飲み込むので、全件失敗しても
      // Promise.all は成功し、onSuccess が走って一覧を再取得します。
      // 「全イメージをスキャンした」と見えて、1件も走っていません。
      try {
        await Promise.all(ids.map(id =>
          apiFetch(`/api/v1/admin/container-security/images/${id}/scan`, { method: 'POST' })
        ))
      } finally {
  ...
  return (
    <div className="min-h-screen bg-[#070d19] p-6">
      <PageDataUnavailable />
      <PageSaveFailed />
      <SaveFailed error={saveErrorOf('イメージのスキャン', scanAllMutation)} />
      {/* Header */}
                ...
                <tbody>
                  {events.length === 0 ? (
                    ...
                    <tr key={ev.id} className="border-b border-[#1e2d42]/60 last:border-0 hover:bg-[#070d19]/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-sm text-xs border font-mono bg-[#070d19] border-[#1e2d42] text-[#7d92b0]">
                          {ev.namespace}
                        </span>
                      </td>
                      <td className="px-4 py-3">

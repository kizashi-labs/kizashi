...
export default function DLPPage() {
  ...
  const { data: rules = [], isLoading: rulesLoading } = useQuery<DLPRule[]>({
    queryKey: ['dlp-rules'],
    queryFn: async () => {
      try { return await apiFetchList<DLPRule>('/api/v1/admin/dlp/rules') } catch { return [] }
    },
    staleTime: 30_000,
    retry: false,
  })
  ...
  const { data: violations = [], isLoading: violationsLoading } = useQuery<DLPViolation[]>({
    ...
    queryFn: async () => {
      try { return await apiFetchList<DLPViolation>('/api/v1/admin/dlp/violations') } catch { return [] }
    },
    staleTime: 30_000,
    retry: false,
  })

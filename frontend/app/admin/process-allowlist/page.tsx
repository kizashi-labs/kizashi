    ...
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-falcon-muted text-sm ml-11">プロセス名・パス・署名者による許可/拒否ルール</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: '拒否ルール', value: stats.deny, icon: ShieldOff, color: 'text-falcon-red' },
      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-lg overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="p-10 text-center text-falcon-muted text-sm">ルールがありません</div>
        ) : (
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-falcon-border">
                {rules.map(rule => (
                  ...
                  >
                    <td className="px-4 py-3 text-sm text-falcon-text max-w-[140px] truncate" title={rule.name}>{rule.name}</td>
                    <td className="px-4 py-3">
                      <span className="font-mono text-sm text-falcon-text">{rule.process_name}</span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-xs">
                      <span className={{ critical: 'text-falcon-red', high: 'text-orange-400', medium: 'text-yellow-400', low: 'text-green-400' }[rule.severity] ?? 'text-falcon-muted'}>
                        {{ critical: 'クリティカル', high: '高', medium: '中', low: '低' }[rule.severity] ?? rule.severity}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs font-medium bg-falcon-border text-falcon-muted border border-falcon-border">
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className="p-1 rounded-sm transition-colors hover:bg-falcon-border"
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs">
          <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-lg mx-4 shadow-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="px-5 py-4 space-y-4 overflow-y-auto">
              ...
              <div>
                <label className="block text-xs font-medium text-falcon-muted mb-1.5">
                  ...
                  <span className="text-falcon-subtle ml-1 font-normal">(glob パターン可)</span>
                </label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                        ...
                        className={`flex-1 py-1.5 rounded text-xs font-medium transition-colors border ${
                          form.rule_type === t
                            ? t === 'allow'
                              ...
                              : 'bg-falcon-red/20 text-falcon-red border-falcon-red/30'
                ...
                <div>
                  <label className="block text-xs font-medium text-falcon-muted mb-1.5">アクション</label>
                  <div className="relative">
                    <select
                      value={form.action}
                      onChange={e => setForm(f => ({ ...f, action: e.target.value as FormState['action'] }))}
                      className="w-full appearance-none bg-[#070d19] border border-falcon-border rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden focus:border-falcon-red/50 pr-8"
                    >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-falcon-muted mb-1.5">重大度</label>
                  <div className="relative">
                    <select
                      value={form.severity}
                ...
                <div>
                  <label className="block text-xs font-medium text-falcon-muted mb-1.5">スコープ</label>
                  <div className="relative">
                    <select
                      value={form.scope}
            ...
            <div className="flex justify-end gap-3 px-5 py-4 border-t border-falcon-border shrink-0">
              <button

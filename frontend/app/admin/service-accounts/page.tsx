...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="p-1 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-[#e2e8f4] transition-colors shrink-0"
      title="コピー"
    >
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#0d1220] border border-[#e8002d]/40 rounded-xl w-full max-w-md mx-4 shadow-2xl">
        <div className="flex items-center gap-3 px-5 py-4 border-b border-[#1e2d42]">
          <div className="w-8 h-8 rounded-lg bg-[#e8002d]/10 flex items-center justify-center shrink-0">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2.5">
              ...
              <button
                onClick={copy}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-sm bg-[#e8002d] hover:bg-[#c8001d] text-white text-xs font-medium transition-colors shrink-0"
              >
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg mx-4 shadow-2xl">
        ...
        <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-hidden focus:border-[#e8002d]/60 transition-colors"
            />
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea
              ...
              rows={2}
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-hidden focus:border-[#e8002d]/60 transition-colors resize-none"
            />
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-hidden focus:border-[#e8002d]/60 transition-colors resize-none font-mono"
            />
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-[#e2e8f4] focus:outline-hidden focus:border-[#e8002d]/60 transition-colors [color-scheme:dark]"
            />
        ...
        <div className="px-5 py-4 border-t border-[#1e2d42] flex gap-3">
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-[#e8002d] hover:bg-[#c8001d] text-white text-sm font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
...
export default function ServiceAccountsPage() {
  ...
  const { data: displayAccounts = [], isLoading } = useQuery<ServiceAccount[]>({
    ...
    queryFn: () => apiFetchList<ServiceAccount>('/api/v1/admin/service-accounts'),
    staleTime: 60_000,
  })
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      setShowCreate(false)
      // サーバが返さなかったときに作らないこと。控えて配った鍵が
      // サーバの知らない値だと、回したつもりのまま古い鍵が生き続けます。
      setSecretValue(data.client_secret ?? null)
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      apiFetch(`/api/v1/admin/service-accounts/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['service-accounts'] })
                      ...
                      <tr key={i} className="animate-pulse">
                        {Array.from({ length: 8 }).map((_, j) => (
                          <td key={j} className="px-4 py-4">
                            <div className="h-4 bg-[#1e2d42] rounded-sm w-full" />
                          </td>
                        ))}
                      </tr>
                    ))
                      ...
                      return (
                        <tr key={acct.id} className="hover:bg-[#0a1120] transition-colors">
                          ...
                          <td className="px-4 py-3 whitespace-nowrap">
                            {acct.expires_at ? (
                              <div className="flex items-center gap-1.5">
                                <CalendarDays className={`w-3.5 h-3.5 shrink-0 ${expired ? 'text-[#e8002d]' : nearExpiry ? 'text-[#f59e0b]' : 'text-[#7d92b0]'}`} />
                          ...
                          <td className="px-4 py-3">
                            ...
                            >
                              <span className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-[#e2e8f4] shadow-sm transition-transform duration-200 ${acct.enabled ? 'translate-x-4' : 'translate-x-0'}`} />
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1">
                              ...
                              <button
                                ...
                                title="シークレット再生成"
                                className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-[#f59e0b] transition-colors"
                              >

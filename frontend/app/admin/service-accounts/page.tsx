    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/60 backdrop-blur-xs">
      <div className="bg-falcon-surface border border-falcon-red/40 rounded-xl w-full max-w-md mx-4 shadow-2xl">
        <div className="flex items-center gap-3 px-5 py-4 border-b border-falcon-border">
          <div className="w-8 h-8 rounded-lg bg-falcon-red/10 flex items-center justify-center shrink-0">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2.5">
              ...
              <button
                onClick={copy}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-sm bg-falcon-red hover:bg-[#c8001d] text-white text-xs font-medium transition-colors shrink-0"
              >
      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-lg mx-4 shadow-2xl">
        <div className="flex items-center gap-3 px-5 py-4 border-b border-falcon-border">
          <div className="w-8 h-8 rounded-lg bg-falcon-red/10 flex items-center justify-center">
            <Key className="w-4 h-4 text-falcon-red" />
          </div>
        ...
        <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea
              ...
              rows={2}
              className="w-full bg-[#070d19] border border-falcon-border rounded px-3 py-2 text-sm text-falcon-text placeholder-falcon-subtle
                         focus:outline-hidden focus:border-falcon-red/60 transition-colors resize-none"
            />
          </div>

          {/* Scopes */}
          <div className="flex flex-col gap-2">
            <label className="text-xs font-medium text-falcon-muted uppercase tracking-wide">スコープ <span className="text-falcon-red">*</span></label>
              ...
              {ALL_SCOPES.map(scope => (
                <label key={scope.value} className="flex items-start gap-2.5 cursor-pointer group">
                  <input
                    ...
                    className="mt-0.5 w-4 h-4 accent-falcon-red"
                  />
                  <div>
          ...
          {/* Allowed IPs */}
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-falcon-muted uppercase tracking-wide">許可IPアドレス</label>
            <textarea
              ...
              rows={3}
              className="w-full bg-[#070d19] border border-falcon-border rounded px-3 py-2 text-sm text-falcon-text placeholder-falcon-subtle
                         focus:outline-hidden focus:border-falcon-red/60 transition-colors resize-none font-mono"
            />
            <p className="text-xs text-falcon-subtle">1行につき1つ。CIDR記法 (例: 10.0.0.0/8) または単一IP</p>
          </div>

          {/* Expires At */}
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-falcon-muted uppercase tracking-wide">有効期限 (オプション)</label>
            <input
              type="date"
              ...
              className="w-full bg-[#070d19] border border-falcon-border rounded px-3 py-2 text-sm text-falcon-text
                         ...
                         scheme-dark"
            />
            <p className="text-xs text-falcon-subtle">空白の場合は無期限</p>
          </div>
        </div>
        ...
        <div className="px-5 py-4 border-t border-falcon-border flex gap-3">
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-falcon-red hover:bg-[#c8001d]
...
curl https://api.falconedr.example.com/api/v1/alerts \\
  ...
  return (
    <div className="bg-falcon-surface border border-falcon-border rounded-lg overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          <Terminal className="w-4 h-4 text-falcon-muted" />
          <span className="text-sm font-medium text-falcon-text">使用ガイド: Client Credentials 認証</span>
        </div>
        ...
        <div className="px-5 pb-5">
          <div className="bg-[#070d19] border border-falcon-border rounded-lg overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2 border-b border-falcon-border">
              <span className="text-xs text-falcon-subtle font-mono">bash</span>
              <CopyButton text={snippet} />
            </div>
            <pre className="p-4 text-xs text-falcon-muted font-mono overflow-x-auto whitespace-pre leading-relaxed">
              {snippet}
            </pre>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-[#070d19] text-falcon-text">
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-falcon-text tracking-tight">サービスアカウント管理</h1>
            <p className="text-sm text-falcon-muted mt-1">機械間認証・API自動化用サービスアカウントの管理</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { label: '総アカウント数', value: String(total), icon: Users, color: 'text-falcon-blue' },
            { label: 'アクティブ', value: String(active), icon: CheckCircle2, color: 'text-falcon-green' },
            { label: '期限切れ', value: String(expired), icon: AlertCircle, color: expired > 0 ? 'text-falcon-red' : 'text-falcon-muted', danger: expired > 0 },
            {
              label: '最終使用',
            ...
            <div key={stat.label} className={`bg-falcon-surface border rounded-lg p-4 ${(stat as any).danger ? 'border-falcon-red/30' : 'border-falcon-border'}`}>
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs text-falcon-muted">{stat.label}</p>
                <stat.icon className={`w-4 h-4 ${stat.color}`} />
              </div>
              <p className={`text-xl font-bold ${(stat as any).danger ? 'text-falcon-red' : 'text-falcon-text'}`}>{stat.value}</p>
            </div>
          ))}
        </div>
                      ...
                      return (
                        <tr key={acct.id} className="hover:bg-[#0a1120] transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <div>
                              <p className="font-medium text-falcon-text">{acct.name}</p>
                              {acct.description && (
                                <p className="text-xs text-falcon-muted mt-0.5 max-w-[160px] truncate" title={acct.description}>
                                  {acct.description}
                                </p>
                              )}
                            </div>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              <code className="text-xs font-mono text-falcon-muted">{maskClientId(acct.client_id)}</code>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex flex-wrap gap-1 max-w-[200px]">
                              ...
                              {acct.scopes.length > 3 && (
                                <Badge color="border-falcon-border text-falcon-subtle">+{acct.scopes.length - 3}</Badge>
                              )}
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3">
                            {acct.allowed_ips.length === 0
                              ? <span className="text-xs text-falcon-subtle">制限なし</span>
                              : (
                                <div className="flex items-center gap-1">
                                  <Network className="w-3 h-3 text-falcon-muted" />
                                  <span className="text-xs text-falcon-muted">{acct.allowed_ips.length} CIDR</span>
                                </div>
                              )
                            }
                          </td>
                          ...
                          <td className="px-4 py-3 whitespace-nowrap">
                            {acct.expires_at ? (
                              <div className="flex items-center gap-1.5">
                                <CalendarDays className={`w-3.5 h-3.5 shrink-0 ${expired ? 'text-falcon-red' : nearExpiry ? 'text-[#f59e0b]' : 'text-falcon-muted'}`} />
                                <span className={`text-xs ${expired ? 'text-falcon-red font-medium' : nearExpiry ? 'text-[#f59e0b]' : 'text-falcon-muted'}`}>
                          ...
                          <td className="px-4 py-3">
                            <div
                              ...
                              className={`relative w-9 h-5 rounded-full cursor-pointer transition-colors duration-200 ${acct.enabled ? 'bg-falcon-green' : 'bg-falcon-border'}`}
                            >
                              <span className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-falcon-text shadow-sm transition-transform duration-200 ${acct.enabled ? 'translate-x-4' : 'translate-x-0'}`} />
                            </div>
                          </td>

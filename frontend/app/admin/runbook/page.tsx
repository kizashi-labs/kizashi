  ...
  return (
    ...
    >
      ...
      <span className="flex items-start gap-2">
        <span className="shrink-0 w-5 h-5 rounded-full bg-[#e8002d]/20 text-[#e8002d] text-[11px] font-bold flex items-center justify-center mt-0.5">
          {index + 1}
        </span>
        ...
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
          ...
          <div className="flex flex-wrap gap-1 mt-3">
            {article.tags.map(tag => (
              <span key={tag} className="px-2 py-0.5 bg-[#1e2d42] text-[#7d92b0] rounded-sm text-xs">#{tag}</span>
            ))}
          </div>
        </div>
            ...
            <div className="mt-4 space-y-2">
              {article.versions.map(v => (
                <div
                  ...
                  className={`p-3 rounded-sm border cursor-pointer transition-colors ${activeVersion === v.version ? 'border-[#e8002d]/40 bg-[#e8002d]/5' : 'border-[#1e2d42] hover:border-[#2e4060]'}`}
                >
        ...
        {related.length > 0 && (
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
            ...
            <div className="space-y-2">
              {related.map(r => (
                <div key={r.id} className="flex items-center gap-3 p-2 rounded-sm border border-[#1e2d42] hover:border-[#2e4060]">
            ...
            {tocItems.map(item => (
              <li key={item.id}>
                <button
                  ...
                  className="w-full text-left text-xs text-[#7d92b0] hover:text-white py-1 px-2 rounded-sm hover:bg-[#1e2d42] transition-colors"
                >
                  {item.heading}
                </button>
              </li>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 mb-4 pb-4 border-b border-[#1e2d42]">
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-sm text-xs transition-colors ${preview ? 'bg-[#1e2d42] text-white' : 'text-[#7d92b0] hover:text-white'}`}
        >
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-1.5 rounded-sm bg-[#1e2d42] text-[#7d92b0] hover:bg-[#2e3d52] hover:text-white text-xs transition-colors"
        >
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-1.5 rounded-sm bg-[#e8002d] text-white hover:bg-[#c00025] text-xs transition-colors"
        >
          <Send className="w-3.5 h-3.5" /> 公開
        </button>
        <button onClick={onCancel} className="p-1.5 rounded-sm text-[#7d92b0] hover:text-white hover:bg-[#1e2d42] transition-colors">
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-4">
          ...
          <div key={section.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="flex-1 bg-transparent border-b border-[#1e2d42] text-white text-sm font-semibold py-0.5 focus:outline-hidden focus:border-[#e8002d]/50"
              />
                ...
                {section.steps.map((step, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1 text-[#c8d6e8] text-xs focus:outline-hidden focus:border-[#e8002d]/50"
                    />
        ...
        <button
          onClick={addSection}
          className="w-full flex items-center justify-center gap-2 p-3 rounded-sm border border-dashed border-[#1e2d42] text-[#3d5068] hover:border-[#e8002d]/40 hover:text-[#e8002d] transition-colors text-sm"
        >
  ...
  return (
    <div className="min-h-screen bg-[#070d19] text-white flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left Sidebar */}
        <aside className="w-56 shrink-0 border-r border-[#1e2d42] flex flex-col">
          <div className="p-3 border-b border-[#1e2d42]">
            <div className="relative">
              ...
              <input
                ...
                placeholder="手順書を検索..."
                className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-sm pl-8 pr-3 py-2 text-sm text-white placeholder-[#3d5068] focus:outline-hidden focus:border-[#e8002d]/50"
              />
            </div>
          </div>
          <nav className="flex-1 overflow-y-auto p-2">
            {CATEGORIES.map(({ key, label, icon: Icon }) => (
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-sm text-sm transition-colors mb-0.5
                  ${selectedCategory === key ? 'bg-[#1d2f4a] text-white' : 'text-[#7d92b0] hover:bg-[#19253d] hover:text-white'}`}
              >
                <Icon className={`w-4 h-4 shrink-0 ${selectedCategory === key ? 'text-[#e8002d]' : 'text-[#3d5068]'}`} />
                <span className="flex-1 text-left">{label}</span>
                <span className="text-xs text-[#3d5068] bg-[#0d1220] px-1.5 py-0.5 rounded-sm">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 mt-2 text-xs text-[#3d5068]">
                          <span className="flex items-center gap-1">
                            <div className="w-5 h-5 rounded-full bg-linear-to-br from-[#1a6bff] to-[#0044cc] flex items-center justify-center text-[9px] font-bold text-white">

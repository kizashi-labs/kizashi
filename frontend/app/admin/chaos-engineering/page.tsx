      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl mx-4 shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-5 flex-1">
          <div className="flex flex-wrap gap-2">
            ...
            <span className="text-xs px-2 py-0.5 rounded border border-[#1e2d42] text-[#7d92b0]">目安 {exp.estimated_duration_min}分</span>
          </div>
            ...
            <ol className="space-y-1.5">
              {exp.execution_steps.map((s, i) => (
                <li key={i} className="flex items-start gap-2.5 text-sm text-[#e2e8f4]">
                  <span className="w-5 h-5 rounded-full bg-[#1e2d42] text-[#7d92b0] text-xs flex items-center justify-center flex-shrink-0 mt-0.5">{i + 1}</span>
                  {s}
                </li>
              ))}
            </ol>
        ...
        <div className="px-6 py-4 border-t border-[#1e2d42] flex items-center justify-end gap-3 flex-shrink-0">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <select multiple value={approvers} onChange={e => setApprovers(Array.from(e.target.selectedOptions, o => o.value))}
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/50 h-28">
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl mx-4 shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-5 flex-1">
          <div className="flex items-center gap-3">
            ...
            {run.rollback_taken && <span className="text-xs px-2 py-0.5 rounded border border-blue-500/30 bg-blue-500/10 text-blue-400">ロールバック実施</span>}
          </div>
            ...
            {filteredExps.map(exp => (
              <div key={exp.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 hover:border-[#2a3f5e] transition-colors">
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  <Badge cfg={CATEGORY_CONFIG[exp.category]} />
                  <Badge cfg={SEVERITY_CONFIG[exp.severity_impact]} />
                  <span className="text-xs px-2 py-0.5 rounded border border-[#1e2d42] text-[#7d92b0]">{exp.target_type}</span>
                  <span className="text-xs px-2 py-0.5 rounded border border-[#1e2d42] text-[#7d92b0] flex items-center gap-1">
              ...
              <div className="mt-4 space-y-2">
                {SAFETY_RULES.map(rule => (
                  <div key={rule.id} className="flex items-center gap-3 p-3 bg-[#070d19] rounded-lg border border-[#1e2d42]">
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 ${rule.active ? 'bg-green-400' : 'bg-[#3d5068]'}`} />

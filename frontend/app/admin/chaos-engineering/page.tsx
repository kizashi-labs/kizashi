      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-2xl mx-4 shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-5 flex-1">
          <div className="flex flex-wrap gap-2">
            ...
            <span className="text-xs px-2 py-0.5 rounded-sm border border-falcon-border text-falcon-muted">目安 {exp.estimated_duration_min}分</span>
          </div>

          <div className="bg-[#070d19] border border-falcon-border rounded-lg p-4">
            <p className="text-xs text-falcon-muted mb-1">仮説</p>
          ...
          <div className="bg-[#070d19] border border-amber-500/20 rounded-lg p-4">
            ...
            <p className="text-falcon-text text-sm">{exp.blast_radius}</p>
          </div>

          <div>
            <p className="text-xs text-falcon-muted mb-2">ステディステートメトリクス</p>
            <ul className="space-y-1">
              {exp.steady_state_metrics.map((m, i) => (
                <li key={i} className="flex items-center gap-2 text-sm text-falcon-text">
          ...
          <div>
            <p className="text-xs text-falcon-muted mb-2">実行ステップ</p>
            <ol className="space-y-1.5">
              {exp.execution_steps.map((s, i) => (
                <li key={i} className="flex items-start gap-2.5 text-sm text-falcon-text">
                  <span className="w-5 h-5 rounded-full bg-falcon-border text-falcon-muted text-xs flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                  {s}
                </li>
              ))}
            </ol>
          ...
          <div>
            <p className="text-xs text-falcon-muted mb-2">ロールバック手順</p>
            <ol className="space-y-1.5">
        ...
        <div className="px-6 py-4 border-t border-falcon-border flex items-center justify-end gap-3 shrink-0">
          ...
          <button onClick={onRunRequest}
            className="px-4 py-2 rounded-lg bg-falcon-red text-white hover:bg-[#c8001f] text-sm transition-colors flex items-center gap-2">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs">
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-lg mx-4 shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-xs text-falcon-muted mb-1">実施理由・正当化</label>
          ...
          <div>
            <label className="block text-xs text-falcon-muted mb-1">承認者</label>
            <select multiple value={approvers} onChange={e => setApprovers(Array.from(e.target.selectedOptions, o => o.value))}
              className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 h-28">
            ...
            <p className="text-xs text-falcon-subtle mt-1">Ctrl+クリックで複数選択</p>
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs">
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-lg mx-4 shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs text-falcon-muted mb-1">ターゲットスコープ</label>
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="radio" checked={runNow} onChange={() => setRunNow(true)} className="accent-falcon-red" />
              <span className="text-sm text-falcon-text">今すぐ実行</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="radio" checked={!runNow} onChange={() => setRunNow(false)} className="accent-falcon-red" />
              <span className="text-sm text-falcon-text">スケジュール実行</span>
            </label>
          </div>
          <label className="flex items-start gap-3 cursor-pointer">
            <input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} className="mt-0.5 accent-falcon-red" />
            <span className="text-sm text-falcon-text">本番環境への影響を理解しています。ロールバック手順を確認済みです。</span>
          </label>
        </div>
        <div className="px-6 py-4 border-t border-falcon-border flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-falcon-border text-falcon-muted hover:text-white text-sm transition-colors">キャンセル</button>
          <button onClick={() => onRun(scope, runNow)} disabled={!scope.trim() || !confirmed}
            className="px-4 py-2 rounded-lg bg-falcon-red text-white hover:bg-[#c8001f] text-sm transition-colors disabled:opacity-50 flex items-center gap-2">
      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-2xl mx-4 shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-falcon-border shrink-0">
          <div>
            ...
            <p className="text-xs text-falcon-muted mt-0.5">{fmtDate(run.started_at)} — {run.executed_by}</p>
          </div>
          <button onClick={onClose} className="text-falcon-muted hover:text-white"><X className="w-5 h-5" /></button>
        </div>
        <div className="overflow-y-auto px-6 py-5 space-y-5 flex-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#070d19] border border-falcon-border rounded-lg p-4">
              <p className="text-xs text-falcon-muted mb-2">メトリクス（実験前）</p>
              {Object.entries(run.metrics_before).map(([k, v]) => (
                <div key={k} className="flex justify-between text-sm py-0.5">
                  <span className="text-falcon-muted text-xs">{k}</span>
                  <span className="text-white font-mono">{v}</span>
                </div>
              ))}
            </div>
            <div className="bg-[#070d19] border border-falcon-border rounded-lg p-4">
              <p className="text-xs text-falcon-muted mb-2">メトリクス（実験後）</p>
          ...
          <div className="bg-[#070d19] border border-falcon-border rounded-lg p-4">
            <p className="text-xs text-falcon-muted mb-1">実際の結果</p>
            <p className="text-falcon-text text-sm">{run.hypothesis_actual}</p>
          </div>

          <div className="bg-[#070d19] border border-falcon-border rounded-lg p-4">
            <p className="text-xs text-falcon-muted mb-1">主な発見</p>
            <p className="text-falcon-text text-sm">{run.findings_summary}</p>
          </div>

          <div>
            <label className="block text-xs text-falcon-muted mb-1">教訓・改善点</label>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-falcon-muted mt-0.5">実験設計・実行・知見管理プラットフォーム</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="flex gap-2">
              {(['all', 'network', 'endpoint', 'auth', 'data', 'detection'] as const).map(c => (
                <button key={c} onClick={() => setFilterCategory(c)}
                  className={`px-3 py-1 rounded-full text-xs font-medium transition-all border ${filterCategory === c ? 'bg-falcon-red/20 border-falcon-red/50 text-falcon-red' : 'border-falcon-border text-falcon-muted hover:text-white'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredExps.map(exp => (
              <div key={exp.id} className="bg-falcon-surface border border-falcon-border rounded-xl p-5 hover:border-[#2a3f5e] transition-colors">
                <div className="flex items-start justify-between mb-3">
                  <h3 className="text-white font-semibold text-sm leading-snug flex-1 pr-2">{exp.name}</h3>
                  <Badge cfg={SAFETY_CONFIG[exp.is_safe]} />
                </div>
                <p className="text-xs text-falcon-muted mb-4 line-clamp-2">{exp.description}</p>
                <div className="flex flex-wrap gap-1.5 mb-4">
                  <Badge cfg={CATEGORY_CONFIG[exp.category]} />
                  <Badge cfg={SEVERITY_CONFIG[exp.severity_impact]} />
                  <span className="text-xs px-2 py-0.5 rounded-sm border border-falcon-border text-falcon-muted">{exp.target_type}</span>
                  <span className="text-xs px-2 py-0.5 rounded-sm border border-falcon-border text-falcon-muted flex items-center gap-1">
                    <Clock className="w-3 h-3" /> {exp.estimated_duration_min}分
                  </span>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setRunExp(exp)}
                    className="flex-1 py-1.5 rounded-lg bg-falcon-red/20 border border-falcon-red/30 text-falcon-red hover:bg-falcon-red/30 text-xs font-medium transition-colors flex items-center justify-center gap-1.5">
                  ...
                  <button onClick={() => setSelectedExp(exp)}
                    className="py-1.5 px-3 rounded-lg border border-falcon-border text-falcon-muted hover:text-white hover:border-[#2a3f5e] text-xs transition-colors">
          ...
          {approvals.length > 0 && (
            <div className="bg-falcon-surface border border-falcon-border rounded-xl p-5">
              <h2 className="text-white font-semibold mb-4 flex items-center gap-2">
                <ClipboardList className="w-4 h-4 text-falcon-red" />
                承認待ちキュー
              ...
              <div className="space-y-3">
                {approvals.filter(a => a.status === 'pending').map(apr => (
                  <div key={apr.id} className="flex items-start justify-between p-4 bg-[#070d19] border border-falcon-border rounded-lg">
                    <div className="flex-1 min-w-0">
                      <p className="text-white font-medium text-sm">{apr.experiment_name}</p>
                      <p className="text-xs text-falcon-muted mt-1 flex items-center gap-1.5">
                        <User className="w-3 h-3" /> {apr.requested_by} — {fmtDate(apr.requested_at)}
                      </p>
                      <p className="text-xs text-falcon-text mt-2 line-clamp-2">{apr.justification}</p>
                    </div>
          ...
          <div className="bg-falcon-surface border border-falcon-border rounded-xl p-5">
            <button onClick={() => setSafetyOpen(v => !v)}
              ...
              {safetyOpen ? <ChevronUp className="w-4 h-4 text-falcon-muted" /> : <ChevronDown className="w-4 h-4 text-falcon-muted" />}
            </button>
            {safetyOpen && (
              <div className="mt-4 space-y-2">
                {SAFETY_RULES.map(rule => (
                  <div key={rule.id} className="flex items-center gap-3 p-3 bg-[#070d19] rounded-lg border border-falcon-border">
                    <div className={`w-2 h-2 rounded-full shrink-0 ${rule.active ? 'bg-green-400' : 'bg-falcon-subtle'}`} />
                    <span className={`text-sm flex-1 ${rule.active ? 'text-falcon-text' : 'text-falcon-subtle line-through'}`}>{rule.rule}</span>
                    <span className={`text-xs ${rule.active ? 'text-green-400' : 'text-falcon-subtle'}`}>{rule.active ? '有効' : '無効'}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {tab === 'history' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-falcon-surface border border-falcon-border rounded-xl p-5">
              <p className="text-xs text-falcon-muted mb-1">仮説確認率</p>
              <p className="text-3xl font-bold text-white">{successRate}<span className="text-lg text-falcon-muted">%</span></p>
              <p className="text-xs text-falcon-muted mt-1">全{runs.length}回の実験</p>
            </div>
            <div className="bg-falcon-surface border border-falcon-border rounded-xl p-5">
              <p className="text-xs text-falcon-muted mb-1">平均発見数/実験</p>
              <p className="text-3xl font-bold text-amber-400">{avgVulnFound}</p>
              <p className="text-xs text-falcon-muted mt-1">新規脆弱性・改善点</p>
            </div>
            <div className="bg-falcon-surface border border-falcon-border rounded-xl p-5">
              <p className="text-xs text-falcon-muted mb-1">ロールバック率</p>
              <p className="text-3xl font-bold text-blue-400">
                {Math.round((runs.filter(r => r.rollback_taken).length / runs.length) * 100)}
                <span className="text-lg text-falcon-muted">%</span>
              </p>
              <p className="text-xs text-falcon-muted mt-1">{runs.filter(r => r.rollback_taken).length}回のロールバック実施</p>
            </div>
          </div>
          ...
          <div className="bg-falcon-surface border border-falcon-border rounded-xl overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {runs.map(run => (
                    <tr key={run.id} className="border-b border-falcon-border/50 hover:bg-[#070d19] transition-colors">
                      <td className="px-4 py-3 text-white font-medium text-xs whitespace-nowrap">{run.experiment_name}</td>
                      <td className="px-4 py-3 text-falcon-muted text-xs">{run.executed_by.split('@')[0]}</td>
                      <td className="px-4 py-3 text-falcon-muted text-xs whitespace-nowrap">{fmtDate(run.started_at)}</td>
                      <td className="px-4 py-3 text-falcon-muted text-xs">{run.duration_min}分</td>
                      <td className="px-4 py-3 text-falcon-muted text-xs max-w-[120px] truncate">{run.scope}</td>
                      <td className="px-4 py-3"><Badge cfg={RESULT_CONFIG[run.result]} /></td>
                      <td className="px-4 py-3 text-falcon-muted text-xs max-w-[200px] truncate">{run.findings_summary}</td>

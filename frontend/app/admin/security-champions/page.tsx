...
function Avatar({ name, size = 'md' }: { name: string; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div className={`${sz} rounded-full bg-linear-to-br from-[#1a6bff] to-[#0044cc] flex items-center justify-center font-bold text-white shrink-0`}>
      {initials(name)}
    </div>
  )
}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div key={m.label} className="bg-[#070d19] rounded-sm border border-[#1e2d42] p-3 text-center">
            ...
            {myActivities.length === 0
              ...
              : <div className="space-y-2">
                  {myActivities.slice(0, 5).map(a => (
                    <div key={a.id} className="flex items-start gap-3 p-3 bg-[#070d19] rounded-sm border border-[#1e2d42]">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-white text-sm focus:border-[#e8002d] focus:outline-hidden" />
            </div>
            <div>
          ...
          <div>
            ...
            <textarea value={form.training_plan} onChange={e => setForm(f => ({ ...f, training_plan: e.target.value }))} rows={3}
              ...
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-white text-sm focus:border-[#e8002d] focus:outline-hidden resize-none" />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2 border border-[#1e2d42] rounded-sm text-[#7d92b0] hover:text-white text-sm">キャンセル</button>
            <button type="submit" className="flex-1 px-4 py-2 bg-[#e8002d] rounded-sm text-white font-medium text-sm hover:bg-[#c0001f]">推薦する</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2 border border-[#1e2d42] rounded-sm text-[#7d92b0] hover:text-white text-sm">キャンセル</button>
            <button type="submit" className="flex-1 px-4 py-2 bg-[#e8002d] rounded-sm text-white font-medium text-sm hover:bg-[#c0001f]">追加する</button>
          </div>
        </form>
      </div>
    </div>
...
export default function SecurityChampionsPage() {
  ...
  const { data: champions = [] } = useQuery<Champion[]>({
    ...
    queryFn: () => apiFetchList<Champion>('/api/v1/admin/security-champions'),
    retry: false,
  })

  const { data: activities = [] } = useQuery<ChampionActivity[]>({
    queryKey: ['security-champions-activities'],
    queryFn: () => apiFetchList<ChampionActivity>('/api/v1/admin/security-champions/activities'),
    retry: false,
  })
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as ChampionStatus | 'all')}
              className="bg-[#0d1220] border border-[#1e2d42] rounded-sm px-3 py-2 text-white text-sm focus:outline-hidden">
                    ...
                    <tr key={c.id} className="hover:bg-[#0d1525] cursor-pointer" onClick={() => setDetailChampion(c)}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                          <button onClick={() => setDetailChampion(c)} className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-white"><ChevronRight className="w-4 h-4" /></button>
                          {c.status === 'inactive'
                            ? <button onClick={() => showToast(`${c.name}を再有効化しました`)} className="p-1.5 rounded-sm hover:bg-green-900/40 text-[#7d92b0] hover:text-green-300"><RefreshCw className="w-4 h-4" /></button>
                            : <button onClick={() => showToast(`${c.name}を非アクティブにしました`)} className="p-1.5 rounded-sm hover:bg-red-900/40 text-[#7d92b0] hover:text-red-300"><Trash2 className="w-4 h-4" /></button>
                          }
                        </div>
                      </td>
                    </tr>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
            ...
            <div className="space-y-3">
              {([] as TrainingAssignment[]).map(t => {
                ...
                return (
                  <div key={t.id} className="flex items-center gap-4 p-3 bg-[#070d19] rounded-sm border border-[#1e2d42]">
                    <div className="flex-1 min-w-0">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
              ...
              <div className="space-y-2">
                {top10.map((c, i) => (
                  <div key={c.id} className="flex items-center gap-3 p-2.5 bg-[#070d19] rounded-sm border border-[#1e2d42]">

...
function RoleBadge({ role }: { role: RoleInProgram }) {
  ...
  return <span className={`inline-flex items-center px-2 py-0.5 rounded-sm text-xs font-medium ${s.bg} ${s.text}`}>{s.label}</span>
}

function StatusBadge({ status }: { status: ChampionStatus }) {
  const s = STATUS_STYLES[status]
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs font-medium ${s.bg} ${s.text}`}>
...
function Avatar({ name, size = 'md' }: { name: string; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div className={`${sz} rounded-full bg-linear-to-br from-falcon-blue to-[#0044cc] flex items-center justify-center font-bold text-white shrink-0`}>
      {initials(name)}
    </div>
  )
}
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2 text-sm">
              <div className="flex items-center gap-2 text-falcon-muted"><Mail className="w-4 h-4" />{champion.email}</div>
              <div className="flex items-center gap-2 text-falcon-muted"><Building2 className="w-4 h-4" />{champion.department}</div>
              <div className="flex items-center gap-2 text-falcon-muted"><User className="w-4 h-4" />マネージャー: {champion.manager}</div>
              <div className="flex items-center gap-2 text-falcon-muted"><Calendar className="w-4 h-4" />推薦日: {fmt(champion.nomination_date)}</div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div key={m.label} className="bg-[#070d19] rounded-sm border border-falcon-border p-3 text-center">
          ...
          <div>
            ...
            {champion.certifications.length === 0
              ? <p className="text-falcon-muted text-sm">未取得</p>
              : <div className="flex flex-wrap gap-2">
                  {champion.certifications.map(c => (
                    <span key={c} className="px-2 py-1 bg-yellow-900/30 border border-yellow-700/40 rounded-sm text-xs text-yellow-300">{c}</span>
                  ))}
                </div>
            }
          </div>
            ...
            <div>
              ...
              <ul className="space-y-1">
                {champion.upcoming_tasks.map((t, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm text-falcon-muted">
                    <CheckCircle className="w-3.5 h-3.5 text-blue-400 shrink-0" />{t}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div>
            ...
            {myActivities.length === 0
              ? <p className="text-falcon-muted text-sm">活動記録なし</p>
              : <div className="space-y-2">
                  {myActivities.slice(0, 5).map(a => (
                    <div key={a.id} className="flex items-start gap-3 p-3 bg-[#070d19] rounded-sm border border-falcon-border">
                      <ActivityTypeBadge type={a.activity_type} />
                      <div className="flex-1 min-w-0">
                        <p className="text-falcon-text text-sm truncate">{a.description}</p>
                        <p className="text-falcon-subtle text-xs mt-0.5">{fmt(a.date)}</p>
                      </div>
                      <span className="text-yellow-400 text-sm font-bold shrink-0">+{a.points_earned}pt</span>
                    </div>
                  ))}
                </div>
            }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-falcon-surface border border-falcon-border rounded-lg w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-falcon-muted text-sm mb-1">氏名 *</label>
            ...
            <div>
              <label className="block text-falcon-muted text-sm mb-1">メールアドレス *</label>
              <input required type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                className="w-full bg-[#070d19] border border-falcon-border rounded-sm px-3 py-2 text-white text-sm focus:border-falcon-red focus:outline-hidden" />
            </div>
          </div>
          <div>
            <label className="block text-falcon-muted text-sm mb-1">部門 *</label>
          ...
          <div>
            <label className="block text-falcon-muted text-sm mb-1">初期トレーニング計画</label>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-falcon-muted text-sm">マネージャー承認が必要</span>
          </label>
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" className="flex-1 px-4 py-2 bg-falcon-red rounded-sm text-white font-medium text-sm hover:bg-[#c0001f]">推薦する</button>
          </div>
        </form>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-falcon-surface border border-falcon-border rounded-lg w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
            <label className="block text-falcon-muted text-sm mb-1">チャンピオン *</label>
          ...
          <div>
            <label className="block text-falcon-muted text-sm mb-1">活動タイプ *</label>
          ...
          <div>
            <label className="block text-falcon-muted text-sm mb-1">説明 *</label>
            <textarea required value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={3}
              className="w-full bg-[#070d19] border border-falcon-border rounded-sm px-3 py-2 text-white text-sm focus:border-falcon-red focus:outline-hidden resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-falcon-muted text-sm mb-1">日付 *</label>
              <input required type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))}
                className="w-full bg-[#070d19] border border-falcon-border rounded-sm px-3 py-2 text-white text-sm focus:border-falcon-red focus:outline-hidden" />
            </div>
            <div>
              <label className="block text-falcon-muted text-sm mb-1">エビデンス</label>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2 border border-falcon-border rounded-sm text-falcon-muted hover:text-white text-sm">キャンセル</button>
            <button type="submit" className="flex-1 px-4 py-2 bg-falcon-red rounded-sm text-white font-medium text-sm hover:bg-[#c0001f]">追加する</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#070d19] p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-linear-to-br from-yellow-500 to-yellow-700 flex items-center justify-center">
            <Star className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="text-white text-2xl font-bold">セキュリティチャンピオンプログラム</h1>
            <p className="text-falcon-muted text-sm">組織全体のセキュリティ文化を推進するリーダーシップ管理</p>
          </div>
        </div>
      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-lg p-5">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <p className="text-falcon-muted text-sm leading-relaxed max-w-3xl">
            ...
            {champions.find(c => c.role_in_program === 'lead_champion') && (
              <div className="flex items-center gap-2 mt-3">
                <span className="text-falcon-muted text-sm">プログラムリード:</span>
      ...
      {/* Tabs */}
      <div className="border-b border-falcon-border flex gap-1">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as ChampionStatus | 'all')}
              className="bg-falcon-surface border border-falcon-border rounded-sm px-3 py-2 text-white text-sm focus:outline-hidden">
            ...
            <span className="text-falcon-muted text-sm ml-auto">{filteredChampions.length}件表示</span>
          </div>
                    ...
                    <tr key={c.id} className="hover:bg-[#0d1525] cursor-pointer" onClick={() => setDetailChampion(c)}>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-falcon-subtle text-xs">{c.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-falcon-muted whitespace-nowrap">{c.department}</td>
                      <td className="px-4 py-3"><RoleBadge role={c.role_in_program} /></td>
                      <td className="px-4 py-3 text-falcon-muted whitespace-nowrap">{fmt(c.nomination_date)}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 bg-falcon-border rounded-full h-1.5 min-w-[60px]">
                            <div className="bg-falcon-red h-1.5 rounded-full" style={{ width: `${Math.min(100, (c.contribution_score / 1000) * 100)}%` }} />
                          </div>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                          <button onClick={() => setDetailChampion(c)} className="p-1.5 rounded-sm hover:bg-falcon-border text-falcon-muted hover:text-white"><ChevronRight className="w-4 h-4" /></button>
                          {c.status === 'inactive'
                            ? <button onClick={() => showToast(`${c.name}を再有効化しました`)} className="p-1.5 rounded-sm hover:bg-green-900/40 text-falcon-muted hover:text-green-300"><RefreshCw className="w-4 h-4" /></button>
                            : <button onClick={() => showToast(`${c.name}を非アクティブにしました`)} className="p-1.5 rounded-sm hover:bg-red-900/40 text-falcon-muted hover:text-red-300"><Trash2 className="w-4 h-4" /></button>
                          }
                        </div>
                      </td>
                    </tr>
          ...
          <div className="bg-falcon-surface border border-falcon-border rounded-lg p-5">
            ...
            <div className="space-y-3">
              {([] as TrainingAssignment[]).map(t => {
                ...
                return (
                  <div key={t.id} className="flex items-center gap-4 p-3 bg-[#070d19] rounded-sm border border-falcon-border">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-white text-sm font-medium truncate">{t.title}</p>
                        <span className={`px-1.5 py-0.5 rounded-sm text-xs ${t.type === 'required' ? 'bg-red-900/40 text-red-300' : 'bg-blue-900/40 text-blue-300'}`}>
                          {t.type === 'required' ? '必須' : '推奨'}
                        </span>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="flex-1 bg-falcon-border rounded-full h-1.5">
                          <div className="bg-falcon-red h-1.5 rounded-full" style={{ width: `${pct}%` }} />
                        </div>
                        <span className="text-falcon-muted text-xs whitespace-nowrap">{t.completed_count}/{t.total_count} 完了</span>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-white font-bold">{pct}%</p>
                      <p className="text-falcon-subtle text-xs">期限: {fmt(t.due_date)}</p>
                    </div>
                  </div>
                )
              })}
        ...
        <div className="space-y-6">
                ...
                <tbody className="divide-y divide-falcon-border">
                  {activities.map(a => (
                    <tr key={a.id} className="hover:bg-[#0d1525]">
                      ...
                      <td className="px-4 py-3 text-falcon-muted max-w-xs truncate">{a.description}</td>
                      <td className="px-4 py-3 text-yellow-400 font-bold whitespace-nowrap">+{a.points_earned}pt</td>
                      <td className="px-4 py-3 text-falcon-muted whitespace-nowrap">{fmt(a.date)}</td>
                      <td className="px-4 py-3">
                        {a.approved
                          ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs bg-green-900/40 text-green-300"><CheckCircle className="w-3 h-3" />承認済み</span>
                          : <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs bg-yellow-900/40 text-yellow-300"><Clock className="w-3 h-3" />審査中</span>
                        }
                      </td>
                    </tr>
                  ))}
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="bg-falcon-surface border border-falcon-border rounded-lg p-5">
              ...
              <div className="space-y-2">
                {top10.map((c, i) => (
                  <div key={c.id} className="flex items-center gap-3 p-2.5 bg-[#070d19] rounded-sm border border-falcon-border">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-falcon-subtle text-xs">{c.department}</p>
                    </div>
                      ...
                      <div className="flex items-end gap-1 h-8">
                        {counts.map((v, i) => (
                          <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                            <div className={`w-full rounded-xs ${s.bg.replace('/40', '/60')}`} style={{ height: `${(v / activityBarMax) * 32}px` }} />
                          </div>
                        ))}
                      </div>
                      <div className="flex justify-between mt-0.5">
                        {MONTHS.map(m => <span key={m} className="text-falcon-subtle text-[9px]">{m}</span>)}
                      </div>
                    </div>
                  )
                })}
                ...
                <thead>
                  <tr className="border-b border-falcon-border">
                    {['部門', 'アクティブチャンピオン数', '全チャンピオン数', 'カバー状況'].map(h => (
                      <th key={h} className="px-4 py-2 text-left text-falcon-muted font-medium text-xs">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-falcon-border">
                  {deptCoverage.map(d => {
                    ...
                    return (
                      <tr key={d.dept} className="hover:bg-[#0d1525]">
                        ...
                        <td className="px-4 py-2.5 text-falcon-muted">{total}</td>
                        <td className="px-4 py-2.5">
                          {d.has
                            ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs bg-green-900/40 text-green-300"><CheckCircle className="w-3 h-3" />カバー済み</span>
                            : <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs bg-red-900/40 text-red-300"><AlertCircle className="w-3 h-3" />未カバー</span>
                          }
                        </td>
                      </tr>
                    )

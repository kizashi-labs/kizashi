...
export default function SecurityGovernancePage() {
  ...
  const { data: policies = [] } = useQuery({
    ...
    queryFn: () => apiFetchList<Policy>('/api/v1/admin/security-governance/policies'),
  })
  const { data: exceptions = [] } = useQuery({
    queryKey: ['sg-exceptions'],
    queryFn: () => apiFetchList<Exception>('/api/v1/admin/security-governance/exceptions'),
  })
                  ...
                  {filtered.map(p => (
                    <>
                      <tr key={p.id} className="border-b border-[#1e2d42] hover:bg-[#1e2d42]/30 cursor-pointer"
                        ...
                        <td className="px-3 py-3"><span className="bg-[#1e2d42] text-[#7d92b0] px-2 py-0.5 rounded-sm text-xs">{p.category}</span></td>
                          ...
                          <td colSpan={10} className="px-6 py-4">
                            <div className="flex gap-8 text-sm">
                              ...
                              <div>
                                ...
                                <div className="flex gap-1">{p.related_controls.map(c => <span key={c} className="bg-[#1e2d42] text-[#7d92b0] px-2 py-0.5 rounded-sm text-xs font-mono">{c}</span>)}</div>
                              </div>
                            </div>
                          </td>
                        </tr>
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
              ...
              <div className="space-y-2">
                {[{date:'2026-03-25',policy:'POL-003',owner:'SOCチーム'},{date:'2026-04-05',policy:'POL-001',owner:'CISO室'},{date:'2026-04-12',policy:'POL-002',owner:'IT統括部'}].map(r => (
                  <div key={r.policy} className="flex items-center gap-3 p-2 rounded-sm bg-[#1e2d42]/30">

      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-falcon-muted text-sm mt-1">ポリシー・例外申請・コンプライアンス管理</p>
          </div>
          <button className="bg-falcon-red hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
            + 新規ポリシー
          </button>
        </div>
            ...
            <div className="flex gap-2 mb-4 flex-wrap">
              {CATEGORIES.map(c => (
                <button key={c} onClick={() => setCatFilter(c)}
                  className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${catFilter === c ? 'bg-falcon-red text-white' : 'bg-falcon-surface border border-falcon-border text-falcon-muted hover:text-white'}`}>
                  {c}
                </button>
              ))}
            </div>
            <div className="bg-falcon-surface border border-falcon-border rounded-lg overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.map(p => (
                    <>
                      <tr key={p.id} className="border-b border-falcon-border hover:bg-falcon-border/30 cursor-pointer"
                        onClick={() => setExpandedPolicy(expandedPolicy === p.id ? null : p.id)}>
                        <td className="px-3 py-3 font-mono text-xs text-falcon-muted">
                        ...
                        <td className="px-3 py-3"><span className="bg-falcon-border text-falcon-muted px-2 py-0.5 rounded-sm text-xs">{p.category}</span></td>
                        <td className="px-3 py-3 text-falcon-muted">v{p.version}</td>
                        <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${STATUS_STYLES[p.status]}`}>{p.status}</span></td>
                        <td className="px-3 py-3 text-falcon-muted">{p.owner}</td>
                        <td className="px-3 py-3 text-falcon-muted text-xs">{p.effective_date || '—'}</td>
                        <td className="px-3 py-3 text-xs"><span className={p.review_date < today ? 'text-red-400' : 'text-falcon-muted'}>{p.review_date}</span></td>
                        <td className="px-3 py-3 w-28">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-falcon-muted w-8">{p.approval_rate}%</span>
                          </div>
                        </td>
                        <td className="px-3 py-3">
                          <div className="flex gap-1">
                            <button className="text-xs text-blue-400 hover:text-blue-300 px-2 py-1 rounded-sm border border-blue-900 hover:bg-blue-900/30">編集</button>
                            <button className="text-xs text-falcon-muted hover:text-white px-1">{expandedPolicy === p.id ? '▲' : '▼'}</button>
                          </div>
                        </td>
                      </tr>
                      {expandedPolicy === p.id && (
                        <tr key={`${p.id}-detail`} className="bg-[#070d19]">
                          <td colSpan={10} className="px-6 py-4">
                            <div className="flex gap-8 text-sm">
                              <div>
                                <div className="text-falcon-muted text-xs mb-1">適用フレームワーク</div>
                                <div className="flex gap-1">{p.frameworks.map(f => <span key={f} className="bg-blue-900 text-blue-300 px-2 py-0.5 rounded-sm text-xs">{f}</span>)}</div>
                              </div>
                              <div>
                                <div className="text-falcon-muted text-xs mb-1">関連コントロール</div>
                                <div className="flex gap-1">{p.related_controls.map(c => <span key={c} className="bg-falcon-border text-falcon-muted px-2 py-0.5 rounded-sm text-xs font-mono">{c}</span>)}</div>
                              </div>
                            </div>
                          </td>
                        </tr>
            ...
            <div className="bg-falcon-surface border border-falcon-border rounded-lg overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {exceptions.map(e => (
                    <>
                      <tr key={e.id} className="border-b border-falcon-border hover:bg-falcon-border/30 cursor-pointer"
                        onClick={() => setExpandedExc(expandedExc === e.id ? null : e.id)}>
                        <td className="px-3 py-3 font-medium">{e.title}</td>
                        <td className="px-3 py-3 text-falcon-muted font-mono text-xs">{e.policy}</td>
                        <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${RISK_STYLES[e.risk_level]}`}>{e.risk_level}</span></td>
                        <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${EXC_STATUS_STYLES[e.status]}`}>{e.status}</span></td>
                        <td className="px-3 py-3 text-falcon-muted">{e.requester}</td>
                        <td className="px-3 py-3 text-falcon-muted text-xs">{e.expires}</td>
                        <td className="px-3 py-3 text-falcon-muted">{e.approver}</td>
                        <td className="px-3 py-3">
                          {e.status === 'pending' ? (
                            <div className="flex gap-1">
                              <button className="text-xs bg-green-900 text-green-300 hover:bg-green-800 px-2 py-1 rounded-sm">承認</button>
                              <button className="text-xs bg-red-900 text-red-300 hover:bg-red-800 px-2 py-1 rounded-sm">却下</button>
                            </div>
                        ...
                        <tr key={`${e.id}-detail`} className="bg-[#070d19]">
                          <td colSpan={8} className="px-6 py-4">
                            <div className="grid grid-cols-2 gap-6 text-sm">
                              <div>
                                <div className="text-falcon-muted text-xs mb-1">申請理由</div>
                                <p className="text-white">{e.justification}</p>
                              </div>
                              <div>
                                <div className="text-falcon-muted text-xs mb-1">補償的コントロール</div>
          ...
          <div className="grid grid-cols-2 gap-6">
                ...
                {[{name:'ISO27001', pct:92, color:'bg-blue-500'},{name:'NIST CSF', pct:78, color:'bg-purple-500'},{name:'PCI-DSS', pct:85, color:'bg-orange-500'},{name:'GDPR', pct:88, color:'bg-green-500'}].map(f => (
                  <div key={f.name}>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-falcon-muted">{f.name}</span>
            ...
            <div className="bg-falcon-surface border border-falcon-border rounded-lg p-5">
              ...
              <div className="space-y-2">
                {[{date:'2026-03-25',policy:'POL-003',owner:'SOCチーム'},{date:'2026-04-05',policy:'POL-001',owner:'CISO室'},{date:'2026-04-12',policy:'POL-002',owner:'IT統括部'}].map(r => (
                  <div key={r.policy} className="flex items-center gap-3 p-2 rounded-sm bg-falcon-border/30">
                    <div className="w-16 text-xs font-mono text-falcon-red">{r.date.slice(5)}</div>
                    <div className="flex-1">
                      <div className="text-sm font-medium">{r.policy}</div>
                      <div className="text-xs text-falcon-muted">{r.owner}</div>
                    </div>
            ...
            <div className="bg-falcon-surface border border-falcon-border rounded-lg p-5">
              ...
              <div className="text-center py-4">
                ...
                <div className="text-falcon-muted text-sm mb-4">全スタッフの 94.7% が基本方針を承認済み</div>
                <div className="bg-falcon-border rounded-full h-4 mx-4">
                  <div className="bg-green-500 h-4 rounded-full" style={{ width:'94.7%' }} />
                </div>
                <div className="flex justify-between text-xs text-falcon-muted mt-1 mx-4">
                  <span>承認済み: 284名</span>
                  <span>未承認: 16名</span>
                </div>
              </div>

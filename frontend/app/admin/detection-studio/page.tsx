    ...
    <div className="text-sm space-y-0.5">
      {CATEGORIES.filter(c => byCategory[c]?.length > 0).map(cat => (
        <div key={cat}>
          <button
            onClick={() => toggle(cat)}
            className="w-full flex items-center gap-1.5 px-2 py-1.5 rounded text-[#7d92b0] hover:text-white hover:bg-[#1e2d42]/40 transition-colors"
          >
            {openCategories[cat]
              ? <FolderOpen className="w-3.5 h-3.5 text-[#e8002d]/60 flex-shrink-0" />
              : <Folder className="w-3.5 h-3.5 text-[#3d5068] flex-shrink-0" />
            }
            <span className="flex-1 text-left text-xs font-semibold uppercase tracking-wide">{cat}</span>
            <span className="text-[10px] text-[#3d5068]">{byCategory[cat].length}</span>
            ...
            <button
              ...
              className={`w-full flex items-center gap-1.5 px-2 py-1.5 pl-6 rounded text-xs transition-colors ${
                selectedId === rule.id
                  ...
                  : 'text-[#7d92b0] hover:bg-[#1e2d42]/40 hover:text-white'
              }`}
            >
              <FileCode className={`w-3 h-3 flex-shrink-0 ${getTypeColor(rule.type)}`} />
              <span className="flex-1 text-left truncate">{rule.name}</span>
              {rule.match_count && rule.match_count > 0 && (
                <span className="text-[10px] text-[#7d92b0]">{rule.match_count}</span>
              )}
            </button>
          ))}
        </div>
          ...
          <button
            ...
            className={`w-full text-left p-3 rounded-lg border transition-colors ${
              selectedVersion?.version === v.version
                ? 'bg-[#1d2f4a] border-[#e8002d]/30'
                : 'bg-[#070d19] border-[#1e2d42] hover:border-[#7d92b0]/30'
            }`}
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-white">v{v.version}</span>
              <span className="text-xs text-[#7d92b0]">{v.date}</span>
            </div>
            <p className="text-xs text-[#7d92b0] mt-0.5">{v.author} · {v.comment}</p>
          </button>
        ))}
      </div>
      {selectedVersion && (
        <div className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              className="text-xs px-2 py-1 bg-[#e8002d]/20 text-[#e8002d] border border-[#e8002d]/30 rounded hover:bg-[#e8002d]/30"
            >
              <RotateCcw className="w-3 h-3 inline mr-1" />
              復元
            </button>
          </div>
          <p className="text-xs text-[#7d92b0] italic">
...
falsepositives:
  ...
  return (
    <div className="min-h-screen bg-[#070d19] flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#1e2d42] bg-[#0d1220]">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-[#e8002d]/10 border border-[#e8002d]/20 flex items-center justify-center">
            <Code className="w-4.5 h-4.5 text-[#e8002d]" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">検出ルールスタジオ</h1>
            <p className="text-xs text-[#7d92b0]">ルール開発・テスト環境</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-[#1e2d42] rounded text-[#7d92b0] hover:text-white hover:border-[#7d92b0]/40 transition-colors"
          >
            <Plus className="w-3.5 h-3.5" />
            新規
          </button>
          <button
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-[#1e2d42] rounded text-[#7d92b0] hover:text-white transition-colors"
            title="コミュニティに公開"
          >
      ...
      <div className="flex flex-1 overflow-hidden" style={{ minHeight: 'calc(100vh - 73px)' }}>
        ...
        <div className="w-[220px] flex-shrink-0 bg-[#0d1220] border-r border-[#1e2d42] flex flex-col overflow-hidden">
          <div className="px-3 py-2 border-b border-[#1e2d42]">
            <p className="text-[10px] font-bold text-[#3d5068] uppercase tracking-wider">ルールライブラリ ({allRules.length})</p>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#070d19]">
          {/* Editor toolbar */}
          <div className="flex items-center gap-2 px-4 py-2 border-b border-[#1e2d42] bg-[#0d1220] flex-wrap gap-y-1.5">
            {/* Rule name */}
            <input
              className="bg-transparent text-sm font-medium text-white border-b border-[#1e2d42] focus:outline-none focus:border-[#e8002d]/50 px-1 py-0.5 w-48 min-w-[120px]"
              value={ruleName}
              onChange={e => setRuleName(e.target.value)}
            />
            {/* Type selector */}
            <div className="flex gap-0.5 ml-2 bg-[#070d19] rounded border border-[#1e2d42] p-0.5">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1.5 text-xs bg-[#e8002d] text-white rounded hover:bg-[#c0001f] disabled:opacity-50 transition-colors"
              >
              ...
              <button
                onClick={handleTest}
                disabled={isTesting}
                className="flex items-center gap-1 px-2.5 py-1.5 text-xs bg-green-800/60 border border-green-700/40 text-green-400 rounded hover:bg-green-800/80 disabled:opacity-50 transition-colors"
              >
                <Play className="w-3.5 h-3.5" />
                テスト
              </button>
              <button onClick={handleFormat} className="flex items-center gap-1 px-2.5 py-1.5 text-xs border border-[#1e2d42] text-[#7d92b0] rounded hover:text-white transition-colors">
                <AlignLeft className="w-3.5 h-3.5" />
                整形
              </button>
              <button onClick={handleCopy} className="flex items-center gap-1 px-2.5 py-1.5 text-xs border border-[#1e2d42] text-[#7d92b0] rounded hover:text-white transition-colors">
                {copyNotice ? <CheckCircle2 className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
              <button onClick={handleExport} className="flex items-center gap-1 px-2.5 py-1.5 text-xs border border-[#1e2d42] text-[#7d92b0] rounded hover:text-white transition-colors">
          ...
          <div className="flex-1 flex overflow-hidden font-mono text-sm">
            {/* Line numbers */}
            <div
              className="select-none bg-[#070d19] border-r border-[#1e2d42] text-[#3d5068] text-xs text-right pr-3 pt-3 leading-6 min-w-[44px]"
            ...
            <textarea
              ref={textareaRef}
              className="flex-1 bg-[#070d19] text-[#e2e8f4] text-sm font-mono resize-none focus:outline-none px-4 pt-3 leading-6 scrollbar-thin"
              spellCheck={false}
              value={content}
          ...
          {/* Status bar */}
          <div className="flex items-center justify-between px-4 py-1.5 border-t border-[#1e2d42] bg-[#0d1220] text-[10px] text-[#3d5068]">
            <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              {selectedRule && (
                <>
                  ...
                  <span className={getStatusBadge(selectedRule.status) + ' px-1.5 py-0.5 rounded text-[9px]'}>{selectedRule.status}</span>
                </>
              )}
            </div>
          </div>
        ...
        <div className="w-[280px] flex-shrink-0 bg-[#0d1220] border-l border-[#1e2d42] flex flex-col overflow-hidden">
          {/* Panel tabs */}
          ...
          <div className="flex-1 overflow-y-auto p-4 scrollbar-thin">
                  ...
                  <div className="space-y-1.5">
                    {docs.items.map(item => (
                      <div key={item.label} className="flex items-center gap-2">
                        <code className="text-xs font-mono text-[#e8002d] bg-[#070d19] px-1.5 py-0.5 rounded min-w-[80px] flex-shrink-0">{item.label}</code>
                        <span className="text-xs text-[#7d92b0]">{item.desc}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {COMMON_PATTERNS.map(p => (
                      <div key={p.label} className="bg-[#070d19] border border-[#1e2d42] rounded p-2">
                        ...
                        <p className="text-[10px] text-[#7d92b0]">{p.desc}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {panelView === 'test' && (
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-xs font-semibold text-[#7d92b0] block mb-1.5">
                    シナリオ選択
                  </label>
                  <div className="flex flex-wrap gap-1.5">
                    {SAMPLE_EVENTS.map(s => (
                      <button
                        ...
                        className="flex items-center gap-1 px-2 py-1 text-[10px] rounded border border-[#1e2d42]
                                   bg-[#0d1220] text-[#7d92b0] hover:text-white hover:border-[#3d5068] transition-colors"
                        title={s.label}
                      >
                        <span className="opacity-50">{s.category}</span>
                        <span className="text-[#e2e8f4]">{s.label}</span>
                      </button>
                    ))}
                  </div>
                </div>
                <div>
                  <label className="text-xs font-semibold text-[#7d92b0] block mb-1.5">サンプルイベント (JSON)</label>
                  <textarea
                    className="w-full h-40 bg-[#070d19] border border-[#1e2d42] rounded p-2 text-xs font-mono text-[#7d92b0] focus:outline-none focus:border-[#e8002d]/50 resize-none"
                    value={testEvent}
                ...
                <button
                  onClick={handleTest}
                  disabled={isTesting}
                  className="w-full flex items-center justify-center gap-2 py-2 bg-green-800/40 border border-green-700/40 text-green-400 text-sm rounded hover:bg-green-800/60 disabled:opacity-50 transition-colors"
                >
                  {isTesting ? (
                  ...
                  <div className="space-y-2">
                    <div className={`flex items-center gap-2 p-3 rounded-lg border ${
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#7d92b0]">評価時間: {testResult.eval_time_ms}ms</p>
                      </div>
                    </div>
                      ...
                      <div className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-3">
                        ...
                        {Object.entries(testResult.matched_fields).map(([k, v]) => (
                          <div key={k} className="mb-1">
                            <span className="text-xs text-[#e8002d]">{k}:</span>
                            <span className="text-xs text-[#7d92b0] ml-1 break-all">{v}</span>
                          </div>
                        ))}
                      </div>
                    )}

                    <button className="w-full py-1.5 text-xs border border-[#1e2d42] text-[#7d92b0] rounded hover:text-white hover:border-[#7d92b0]/40 transition-colors">
            ...
            {panelView === 'history' && (
              selectedRule
                ...
                : <p className="text-xs text-[#7d92b0] text-center py-8">ルールを選択してください</p>
            )}
            ...
            {panelView === 'ai' && (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <Bot className="w-5 h-5 text-[#e8002d]" />
                  <h3 className="text-sm font-bold text-white">AIアシスト</h3>
                </div>
                <p className="text-xs text-[#7d92b0]">検出したい脅威シナリオを説明すると、ルールテンプレートを生成します。</p>

                <div>
                  <label className="text-xs text-[#7d92b0] mb-1 block">脅威シナリオの説明</label>
                  <textarea
                    className="w-full h-24 bg-[#070d19] border border-[#1e2d42] rounded p-2 text-xs text-white placeholder-[#3d5068] focus:outline-none focus:border-[#e8002d]/50 resize-none"
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2 bg-[#e8002d]/20 border border-[#e8002d]/30 text-[#e8002d] text-sm rounded hover:bg-[#e8002d]/30 disabled:opacity-50 transition-colors"
                >
                  {aiGenerating
                    ...
                    <div className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                          ...
                          className="text-xs px-2 py-0.5 bg-[#e8002d]/20 text-[#e8002d] border border-[#e8002d]/30 rounded hover:bg-[#e8002d]/30 transition-colors"
                        >
                          エディタに適用
                        </button>
                      </div>
                      <pre className="text-xs font-mono text-[#7d92b0] overflow-x-auto whitespace-pre-wrap">{aiResult}</pre>
                    </div>
                    <p className="text-[10px] text-[#3d5068] italic">* AIが生成したルールは必ずレビューしてください</p>
                  </div>
                )}
              </div>
            )}

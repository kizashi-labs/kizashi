    ...
    <div className="text-sm space-y-0.5">
      {CATEGORIES.filter(c => byCategory[c]?.length > 0).map(cat => (
        <div key={cat}>
          <button
            onClick={() => toggle(cat)}
            className="w-full flex items-center gap-1.5 px-2 py-1.5 rounded-sm text-[#7d92b0] hover:text-white hover:bg-[#1e2d42]/40 transition-colors"
          >
            {openCategories[cat]
              ? <FolderOpen className="w-3.5 h-3.5 text-[#e8002d]/60 shrink-0" />
              : <Folder className="w-3.5 h-3.5 text-[#3d5068] shrink-0" />
            }
          ...
          {openCategories[cat] && byCategory[cat].map(rule => (
            <button
              ...
              className={`w-full flex items-center gap-1.5 px-2 py-1.5 pl-6 rounded-sm text-xs transition-colors ${
                selectedId === rule.id
        ...
        <div className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              className="text-xs px-2 py-1 bg-[#e8002d]/20 text-[#e8002d] border border-[#e8002d]/30 rounded-sm hover:bg-[#e8002d]/30"
            >
...
export default function DetectionStudioPage() {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      // ここは Math.random() > 0.3 でした。7割の確率で「マッチ!」と表示し、
      // 一致した項目として encoded PowerShell のコマンドラインまで
      // 作って見せていました。ルールを書いている最中の人が、それを
      // 自分のルールが効いた証拠として読みます。
      setTestResult(null)
  ...
  return (
    <div className="min-h-screen bg-[#070d19] flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#1e2d42] bg-[#0d1220]">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-[#1e2d42] rounded-sm text-[#7d92b0] hover:text-white hover:border-[#7d92b0]/40 transition-colors"
          >
            <Plus className="w-3.5 h-3.5" />
            新規
          </button>
          <button
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-[#1e2d42] rounded-sm text-[#7d92b0] hover:text-white transition-colors"
            title="コミュニティに公開"
          >
      ...
      <div className="flex flex-1 overflow-hidden" style={{ minHeight: 'calc(100vh - 73px)' }}>
        ...
        <div className="w-[220px] shrink-0 bg-[#0d1220] border-r border-[#1e2d42] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#070d19]">
          ...
          <div className="flex items-center gap-2 px-4 py-2 border-b border-[#1e2d42] bg-[#0d1220] flex-wrap gap-y-1.5">
            {/* Rule name */}
            <input
              className="bg-transparent text-sm font-medium text-white border-b border-[#1e2d42] focus:outline-hidden focus:border-[#e8002d]/50 px-1 py-0.5 w-48 min-w-[120px]"
              value={ruleName}
              onChange={e => setRuleName(e.target.value)}
            />
            {/* Type selector */}
            <div className="flex gap-0.5 ml-2 bg-[#070d19] rounded-sm border border-[#1e2d42] p-0.5">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1.5 text-xs bg-[#e8002d] text-white rounded-sm hover:bg-[#c0001f] disabled:opacity-50 transition-colors"
              >
              ...
              <button onClick={handleFormat} className="flex items-center gap-1 px-2.5 py-1.5 text-xs border border-[#1e2d42] text-[#7d92b0] rounded-sm hover:text-white transition-colors">
                <AlignLeft className="w-3.5 h-3.5" />
                整形
              </button>
              <button onClick={handleCopy} className="flex items-center gap-1 px-2.5 py-1.5 text-xs border border-[#1e2d42] text-[#7d92b0] rounded-sm hover:text-white transition-colors">
                {copyNotice ? <CheckCircle2 className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
              <button onClick={handleExport} className="flex items-center gap-1 px-2.5 py-1.5 text-xs border border-[#1e2d42] text-[#7d92b0] rounded-sm hover:text-white transition-colors">
          ...
          <div className="flex-1 flex overflow-hidden font-mono text-sm">
            ...
            <textarea
              ref={textareaRef}
              className="flex-1 bg-[#070d19] text-[#e2e8f4] text-sm font-mono resize-none focus:outline-hidden px-4 pt-3 leading-6 scrollbar-thin"
              spellCheck={false}
              value={content}
        ...
        <div className="w-[280px] shrink-0 bg-[#0d1220] border-l border-[#1e2d42] flex flex-col overflow-hidden">
          {/* Panel tabs */}
                  ...
                  <div className="space-y-1.5">
                    {docs.items.map(item => (
                      <div key={item.label} className="flex items-center gap-2">
                        <code className="text-xs font-mono text-[#e8002d] bg-[#070d19] px-1.5 py-0.5 rounded-sm min-w-[80px] shrink-0">{item.label}</code>
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {COMMON_PATTERNS.map(p => (
                      <div key={p.label} className="bg-[#070d19] border border-[#1e2d42] rounded-sm p-2">
            ...
            {panelView === 'test' && (
              <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {SAMPLE_EVENTS.map(s => (
                      <button
                        ...
                        className="flex items-center gap-1 px-2 py-1 text-[10px] rounded-sm border border-[#1e2d42] bg-[#0d1220] text-[#7d92b0] hover:text-white hover:border-[#3d5068] transition-colors"
                        title={s.label}
                      >
                ...
                <div>
                  ...
                  <textarea
                    className="w-full h-40 bg-[#070d19] border border-[#1e2d42] rounded-sm p-2 text-xs font-mono text-[#7d92b0] focus:outline-hidden focus:border-[#e8002d]/50 resize-none"
                    value={testEvent}
                ...
                {testResult && (
                  <div className="space-y-2">
                    ...
                    <button className="w-full py-1.5 text-xs border border-[#1e2d42] text-[#7d92b0] rounded-sm hover:text-white hover:border-[#7d92b0]/40 transition-colors">
            ...
            {panelView === 'ai' && (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <textarea
                    className="w-full h-24 bg-[#070d19] border border-[#1e2d42] rounded-sm p-2 text-xs text-white placeholder-[#3d5068] focus:outline-hidden focus:border-[#e8002d]/50 resize-none"
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2 bg-[#e8002d]/20 border border-[#e8002d]/30 text-[#e8002d] text-sm rounded-sm hover:bg-[#e8002d]/30 disabled:opacity-50 transition-colors"
                >
                  {aiGenerating
                    ...
                    <div className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                          ...
                          className="text-xs px-2 py-0.5 bg-[#e8002d]/20 text-[#e8002d] border border-[#e8002d]/30 rounded-sm hover:bg-[#e8002d]/30 transition-colors"
                        >
                          エディタに適用
                        </button>
                      </div>

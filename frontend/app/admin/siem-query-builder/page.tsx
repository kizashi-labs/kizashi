...
export default function SiemQueryBuilderPage() {
  ...
  const { data: savedQueries, isLoading: loadingSaved } = useQuery<SavedQuery[]>({
    ...
    queryFn: () =>
      apiFetchList<SavedQuery>('/api/v1/admin/siem-queries').catch(() => []),
    staleTime: 60_000,
  })
  ...
  const saveQueryMutation = useMutation({
    mutationFn: (body: { name: string; description: string; query: string }) =>
      apiFetch('/api/v1/admin/siem-queries', { method: 'POST', body: JSON.stringify(body) }).catch(() => body),
    onSuccess: () => {
  ...
  const deleteQueryMutation = useMutation({
    mutationFn: (id: string) =>
      apiFetch(`/api/v1/admin/siem-queries/${id}`, { method: 'DELETE' }).catch(() => ({})),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['siem-saved-queries'] })

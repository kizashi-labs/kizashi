...
export default function SecurityMetricsPage() {
  ...
  const { data: namesData = {} as MetricNames } = useQuery<MetricNames>({
    queryKey: ['security-metric-names'],
    queryFn: () => apiFetch<MetricNames>('/api/v1/admin/security-metrics/names'),
  })
            ...
            {metricsLoading ? (
              <div className="space-y-2">
                {Array.from({ length: 7 }).map((_, i) => (
                  <div key={i} className="h-8 bg-[#1e2d42] rounded-sm animate-pulse" />
                ))}
              </div>
            ) : points.length === 0 ? (

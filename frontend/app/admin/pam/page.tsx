  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-6 w-full max-w-md mx-4">
        ...
        <textarea
          className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm resize-none focus:outline-hidden focus:border-[#e8002d] h-28"
          placeholder="却下理由を入力..."
          value={reason}
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm resize-none focus:outline-hidden focus:border-[#e8002d] h-24"
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3 p-3 bg-[#0d1220] border border-[#1e2d42] rounded-lg">
        <select
          className="bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1.5 text-white text-xs focus:outline-hidden focus:border-[#e8002d]"
          value={statusFilter}
        ...
        <input
          type="text"
          className="bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1.5 text-white text-xs focus:outline-hidden focus:border-[#e8002d] w-40"
...
export default function PAMPage() {
  ...
  const { data: statsData } = useQuery<PAMStats>({
    queryKey: ['pam-stats'],
    queryFn: () => apiFetch('/api/v1/pam/stats'),
    staleTime: 30_000,
    retry: false,
  })
  ...
  const { data: requestsData = [], refetch: refetchRequests } = useQuery<PAMRequest[]>({
    ...
    queryFn: () => apiFetchList<PAMRequest>('/api/v1/pam/requests'),
    staleTime: 30_000,
    retry: false,
  })
  ...
  const { data: sessionsData = [], refetch: refetchSessions } = useQuery<PAMSession[]>({
    ...
    queryFn: () => apiFetchList<PAMSession>('/api/v1/pam/sessions'),
    staleTime: 30_000,
    retry: false,

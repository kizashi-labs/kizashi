  ...
  return (
    <div>
      ...
      >
        ...
        <span className="text-xs bg-[#1e2d42] text-[#7d92b0] px-1.5 py-0.5 rounded">{group.endpoint_count}</span>
      </div>
      ...
      <div className="flex gap-4">
        ...
        <div className="w-[35%] bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4 flex flex-col gap-2">
          <div className="flex items-center justify-between mb-1">
            ...
            <button className="flex items-center gap-1 text-xs bg-[#e8002d] hover:bg-[#c0001f] text-white px-2 py-1 rounded transition-colors">
            ...
            <>
              ...
              <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={addRule} className="flex items-center gap-1 text-xs text-[#7d92b0] hover:text-white bg-[#1e2d42] hover:bg-[#2e4060] px-2 py-1 rounded transition-colors">
                ...
                <div className="space-y-2">
                  {editingRules.map(rule => (
                    <div key={rule.id} className="flex items-center gap-2">
                      <select value={rule.field} onChange={e => updateRule(rule.id, 'field', e.target.value)} className="bg-[#070d19] border border-[#1e2d42] text-white text-sm px-2 py-1.5 rounded focus:outline-none focus:border-[#e8002d]/50">
                        {RULE_FIELDS.map(f => <option key={f} value={f}>{FIELD_LABELS[f]}</option>)}
                      </select>
                      <select value={rule.operator} onChange={e => updateRule(rule.id, 'operator', e.target.value)} className="bg-[#070d19] border border-[#1e2d42] text-white text-sm px-2 py-1.5 rounded focus:outline-none focus:border-[#e8002d]/50">
                        {RULE_OPERATORS.map(o => <option key={o} value={o}>{OP_LABELS[o]}</option>)}
                      </select>
                      <input value={rule.value} onChange={e => updateRule(rule.id, 'value', e.target.value)} placeholder="値を入力..." className="flex-1 bg-[#070d19] border border-[#1e2d42] text-white text-sm px-2 py-1.5 rounded focus:outline-none focus:border-[#e8002d]/50" />
              ...
              <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4">
                      ...
                      <tr key={ep.id} className="border-b border-[#1e2d42]/50 hover:bg-[#1e2d42]/20 transition-colors">
                        ...
                        <td className="py-2 pr-3">
                          <div className="flex gap-1">
                            <button className="text-xs text-blue-400 hover:text-blue-300 bg-blue-900/30 hover:bg-blue-900/50 px-1.5 py-0.5 rounded transition-colors">手動追加</button>
                            <button className="text-xs text-red-400 hover:text-red-300 bg-red-900/30 hover:bg-red-900/50 px-1.5 py-0.5 rounded transition-colors">除外</button>
                          </div>
                        </td>
                      </tr>
                    ))}
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-end gap-2 mt-3">
                    <button onClick={() => setEndpointPage(p => Math.max(1, p - 1))} disabled={endpointPage === 1} className="text-xs text-[#7d92b0] hover:text-white disabled:opacity-40 px-2 py-1 bg-[#1e2d42] rounded">前</button>
                    <span className="text-xs text-[#7d92b0]">{endpointPage} / {totalPages}</span>
                    <button onClick={() => setEndpointPage(p => Math.min(totalPages, p + 1))} disabled={endpointPage === totalPages} className="text-xs text-[#7d92b0] hover:text-white disabled:opacity-40 px-2 py-1 bg-[#1e2d42] rounded">次</button>
                  </div>
                )}
              </div>

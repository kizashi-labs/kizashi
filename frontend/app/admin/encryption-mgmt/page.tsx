...
export default function EncryptionMgmtPage() {
  ...
  const { data: policies = [] } = useQuery<EncPolicy[]>({
    ...
    queryFn: () => apiFetchList<EncPolicy>('/api/v1/admin/encryption/policies').catch(() => []),
    staleTime: 30_000,
  })

  const { data: endpoints = [] } = useQuery<EndpointEnc[]>({
    queryKey: ['enc-endpoints'],
    queryFn: () => apiFetchList<EndpointEnc>('/api/v1/admin/encryption/endpoints').catch(() => []),
    staleTime: 30_000,
  })

  const togglePolicy = useMutation({
    mutationFn: (id: string) =>
      apiFetch(`/api/v1/admin/encryption/policies/${id}/toggle`, { method: 'POST' }).catch(() => null),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['enc-policies'] }),
  })

  const forceEncrypt = useMutation({
    mutationFn: (ids: string[]) =>
      apiFetch('/api/v1/admin/encryption/enforce', { method: 'POST', body: JSON.stringify({ endpoint_ids: ids }) }).catch(() => null),
  ...
  const STATS = [
    { label: '総エンドポイント', value: totalEps.toString(), color: 'text-white', bg: 'bg-falcon-surface border-falcon-border' },
    ...
    <div className="min-h-screen bg-[#070d19] text-white">
      <div className="max-w-[1400px] mx-auto px-6 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-falcon-muted text-sm ml-11">エンドポイントの暗号化ポリシーと適用状況を管理します</p>
        </div>
                ...
                <tbody>
                  {policies.map(p => (
                    <tr key={p.id} className="border-b border-falcon-border/50 hover:bg-[#131d31]/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-falcon-muted font-mono text-xs">{p.algorithm}</td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-falcon-muted">{p.target_endpoints}</td>
              ...
              <div className="flex items-center gap-3">
                {unencryptedEps.length > 0 && (
                  <button onClick={selectAllUnencrypted}
                    className="text-xs text-falcon-muted hover:text-white transition-colors underline">
                    未暗号化をすべて選択
                  </button>
                )}
                {selected.size > 0 && (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredEndpoints.map(e => (
                    <tr key={e.id} className={`border-b border-falcon-border/50 hover:bg-[#131d31]/50 transition-colors ${selected.has(e.id) ? 'bg-[#131d31]/70' : ''}`}>
                      <td className="px-4 py-3">
                        {e.status === 'unencrypted' && (
                          <input type="checkbox" checked={selected.has(e.id)} onChange={() => toggleSelect(e.id)}
                            className="accent-falcon-red w-4 h-4 cursor-pointer" />
                        )}
                      </td>
                      <td className="px-4 py-3 text-white font-medium">{e.hostname}</td>
                      <td className="px-4 py-3 text-falcon-muted font-mono text-xs">{e.id}</td>
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-0.5 rounded-sm border ${statusCls[e.status]}`}>
                          {statusLabel[e.status]}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-falcon-muted font-mono text-xs">{e.algorithm}</td>
                      <td className="px-4 py-3">
                        {e.compliance
                      ...
                      <td className="px-4 py-3 text-falcon-muted text-xs whitespace-nowrap">{fmtDate(e.last_checked)}</td>
                      <td className="px-4 py-3">
                        {e.status !== 'encrypted' && (
                          <button onClick={() => forceEncrypt.mutate([e.id])}
                            className="flex items-center gap-1 px-3 py-1 text-xs bg-falcon-red/20 hover:bg-falcon-red/40 text-red-300 border border-red-700/50 rounded-lg transition-colors">
        ...
        <div className="mt-8 bg-falcon-surface rounded-xl border border-falcon-border p-6">
          <p className="text-xs text-falcon-muted uppercase tracking-widest mb-6">コンプライアンス概要</p>
          <div className="flex items-center gap-12">
            ...
            <div className="relative shrink-0">
              <div className="w-36 h-36 rounded-full flex items-center justify-center"
                ...
                <div className="w-24 h-24 rounded-full bg-falcon-surface flex flex-col items-center justify-center">
                  <p className="text-2xl font-bold text-white">{complianceRate}%</p>
                  <p className="text-xs text-falcon-muted">準拠率</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 flex-1">
              {[
                ...
                <div key={item.label} className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-falcon-muted flex-1">{item.label}</span>
                  <span className="text-sm font-medium text-white w-8 text-right">{item.value}</span>
                  <span className="text-xs text-falcon-muted w-10 text-right">{item.pct}%</span>
                </div>
              ))}
              <div className="border-t border-falcon-border pt-3 flex items-center gap-3">
                <span className="w-3 h-3 rounded-full shrink-0 bg-falcon-border" />
                <span className="text-sm text-falcon-muted flex-1">合計</span>
                <span className="text-sm font-bold text-white w-8 text-right">{totalEps}</span>
                <span className="text-xs text-falcon-muted w-10 text-right">100%</span>
              </div>
            </div>
          </div>
        </div>

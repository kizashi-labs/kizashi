...
function EventBubble({ event }: { event: SecurityEvent }) {
  ...
  return (
    <div className="flex gap-4 relative group">
      ...
      {/* Event card */}
      <div
        className="flex-1 bg-[#0d1220] border border-[#1e2d42] rounded-lg p-3 mb-3 cursor-pointer
                   hover:border-[#2a3f5c] transition-colors"
        onClick={() => setExpanded(e => !e)}
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`text-xs px-1.5 py-0.5 rounded flex items-center gap-1 ${EVENT_TYPE_STYLES[event.event_type]}`}>
    ...
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={() => refetch()}
          disabled={isFetching}
          className="flex items-center gap-2 px-3 py-2 bg-[#0d1220] border border-[#1e2d42] hover:bg-[#19253d]
                     text-[#7d92b0] hover:text-white rounded-lg text-sm transition-colors disabled:opacity-50"
        >

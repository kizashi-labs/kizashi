  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between items-center">
        ...
        <span className="text-sm font-bold text-white min-w-[3rem] text-right">{value}</span>
      </div>
      ...
      <div
        ...
        className={`w-5 h-5 rounded-sm border-2 flex items-center justify-center transition-colors shrink-0 ${
          checked
            ...
            : 'border-[#1e2d42] group-hover:border-blue-500/50'
        }`}
      >
...
function StrengthMeter({ score }: { score: number }) {
  const levels = [
    { min: 0, max: 0, label: '入力してください', color: 'bg-[#1e2d42]' },
    { min: 1, max: 1, label: '非常に弱い',       color: 'bg-[#e8002d]' },
...
function ConfirmDialog({ message, onConfirm, onCancel }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-sm shadow-2xl">
  ...
  return (
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-[#7d92b0] text-sm">組織のパスワード強度・有効期限・ロックアウト設定</p>
      </div>

      {/* Current Policy Banner */}
      <div className="flex items-center justify-between bg-[#0d1220] border border-[#1e2d42] rounded-lg px-5 py-3 mb-6">
        <div className="flex items-center gap-3">
          <div className="w-2 h-2 rounded-full bg-green-400" />
          <div>
            <span className="text-xs text-[#7d92b0]">現在のポリシー</span>
            <span className="ml-3 text-white font-semibold">{policy.name}</span>
          </div>
          <span className="text-xs text-[#7d92b0] ml-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleReset}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-sm text-xs text-[#7d92b0] border border-[#1e2d42] hover:bg-[#1e2d42] transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-1.5 rounded-sm text-xs font-semibold bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-40 transition-colors"
          >
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-6">
          ...
          <SectionCard title="パスワード要件" icon={Lock}>
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 items-center">
                <div>
                  <label className="block text-xs text-[#7d92b0] mb-1.5">最大文字数</label>
                  <input
                    type="number"
                    ...
                    className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-white text-sm focus:outline-hidden focus:border-blue-500"
                  />
                </div>
                <div>
                  <label className="block text-xs text-[#7d92b0] mb-1.5">最低特殊文字数</label>
                  <input
                    type="number"
            ...
            <div className="space-y-6">
              <div>
                ...
                {policy.history_count === 0 && (
                  <p className="text-xs text-[#7d92b0] mt-1 flex items-center gap-1">
              ...
              <div className="grid grid-cols-2 gap-4 items-center">
                <div>
                  <label className="block text-xs text-[#7d92b0] mb-1.5">
                    最低有効日数 <span className="text-[#3d5068]">(最低N日は変更不可)</span>
                  </label>
                  <input
                    type="number"
                    ...
                    className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-white text-sm focus:outline-hidden focus:border-blue-500"
                  />
                  <p className="text-[10px] text-[#7d92b0] mt-1">最低{policy.min_age_days}日は変更不可</p>
                </div>
              </div>
            </div>
          </SectionCard>
          ...
          <SectionCard title="アカウントロックアウト" icon={Shield}>
            <div className="space-y-6">
              ...
              {/* Summary */}
              <div className="bg-[#070d19] border border-[#1e2d42] rounded-lg px-4 py-3 grid grid-cols-3 gap-4">
                <div className="text-center">
                  <p className="text-2xl font-bold text-white">{policy.lockout_attempts}</p>
                  <p className="text-xs text-[#7d92b0] mt-0.5">最大試行回数</p>
                </div>
                <div className="text-center border-x border-[#1e2d42]">
                  <p className="text-2xl font-bold text-white">{policy.lockout_duration_minutes}</p>
                  <p className="text-xs text-[#7d92b0] mt-0.5">ロック時間(分)</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-white">{policy.min_length}</p>
                  <p className="text-xs text-[#7d92b0] mt-0.5">最小文字数</p>
                </div>
              </div>
            </div>
          </SectionCard>
        ...
        <div className="col-span-1">
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg sticky top-6">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-[#1e2d42]">
            ...
            <div className="px-5 py-5 space-y-4">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 pr-10 text-white text-sm focus:outline-hidden focus:border-blue-500 placeholder-[#3d5068]"
                />
                <button
                  onClick={() => setShowPassword(v => !v)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[#7d92b0] hover:text-white"
                >
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-sm text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
              ...
              {validation && (
                <div className="space-y-2">
                  {validation.valid ? (
                    ...
                    <div className="space-y-1.5">
                      <p className="text-xs font-semibold text-[#7d92b0] uppercase tracking-wider">違反項目</p>
                      {validation.violations.map((v, i) => (
                        <div key={i} className="flex items-start gap-2 text-xs text-[#e8002d]">
              ...
              <div className="border-t border-[#1e2d42] pt-4 space-y-2">
                <p className="text-xs font-semibold text-[#7d92b0] uppercase tracking-wider">現在の要件</p>
                {[
                  ...
                  <div key={i} className="flex items-center gap-2">
                    {!testPassword
                      ? <div className="w-3.5 h-3.5 rounded-full border border-[#1e2d42] shrink-0" />
                      : r.ok
                      ? <CheckCircle2 className="w-3.5 h-3.5 text-green-400 shrink-0" />
                      : <XCircle className="w-3.5 h-3.5 text-[#e8002d] shrink-0" />
                    }
                    <span className={`text-xs ${
                      !testPassword ? 'text-[#7d92b0]' :
                      r.ok ? 'text-green-400' : 'text-[#e8002d]'
                    }`}>{r.label}</span>
                  </div>
                ))}
              </div>
      ...
      {/* Bottom save bar */}
      <div className="mt-6 flex items-center justify-end gap-3 bg-[#0d1220] border border-[#1e2d42] rounded-lg px-5 py-3">
        <button
          onClick={handleReset}
          className="flex items-center gap-2 px-4 py-2 rounded-sm text-sm text-[#7d92b0] border border-[#1e2d42] hover:bg-[#1e2d42] transition-colors"
        >
        ...
        <button
          onClick={handleSave}
          disabled={saveMutation.isPending}
          className="flex items-center gap-2 px-6 py-2 rounded-sm text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-40 transition-colors"
        >
          {saveMutation.isPending
            ? <RefreshCw className="w-4 h-4 animate-spin" />

    ...
    <div>
      <div className="flex gap-2 mb-4">
        {filters.map(f => (
          <button
            ...
            className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filter === f.key ? 'bg-falcon-red text-white' : 'bg-[#070d19] text-falcon-muted border border-falcon-border hover:text-white'}`}
          >
            {f.label}
          </button>
        ))}
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map(v => (
              <tr key={v.id} className="border-b border-falcon-border hover:bg-[#070d19] transition-colors">
                <td className="px-4 py-3 text-white font-medium">{v.name}</td>
                <td className="px-4 py-3 text-falcon-muted">{v.type}</td>
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs ${v.criticality === 'critical' ? 'text-red-400' : v.criticality === 'important' ? 'text-orange-400' : 'text-falcon-muted'}`}>
                    {v.criticality === 'critical' ? '重要' : v.criticality === 'important' ? '重要度高' : '標準'}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span className={`text-xs ${v.assessment_status === '完了' ? 'text-green-400' : v.assessment_status === '実施中' ? 'text-blue-400' : 'text-falcon-muted'}`}>
                    {v.assessment_status}
                  </span>
                </td>
                <td className="px-4 py-3 text-falcon-muted text-xs">{v.last_assessed}</td>
                <td className="px-4 py-3">
                  <button className="flex items-center gap-1 px-2.5 py-1 bg-falcon-surface border border-falcon-border text-falcon-muted rounded-sm text-xs hover:text-white hover:border-falcon-red transition-colors">
                    <RefreshCw size={10} />
                    評価実行
                  </button>
                </td>
  ...
  return (
    <div className="space-y-3">
      {incidents.map(inc => {
        ...
        return (
          <div key={inc.id} className="p-4 bg-[#070d19] border border-falcon-border rounded-lg">
            ...
            <div className="flex items-center gap-4 text-xs text-falcon-muted mb-2">
              <span className="flex items-center gap-1"><Building2 size={11} />{inc.vendor_name}</span>
              <span>{inc.reported_at}</span>
            </div>
            <p className="text-sm text-falcon-muted">{inc.description}</p>
          </div>
        )
      })}
    </div>
    ...
    <div>
      <div className="flex gap-6">
        <div className="flex-1">
          <div className="text-center text-falcon-muted text-xs mb-2">発生可能性 →</div>
          <div className="flex items-start gap-1">
            <div className="flex flex-col justify-around h-[224px] mr-1">
              {yLabels.map(l => <span key={l} className="text-falcon-muted text-xs">{l}</span>)}
            </div>
            <div className="flex-1">
              <div className="grid grid-cols-4 gap-1">
                {grid.map((row, ri) =>
                  row.map((cell, ci) => (
                    <div key={`${ri}-${ci}`} className={`${cellColor(ri, ci)} border border-falcon-border rounded-sm h-14 p-1 flex flex-wrap gap-1 items-start content-start`}>
                      {cell.map(v => (
                        <div
                          key={v.id}
                          title={v.name}
                          className={`w-3 h-3 rounded-full shrink-0 ${riskBadge[v.risk_level].split(' ')[0]}`}
                        />
                      ))}
                    </div>
                  ))
              ...
              <div className="grid grid-cols-4 gap-1 mt-1">
                {xLabels.map(l => <div key={l} className="text-center text-falcon-muted text-xs">{l}</div>)}
              </div>
            </div>
          </div>
          <div className="text-center text-falcon-muted text-xs mt-1">← 影響度</div>
        </div>

        <div className="w-48">
          <div className="text-falcon-muted text-xs font-medium uppercase tracking-wider mb-2">凡例</div>
          <div className="space-y-2">
            {vendors.map(v => (
              <div key={v.id} className="flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full shrink-0 ${riskBadge[v.risk_level].split(' ')[0]}`} />
                <span className="text-sm text-falcon-muted truncate">{v.name}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3 mt-6 pt-4 border-t border-falcon-border">
        {(['critical', 'high', 'medium', 'low'] as RiskLevel[]).map(level => {
          const count = vendors.filter(v => v.risk_level === level).length
          return (
            <div key={level} className="bg-[#070d19] border border-falcon-border rounded-lg p-3 text-center">
              <div className={`text-xl font-bold ${riskColor[level]}`}>{count}</div>
              <div className="text-falcon-muted text-xs mt-1">{riskLabel[level]}リスク</div>
            </div>
          )
        })}
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-falcon-muted text-sm mt-1">Supply Chain Risk Management</p>
      </div>

          ...
          <tbody>
            {filtered.map(v => (
              <tr key={v.id} className="border-b border-[#1e2d42] hover:bg-[#070d19] transition-colors">
                ...
                <td className="px-4 py-3">
                  <button className="flex items-center gap-1 px-2.5 py-1 bg-[#0d1220] border border-[#1e2d42] text-[#7d92b0] rounded-sm text-xs hover:text-white hover:border-[#e8002d] transition-colors">
                    <RefreshCw size={10} />
                    評価実行
                  </button>
                </td>
          ...
          <div className="flex items-start gap-1">
            ...
            <div className="flex-1">
              <div className="grid grid-cols-4 gap-1">
                {grid.map((row, ri) =>
                  row.map((cell, ci) => (
                    <div key={`${ri}-${ci}`} className={`${cellColor(ri, ci)} border border-[#1e2d42] rounded-sm h-14 p-1 flex flex-wrap gap-1 items-start content-start`}>
                      {cell.map(v => (
                        <div
                          key={v.id}
                          title={v.name}
...
export default function SupplyChainRiskPage() {
  ...
  const { data: vendors = [] } = useQuery<Vendor[]>({
    ...
    queryFn: () =>
      apiFetchList<Vendor>('/api/v1/admin/supply-chain-risk/vendors'),
  })

  const { data: incidents = [] } = useQuery<SCIncident[]>({
    queryKey: ['supply-chain-incidents'],
    queryFn: () =>
      apiFetchList<SCIncident>('/api/v1/admin/supply-chain-risk/incidents'),
  })

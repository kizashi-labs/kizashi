...
function Toggle({
  ...
  return (
    ...
    >
      <span
        className={`absolute top-0.5 w-4 h-4 rounded-full bg-[#e2e8f4] shadow-sm transition-transform duration-200 ${
...
function StatCard({
  ...
  return (
    <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4 flex items-start gap-3">
      <div className="w-9 h-9 rounded-lg bg-[#1e2d42] flex items-center justify-center shrink-0">
...
function EmailTagInput({
  ...
  return (
    <div className="flex flex-wrap gap-2 p-2 rounded-sm bg-[#070d19] border border-[#1e2d42] min-h-[42px]">
      {emails.map((email) => (
        <span
          key={email}
          className="flex items-center gap-1.5 px-2 py-1 rounded-sm bg-[#1e2d42] text-xs text-[#e2e8f4]"
        >
          {email}
          <button
      ...
      <input
        ...
        className="flex-1 min-w-[180px] bg-transparent text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-hidden"
      />
      <button
        onClick={addEmail}
        className="p-1 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-[#e2e8f4] transition-colors"
      >
    ...
    >
      ...
      >
        ...
        <div className="p-5 max-h-[70vh] overflow-y-auto space-y-4">
          {/* Email header */}
          <div className="p-4 rounded-lg bg-linear-to-r from-[#e8002d]/20 to-[#070d19] border border-[#e8002d]/30">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3 rounded-sm bg-[#070d19] border border-[#1e2d42] text-center">
          ...
          <div>
            ...
            <div className="rounded-sm bg-[#070d19] border border-[#1e2d42] overflow-hidden">
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {mockAgents.map((a, i) => (
                <div
                  key={a.name}
                  className="flex items-center gap-3 p-2 rounded-sm bg-[#070d19] border border-[#1e2d42]"
                >
...
export default function AlertDigestPage() {
  ...
  // .catch(() => null) の外で無条件に「送信しました」を出していました。
  // ダイジェストは届いたかどうかを受け取り手が知らせてくれない種類の
  // 通知なので、送れなかったことはこの画面でしか分かりません。
  const triggerDigest = async (period: 'daily' | 'weekly') => {
    setSendingPeriod(period)
    const ok = await persist('ダイジェストの送信', '/api/v1/admin/digest/trigger', {
      method: 'POST',
      body: JSON.stringify({ period }),
    })
    setSendingPeriod(null)
    if (ok) showToast('ダイジェストを送信しました')
  }
        ...
        <div className="max-w-3xl space-y-6">
            ...
            >
              ...
              <div className="flex justify-end pt-2 border-t border-[#1e2d42]">
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 rounded-sm text-sm font-medium bg-[#1e2d42] text-[#7d92b0] hover:bg-[#243448] hover:text-[#e2e8f4] disabled:opacity-50 transition-colors"
                >
                  {sendingPeriod === 'daily' ? (
          ...
          <div className="flex justify-end pt-2">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-lg font-semibold text-sm bg-[#e8002d] hover:bg-[#c0001f] text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-sm text-sm font-medium bg-[#1e2d42] text-[#7d92b0] hover:bg-[#243448] hover:text-[#e2e8f4] transition-colors"
            >
                  ...
                  <tr key={record.id} className="border-b border-[#1e2d42] last:border-0 hover:bg-[#070d19] transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        ...
                        className="flex items-center gap-1.5 ml-auto px-2.5 py-1 rounded-sm text-xs font-medium bg-[#1e2d42] text-[#7d92b0] hover:bg-[#243448] hover:text-[#e2e8f4] transition-colors"
                      >

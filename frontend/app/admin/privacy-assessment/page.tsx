        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input value={form.title} onChange={e => set('title', e.target.value)} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 transition-colors" placeholder="例: 顧客分析システム DPIA" />
            </div>
            <div>
              <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">システム/プロセス名</label>
            ...
            <div>
              <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">評価種別</label>
              <select value={form.assessment_type} onChange={e => set('assessment_type', e.target.value)} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-falcon-muted text-sm focus:outline-hidden focus:border-falcon-red/50">
                <option value="PIA">PIA（プライバシー影響評価）</option>
                <option value="DPIA">DPIA（データ保護影響評価）</option>
              </select>
            </div>
            <div className="col-span-2">
              <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">データ管理者</label>
              <input value={form.data_controller} onChange={e => set('data_controller', e.target.value)} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 transition-colors" />
            </div>
            <div className="col-span-2">
              <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">処理目的</label>
              <input value={form.processing_purpose} onChange={e => set('processing_purpose', e.target.value)} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 transition-colors" />
            </div>
            <div className="col-span-2">
              <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-2">データカテゴリ（複数選択可）</label>
                ...
                {(Object.keys(DATA_CAT_CONFIG) as DataCategory[]).map(cat => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-sm border text-xs font-medium transition-colors ${selected ? DATA_CAT_CONFIG[cat].cls : 'bg-[#070d19] border-falcon-border text-falcon-subtle hover:text-falcon-muted'}`}
                    >
...
function PIAWizard({ pia, onClose }: { pia: PIA; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/80 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-4xl max-h-[92vh] flex flex-col" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-start gap-4 p-6 border-b border-falcon-border">
          <div className="flex-1">
            ...
            <p className="text-falcon-muted text-sm">{pia.system_name}</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => alert('PDFエクスポートは実装予定です')}
              className="flex items-center gap-1.5 px-3 py-2 bg-[#0a1628] border border-falcon-border text-falcon-muted hover:text-white rounded-lg text-sm transition-colors"
            >
              ...
              <div key={s.num} className="flex items-center flex-1 last:flex-none">
                ...
                >
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
                    step === s.num ? 'bg-falcon-red text-white' :
                    s.num < step ? 'bg-green-900/50 border border-green-700 text-green-300' :
                    'bg-[#0a1628] border border-falcon-border text-falcon-subtle'
                  }`}>
                    {s.num < step ? <Check className="w-4 h-4" /> : s.num}
                  </div>
                  <span className={`text-xs mt-1 text-center max-w-[80px] leading-tight transition-colors ${step === s.num ? 'text-white' : 'text-falcon-subtle'}`}>{s.label}</span>
                </button>
                {i < STEPS.length - 1 && (
                  <div className={`flex-1 h-px mx-2 mb-4 ${s.num < step ? 'bg-green-700' : 'bg-falcon-border'}`} />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">処理目的</label>
                  <textarea value={form.processing_purpose} onChange={e => setF('processing_purpose', e.target.value)} rows={2} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 transition-colors resize-none" />
                </div>
                <div>
                  <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">法的根拠</label>
                  <select value={form.legal_basis} onChange={e => setF('legal_basis', e.target.value)} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-falcon-muted text-sm focus:outline-hidden focus:border-falcon-red/50">
                ...
                <div>
                  <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">保存期間</label>
                  <input value={form.retention_period} onChange={e => setF('retention_period', e.target.value)} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 transition-colors" />
                </div>
                <div className="col-span-2">
                  <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">データ主体</label>
                  <input value={form.data_subjects} onChange={e => setF('data_subjects', e.target.value)} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 transition-colors" />
                </div>
                <div className="col-span-2">
                  <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">データ受領者</label>
                  <input value={form.data_recipients} onChange={e => setF('data_recipients', e.target.value)} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 transition-colors" />
                </div>
                <div className="col-span-2">
                  <div className="flex items-center gap-3">
                    <button
                      onClick={() => setF('international_transfers', !form.international_transfers)}
                      className={`w-12 h-6 rounded-full relative transition-colors ${form.international_transfers ? 'bg-falcon-red' : 'bg-falcon-border'}`}
                    >
                      <span className={`absolute top-1 w-4 h-4 rounded-full bg-falcon-text transition-transform ${form.international_transfers ? 'translate-x-7' : 'translate-x-1'}`} />
                    </button>
                ...
                <div className="col-span-2">
                  <p className="text-falcon-muted text-xs uppercase tracking-wider mb-2">処理するデータカテゴリ</p>
                  <div className="flex flex-wrap gap-2">
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <p className="text-falcon-muted text-sm">各データカテゴリが処理目的に対して必要かどうかを評価してください。</p>
              <div className="space-y-3">
                {form.necessities.map((n, idx) => (
                  <div key={idx} className="bg-[#070d19] border border-falcon-border rounded-xl p-4">
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded border text-sm font-medium transition-colors ${
                            n.is_necessary === val
                              ...
                              : 'bg-falcon-surface border-falcon-border text-falcon-subtle hover:text-falcon-muted'
                          }`}
                        >
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full bg-falcon-surface border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 transition-colors resize-none"
                    />
                  </div>
                ))}
              </div>
              <div className="bg-[#070d19] border border-falcon-border rounded-xl p-4">
                <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-2">データ最小化措置</label>
                <textarea
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <p className="text-falcon-muted text-sm">各プライバシーリスクの可能性（1-5）と影響度（1-5）を評価してください。</p>

              {/* Risk Matrix Legend */}
              <div className="bg-[#070d19] border border-falcon-border rounded-xl p-4">
                <p className="text-falcon-muted text-xs mb-3">リスクスコア = 可能性 × 影響度</p>
              ...
              <div className="space-y-3">
                {form.risks.map(risk => (
                  <div key={risk.id} className="bg-[#070d19] border border-falcon-border rounded-xl p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-falcon-muted text-xs block mb-2">可能性 ({risk.likelihood}/5)</label>
                        <div className="flex gap-1">
                          {[1, 2, 3, 4, 5].map(v => (
                            <button
                              key={v}
                              onClick={() => updateRisk(risk.id, 'likelihood', v)}
                              className={`w-8 h-8 rounded-sm text-xs font-bold transition-colors ${risk.likelihood >= v ? riskCellColor(v) + ' text-white' : 'bg-falcon-surface text-falcon-subtle'}`}
                            >
                              {v}
                            </button>
                          ))}
                      ...
                      <div>
                        <label className="text-falcon-muted text-xs block mb-2">影響度 ({risk.impact}/5)</label>
                        <div className="flex gap-1">
                          {[1, 2, 3, 4, 5].map(v => (
                            <button
                              key={v}
                              onClick={() => updateRisk(risk.id, 'impact', v)}
                              className={`w-8 h-8 rounded-sm text-xs font-bold transition-colors ${risk.impact >= v ? riskCellColor(v) + ' text-white' : 'bg-falcon-surface text-falcon-subtle'}`}
                            >
                              {v}
                            </button>
                          ))}
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 transition-colors"
                />
                <button onClick={addCustomRisk} className="px-4 py-2 bg-falcon-red/10 border border-falcon-red/30 text-falcon-red rounded-lg text-sm font-medium hover:bg-falcon-red/20 transition-colors">
          ...
          {step === 4 && (
            <div className="space-y-4">
              ...
              <p className="text-falcon-muted text-sm">各リスクに対する軽減措置と残余リスクを設定してください。</p>
              <div className="space-y-4">
                {form.risks.map(risk => (
                  <div key={risk.id} className="bg-[#070d19] border border-falcon-border rounded-xl p-4">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="mb-3">
                      <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">軽減措置</label>
                      <textarea
                    ...
                    <div>
                      <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-2">措置後の残余リスク</label>
                      <div className="flex gap-2">
                        {(['low', 'medium', 'high'] as const).map(level => (
                          <button
                            key={level}
                            onClick={() => updateRisk(risk.id, 'residual_risk', level)}
                            className={`px-3 py-1.5 rounded-sm border text-xs font-medium transition-colors ${risk.residual_risk === level ? RESIDUAL_RISK_CONFIG[level] : 'bg-falcon-surface border-falcon-border text-falcon-subtle hover:text-falcon-muted'}`}
                          >
              ...
              <div className="bg-[#070d19] border border-falcon-border rounded-xl p-4">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <button
                    ...
                    className={`w-12 h-6 rounded-full relative transition-colors ${form.dpo_consulted ? 'bg-falcon-red' : 'bg-falcon-border'}`}
                  >
                    <span className={`absolute top-1 w-4 h-4 rounded-full bg-falcon-text transition-transform ${form.dpo_consulted ? 'translate-x-7' : 'translate-x-1'}`} />
                  </button>
                ...
                {form.dpo_consulted && (
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">協議日</label>
                      <input type="date" value={form.dpo_consultation_date} onChange={e => setF('dpo_consultation_date', e.target.value)} className="w-full bg-falcon-surface border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 transition-colors" />
                    </div>
                    <div className="col-span-2">
                      <label className="text-falcon-muted text-xs uppercase tracking-wider block mb-1.5">DPOコメント</label>
                      <textarea value={form.dpo_notes} onChange={e => setF('dpo_notes', e.target.value)} rows={3} className="w-full bg-falcon-surface border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 transition-colors resize-none" />
                    </div>
                  </div>
                )}
              </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between p-6 border-t border-falcon-border">
          <button
            onClick={() => setStep(s => Math.max(1, s - 1))}
            disabled={step === 1}
            className="flex items-center gap-1.5 px-4 py-2 bg-falcon-surface border border-falcon-border text-falcon-muted rounded-lg text-sm font-medium hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
          >
            ← 前へ
          </button>
          <div className="flex items-center gap-2">
            <button onClick={handleSave} className="px-4 py-2 bg-falcon-surface border border-falcon-border text-falcon-muted rounded-lg text-sm hover:text-white transition-colors">
              保存
            </button>
            {step < 5 && (
              <button
                onClick={() => setStep(s => Math.min(5, s + 1))}
                className="flex items-center gap-1.5 px-4 py-2 bg-falcon-red text-white rounded-lg text-sm font-semibold hover:bg-[#c0001f] transition-colors"
              >
                次へ →
              </button>
            )}
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-falcon-muted text-sm">Privacy Impact Assessment / Data Protection Impact Assessment</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
                ...
                return (
                  <tr key={pia.id} className="hover:bg-[#0a1628] transition-colors">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-falcon-subtle text-xs">{pia.system_name}</p>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-falcon-muted text-sm max-w-[140px] truncate">{pia.data_controller}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      {pia.dpo_consulted
                        ? <CheckCircle className="w-4 h-4 text-green-400" />
                        : <AlertCircle className="w-4 h-4 text-falcon-subtle" />
                      }
                    </td>
                    <td className="px-4 py-3 text-falcon-muted text-sm whitespace-nowrap">{pia.started_date}</td>
                    <td className="px-4 py-3 text-falcon-muted text-sm whitespace-nowrap">{pia.completed_date ?? '—'}</td>
                    <td className="px-4 py-3 text-falcon-muted text-sm whitespace-nowrap">{pia.next_review_date ?? '—'}</td>
                    <td className="px-4 py-3">
                      <button

...
function CreatePIAModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    if (!(await persist('プライバシー影響評価', '/api/v1/admin/pia', { method: 'POST', body: JSON.stringify(form) }))) return
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={onClose}>
      <div className="absolute top-4 left-1/2 -translate-x-1/2 w-full max-w-2xl px-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input value={form.title} onChange={e => set('title', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 transition-colors" placeholder="例: 顧客分析システム DPIA" />
            </div>
            <div>
            ...
            <div>
              ...
              <select value={form.assessment_type} onChange={e => set('assessment_type', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-[#7d92b0] text-sm focus:outline-hidden focus:border-[#e8002d]/50">
            ...
            <div className="col-span-2">
              ...
              <input value={form.data_controller} onChange={e => set('data_controller', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 transition-colors" />
            </div>
            <div className="col-span-2">
              <label className="text-[#7d92b0] text-xs uppercase tracking-wider block mb-1.5">処理目的</label>
              <input value={form.processing_purpose} onChange={e => set('processing_purpose', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 transition-colors" />
            </div>
            <div className="col-span-2">
                ...
                {(Object.keys(DATA_CAT_CONFIG) as DataCategory[]).map(cat => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-sm border text-xs font-medium transition-colors ${selected ? DATA_CAT_CONFIG[cat].cls : 'bg-[#070d19] border-[#1e2d42] text-[#3d5068] hover:text-[#7d92b0]'}`}
                    >
...
function PIAWizard({ pia, onClose }: { pia: PIA; onClose: () => void }) {
  ...
  // 評価の途中経過。保存できていないのに閉じると、書いた内容が消えます。
  const handleSave = async () => {
    if (await persist('評価内容', `/api/v1/admin/pia/${pia.id}`, { method: 'PUT', body: JSON.stringify({ ...form, current_step: step }) })) {
      onClose()
    }
  }
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="text-[#7d92b0] text-xs uppercase tracking-wider block mb-1.5">処理目的</label>
                  <textarea value={form.processing_purpose} onChange={e => setF('processing_purpose', e.target.value)} rows={2} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 transition-colors resize-none" />
                </div>
                <div>
                  <label className="text-[#7d92b0] text-xs uppercase tracking-wider block mb-1.5">法的根拠</label>
                  <select value={form.legal_basis} onChange={e => setF('legal_basis', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-[#7d92b0] text-sm focus:outline-hidden focus:border-[#e8002d]/50">
                ...
                <div>
                  ...
                  <input value={form.retention_period} onChange={e => setF('retention_period', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 transition-colors" />
                </div>
                <div className="col-span-2">
                  <label className="text-[#7d92b0] text-xs uppercase tracking-wider block mb-1.5">データ主体</label>
                  <input value={form.data_subjects} onChange={e => setF('data_subjects', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 transition-colors" />
                </div>
                <div className="col-span-2">
                  <label className="text-[#7d92b0] text-xs uppercase tracking-wider block mb-1.5">データ受領者</label>
                  <input value={form.data_recipients} onChange={e => setF('data_recipients', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 transition-colors" />
                </div>
                <div className="col-span-2">
                ...
                {form.necessities.map((n, idx) => (
                  <div key={idx} className="bg-[#070d19] border border-[#1e2d42] rounded-xl p-4">
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 transition-colors resize-none"
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-4">
                        ...
                        <div className="flex gap-1">
                          {[1, 2, 3, 4, 5].map(v => (
                            <button
                              ...
                              className={`w-8 h-8 rounded-sm text-xs font-bold transition-colors ${risk.likelihood >= v ? riskCellColor(v) + ' text-white' : 'bg-[#0d1220] text-[#3d5068]'}`}
                            >
                              {v}
                            </button>
                          ))}
                        ...
                        <div className="flex gap-1">
                          {[1, 2, 3, 4, 5].map(v => (
                            <button
                              ...
                              className={`w-8 h-8 rounded-sm text-xs font-bold transition-colors ${risk.impact >= v ? riskCellColor(v) + ' text-white' : 'bg-[#0d1220] text-[#3d5068]'}`}
                            >
                              {v}
                            </button>
                          ))}
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 transition-colors"
                />
                      ...
                      <div className="flex gap-2">
                        {(['low', 'medium', 'high'] as const).map(level => (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-sm border text-xs font-medium transition-colors ${risk.residual_risk === level ? RESIDUAL_RISK_CONFIG[level] : 'bg-[#0d1220] border-[#1e2d42] text-[#3d5068] hover:text-[#7d92b0]'}`}
                          >
                ...
                {form.dpo_consulted && (
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="date" value={form.dpo_consultation_date} onChange={e => setF('dpo_consultation_date', e.target.value)} className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 transition-colors" />
                    </div>
                    <div className="col-span-2">
                      <label className="text-[#7d92b0] text-xs uppercase tracking-wider block mb-1.5">DPOコメント</label>
                      <textarea value={form.dpo_notes} onChange={e => setF('dpo_notes', e.target.value)} rows={3} className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 transition-colors resize-none" />
                    </div>
                  </div>
                )}
              </div>

...
const healthColor: Record<HealthStatus, string> = {
  healthy: 'text-green-400', degraded: 'text-yellow-400', error: 'text-red-400', unknown: 'text-falcon-muted',
}
...
function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <Star key={i} className={`w-3 h-3 ${i <= Math.round(rating) ? 'text-yellow-400 fill-yellow-400' : 'text-falcon-subtle'}`} />
      ))}
      <span className="text-[11px] text-falcon-muted ml-1">{rating}</span>
    </div>
  )
}
      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-falcon-border">
          <div className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-xl bg-linear-to-br ${integration.logo_color} flex items-center justify-center text-white font-bold text-sm`}>
              {integration.logo_placeholder}
            </div>
            <div>
              <h3 className="text-white font-semibold">{integration.name}</h3>
              <p className="text-xs text-falcon-muted">{integration.vendor} · v{integration.version}</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5">
          <div className="flex items-center gap-3">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-sm ${priceColor[integration.price_type]}`}>
              {priceLabel[integration.price_type]}{integration.price ? ` (${integration.price})` : ''}
            </span>
            <StarRating rating={integration.rating} />
            <span className="text-xs text-falcon-muted">{integration.review_count}レビュー</span>
            <span className="text-xs text-falcon-muted ml-auto">{(integration.install_count ?? 0).toLocaleString()}件インストール</span>
          </div>

          <p className="text-sm text-falcon-muted leading-relaxed">{integration.long_description}</p>

          <div>
            <p className="text-xs font-medium text-white mb-2">主な機能</p>
            <div className="grid grid-cols-2 gap-1.5">
              {integration.features.map(f => (
                <div key={f} className="flex items-center gap-1.5 text-xs text-falcon-muted">
          ...
          <div>
            ...
            <ul className="space-y-1">
              {integration.requirements.map(r => (
                <li key={r} className="flex items-center gap-1.5 text-xs text-falcon-muted">
                  <ChevronRight className="w-3.5 h-3.5 text-falcon-red" /> {r}
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-1">
              {integration.changelog.map(c => (
                <li key={c} className="text-xs text-falcon-muted">· {c}</li>
              ))}
            </ul>
          </div>
        </div>
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-falcon-border">
          <button onClick={onClose} className="px-4 py-2 text-sm text-falcon-muted hover:text-white">閉じる</button>
          {integration.installed ? (
            ...
            <button onClick={onInstall} disabled={isLoading}
              className="flex items-center gap-2 px-4 py-2 bg-falcon-red hover:bg-[#c0001f] text-white text-sm rounded-lg transition-colors disabled:opacity-40">
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-md">
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-falcon-muted">Kizashi Integration SDKを使用してカスタム統合を開発できます。</p>
          <div className="space-y-2">
            {[
              ...
              <div key={d.title} className="flex items-center gap-3 p-3 bg-[#070d19] border border-falcon-border rounded-lg hover:border-falcon-muted/30 cursor-pointer transition-colors group">
                <Code2 className="w-4 h-4 text-falcon-red" />
                <div className="flex-1">
                  <p className="text-sm text-white">{d.title}</p>
                  <p className="text-[11px] text-falcon-muted">{d.desc}</p>
                </div>
                <ExternalLink className="w-3.5 h-3.5 text-falcon-subtle group-hover:text-falcon-muted" />
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="min-h-screen bg-[#070d19] p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-falcon-red/10 border border-falcon-red/30 flex items-center justify-center">
            <ShoppingBag className="w-5 h-5 text-falcon-red" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">統合マーケットプレース</h1>
            <p className="text-xs text-falcon-muted">{integrations.length}個の統合 · {installed.length}個インストール済み</p>
          </div>
        </div>
        <button
          onClick={() => setShowDevDoc(true)}
          className="flex items-center gap-2 px-4 py-2 border border-falcon-border text-falcon-muted hover:text-white hover:border-falcon-muted/40 text-sm rounded-lg transition-colors"
        >
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48 max-w-64">
          ...
          <input
            className="w-full bg-falcon-surface border border-falcon-border rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder-falcon-muted focus:outline-hidden focus:border-falcon-red/40"
            placeholder="統合を検索..."
        ...
        <div className="flex gap-1">
          {(['all', 'installed', 'available'] as const).map(f => (
            <button key={f} onClick={() => setShowFilter(f)}
              className={`px-3 py-1.5 rounded-sm text-xs font-medium transition-colors ${showFilter === f ? 'bg-falcon-active text-white' : 'text-falcon-muted hover:text-white'}`}>
        ...
        <div className="flex gap-1 flex-wrap">
          {categories.map(c => (
            <button key={c} onClick={() => setCatFilter(c)}
              className={`px-2 py-1 rounded-sm text-xs transition-colors ${catFilter === c ? 'bg-falcon-active text-white' : 'text-falcon-subtle hover:text-falcon-muted'}`}>
              {c === 'all' ? '全カテゴリ' : c}
            </button>
          ))}
        </div>
      ...
      {installed.length > 0 && showFilter !== 'available' && (
        <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {installed.map(i => (
              <div key={i.id} className="bg-falcon-surface border border-green-500/20 rounded-xl p-3 flex items-center gap-3">
                <div className={`w-10 h-10 rounded-lg bg-linear-to-br ${i.logo_color} flex items-center justify-center text-white font-bold text-xs shrink-0`}>
                  {i.logo_placeholder}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white font-medium truncate">{i.name}</p>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className={`text-[10px] ${i.health ? healthColor[i.health] : 'text-falcon-muted'}`}>
                      ● {i.health ? healthLabel[i.health] : '不明'}
                    </span>
                    <span className="text-[10px] text-falcon-subtle">v{i.installed_version}</span>
                  </div>
                </div>
                <div className="flex gap-1">
                  <button className="p-1.5 text-falcon-muted hover:text-white border border-falcon-border rounded-sm transition-colors">
                    <Settings className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => handleUninstall(i.id)} className="p-1.5 text-falcon-muted hover:text-red-400 border border-falcon-border rounded-sm transition-colors">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {filtered.map(i => (
            <div key={i.id} className={`bg-falcon-surface border rounded-xl p-4 flex flex-col gap-3 hover:border-falcon-red/30 transition-all cursor-pointer ${i.installed ? 'border-green-500/20' : 'border-falcon-border'}`}
              onClick={() => setSelectedIntegration(i)}>
              <div className="flex items-start gap-3">
                <div className={`w-12 h-12 rounded-xl bg-linear-to-br ${i.logo_color} flex items-center justify-center text-white font-bold text-sm shrink-0`}>
                  {i.logo_placeholder}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <p className="text-sm text-white font-medium truncate">{i.name}</p>
                    {i.installed && <CheckCircle className="w-3.5 h-3.5 text-green-400 shrink-0" />}
                  </div>
                  <p className="text-[11px] text-falcon-subtle">{i.vendor}</p>
                </div>
              </div>
              <p className="text-xs text-falcon-muted leading-relaxed flex-1">{i.description}</p>
              <div className="flex items-center gap-2 flex-wrap">
                <Badge text={i.category} cls={categoryColor[i.category]} />
                <span className={`text-[11px] px-2 py-0.5 rounded-sm ${priceColor[i.price_type]}`}>
                  {priceLabel[i.price_type]}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <StarRating rating={i.rating} />
                <span className="text-[10px] text-falcon-subtle">{(i.install_count ?? 0).toLocaleString()}件</span>
              </div>
              <button

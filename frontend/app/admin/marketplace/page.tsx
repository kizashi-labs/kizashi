...
export default function MarketplacePage() {
  ...
  // 失敗を捨ててから installed: true, health: 'healthy' に書き換えていました。
  // /api/v1/marketplace/* にはサーバ側のルートがありません。連携が入って
  // いないのに「導入済み・正常」と表示されます。
  const handleInstall = async (id: string) => {
    setLoadingId(id)
    const ok = await persist('連携の導入', `/api/v1/marketplace/integrations/${id}/install`, { method: 'POST' })
    setLoadingId(null)
    if (!ok) return
  ...
  const handleUninstall = async (id: string) => {
    setLoadingId(id)
    const ok = await persist('連携の削除', `/api/v1/marketplace/integrations/${id}/uninstall`, { method: 'DELETE' })
    setLoadingId(null)
    if (!ok) return
    ...
    <div className="min-h-screen bg-[#070d19] p-6 space-y-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48 max-w-64">
          ...
          <input
            className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder-[#7d92b0] focus:outline-hidden focus:border-[#e8002d]/40"
            placeholder="統合を検索..."
        ...
        <div className="flex gap-1">
          {(['all', 'installed', 'available'] as const).map(f => (
            <button key={f} onClick={() => setShowFilter(f)}
              className={`px-3 py-1.5 rounded-sm text-xs font-medium transition-colors ${showFilter === f ? 'bg-[#1d2f4a] text-white' : 'text-[#7d92b0] hover:text-white'}`}>
        ...
        <div className="flex gap-1 flex-wrap">
          {categories.map(c => (
            <button key={c} onClick={() => setCatFilter(c)}
              className={`px-2 py-1 rounded-sm text-xs transition-colors ${catFilter === c ? 'bg-[#1d2f4a] text-white' : 'text-[#3d5068] hover:text-[#7d92b0]'}`}>
              {c === 'all' ? '全カテゴリ' : c}
            </button>
          ))}
        </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {installed.map(i => (
              <div key={i.id} className="bg-[#0d1220] border border-green-500/20 rounded-xl p-3 flex items-center gap-3">
                ...
                <div className="flex gap-1">
                  <button className="p-1.5 text-[#7d92b0] hover:text-white border border-[#1e2d42] rounded-sm transition-colors">
                    <Settings className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => handleUninstall(i.id)} className="p-1.5 text-[#7d92b0] hover:text-red-400 border border-[#1e2d42] rounded-sm transition-colors">

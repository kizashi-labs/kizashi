...
function Badge({ cfg }: { cfg: { label: string; color: string } }) {
  return <span className={`text-xs px-2 py-0.5 rounded border font-medium ${cfg.color}`}>{cfg.label}</span>
}
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl mx-4 shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1e2d42] flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-[#7d92b0] mt-0.5">{CLAIM_TYPE_LABELS[claim.claim_type]} — {claim.incident_date}</p>
          </div>
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-5 flex-1">
          ...
          <div className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-4">
            <p className="text-xs text-[#7d92b0] mb-1">インシデント概要</p>
            <p className="text-[#e2e8f4] text-sm">{claim.description}</p>
          </div>

          {/* Timeline */}
          <div>
            <p className="text-xs text-[#7d92b0] mb-3">対応タイムライン</p>
            <div className="relative space-y-3 pl-5">
              <div className="absolute left-1.5 top-0 bottom-0 w-px bg-[#1e2d42]" />
              {claim.timeline.map((t, i) => (
                <div key={i} className="relative">
                  <div className="absolute -left-3.5 top-1 w-2.5 h-2.5 rounded-full bg-[#e8002d] border-2 border-[#070d19]" />
                  <p className="text-xs text-[#7d92b0]">{t.date}</p>
                  <p className="text-sm text-white font-medium">{t.event}</p>
                  {t.note && <p className="text-xs text-[#7d92b0] mt-0.5">{t.note}</p>}
                </div>
              ))}
            </div>
          </div>
          ...
          {claim.adjuster_notes && (
            <div className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-4">
              <p className="text-xs text-[#7d92b0] mb-1">調査員ノート</p>
              <p className="text-[#e2e8f4] text-sm">{claim.adjuster_notes}</p>
            </div>
          )}
        </div>
        <div className="px-6 py-4 border-t border-[#1e2d42] flex justify-end flex-shrink-0">
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg mx-4 shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-[#7d92b0] mb-1">申請タイプ</label>
            ...
            <div>
              <label className="block text-xs text-[#7d92b0] mb-1">インシデント日付</label>
              <input type="date" value={form.incident_date} onChange={e => setForm(p => ({ ...p, incident_date: e.target.value }))}
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/50" />
            </div>
          </div>
          <div>
            <label className="block text-xs text-[#7d92b0] mb-1">インシデント概要</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-[#7d92b0] mb-1">推定損失額（円）</label>
              <input type="number" value={form.estimated_loss} onChange={e => setForm(p => ({ ...p, estimated_loss: e.target.value }))}
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/50" />
            </div>
            <div>
              <label className="block text-xs text-[#7d92b0] mb-1">請求金額（円）</label>
              <input type="number" value={form.claimed_amount} onChange={e => setForm(p => ({ ...p, claimed_amount: e.target.value }))}
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/50" />
            </div>
          </div>
          <div>
            <label className="block text-xs text-[#7d92b0] mb-1">証拠書類（アップロード）</label>
            <div className="border border-dashed border-[#1e2d42] rounded-lg p-4 text-center cursor-pointer hover:border-[#2a3f5e] transition-colors">
              <Upload className="w-5 h-5 text-[#3d5068] mx-auto mb-1" />
              <p className="text-xs text-[#7d92b0]">ファイルをドラッグ＆ドロップ または クリックして選択</p>
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-[#070d19] p-6 space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-[#7d92b0] mt-0.5">保険証券・申請・リスク評価の一元管理</p>
        </div>
      </div>
      ...
      {tab === 'policy' && (
        <div className="space-y-6">
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => showToast('証券をエクスポートしました')}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[#1e2d42] text-[#7d92b0] hover:text-white text-xs transition-colors">
                ...
                {editingPolicy && (
                  <button onClick={() => savePolicyMutation.mutate({})}
                    className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#e8002d] text-white text-xs transition-colors">
                    保存
                  </button>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {[
                ...
                <div key={f.label}>
                  <p className="text-xs text-[#7d92b0] mb-1">{f.label}</p>
                  {editingPolicy ? (
                    <input defaultValue={f.value}
                      className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-white text-sm focus:outline-none focus:border-[#e8002d]/50" />
                  ) : (
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead><tr className="border-b border-[#1e2d42]">
                <th className="px-5 py-3 text-left text-xs text-[#7d92b0] font-medium">リスク種別</th>
                <th className="px-5 py-3 text-right text-xs text-[#7d92b0] font-medium">補償上限</th>
                <th className="px-5 py-3 text-center text-xs text-[#7d92b0] font-medium">補償状態</th>
              </tr></thead>
              <tbody>
                {policy.coverages.map(c => (
                  <tr key={c.risk} className="border-b border-[#1e2d42]/50 hover:bg-[#070d19] transition-colors">
                    <td className="px-5 py-3 text-[#e2e8f4]">{c.risk_label}</td>
                    <td className="px-5 py-3 text-right font-mono text-sm">
                      {c.included ? <span className="text-white">{fmtYenFull(c.coverage_limit)}</span> : <span className="text-[#3d5068]">—</span>}
                    </td>
                    <td className="px-5 py-3 text-center">
                      {c.included ? (
                        <span className="text-xs px-2 py-0.5 rounded border bg-green-500/10 text-green-400 border-green-500/30">対象</span>
                      ) : (
                        <span className="text-xs px-2 py-0.5 rounded border bg-red-500/10 text-red-400 border-red-500/30">除外</span>
                      )}
                    </td>
                  </tr>
                ))}
            ...
            <div className="space-y-2">
              {policy.exclusions.map((ex, i) => (
                <div key={i} className="flex items-start gap-2.5 p-3 bg-[#070d19] rounded-lg border border-amber-500/10">
                  ...
                  <span className="text-sm text-[#e2e8f4]">{ex}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
            <h2 className="text-white font-semibold mb-4 flex items-center gap-2">
              <Building2 className="w-4 h-4 text-[#e8002d]" /> ブローカー情報
            </h2>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded bg-[#1e2d42] flex items-center justify-center flex-shrink-0">
                  <Building2 className="w-3.5 h-3.5 text-[#7d92b0]" />
                </div>
                <div>
                  <p className="text-xs text-[#7d92b0]">会社名</p>
                  <p className="text-white text-sm">{policy.broker_company}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded bg-[#1e2d42] flex items-center justify-center flex-shrink-0">
                  <Mail className="w-3.5 h-3.5 text-[#7d92b0]" />
                </div>
                <div>
                  <p className="text-xs text-[#7d92b0]">メールアドレス</p>
                  <p className="text-white text-sm">{policy.broker_email}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded bg-[#1e2d42] flex items-center justify-center flex-shrink-0">
                  <Phone className="w-3.5 h-3.5 text-[#7d92b0]" />
                </div>
                <div>
                  <p className="text-xs text-[#7d92b0]">担当者</p>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs text-[#7d92b0]">電話番号</p>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={doc.name} className="flex items-center justify-between p-3 bg-[#070d19] border border-[#1e2d42] rounded-lg">
                  <div className="flex items-center gap-3">
                    <FileText className="w-4 h-4 text-blue-400" />
                    <div>
                      <p className="text-sm text-white">{doc.name}</p>
                      <p className="text-xs text-[#7d92b0]">{doc.date} — {doc.size}</p>
                    </div>
                  </div>
                  <button onClick={() => showToast('ダウンロードを開始しました')}
                    className="p-1.5 rounded border border-[#1e2d42] text-[#7d92b0] hover:text-white transition-colors">
                    <Download className="w-3.5 h-3.5" />
                  </button>
                </div>
              ))}
              <button onClick={() => showToast('ファイル選択ダイアログ（モック）')}
                className="w-full flex items-center justify-center gap-2 p-3 border border-dashed border-[#1e2d42] rounded-lg text-xs text-[#7d92b0] hover:border-[#2a3f5e] hover:text-white transition-colors">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-4">
              <p className="text-xs text-[#7d92b0] mb-1">総請求額</p>
              <p className="text-xl font-bold text-white">{fmtYen(totalClaimed)}</p>
            </div>
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-4">
              <p className="text-xs text-[#7d92b0] mb-1">総支払額</p>
              <p className="text-xl font-bold text-teal-400">{fmtYen(totalPaid)}</p>
            </div>
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-4">
              <p className="text-xs text-[#7d92b0] mb-1">審査中金額</p>
              <p className="text-xl font-bold text-amber-400">{fmtYen(pendingAmount)}</p>
            </div>
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-4">
              <p className="text-xs text-[#7d92b0] mb-1">承認率</p>
              <p className="text-xl font-bold text-white">{approvalRate}<span className="text-sm text-[#7d92b0]">%</span></p>
            </div>
          </div>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#1e2d42]">
              ...
              <button onClick={() => setNewClaimOpen(true)}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#e8002d]/20 border border-[#e8002d]/30 text-[#e8002d] hover:bg-[#e8002d]/30 text-xs transition-colors">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {claims.map(c => (
                    <tr key={c.id} className="border-b border-[#1e2d42]/50 hover:bg-[#070d19] transition-colors">
                      <td className="px-4 py-3 font-mono text-xs text-[#e2e8f4] whitespace-nowrap">{c.claim_id}</td>
                      <td className="px-4 py-3 text-[#7d92b0] text-xs whitespace-nowrap">{c.incident_date}</td>
                      <td className="px-4 py-3 text-xs text-[#e2e8f4] whitespace-nowrap">{CLAIM_TYPE_LABELS[c.claim_type]}</td>
                      <td className="px-4 py-3 text-xs font-mono text-[#e2e8f4] whitespace-nowrap">{fmtYen(c.estimated_loss)}</td>
                      <td className="px-4 py-3 text-xs font-mono text-white whitespace-nowrap">{fmtYen(c.claimed_amount)}</td>
                      <td className="px-4 py-3 text-xs font-mono whitespace-nowrap">
                        {c.settled_amount !== null ? <span className="text-teal-400">{fmtYen(c.settled_amount)}</span> : <span className="text-[#3d5068]">—</span>}
                      </td>
                      <td className="px-4 py-3"><Badge cfg={CLAIM_STATUS_CONFIG[c.status]} /></td>
                      <td className="px-4 py-3 text-xs text-[#7d92b0] whitespace-nowrap">{c.adjuster_name === '未割当' ? <span className="text-[#3d5068]">未割当</span> : c.adjuster_name.split('（')[0]}</td>
                      <td className="px-4 py-3 text-xs text-[#7d92b0] whitespace-nowrap">{c.filed_date}</td>
      ...
      {tab === 'risk' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className={`bg-[#0d1220] border rounded-xl p-5 ${premiumImpact < 0 ? 'border-green-500/30' : 'border-red-500/30'}`}>
              <p className="text-xs text-[#7d92b0] mb-1">推定保険料インパクト</p>
              <div className="flex items-center gap-2">
                {premiumImpact < 0
              ...
              <p className="text-xs text-[#7d92b0] mt-1">ベースライン比</p>
            </div>
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
              <p className="text-xs text-[#7d92b0] mb-1">ポジティブ要素</p>
              <p className="text-3xl font-bold text-green-400">
                {([] as QuestionnaireItem[]).filter(q => q.is_positive && (answers[q.id] === 'yes' || q.response_type !== 'yes_no')).length}
              </p>
              <p className="text-xs text-[#7d92b0] mt-1">保険料削減効果のある項目</p>
            </div>
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
              <p className="text-xs text-[#7d92b0] mb-1">改善対象項目</p>
              <p className="text-3xl font-bold text-amber-400">{gaps.length}</p>
              <p className="text-xs text-[#7d92b0] mt-1">保険料改善余地のある項目</p>
            </div>
          </div>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-[#1e2d42]">
              ...
              <p className="text-xs text-[#7d92b0] mt-0.5">回答を変更すると保険料インパクトがリアルタイムで更新されます</p>
            </div>
            <div className="divide-y divide-[#1e2d42]">
              {([] as QuestionnaireItem[]).map(q => (
                <div key={q.id} className="px-5 py-4 flex items-start gap-4 hover:bg-[#070d19] transition-colors">
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 mt-2 ${q.is_positive ? 'bg-green-400' : 'bg-amber-400'}`} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <span className="text-xs text-[#3d5068] uppercase tracking-wide">{q.category}</span>
                        <p className="text-sm text-[#e2e8f4] mt-0.5">{q.question}</p>
                      </div>
                      <div className="flex items-center gap-3 flex-shrink-0">
                        {q.response_type === 'yes_no' ? (
                          <div className="flex gap-2">
                            <button onClick={() => setAnswers(p => ({ ...p, [q.id]: 'yes' }))}
                              className={`px-3 py-1 rounded text-xs font-medium transition-all border ${answers[q.id] === 'yes' ? 'bg-green-500/20 border-green-500/40 text-green-400' : 'border-[#1e2d42] text-[#7d92b0] hover:text-white'}`}>
                              はい
                            </button>
                            <button onClick={() => setAnswers(p => ({ ...p, [q.id]: 'no' }))}
                              className={`px-3 py-1 rounded text-xs font-medium transition-all border ${answers[q.id] === 'no' ? 'bg-red-500/20 border-red-500/40 text-red-400' : 'border-[#1e2d42] text-[#7d92b0] hover:text-white'}`}>
                              いいえ
                            </button>
                          </div>
                        ) : (
                          <input type="number" value={answers[q.id] as number}
                            ...
                            className="w-24 bg-[#070d19] border border-[#1e2d42] rounded px-2 py-1 text-white text-sm text-right focus:outline-none focus:border-[#e8002d]/50" />
                        )}
          ...
          {gaps.length > 0 && (
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
              ...
              <div className="space-y-3">
                {gaps.map((q, i) => (
                  <div key={q.id} className="flex items-center gap-4 p-3 bg-[#070d19] border border-[#1e2d42] rounded-lg">
                    <span className="w-6 h-6 rounded-full bg-[#1e2d42] text-[#7d92b0] text-xs flex items-center justify-center flex-shrink-0 font-bold">{i + 1}</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-[#e2e8f4] truncate">{q.question}</p>
                      <p className="text-xs text-[#7d92b0] mt-0.5">{q.category}</p>
                    </div>

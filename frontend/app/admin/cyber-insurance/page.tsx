...
export default function CyberInsurancePage() {
  ...
  const savePolicyMutation = useMutation({
    mutationFn: (d: Partial<Policy>) => apiFetch('/api/v1/admin/cyber-insurance/policy', { method: 'PUT', body: JSON.stringify(d) }),
  ...
  const newClaimMutation = useMutation({
    mutationFn: (d: Partial<Claim>) => apiFetch('/api/v1/admin/cyber-insurance/claims', { method: 'POST', body: JSON.stringify(d) }),
                ...
                <div key={f.label}>
                  ...
                  {editingPolicy ? (
                    <input defaultValue={f.value}
                      className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50" />
                  ) : (
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-sm bg-[#1e2d42] flex items-center justify-center shrink-0">
                        ...
                        {q.response_type === 'yes_no' ? (
                          <div className="flex gap-2">
                            <button onClick={() => setAnswers(p => ({ ...p, [q.id]: 'yes' }))}
                              className={`px-3 py-1 rounded-sm text-xs font-medium transition-all border ${answers[q.id] === 'yes' ? 'bg-green-500/20 border-green-500/40 text-green-400' : 'border-[#1e2d42] text-[#7d92b0] hover:text-white'}`}>
                              はい
                            </button>
                            <button onClick={() => setAnswers(p => ({ ...p, [q.id]: 'no' }))}
                              className={`px-3 py-1 rounded-sm text-xs font-medium transition-all border ${answers[q.id] === 'no' ? 'bg-red-500/20 border-red-500/40 text-red-400' : 'border-[#1e2d42] text-[#7d92b0] hover:text-white'}`}>
                              いいえ
                            </button>
                          </div>
                        ) : (
                          <input type="number" value={answers[q.id] as number}
                            ...
                            className="w-24 bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1 text-white text-sm text-right focus:outline-hidden focus:border-[#e8002d]/50" />
                        )}
              ...
              <div className="space-y-3">
                {gaps.map((q, i) => (
                  <div key={q.id} className="flex items-center gap-4 p-3 bg-[#070d19] border border-[#1e2d42] rounded-lg">
                    <span className="w-6 h-6 rounded-full bg-[#1e2d42] text-[#7d92b0] text-xs flex items-center justify-center shrink-0 font-bold">{i + 1}</span>
                    <div className="flex-1 min-w-0">

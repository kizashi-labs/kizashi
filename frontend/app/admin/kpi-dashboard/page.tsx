    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs">
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-lg mx-4 shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
          <div>
            <label className="block text-xs text-falcon-muted mb-1">KPI名</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-falcon-muted mb-1">カテゴリ</label>
            ...
            <div>
              <label className="block text-xs text-falcon-muted mb-1">単位</label>
...
function KPIDetailModal({ kpi, measurements, onClose, onAddMeasurement, onDeleteMeasurement }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs">
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-2xl mx-4 shadow-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          {/* History Chart */}
          <div>
            <h3 className="text-xs font-semibold text-falcon-muted uppercase tracking-wider mb-3">6ヶ月推移</h3>
            <div className="flex items-end gap-2 h-24 bg-[#070d19] rounded-lg p-3 relative">
              ...
              {measurements.slice(-6).map(m => (
                <div key={m.id} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <span className="text-[8px] text-falcon-subtle truncate">{m.period.slice(5)}</span>
                </div>
              ))}
              {measurements.length === 0 && <div className="w-full text-center text-xs text-falcon-subtle self-center">データなし</div>}
            </div>
          </div>

          {/* Add measurement */}
          <div className="bg-[#070d19] border border-falcon-border rounded-lg p-4">
            <h3 className="text-xs font-semibold text-falcon-muted uppercase tracking-wider mb-3">測定値を追加</h3>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-xs text-falcon-muted mb-1">値 ({kpi.unit})</label>
                <input type="number" value={form.value} onChange={e => setForm(p => ({ ...p, value: e.target.value }))}
                  className="w-full bg-falcon-surface border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs text-falcon-muted mb-1">メモ</label>
            ...
            <button onClick={() => { if (form.value) { onAddMeasurement(kpi.id, { value: Number(form.value), period: form.period, notes: form.notes }); setForm(p => ({ ...p, value: '' })) } }}
              className="mt-3 px-3 py-1.5 rounded-lg text-xs bg-falcon-red hover:bg-[#c0001f] text-white font-medium transition-colors">追加</button>
          </div>

          {/* History table */}
          <div>
            <h3 className="text-xs font-semibold text-falcon-muted uppercase tracking-wider mb-2">測定履歴</h3>
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-falcon-border">
                  {['期間','値','メモ','操作'].map(h => <th key={h} className="px-3 py-2 text-left text-falcon-muted">{h}</th>)}
                </tr>
              </thead>
              <tbody className="divide-y divide-falcon-border">
                {measurements.map(m => (
                  <tr key={m.id} className="hover:bg-[#070d19]/50">
                    ...
                    <td className="px-3 py-2 text-falcon-muted">{m.notes || '—'}</td>
                    <td className="px-3 py-2">
                      <button onClick={() => onDeleteMeasurement(kpi.id, m.id)}
                        className="p-1 rounded-sm hover:bg-red-900/20 text-falcon-muted hover:text-red-400 transition-colors">
                ...
                {measurements.length === 0 && (
                  <tr><td colSpan={4} className="px-3 py-4 text-center text-falcon-subtle">測定値がありません</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          {kpis.map(k => (
            <div key={k.id} className="flex items-center gap-3">
              ...
              <input type="number" value={targets[k.id]} onChange={e => setTargets(p => ({ ...p, [k.id]: e.target.value }))}
                className="w-24 bg-[#070d19] border border-falcon-border rounded-lg px-3 py-1.5 text-white text-sm text-right focus:outline-hidden focus:border-falcon-red/50" />
              <span className="text-xs text-falcon-muted w-8">{k.unit}</span>
            </div>
          ))}
        </div>
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-falcon-border">
          ...
          <button onClick={() => { onSave(kpis.map(k => ({ id: k.id, target_value: Number(targets[k.id]) }))); onClose() }}
            className="px-4 py-2 rounded-lg text-sm bg-falcon-red hover:bg-[#c0001f] text-white font-medium transition-colors">更新</button>
        </div>
      </div>
    </div>
  )
...
const STATUS_CONFIG: Record<KPIStatus, { label: string; color: string; badge: string; bg: string }> = {
  ...
  no_data: { label: 'データなし', color: 'text-falcon-muted', badge: 'bg-falcon-muted/10 text-falcon-muted border-falcon-muted/30', bg: 'bg-falcon-muted/10' },
}
...
export default function KPIDashboardPage() {
  ...
  function addMeasurement(kpiId: string, d: { value: number; period: string; notes: string }) {
    const m: KPIMeasurement = { id: Math.random().toString(), kpi_id: kpiId, ...d, created_at: new Date().toISOString() }
    setMeasurements(prev => ({ ...prev, [kpiId]: [...(prev[kpiId] ?? []), m] }))
    apiFetch(`/api/v1/admin/kpi/${kpiId}/measurements`, { method: 'POST', body: JSON.stringify(d) }).catch(() => {})
  }
  ...
  return (
    <div className="min-h-screen bg-[#070d19] text-falcon-muted">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-falcon-muted mt-0.5">セキュリティ指標の目標達成状況を追跡・管理</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setShowBulkModal(true)}
              className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-falcon-muted hover:text-white border border-falcon-border hover:border-falcon-muted/50 transition-colors">
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="bg-falcon-surface border border-falcon-border rounded-xl p-6 flex items-center gap-8">
          ...
          <div>
            <p className="text-sm text-falcon-muted">総合セキュリティスコア</p>
            <p className="text-5xl font-black text-white mt-1">{avgAchievement}<span className="text-2xl text-falcon-muted">%</span></p>
        ...
        <div className="flex gap-1 flex-wrap">
          {categories.map(c => (
            <button key={c} onClick={() => setCategoryFilter(c)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${categoryFilter === c ? 'bg-falcon-red text-white' : 'bg-falcon-surface text-falcon-muted hover:text-white border border-falcon-border'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(k => {
            ...
            return (
              <div key={k.id} className="bg-falcon-surface border border-falcon-border rounded-xl p-5 hover:border-falcon-muted/30 transition-colors">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-falcon-muted mt-0.5 truncate">{k.description}</p>
                  </div>
                ...
                <div className="flex items-end justify-between">
                  <div>
                    <p className="text-2xl font-bold text-white">{k.current_value ?? '—'} <span className="text-sm font-normal text-falcon-muted">{k.unit}</span></p>
                    <p className="text-xs text-falcon-muted mt-0.5">目標: {k.target_value} {k.unit}</p>
                ...
                <div className="flex items-center gap-2 mt-4 pt-3 border-t border-falcon-border">
                  <button onClick={() => setDetailKPI(k)}
                    className="flex-1 py-1.5 rounded-lg text-xs text-falcon-muted hover:text-white bg-[#070d19] hover:bg-falcon-border border border-falcon-border transition-colors">詳細</button>
                  <button onClick={() => { setEditingKPI(k); setShowKPIModal(true) }}
                    className="p-1.5 rounded-lg text-falcon-muted hover:text-white bg-[#070d19] hover:bg-falcon-border border border-falcon-border transition-colors">
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => deleteKPI.mutate(k.id)}
                    className="p-1.5 rounded-lg text-falcon-muted hover:text-red-400 bg-[#070d19] hover:bg-red-900/20 border border-falcon-border transition-colors">

            ...
            <div className="flex items-end gap-2 h-24 bg-[#070d19] rounded-lg p-3 relative">
              ...
              {measurements.slice(-6).map(m => (
                <div key={m.id} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-sm transition-all" style={{ height: `${(m.value / maxVal) * 80}%`, backgroundColor: color, minHeight: 4 }} />
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input type="number" value={form.value} onChange={e => setForm(p => ({ ...p, value: e.target.value }))}
                  className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/50" />
              </div>
              <div>
                  ...
                  <tr key={m.id} className="hover:bg-[#070d19]/50">
                    ...
                    <td className="px-3 py-2">
                      <button onClick={() => onDeleteMeasurement(kpi.id, m.id)}
                        className="p-1 rounded hover:bg-red-900/20 text-[#7d92b0] hover:text-red-400 transition-colors">
          ...
          {kpis.map(k => (
            <div key={k.id} className="flex items-center gap-3">
              ...
              <input type="number" value={targets[k.id]} onChange={e => setTargets(p => ({ ...p, [k.id]: e.target.value }))}
                className="w-24 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-white text-sm text-right focus:outline-none focus:border-[#e8002d]/50" />
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${CATEGORY_COLORS[k.category]}`}>

...
function SeverityBadge({ severity }: { severity: PatchDeployment['severity'] }) {
  ...
  return <span className={`px-2 py-0.5 rounded-sm text-[11px] font-medium ${map[severity]}`}>{labels[severity]}</span>
}
    ...
    <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 space-y-4 hover:border-[#2a3f5c] transition-colors">
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        {patch.require_reboot && (
          <span className="px-2 py-0.5 rounded-sm text-[11px] font-medium bg-orange-900/30 text-orange-300 border border-orange-700/30">再起動必要</span>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs">
        ...
        {patch.cve_ids.map(cve => (
          <span key={cve} className="px-1.5 py-0.5 bg-red-900/20 text-red-400 border border-red-900/40 rounded-sm text-[10px]">{cve}</span>
        ))}
      </div>

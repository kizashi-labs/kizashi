        ...
        <div>
          ...
          <div className="flex gap-3 mb-4 flex-wrap">
            ...
            <select value={sevFilter} onChange={e => setSevFilter(e.target.value)}
              ...
              className="bg-[#0d1220] border border-[#1e2d42] text-white text-sm rounded px-3 py-1.5">
                ...
                {filteredDetections.map((d: any) => (
                  ...
                  <>
                    ...
                    <tr key={d.id} onClick={() => setExpandedRow(expandedRow === d.id ? null : d.id)}
                      ...
                      <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded text-xs font-medium ${THREAT_BADGE[d.threat_type] || 'bg-gray-700 text-gray-300'}`}>{d.threat_type.replace('_',' ')}</span></td>

                      <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded text-xs font-medium ${SEV_BADGE[d.severity]}`}>{d.severity}</span></td>
                      ...
                      <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded text-xs ${STATUS_BADGE[d.status]}`}>{d.status}</span></td>
                      ...
                      <td className="px-3 py-3">
                        ...
                        {d.status === 'open' && (
                          ...
                          <button onClick={e => { e.stopPropagation(); investigateMutation.mutate(d.id) }}
                            ...
                            className="px-2 py-1 bg-[#e8002d]/20 border border-[#e8002d]/40 text-[#e8002d] text-xs rounded hover:bg-[#e8002d]/30 whitespace-nowrap">
            ...
            <div className="flex gap-1">
              ...
              {['全て','signature','behavioral','ml','threshold'].map((t: any) => (
                ...
                <button key={t} onClick={() => setRuleTypeFilter(t)}
                  ...
                  className={`px-3 py-1.5 text-xs rounded transition-colors ${ruleTypeFilter === t ? 'bg-[#1e2d42] text-white' : 'text-[#7d92b0] hover:text-white'}`}>{t}</button>

              ))}

            </div>

            <button className="ml-auto px-3 py-1.5 bg-[#e8002d] text-white text-xs rounded hover:bg-[#c0001f]">+ 新規ルール</button>
              ...
              <tbody>
                ...
                {filteredRules.map((r: any) => (
                  ...
                  <tr key={r.id} className="border-b border-[#1e2d42] hover:bg-[#1e2d42]/20">
                    ...
                    <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded text-xs ${RULE_TYPE_BADGE[r.type]}`}>{r.type}</span></td>

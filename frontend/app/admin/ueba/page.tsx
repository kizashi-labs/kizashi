      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            {[
              ...
              return (
                <div key={row.label} className="flex items-center gap-3">
                  <span className="text-xs text-[#7d92b0] w-28 flex-shrink-0">{row.label}</span>
          ...
          <div className="space-y-2">
            {anomaly.ml_features.map(f => (
              <div key={f.name} className="flex items-center gap-3">
                <span className="text-xs text-[#7d92b0] font-mono w-40 flex-shrink-0">{f.name}</span>
          ...
          <div className="bg-[#070d19] rounded-lg p-4 border border-[#1e2d42] mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {anomaly.related_events.map(e => (
                <span key={e} className="text-xs font-mono bg-[#1e2d42] text-[#7d92b0] px-2 py-1 rounded">{e}</span>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3">
          <button onClick={() => { onAction(anomaly.id, 'confirmed'); onClose() }}
            className="flex-1 py-2 rounded bg-orange-700 text-white text-sm font-medium hover:bg-orange-600 transition-colors flex items-center justify-center gap-2">
            <CheckCircle className="w-4 h-4" /> 確認済み
          </button>
          <button onClick={() => { onAction(anomaly.id, 'false_positive'); onClose() }}
            className="flex-1 py-2 rounded bg-gray-700 text-white text-sm font-medium hover:bg-gray-600 transition-colors flex items-center justify-center gap-2">
        ...
        {heatmap.map((row, dayIdx) => (
          <div key={dayIdx} className="flex items-center gap-1 mb-0.5">
            ...
            {row.map((intensity, hr) => (
              <div key={hr} className="w-4 h-4 rounded-sm"
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <div className="flex gap-0.5">
            {[0.1, 0.3, 0.5, 0.7, 0.9].map(v => (
              <div key={v} className="w-3 h-3 rounded-sm" style={{ backgroundColor: `rgba(232, 0, 45, ${v * 0.8})` }} />
            ))}
          </div>
        ...
        {userAnomalies.slice(0, 5).map(a => {
          ...
          return (
            <div key={a.id} className="flex items-center gap-3 py-1.5 border-b border-[#1e2d42]/50 last:border-0">
              <span className="text-xs text-[#7d92b0] w-24 flex-shrink-0">{fmt(a.timestamp)}</span>
              <span className={`text-xs px-1.5 py-0.5 rounded-full ${tc.bg} ${tc.text} flex-shrink-0`}>{tc.label}</span>
              <span className={`text-xs px-1.5 py-0.5 rounded font-bold ${sc.bg} ${sc.text} flex-shrink-0`}>{sc.label}</span>
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            ...
            <div className="flex items-center gap-2 bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2">
              ...
              <input value={filterUser} onChange={e => setFilterUser(e.target.value)}
                ...
                className="bg-transparent text-sm text-[#7d92b0] focus:outline-none focus:text-white w-36" />
            </div>
          ...
          {selectedIds.size > 0 && (
            <div className="flex items-center gap-3 mb-4 p-3 bg-[#0d1220] border border-[#1e2d42] rounded-lg">
              ...
              <button onClick={() => handleBulkAction('reviewed')}
                className="px-3 py-1.5 bg-blue-900/40 text-blue-300 border border-blue-700/30 rounded text-xs hover:bg-blue-900/60 transition-colors">
                レビュー済みにする
              </button>
              <button onClick={() => handleBulkAction('false_positive')}
                className="px-3 py-1.5 bg-gray-800 text-gray-300 border border-gray-700/30 rounded text-xs hover:bg-gray-700 transition-colors">
                誤検知にする
              </button>
            </div>
          )}
          ...
          <div className="flex items-center gap-3 mb-6">
            <div className="relative flex-1 max-w-sm">
              ...
              <input
                ...
                className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg pl-10 pr-4 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50"
              />
            </div>
            <div className="flex flex-wrap gap-2">

                  ...
                  <div className="flex items-center justify-between px-5 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex gap-1.5">
                        {p.severities.map(s => (
                          <span key={s} className={`text-xs px-2 py-0.5 rounded border capitalize ${severityCls[s]}`}>{s}</span>
                        ))}
                      </div>
                      <span className={`text-xs px-2 py-0.5 rounded border ${p.auto_approve ? 'bg-green-900/30 text-green-300 border-green-700/40' : 'bg-gray-700/30 text-gray-400 border-gray-600/40'}`}>
                    ...
                    <div className="border-t border-[#1e2d42] px-5 py-4 bg-[#070d19]/50">
                      <div className="grid grid-cols-3 gap-4 text-sm">
                        ...
                        <div><p className="text-xs text-[#7d92b0] mb-1">対象深刻度</p>
                          <div className="flex gap-1 mt-1">{p.severities.map(s => <span key={s} className={`text-xs px-2 py-0.5 rounded border capitalize ${severityCls[s]}`}>{s}</span>)}</div>
                        </div>
                      </div>
                    </div>
                  )}
              ...
              <div className="bg-[#0d1220] rounded-xl border border-[#1e2d42] p-5 mb-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#1a6bff]/60" placeholder="例: 緊急パッチ適用" />
                  </div>
                  <div>
                    <label className="block text-xs text-[#7d92b0] mb-1.5">ポリシー選択</label>
                    <select className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#1a6bff]/60">
                    ...
                    return (
                      <tr key={j.id} className="border-b border-[#1e2d42]/50 hover:bg-[#131d31]/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded border capitalize ${jobStatusCls[j.status]} ${j.status === 'running' ? 'animate-pulse' : ''}`}>
                ...
                <tbody>
                  {sortedMissing.map(p => (
                    <tr key={p.id} className="border-b border-[#1e2d42]/50 hover:bg-[#131d31]/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className="text-xs font-mono text-[#7d92b0] bg-[#070d19] px-2 py-0.5 rounded">{p.cve}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-0.5 rounded border capitalize ${severityCls[p.severity]}`}>{p.severity}</span>
                      </td>

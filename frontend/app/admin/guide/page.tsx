...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handle}
      className="p-1.5 rounded-sm text-[#5a6a7a] hover:text-[#e2e8f4] hover:bg-[#1e2d42] transition-colors"
    >
...
export default function AdminGuidePage() {
  ...
  return (
    <div className="flex h-full bg-[#080c14] text-[#e2e8f4]">
      ...
      {/* ── 左サイドバー ─────────────────────────────────────────── */}
      <aside className="w-56 shrink-0 border-r border-[#1e2d42] sticky top-0 h-screen overflow-y-auto">
          ...
          <Section id="alerts" title="アラート・インシデント対応" icon={<Bell className="w-4 h-4" />}>
            ...
            <div className="flex items-center gap-2 flex-wrap mb-4 text-xs">
              {['open', '→ in_progress', '→ resolved', '→ closed'].map((s, i) => (
                <span key={i} className={`px-2 py-1 rounded-sm font-mono ${i === 0 ? 'bg-[#3b82f6]/20 text-[#3b82f6]' : i === 3 ? 'bg-[#5a6a7a]/20 text-[#5a6a7a]' : 'bg-[#1e2d42] text-[#8899aa]'}`}>
                  {s}
                </span>
              ))}
            </div>

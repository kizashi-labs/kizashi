...
const TRIGGER_COLORS: Record<TriggerType, string> = {
  alert: 'bg-[#e8002d]/20 text-[#e8002d]',
...
function WorkflowModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <div className="px-6 py-4 space-y-6">
          {/* Section 1: Basic */}
          <div>
            <h3 className="text-[#7d92b0] text-xs font-semibold uppercase tracking-wider mb-3">基本設定</h3>
            <div className="space-y-3">
              <div>
                <label className="text-[#7d92b0] text-xs mb-1 block">ワークフロー名</label>
                <input
                  value={form.name}
                  onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#4a6fa5]"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#4a6fa5] resize-none"
                  placeholder="ワークフローの説明"
                />
              </div>
              <div>
                <label className="text-[#7d92b0] text-xs mb-1 block">トリガータイプ</label>
                <select
          ...
          <div>
            <h3 className="text-[#7d92b0] text-xs font-semibold uppercase tracking-wider mb-3">トリガー条件</h3>
            {form.trigger_type === 'alert' && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[#7d92b0] text-xs mb-1 block">重要度</label>
                  <select
                ...
                <div>
                  <label className="text-[#7d92b0] text-xs mb-1 block">アラートタイプ</label>
                  <select
            ...
            {form.trigger_type === 'schedule' && (
              <div>
                <label className="text-[#7d92b0] text-xs mb-1 block">Cron式</label>
                <input
                  value={form.conditions.cron ?? ''}
                  onChange={e => setForm(f => ({ ...f, conditions: { ...f.conditions, cron: e.target.value } }))}
                  className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white font-mono focus:outline-hidden focus:border-[#4a6fa5]"
                  placeholder="0 8 * * * (毎日8時)"
                />
                <p className="text-[#3d5068] text-xs mt-1">例: 0 8 * * * = 毎日8時, 0 */6 * * * = 6時間ごと</p>
              </div>
            )}
            {form.trigger_type === 'manual' && (
              <p className="text-[#7d92b0] text-sm">手動トリガーには条件は不要です。</p>
            )}
            {form.trigger_type === 'webhook' && (
              <div>
                <label className="text-[#7d92b0] text-xs mb-1 block">Webhookエンドポイント（自動生成）</label>
                <div className="bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-[#7d92b0] font-mono">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-[#7d92b0] text-xs font-semibold uppercase tracking-wider">アクション設定</h3>
              <button
                onClick={addAction}
            ...
            {form.actions.length === 0 && (
              <div className="text-center py-6 border border-dashed border-[#1e2d42] rounded-sm text-[#3d5068] text-sm">
                アクションを追加してください
              </div>
            )}
            <div className="space-y-3">
              {form.actions.map((action, idx) => {
                ...
                return (
                  <div key={action.id} className="border border-[#1e2d42] rounded-sm p-3 bg-[#070d19]">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[#7d92b0] text-xs font-medium">アクション {idx + 1}</span>
                      <button onClick={() => removeAction(idx)} className="text-[#3d5068] hover:text-[#e8002d] transition-colors">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <select
                        ...
                        className="flex-1 bg-[#0d1220] border border-[#1e2d42] rounded-sm px-2 py-1.5 text-sm text-white focus:outline-hidden focus:border-[#4a6fa5]"
                      >
                    ...
                    {action.type === 'create_ticket' && (
                      <div>
                        ...
                        <select
                          ...
                          className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-sm px-2 py-1.5 text-sm text-white focus:outline-hidden focus:border-[#4a6fa5]"
                        >
                          <option value="critical">クリティカル</option>
                          <option value="high">高</option>
                          <option value="medium">中</option>
                    ...
                    {action.type === 'send_notification' && (
                      <div>
                        <label className="text-[#7d92b0] text-xs mb-1 block">メッセージ</label>
                        <textarea
                          value={action.params.message ?? ''}
                          onChange={e => updateAction(idx, { params: { ...action.params, message: e.target.value } })}
                          rows={2}
                          className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-sm px-2 py-1.5 text-sm text-white focus:outline-hidden focus:border-[#4a6fa5] resize-none"
                          placeholder="通知メッセージ"
                        />
                      </div>
                    )}
                    {action.type === 'quarantine_agent' && (
                      <div>
                        <label className="text-[#7d92b0] text-xs mb-1 block">エージェント</label>
                        <select
                    ...
                    {action.type === 'run_playbook' && (
                      <div>
                        <label className="text-[#7d92b0] text-xs mb-1 block">プレイブックID</label>
                        <input
        ...
        <div className="px-6 py-4 border-t border-[#1e2d42] flex justify-end gap-3">
          ...
          <button
            ...
            disabled={!form.name}
            className="px-4 py-2 text-sm bg-[#e8002d] hover:bg-[#c0001f] text-white rounded-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            保存
          </button>
        </div>
...
function TriggerModal({ workflow, onClose, onConfirm }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-full bg-[#e8002d]/20 flex items-center justify-center">
            <Play className="w-5 h-5 text-[#e8002d]" />
          </div>
          <div>
            <h2 className="text-white font-semibold">ワークフロー実行確認</h2>
            <p className="text-[#7d92b0] text-xs">{workflow.name}</p>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            ...
            className="px-4 py-2 text-sm bg-[#e8002d] hover:bg-[#c0001f] text-white rounded-sm flex items-center gap-2 transition-colors"
          >
...
function ExecDetailModal({ exec, onClose }: { exec: WorkflowExecution; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1e2d42]">
        ...
        <div className="px-6 py-4">
          <p className="text-[#7d92b0] text-xs mb-1">ワークフロー</p>
          <p className="text-white font-medium mb-3">{exec.workflow_name}</p>
          <div className="grid grid-cols-3 gap-3 mb-4 text-xs">
            <div className="bg-[#070d19] rounded-sm p-2 text-center">
              <p className="text-[#3d5068]">トリガー</p>
              <p className="text-[#7d92b0] mt-1 truncate">{exec.trigger}</p>
            </div>
            <div className="bg-[#070d19] rounded-sm p-2 text-center">
              <p className="text-[#3d5068]">所要時間</p>
              <p className="text-[#7d92b0] mt-1">{formatDuration(exec.duration_ms)}</p>
            </div>
            <div className="bg-[#070d19] rounded-sm p-2 text-center">
              <p className="text-[#3d5068]">開始日時</p>
              <p className="text-[#7d92b0] mt-1">{formatDate(exec.started_at)}</p>
            </div>
          </div>
          <h3 className="text-[#7d92b0] text-xs font-semibold uppercase tracking-wider mb-2">実行ステップ</h3>
          <div className="space-y-2">
            {exec.steps.map((step, i) => (
              <div key={i} className="flex items-start gap-3 p-3 bg-[#070d19] rounded-sm border border-[#1e2d42]">
                <div className="shrink-0 mt-0.5">
                  {step.status === 'success' && <CheckCircle className="w-4 h-4 text-green-400" />}
                  {step.status === 'failed' && <XCircle className="w-4 h-4 text-[#e8002d]" />}
                  {step.status === 'skipped' && <Clock className="w-4 h-4 text-[#3d5068]" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-white">{ACTION_LABELS[step.action_type]}</span>
                    <span className="text-xs text-[#3d5068]">{formatDuration(step.duration_ms)}</span>
                  </div>
                  {step.error && <p className="text-xs text-[#e8002d] mt-1">{step.error}</p>}
                  {step.status === 'skipped' && <p className="text-xs text-[#3d5068] mt-1">前のステップの失敗によりスキップ</p>}
                </div>
              </div>
            ))}
          </div>
...
export default function SoarPage() {
  ...
  const statusBadge = (status: ExecStatus) => {
    ...
    return (
      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-sm text-xs bg-[#e8002d]/20 text-[#e8002d]">
    ...
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Workflow className="w-6 h-6 text-[#e8002d]" />
            SOARワークフロー
          </h1>
          <p className="text-[#7d92b0] text-sm mt-1">セキュリティオーケストレーション・自動対応ワークフローの管理</p>
        </div>
        <button
          onClick={() => setShowCreateModal(true)}
          className="flex items-center gap-2 px-4 py-2 bg-[#e8002d] hover:bg-[#c0001f] text-white rounded-sm text-sm font-medium transition-colors"
        >
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: '成功率', value: `${successRate}%`, icon: Zap, color: 'text-[#e8002d]' },
      ...
      {activeTab === 'workflows' && (
        <div className="grid grid-cols-1 gap-4">
          {workflows.map(wf => (
            <div key={wf.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[#7d92b0] text-sm mb-3">{wf.description}</p>
                  <div className="flex items-center gap-4 text-xs text-[#3d5068]">
                ...
                <div className="flex items-center gap-2 ml-4 shrink-0">
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-sm transition-colors ${wf.enabled ? 'text-green-400 hover:bg-green-400/10' : 'text-[#3d5068] hover:bg-[#1e2d42]'}`}
                  ...
                  <button className="p-1.5 text-[#7d92b0] hover:text-white hover:bg-[#1e2d42] rounded-sm transition-colors">
              ...
              {/* Action list */}
              <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-[#1e2d42]">
                {wf.actions.map((action, i) => {
                  const Icon = ACTION_ICONS[action.type]
                  return (
                    <span key={i} className="inline-flex items-center gap-1 px-2 py-1 bg-[#070d19] rounded-sm text-xs text-[#7d92b0]">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <select
              ...
              className="bg-[#0d1220] border border-[#1e2d42] rounded-sm px-3 py-1.5 text-sm text-white focus:outline-hidden"
            >
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filteredExecs.map(exec => (
                  <tr key={exec.id} className="border-b border-[#1e2d42] hover:bg-[#070d19] transition-colors">
                    <td className="px-4 py-3 text-white font-medium">{exec.workflow_name}</td>
                    <td className="px-4 py-3">
                      <span className="text-xs text-[#7d92b0] font-mono bg-[#070d19] px-2 py-0.5 rounded-sm">
                        {exec.trigger}
                      </span>
                    </td>
                    <td className="px-4 py-3">{statusBadge(exec.status)}</td>
                    <td className="px-4 py-3 text-[#7d92b0]">
                      {exec.actions_completed}/{exec.actions_total}
                    </td>
                    <td className="px-4 py-3 text-[#7d92b0]">{formatDuration(exec.duration_ms)}</td>
                    <td className="px-4 py-3 text-[#7d92b0] text-xs">{formatDate(exec.started_at)}</td>
                    <td className="px-4 py-3">
                      <button

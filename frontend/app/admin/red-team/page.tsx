  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      <div className="bg-[#0d1220] border-l border-[#1e2d42] w-full max-w-xl h-full flex flex-col overflow-hidden shadow-2xl"
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-[#1e2d42] shrink-0">
          <div>
          ...
          <button onClick={onClose} className="text-[#7d92b0] hover:text-white shrink-0"><X className="w-5 h-5" /></button>
        </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {['screenshot_001.png', 'memory_dump.bin', 'network_capture.pcap'].map(f => (
                <div key={f} className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-2 flex items-center gap-2 cursor-pointer hover:border-[#7d92b0]/30 transition-colors">
                  <FileText className="w-3.5 h-3.5 text-[#7d92b0] shrink-0" />
...
export default function RedTeamPage() {
  ...
  const { data: exercises = [] } = useQuery<Exercise[]>({
    ...
    queryFn: () => apiFetchList<Exercise>('/api/v1/admin/red-team/exercises'),
  })

  const { data: findings = [] } = useQuery<Finding[]>({
    queryKey: ['red-team-findings'],
    queryFn: () => apiFetchList<Finding>('/api/v1/admin/red-team/findings'),
  })

  const createExerciseMutation = useMutation({
    mutationFn: async (data: Partial<Exercise>) => {
      return await apiFetch('/api/v1/admin/red-team/exercises', { method: 'POST', body: JSON.stringify(data) })
    },
  ...
  const deleteExerciseMutation = useMutation({
    mutationFn: async (id: string) => {
      return await apiFetch(`/api/v1/admin/red-team/exercises/${id}`, { method: 'DELETE' })
    },
  ...
  const createFindingMutation = useMutation({
    mutationFn: async (data: Partial<Finding>) => {
      return await apiFetch('/api/v1/admin/red-team/findings', { method: 'POST', body: JSON.stringify(data) })
    },
  ...
  const updateFindingMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: FindingStatus }) => {
      return await apiFetch(`/api/v1/admin/red-team/findings/${id}`, { method: 'PUT', body: JSON.stringify({ status }) })
    },
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[200px] max-w-xs">
              ...
              <input value={findingSearch} onChange={e => setFindingSearch(e.target.value)}
                placeholder="所見を検索..."
                className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg pl-9 pr-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50" />
            </div>
            <select value={exerciseFilter} onChange={e => setExerciseFilter(e.target.value)}
              className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden">
                  ...
                  <tr key={f.id} className="border-b border-[#1e2d42]/60 last:border-0 hover:bg-[#070d19]/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => setDetailFinding(f)}
                        className="px-2 py-1 rounded-sm text-xs bg-[#1e2d42] hover:bg-[#2a3a52] text-white transition-colors">
                        詳細
                      </button>
                    </td>
                  </tr>

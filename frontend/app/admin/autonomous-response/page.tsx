      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-[#7d92b0] mb-1 block">ポリシー名</label>
            ...
            <div>
              <label className="text-xs text-[#7d92b0] mb-1 block">最大スコープ</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs text-[#7d92b0] font-medium uppercase tracking-wider">トリガー条件</label>
              ...
              {form.conditions.map((c, i) => (
                <div key={i} className="flex items-center gap-2">
                  <select value={c.field} onChange={e => setForm(p => ({ ...p, conditions: p.conditions.map((cc, idx) => idx === i ? { ...cc, field: e.target.value } : cc) }))}
                    className="bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1.5 text-xs text-[#7d92b0] focus:outline-hidden">
                  ...
                  <input value={c.value} onChange={e => setForm(p => ({ ...p, conditions: p.conditions.map((cc, idx) => idx === i ? { ...cc, value: e.target.value } : cc) }))}
                    placeholder="値..."
                    className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1.5 text-xs text-white focus:outline-hidden" />
                  {form.conditions.length > 1 && (
                    <button onClick={() => removeCondition(i)} className="text-[#7d92b0] hover:text-red-400"><X className="w-3.5 h-3.5" /></button>
                  )}
                </div>
              ))}
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs text-[#7d92b0] font-medium uppercase tracking-wider">レスポンスアクション (順序付き)</label>
            ...
            <div className="space-y-2">
              {form.actions.map((a, i) => (
                <div key={a.id} className="p-3 bg-[#070d19] rounded-lg border border-[#1e2d42]">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs text-[#3d5068] font-mono w-4">{i + 1}.</span>
                    <select value={a.action_type} onChange={e => setForm(p => ({ ...p, actions: p.actions.map((aa, idx) => idx === i ? { ...aa, action_type: e.target.value as ActionType } : aa) }))}
                      className="flex-1 bg-[#0d1220] border border-[#1e2d42] rounded-sm px-2 py-1.5 text-xs text-white focus:outline-hidden">
                    ...
                    <select value={a.on_failure} onChange={e => setForm(p => ({ ...p, actions: p.actions.map((aa, idx) => idx === i ? { ...aa, on_failure: e.target.value as OnFailure } : aa) }))}
                      className="bg-[#0d1220] border border-[#1e2d42] rounded-sm px-2 py-1.5 text-xs text-[#7d92b0] focus:outline-hidden">
                      {(Object.keys(ON_FAILURE_LABELS) as OnFailure[]).map(f => <option key={f} value={f}>失敗時: {ON_FAILURE_LABELS[f]}</option>)}
                    </select>
                    <div className="flex gap-0.5">
                      <button onClick={() => moveAction(i, -1)} className="text-[#3d5068] hover:text-white p-0.5"><ArrowUp className="w-3 h-3" /></button>
                      <button onClick={() => moveAction(i, 1)} className="text-[#3d5068] hover:text-white p-0.5"><ArrowDown className="w-3 h-3" /></button>
                    </div>
                    <button onClick={() => removeAction(i)} className="text-[#7d92b0] hover:text-red-400"><X className="w-3.5 h-3.5" /></button>
                  </div>
                  <div className="flex items-center gap-2 pl-5">
                    <span className="text-xs text-[#3d5068]">タイムアウト:</span>
                    <input type="number" value={a.timeout_s} onChange={e => setForm(p => ({ ...p, actions: p.actions.map((aa, idx) => idx === i ? { ...aa, timeout_s: Number(e.target.value) } : aa) }))}
                      className="w-20 bg-[#0d1220] border border-[#1e2d42] rounded-sm px-2 py-1 text-xs text-white focus:outline-hidden" />
                    <span className="text-xs text-[#3d5068]">秒</span>
                  </div>
                </div>
              ))}
              {form.actions.length === 0 && (
                <p className="text-xs text-[#3d5068] text-center py-3">アクションを追加してください</p>
              )}
            </div>
          </div>

          {/* Approval */}
          <div className="flex items-center justify-between p-3 bg-[#070d19] rounded-lg border border-[#1e2d42]">
            <div>
              <p className="text-sm text-[#e2e8f4] font-medium">承認を必要とする</p>
              <p className="text-xs text-[#7d92b0]">実行前に管理者の承認が必要</p>
            </div>
            <button onClick={() => setForm(p => ({ ...p, requires_approval: !p.requires_approval }))}>
              {form.requires_approval ? <ToggleRight className="w-6 h-6 text-yellow-400" /> : <ToggleLeft className="w-6 h-6 text-[#3d5068]" />}
            </button>
          </div>
          {form.requires_approval && (
            <div>
              <label className="text-xs text-[#7d92b0] mb-1 block">承認タイムアウト (秒)</label>
...
function PolicyTestModal({ policies, onClose }: { policies: ResponsePolicy[]; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-xl p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          {Object.keys(alertInput).map(f => (
            <div key={f}>
              <label className="text-xs text-[#7d92b0] mb-1 block">{f}</label>
              <input value={(alertInput as any)[f]} onChange={e => setAlertInput(p => ({ ...p, [f]: e.target.value }))}
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-xs text-white focus:outline-hidden" />
            </div>
          ))}
        </div>
        <button onClick={runTest}
          className="w-full py-2 rounded-sm bg-[#e8002d] text-white text-sm font-medium hover:bg-[#c8001e] transition-colors flex items-center justify-center gap-2 mb-4">
          <Play className="w-4 h-4" /> ドライラン実行
        </button>
        {results && (
          <div className="space-y-2">
            <p className="text-xs text-[#7d92b0] mb-2 uppercase tracking-wider">
              {results.filter(r => r.matched).length}/{results.length} ポリシーがマッチ
            </p>
            {results.map(({ policy, matched }) => (
              <div key={policy.id} className={`p-3 rounded-lg border flex items-center gap-3 ${matched ? 'border-[#e8002d]/30 bg-[#e8002d]/10' : 'border-[#1e2d42] opacity-50'}`}>
                {matched ? <Zap className="w-4 h-4 text-[#e8002d] shrink-0" /> : <XCircle className="w-4 h-4 text-[#3d5068] shrink-0" />}
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium ${matched ? 'text-white' : 'text-[#7d92b0]'}`}>{policy.name}</p>
                  {matched && (
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-[#7d92b0] text-xs mt-0.5">{execution.trigger_summary}</p>
          </div>
          <button onClick={onClose} className="text-[#7d92b0] hover:text-white"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-xs text-[#7d92b0]">{fmt(execution.created_at)}</span>
          {execution.affected_hosts.length > 0 && (
            <div className="flex gap-1">
              {execution.affected_hosts.map(h => (
                <span key={h} className="text-xs font-mono bg-[#1e2d42] px-2 py-0.5 rounded-sm text-[#e2e8f4]">{h}</span>
              ))}
            </div>
          )}
        </div>
        ...
        {/* Trigger event */}
        <div className="mb-4">
          <p className="text-xs text-[#7d92b0] mb-2 font-medium uppercase tracking-wider">トリガーイベント</p>
          <pre className="bg-[#070d19] border border-[#1e2d42] rounded-sm p-3 text-xs font-mono text-[#7d92b0] overflow-x-auto">
        ...
        {execution.actions_taken.length > 0 && (
          <div className="mb-4">
            <p className="text-xs text-[#7d92b0] mb-2 font-medium uppercase tracking-wider">実行アクション</p>
            <div className="space-y-2">
              {execution.actions_taken.map((a, i) => (
                <div key={i} className={`p-3 rounded-lg border flex items-start gap-3 ${a.status === 'success' ? 'border-green-700/30 bg-green-900/10' : a.status === 'failed' ? 'border-red-700/30 bg-red-900/10' : 'border-[#1e2d42]'}`}>
                  {a.status === 'success' ? <CheckCircle className="w-4 h-4 text-green-400 shrink-0 mt-0.5" /> :
                    ...
                    <Circle className="w-4 h-4 text-[#3d5068] shrink-0 mt-0.5" />}
                  <div className="flex-1">
                    <p className="text-sm text-white font-medium">{ACTION_TYPE_LABELS[a.action_type]}</p>
                    <p className="text-xs text-[#7d92b0]">{a.result}</p>
                    <p className="text-xs text-[#3d5068] mt-0.5">{fmt(a.timestamp)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {execution.approval_chain.length > 0 && (
          <div>
            <p className="text-xs text-[#7d92b0] mb-2 font-medium uppercase tracking-wider">承認チェーン</p>
            {execution.approval_chain.map((ac, i) => (
              <div key={i} className="flex items-center gap-3 p-3 bg-[#070d19] rounded-lg border border-[#1e2d42]">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#7d92b0]">{ac.action === 'approved' ? '承認' : '拒否'} — {fmt(ac.timestamp)}</p>
                </div>
              </div>
            ))}
          </div>
...
export default function AutonomousResponsePage() {
  ...
  // 自動対応ポリシーは端末で隔離やプロセス停止を自動で走らせる設定で、
  // 承認/拒否はその実行を止める最後の関門です。どれも保存の失敗を捨てて
  // 画面だけを進めていました。承認したはずの実行が承認されておらず、
  // 拒否したはずの実行が止まっていない、という状態になります。
  const handleTogglePolicy = async (p: ResponsePolicy) => {
    if (await persist('ポリシーの有効/無効', `/api/v1/admin/autonomous-response/policies/${p.id}/toggle`, { method: 'POST' })) {
      setLocalPolicies(prev => prev.map(x => x.id === p.id ? { ...x, is_active: !x.is_active } : x))
    }
  }

  const handleDeletePolicy = async (p: ResponsePolicy) => {
    if (await persist(`ポリシー「${p.name}」の削除`, `/api/v1/admin/autonomous-response/policies/${p.id}`, { method: 'DELETE' })) {
      setLocalPolicies(prev => prev.filter(x => x.id !== p.id))
      showToast(`ポリシー「${p.name}」を削除しました`)
    }
  }

  const handleSavePolicy = async (form: Omit<ResponsePolicy, 'id' | 'execution_count' | 'success_rate' | 'created_at'>) => {
    if (editPolicy) {
    ...
    if (await persist(`ポリシー「${form.name}」`, '/api/v1/admin/autonomous-response/policies', { method: 'POST', body: JSON.stringify(form) })) {
  ...
  const handleApprove = async (exec: Execution) => {
    if (await persist('実行の承認', `/api/v1/admin/autonomous-response/executions/${exec.id}/approve`, { method: 'POST' })) {
  ...
  const handleReject = async (exec: Execution) => {
    if (await persist('実行の拒否', `/api/v1/admin/autonomous-response/executions/${exec.id}/reject`, { method: 'POST' })) {
    ...
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[#7d92b0] text-sm">アラートトリガーに基づく自動レスポンスポリシー</p>
          </div>
        </div>
        {/* Master switch */}
        <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${masterEnabled ? 'border-[#e8002d]/50 bg-[#e8002d]/10' : 'border-[#1e2d42] bg-[#0d1220]'}`}>
          {masterEnabled && <AlertTriangle className="w-4 h-4 text-[#e8002d]" />}
          <div>
            <p className="text-sm text-white font-medium">自動実行</p>
            <p className={`text-xs ${masterEnabled ? 'text-[#e8002d]' : 'text-[#3d5068]'}`}>{masterEnabled ? '有効 — 注意して使用' : '無効'}</p>
          </div>
          <button onClick={() => setMasterEnabled(!masterEnabled)}>
            {masterEnabled ? <ToggleRight className="w-7 h-7 text-[#e8002d]" /> : <ToggleLeft className="w-7 h-7 text-[#3d5068]" />}
          </button>
        </div>
      </div>

      {masterEnabled && (
        <div className="p-3 bg-[#e8002d]/10 border border-[#e8002d]/30 rounded-lg mb-6 flex items-start gap-2">
          <AlertTriangle className="w-4 h-4 text-[#e8002d] shrink-0 mt-0.5" />
          <p className="text-xs text-[#e8002d]">自動実行が有効です。ポリシーは承認なしに実行されます。承認が必要なポリシーを除いて、すべての自動アクションが即時実行されます。</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: '承認待ち', value: awaitingApproval, color: awaitingApproval > 0 ? 'text-yellow-400' : 'text-[#7d92b0]' },
      ...
      {tab === 'policies' && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <p className="text-[#7d92b0] text-sm">{localPolicies.length} ポリシー</p>
            ...
            <table className="w-full">
              ...
              <tbody>
                {localPolicies.map(p => {
                  ...
                  return (
                    <tr key={p.id} className={`border-b border-[#1e2d42]/50 hover:bg-[#070d19]/50 transition-colors ${!p.is_active ? 'opacity-50' : ''}`}>
                      <td className="px-4 py-3 max-w-[160px]">
                        <p className="text-white text-sm font-medium truncate" title={p.name}>{p.name}</p>
                        <p className="text-[#7d92b0] text-xs truncate" title={p.description}>{p.description}</p>
                      </td>
                      <td className="px-4 py-3 max-w-[140px]">
                        <p className="text-xs font-mono text-[#7d92b0] truncate" title={triggerSummary}>{triggerSummary}</p>
                      </td>
                      <td className="px-4 py-3 max-w-[160px]">
                        <p className="text-xs text-[#7d92b0] truncate" title={actionSummary}>{actionSummary}</p>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {p.requires_approval ? (
                          ...
                          <span className="text-xs text-[#3d5068]">不要</span>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => handleTogglePolicy(p)}>
                          {p.is_active ? <ToggleRight className="w-5 h-5 text-green-400" /> : <ToggleLeft className="w-5 h-5 text-[#3d5068]" />}
                        </button>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button onClick={() => { setEditPolicy(p); setShowPolicyForm(true) }} className="text-[#7d92b0] hover:text-white transition-colors">
                            <Edit2 className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => handleDeletePolicy(p)} className="text-[#7d92b0] hover:text-red-400 transition-colors">
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            <div className="flex items-center gap-2 bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2">
              ...
              <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as any)}
                className="bg-transparent text-sm text-[#7d92b0] focus:outline-hidden">
            ...
            <table className="w-full">
              ...
              <tbody>
                {filtered.map(exec => {
                  ...
                  return (
                    <tr key={exec.id} className="border-b border-[#1e2d42]/50 hover:bg-[#070d19]/50 transition-colors">
                      <td className="px-4 py-3 max-w-[140px]">
                        <p className="text-white text-xs font-medium truncate" title={exec.policy_name}>{exec.policy_name}</p>
                      </td>
                      <td className="px-4 py-3 max-w-[200px]">
                        <p className="text-xs text-[#7d92b0] truncate" title={exec.trigger_summary}>{exec.trigger_summary}</p>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {exec.affected_hosts.slice(0, 2).map(h => (
                            <span key={h} className="text-[10px] font-mono bg-[#1e2d42] px-1.5 py-0.5 rounded-sm text-[#7d92b0]">{h}</span>
                          ))}
                          {exec.affected_hosts.length > 2 && <span className="text-[10px] text-[#3d5068]">+{exec.affected_hosts.length - 2}</span>}
                        </div>
                      </td>
                      <td className="px-4 py-3 text-xs text-[#7d92b0] whitespace-nowrap">{fmt(exec.created_at)}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <button onClick={() => setDetailExecution(exec)} className="text-[#7d92b0] hover:text-white transition-colors">
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={7} className="text-center py-10 text-[#7d92b0] text-sm">条件に一致する実行ログがありません</td></tr>
                )}
              </tbody>
            </table>
          </div>
            ...
            <div className="flex items-end gap-4 h-16">
              {effectivenessData.map(d => (
                <div key={d.label} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-[9px] text-[#7d92b0]">{d.value}%</span>

      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
              ...
              {form.conditions.map((c, i) => (
                <div key={i} className="flex items-center gap-2">
                  <select value={c.field} onChange={e => setForm(p => ({ ...p, conditions: p.conditions.map((cc, idx) => idx === i ? { ...cc, field: e.target.value } : cc) }))}
                    className="bg-[#070d19] border border-[#1e2d42] rounded px-2 py-1.5 text-xs text-[#7d92b0] focus:outline-none">
                ...
                <div key={a.id} className="p-3 bg-[#070d19] rounded-lg border border-[#1e2d42]">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <select value={a.action_type} onChange={e => setForm(p => ({ ...p, actions: p.actions.map((aa, idx) => idx === i ? { ...aa, action_type: e.target.value as ActionType } : aa) }))}
                      className="flex-1 bg-[#0d1220] border border-[#1e2d42] rounded px-2 py-1.5 text-xs text-white focus:outline-none">
                    ...
                    <select value={a.on_failure} onChange={e => setForm(p => ({ ...p, actions: p.actions.map((aa, idx) => idx === i ? { ...aa, on_failure: e.target.value as OnFailure } : aa) }))}
                      className="bg-[#0d1220] border border-[#1e2d42] rounded px-2 py-1.5 text-xs text-[#7d92b0] focus:outline-none">
                  ...
                  <div className="flex items-center gap-2 pl-5">
                    ...
                    <input type="number" value={a.timeout_s} onChange={e => setForm(p => ({ ...p, actions: p.actions.map((aa, idx) => idx === i ? { ...aa, timeout_s: Number(e.target.value) } : aa) }))}
                      className="w-20 bg-[#0d1220] border border-[#1e2d42] rounded px-2 py-1 text-xs text-white focus:outline-none" />
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button onClick={() => { if (form.name && form.actions.length > 0) { onSave(form); onClose() } }}
            className="flex-1 py-2 rounded bg-[#e8002d] text-white text-sm font-medium hover:bg-[#c8001e] transition-colors">保存</button>
        </div>
      </div>
    </div>
  )
          ...
          {Object.keys(alertInput).map(f => (
            <div key={f}>
              ...
              <input value={(alertInput as any)[f]} onChange={e => setAlertInput(p => ({ ...p, [f]: e.target.value }))}
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded px-3 py-2 text-xs text-white focus:outline-none" />
            </div>
          ))}
        </div>
        <button onClick={runTest}
          className="w-full py-2 rounded bg-[#e8002d] text-white text-sm font-medium hover:bg-[#c8001e] transition-colors flex items-center justify-center gap-2 mb-4">
          ...
          <div className="space-y-2">
            ...
            {results.map(({ policy, matched }) => (
              <div key={policy.id} className={`p-3 rounded-lg border flex items-center gap-3 ${matched ? 'border-[#e8002d]/30 bg-[#e8002d]/10' : 'border-[#1e2d42] opacity-50'}`}>
                {matched ? <Zap className="w-4 h-4 text-[#e8002d] flex-shrink-0" /> : <XCircle className="w-4 h-4 text-[#3d5068] flex-shrink-0" />}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
          ...
          {execution.affected_hosts.length > 0 && (
            <div className="flex gap-1">
              {execution.affected_hosts.map(h => (
                <span key={h} className="text-xs font-mono bg-[#1e2d42] px-2 py-0.5 rounded text-[#e2e8f4]">{h}</span>
              ))}
            </div>
          )}
        </div>
        ...
        <div className="mb-4">
          ...
          <pre className="bg-[#070d19] border border-[#1e2d42] rounded p-3 text-xs font-mono text-[#7d92b0] overflow-x-auto">
            ...
            <div className="space-y-2">
              {execution.actions_taken.map((a, i) => (
                <div key={i} className={`p-3 rounded-lg border flex items-start gap-3 ${a.status === 'success' ? 'border-green-700/30 bg-green-900/10' : a.status === 'failed' ? 'border-red-700/30 bg-red-900/10' : 'border-[#1e2d42]'}`}>
                  {a.status === 'success' ? <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0 mt-0.5" /> :
                    a.status === 'failed' ? <XCircle className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" /> :
                    <Circle className="w-4 h-4 text-[#3d5068] flex-shrink-0 mt-0.5" />}
    ...
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      {masterEnabled && (
        <div className="p-3 bg-[#e8002d]/10 border border-[#e8002d]/30 rounded-lg mb-6 flex items-start gap-2">
          <AlertTriangle className="w-4 h-4 text-[#e8002d] flex-shrink-0 mt-0.5" />
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            <div className="flex items-center gap-2 bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2">
              ...
              <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as any)}
                className="bg-transparent text-sm text-[#7d92b0] focus:outline-none">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {exec.affected_hosts.slice(0, 2).map(h => (
                            <span key={h} className="text-[10px] font-mono bg-[#1e2d42] px-1.5 py-0.5 rounded text-[#7d92b0]">{h}</span>
                          ))}
                          ...
                          {exec.status === 'awaiting_approval' && (
                            <>
                              <button onClick={() => handleApprove(exec)}
                                className="px-2 py-1 rounded text-xs font-medium bg-green-900/40 text-green-300 hover:bg-green-900/60 transition-colors">承認</button>
                              <button onClick={() => handleReject(exec)}
                                className="px-2 py-1 rounded text-xs font-medium bg-red-900/40 text-red-300 hover:bg-red-900/60 transition-colors">拒否</button>
                            </>
                          )}

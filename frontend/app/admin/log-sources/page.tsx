  ...
  const mutation = useMutation({
    mutationFn: async (data: { name: string; description: string; format: string }) => {
      try {
        ...
        // 404 のとき 'ls_' + 乱数32文字の取り込みトークンをブラウザ側で
        // 作って返していました。運用担当はそれをコピーしてログ送信元に
        // 設定します。そのトークンを知っているサーバは存在しないので、
        // ログは一件も届かないまま「取り込み元を登録した」画面になります。
        // 届いていないことに気づくのは、そのログが必要になったときです。
        if (e instanceof Error && e.message.includes('404')) {
          throw new Error(
            'ログ取り込み元を登録する API がまだありません。トークンは発行されていません'
          )
        }
        throw e
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-hidden focus:border-[#e8002d]/50 transition-colors"
              required
            />
            {nameError && (
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="説明（任意）"
              rows={2}
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-[#e2e8f4] placeholder-[#3d5068] resize-none focus:outline-hidden focus:border-[#e8002d]/50 transition-colors"
            />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              ...
              className="px-4 py-2 text-sm bg-[#e8002d] hover:bg-[#c0001e] text-white rounded-sm font-medium transition-colors disabled:opacity-50"
            >
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-md p-6 shadow-2xl">
        ...
        <div className="bg-[#070d19] border border-[#1e2d42] rounded-sm p-3 mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="shrink-0 p-1.5 rounded-sm bg-[#1e2d42] hover:bg-[#2a3f5c] text-[#7d92b0] transition-colors"
            >
        ...
        <button
          onClick={onClose}
          className="w-full py-2 text-sm bg-[#e8002d] hover:bg-[#c0001e] text-white rounded-sm font-medium transition-colors"
        >
          トークンを保存しました
        </button>
      </div>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-sm p-6 shadow-2xl">
        ...
        <div className="flex gap-3">
          <button
            onClick={onCancel}
            className="flex-1 py-2 text-sm border border-[#1e2d42] text-[#7d92b0] hover:text-[#e2e8f4] rounded-sm transition-colors"
          >
            キャンセル
          </button>
          <button
...
function SourceDetailPanel({ source, onRegenerateToken, isRegenerating, regenError }: DetailPanelProps) {
  const { copied, copy } = useCopyToClipboard()
  const [showRegenConfirm, setShowRegenConfirm] = useState(false)

  // 取り込み量のグラフは、統計 API が失敗すると 200〜700 を基準に
  // 乱数で 24時間分を描いていました。日中に1.5倍の山を作り夜は0.5倍に
  // 落とすところまで作ってあるので、実測のグラフと見分けがつきません。
  // ログが1件も届いていない取り込み元が、順調に流れているように見えます。
  const { data: stats, error: statsError } = useQuery<LogSourceStats>({
    queryKey: ['log-source-stats', source.id],
    queryFn: () => apiFetch<LogSourceStats>(`/api/v1/admin/log-sources/${source.id}/stats`),
    staleTime: 60_000,
  ...
  return (
    <div className="border-t border-[#1e2d42] bg-[#070d19]/60 p-5 space-y-5">
      ...
      <div>
        ...
        <div className="flex items-center gap-2 bg-[#0d1220] border border-[#1e2d42] rounded-sm p-3">
          ...
          <button
            ...
            className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 text-xs bg-[#1e2d42] hover:bg-[#2a3f5c] text-[#7d92b0] hover:text-[#e2e8f4] rounded-sm transition-colors"
          >
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded-sm transition-colors"
            >
              ...
              <div className="absolute right-0 top-9 bg-[#0d1220] border border-[#1e2d42] rounded-lg p-3 shadow-xl z-10 w-56">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 text-xs py-1.5 border border-[#1e2d42] text-[#7d92b0] rounded-sm transition-colors hover:text-[#e2e8f4]"
                  >
                    キャンセル
                  </button>
                  <button
          ...
          {regenError && (
            <p className="text-[11px] text-amber-400 mt-2">{regenError}</p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        {statsError ? (
          <DataUnavailable what="取り込み数の推移" error={statsError} />
        ) : (
      ...
      <div>
        ...
        <div className="bg-[#070d19] border border-[#1e2d42] rounded-sm">
  ...
  const regenMutation = useMutation({
    mutationFn: async (id: string) => {
      try {
        ...
        // 再発行も同じで、'ls_regen_' + 乱数を返していました。古い
        // トークンは無効にならず、新しいトークンはどこにも登録されない
        // ので、鍵を替えたつもりで替わっていない状態になります。
        if (e instanceof Error && e.message.includes('404')) {
          throw new Error(
            'トークンを再発行する API がまだありません。トークンは変更されていません'
          )
        }
        throw e
      }
    },
    onSuccess: () => { setRegenError(null); qc.invalidateQueries({ queryKey: ['log-sources'] }) },
    onError: (e: Error) => setRegenError(e.message),
  })
    ...
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2.5 mb-1">
            <div className="w-7 h-7 rounded-sm bg-[#e8002d]/20 flex items-center justify-center">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-[#e8002d] hover:bg-[#c0001e] text-white text-sm font-medium rounded-sm transition-colors"
        >
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg overflow-hidden">
        ...
        {!isLoading && !isError && sources.length > 0 && (
          <div>
            {/* Table Header */}
            <div className="grid grid-cols-[1fr_100px_80px_120px_160px_180px_120px] gap-4 px-5 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-[#3d5068] border-b border-[#1e2d42]">
              <div>名前</div>
              <div>フォーマット</div>
              <div>ステータス</div>
              <div>総取り込み数</div>
            ...
            {sources.map(source => {
              ...
              return (
                <div key={source.id} className="border-b border-[#1e2d42] last:border-0">
                  <div
                    className="grid grid-cols-[1fr_100px_80px_120px_160px_180px_120px] gap-4 px-5 py-3.5 items-center hover:bg-[#0a1525] transition-colors cursor-pointer"
                    onClick={() => setExpandedId(isExpanded ? null : source.id)}
                  >
                    {/* Name */}
                    <div className="flex items-center gap-2 min-w-0">
                      {isExpanded
                        ? <ChevronUp className="w-3.5 h-3.5 text-[#3d5068] shrink-0" />
                        : <ChevronDown className="w-3.5 h-3.5 text-[#3d5068] shrink-0" />
                      }
                    ...
                    <div onClick={e => e.stopPropagation()}>
                      ...
                      >
                        <span className={`absolute top-0.5 w-4.5 h-4.5 bg-[#e2e8f4] rounded-full shadow-sm transition-transform ${
                    ...
                    >
                      <button
                        ...
                        className="p-1.5 rounded-sm text-[#3d5068] hover:text-[#7d92b0] hover:bg-[#1e2d42] transition-colors"
                        title="Copy token"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-sm text-[#3d5068] hover:text-red-400 hover:bg-red-500/10 transition-colors"
                        title="Delete source"
                      >

      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50" placeholder="例: Azure AD - 本社" />
          </div>
          <div>
            <label className="block text-[#7d92b0] text-sm mb-1.5">プロバイダータイプ</label>
            <select value={form.provider_type} onChange={e => setForm(f => ({ ...f, provider_type: e.target.value as ProviderType }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50">
          ...
          <div>
            ...
            <input value={form.tenant_id} onChange={e => setForm(f => ({ ...f, tenant_id: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm font-mono focus:outline-hidden focus:border-[#e8002d]/50" />
          </div>
          <div>
            ...
            <textarea value={form.config_json} onChange={e => setForm(f => ({ ...f, config_json: e.target.value }))} rows={6} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm font-mono focus:outline-hidden focus:border-[#e8002d]/50 resize-none" />
          </div>
        </div>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-md shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <select value={localUserId} onChange={e => setLocalUserId(e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50">
...
export default function CloudIdentityPage() {
  ...
  // 以前はサーバを呼ばずに2.5秒待ち、sync_status を synced にして
  // last_sync を今の時刻に書き換え、利用者数を Math.random() の分だけ
  // 増やしていました。同期は一度も行われていません。
  const handleSync = (id: string) => {
    const p = localProviders.find(x => x.id === id)
    showToast(`${p?.name ?? 'このプロバイダ'}の同期はサーバ側が未実装です`, 'error')
  }
                ...
                <tr key={p.id} className="hover:bg-[#0d1826] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => handleSync(p.id)} disabled={syncingIds.has(p.id)} title="同期" className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-blue-400 transition-colors disabled:opacity-50">
                        <RefreshCw className={`w-3.5 h-3.5 ${syncingIds.has(p.id) ? 'animate-spin' : ''}`} />
                      </button>
                      <button onClick={() => { setEditingProvider(p); setShowProviderModal(true) }} title="編集" className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-white transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => handleDeleteProvider(p.id)} title="削除" className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-[#e8002d] transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative">
              ...
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="メール/名前で検索..." className="bg-[#0d1220] border border-[#1e2d42] rounded-lg pl-9 pr-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 w-56" />
            </div>
            <select value={filterProvider} onChange={e => setFilterProvider(e.target.value)} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50">
              <option value="all">全プロバイダー</option>
              {providers.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            <select value={filterLinked} onChange={e => setFilterLinked(e.target.value as 'all' | 'linked' | 'unlinked')} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50">
              <option value="all">リンク状態: 全て</option>
              <option value="linked">リンク済み</option>
              <option value="unlinked">未リンク</option>
            </select>
            <select value={filterRisk} onChange={e => setFilterRisk(e.target.value as RiskIndicator | 'all')} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50">
                  ...
                  <tr key={id.id} className="hover:bg-[#0d1826] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {id.groups.slice(0, 2).map(g => <span key={g} className="px-1.5 py-0.5 bg-[#1e2d42] text-[#7d92b0] rounded-sm text-xs">{g}</span>)}
                        {id.groups.length > 2 && <span className="px-1.5 py-0.5 bg-[#1e2d42] text-[#7d92b0] rounded-sm text-xs">+{id.groups.length - 2}</span>}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {id.roles.slice(0, 2).map(r => <span key={r} className="px-1.5 py-0.5 bg-[#1e2d42] text-[#7d92b0] rounded-sm text-xs">{r}</span>)}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      {id.local_user_id ? (
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => setLinkingIdentity(id)} title={id.local_user_id ? 'リンク変更' : 'リンク'} className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-blue-400 transition-colors">

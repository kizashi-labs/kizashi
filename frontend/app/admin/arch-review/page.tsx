...
function CreateReviewModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    try {
      await apiFetch('/api/v1/admin/arch-reviews', { method: 'POST', body: JSON.stringify(form) })
    } catch { /* mock */ }
...
function AddFindingModal({ reviewId, onClose }: { reviewId: string; onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    try {
      await apiFetch(`/api/v1/admin/arch-reviews/${reviewId}/findings`, { method: 'POST', body: JSON.stringify(form) })
    } catch { /* mock */ }
    onClose()
  }

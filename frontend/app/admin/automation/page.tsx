        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          <div className="space-y-3">
            <div>
              ...
              <input value={name} onChange={e => setName(e.target.value)} placeholder="ワークフロー名を入力" className="w-full px-3 py-2 rounded-lg bg-[#070d19] border border-[#1e2d42] text-white text-sm placeholder-[#3d5068] focus:outline-none focus:border-[#7d92b0]/50" />
            </div>
            <div>
              <label className="block text-xs text-[#7d92b0] mb-1.5">説明</label>
              <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2} placeholder="ワークフローの説明" className="w-full px-3 py-2 rounded-lg bg-[#070d19] border border-[#1e2d42] text-white text-sm placeholder-[#3d5068] focus:outline-none focus:border-[#7d92b0]/50 resize-none" />
            </div>
          </div>
          ...
          <div>
            <h4 className="text-white font-medium mb-3 flex items-center gap-2">
              <div className="w-5 h-5 rounded bg-[#e8002d]/20 flex items-center justify-center text-[#e8002d]"><Zap className="w-3 h-3" /></div>
              トリガー設定
            </h4>
              ...
              {trigger.type === 'alert' && (
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <select className="px-2 py-1.5 rounded bg-[#0d1220] border border-[#1e2d42] text-[#7d92b0] focus:outline-none">
                      <option>severity</option><option>category</option><option>confidence</option>
                    </select>
                    <select className="px-2 py-1.5 rounded bg-[#0d1220] border border-[#1e2d42] text-[#7d92b0] focus:outline-none">
                      <option>{'>'}{`=`}</option><option>{'='}</option><option>{'!='}</option>
                    </select>
                    <input placeholder="値" className="px-2 py-1.5 rounded bg-[#0d1220] border border-[#1e2d42] text-white placeholder-[#3d5068] focus:outline-none" />
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs mb-2">
                    {['毎時', '毎日8:00', '毎週月曜', '毎月1日'].map(p => (
                      <button key={p} className="px-2 py-1.5 rounded bg-[#0d1220] border border-[#1e2d42] text-[#7d92b0] hover:text-white hover:border-[#7d92b0]/40 transition-colors text-left">{p}</button>
                    ))}
                  </div>
                  <input value={trigger.cron ?? ''} onChange={e => setTrigger(prev => ({ ...prev, cron: e.target.value }))} placeholder="カスタムCRON (例: 0 8 * * *)" className="w-full px-3 py-1.5 rounded bg-[#0d1220] border border-[#1e2d42] text-white text-xs placeholder-[#3d5068] font-mono focus:outline-none" />
                </div>
              )}
          ...
          <div>
            <h4 className="text-white font-medium mb-3 flex items-center gap-2">
              <div className="w-5 h-5 rounded bg-blue-900/30 flex items-center justify-center text-blue-400"><Settings className="w-3 h-3" /></div>
              ...
              {actions.map((action, idx) => (
                <div key={action.id} className="bg-[#070d19] border border-[#1e2d42] rounded-xl p-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-[#1e2d42] flex items-center justify-center text-xs text-[#7d92b0] flex-shrink-0">{idx + 1}</span>
                      <span className={`px-2 py-0.5 rounded text-xs ${CategoryColor(action.category)}`}>{action.category}</span>
                      <span className="text-white text-sm">{action.label}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <button onClick={() => moveAction(action.id, 'up')} disabled={idx === 0} className="p-1 rounded hover:bg-[#1e2d42] text-[#7d92b0] disabled:opacity-30 transition-colors"><ArrowUp className="w-3.5 h-3.5" /></button>
                      <button onClick={() => moveAction(action.id, 'down')} disabled={idx === actions.length - 1} className="p-1 rounded hover:bg-[#1e2d42] text-[#7d92b0] disabled:opacity-30 transition-colors"><ArrowDown className="w-3.5 h-3.5" /></button>
                      <button onClick={() => setConfigTarget(configTarget === action.id ? null : action.id)} className="p-1 rounded hover:bg-[#1e2d42] text-[#7d92b0] transition-colors"><Settings className="w-3.5 h-3.5" /></button>
                      <button onClick={() => removeAction(action.id)} className="p-1 rounded hover:bg-red-900/30 text-[#7d92b0] hover:text-red-400 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-[#1e2d42] space-y-2">
                      {Object.entries(action.config).map(([key, val]) => (
                        <div key={key} className="flex items-center gap-2">
                          <span className="text-xs text-[#7d92b0] w-24 flex-shrink-0 font-mono">{key}</span>
                          <input
                            value={String(val)}
                            onChange={e => setActions(prev => prev.map(a => a.id === action.id ? { ...a, config: { ...a.config, [key]: e.target.value } } : a))}
                            className="flex-1 px-2 py-1 rounded bg-[#0d1220] border border-[#1e2d42] text-white text-xs font-mono focus:outline-none focus:border-[#7d92b0]/50"
                          />
                        </div>
                      ))}
                    </div>
              ...
              <div className="space-y-2">
                {testResults.map((step, i) => (
                  <div key={i} className="flex items-center gap-3 text-sm">
                    {step.status === 'success' ? <CheckCircle className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                      : step.status === 'failure' ? <XCircle className="w-4 h-4 text-red-400 flex-shrink-0" />
                      : step.status === 'running' ? <Loader2 className="w-4 h-4 text-blue-400 animate-spin flex-shrink-0" />
                      : <div className="w-4 h-4 rounded-full border border-[#3d5068] flex-shrink-0" />}
                ...
                <div key={cat.category}>
                  ...
                  <div className="space-y-1.5">
                    {cat.actions.map(action => (
                      ...
                      >
                        <span className={`px-1.5 py-0.5 rounded text-[10px] ${CategoryColor(cat.category)}`}>{cat.category}</span>
                        {action.label}
                      </button>
                    ))}
                  </div>
            ...
            {activeWorkflows.map(wf => (
              <div key={wf.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 hover:border-[#2a3d5a] transition-colors">
                ...
                <div className="grid grid-cols-3 gap-3 mb-3 text-xs">
                  <div className="text-center p-2 rounded bg-[#1e2d42]/40">
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-[#1e2d42]">
                  <button onClick={() => setEditingWorkflow(wf)} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded text-xs bg-[#1e2d42] hover:bg-[#2a3d5a] text-white transition-colors"><Edit3 className="w-3 h-3" />編集</button>
                  <button className="flex items-center gap-1.5 px-2.5 py-1.5 rounded text-xs bg-[#1e2d42] hover:bg-[#2a3d5a] text-white transition-colors"><Copy className="w-3 h-3" />複製</button>
                  {wf.trigger.type === 'manual' && (
                    <button
                      onClick={() => runMutation.mutate(wf.id)}
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded text-xs bg-blue-900/30 hover:bg-blue-900/50 text-blue-400 border border-blue-700/30 transition-colors"
                    >
                  ...
                  <button
                    ...
                    className="ml-auto flex items-center gap-1.5 px-2.5 py-1.5 rounded text-xs bg-red-900/20 hover:bg-red-900/40 text-red-400 border border-red-700/30 transition-colors"
                  >
            ...
            {(runHistory ?? []).map(run => {
              ...
              return (
                <div key={run.id}>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      {run.status === 'success' ? <CheckCircle className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                        : run.status === 'failure' ? <XCircle className="w-4 h-4 text-red-400 flex-shrink-0" />
                    ...
                    <div className="px-5 pb-4 space-y-2">
                      {run.steps.map((step, i) => (
                        <div key={i} className="flex items-center gap-3 text-sm ml-7">
                          {step.status === 'success' ? <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                            : step.status === 'failure' ? <XCircle className="w-3.5 h-3.5 text-red-400 flex-shrink-0" />
                            : step.status === 'skipped' ? <div className="w-3.5 h-3.5 rounded-full border border-[#3d5068] flex-shrink-0" />
            ...
            {([] as Template[]).map(tmpl => (
              <div key={tmpl.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 flex flex-col">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className="text-xs text-[#7d92b0] bg-[#1e2d42] px-2 py-0.5 rounded flex-shrink-0 ml-2">{tmpl.action_count} アクション</span>
                </div>
                <p className="text-[#7d92b0] text-xs flex-1 mb-4">{tmpl.description}</p>
                <div className="flex flex-wrap gap-1 mb-4">
                  {tmpl.tags.map(tag => (
                    <span key={tag} className="px-2 py-0.5 rounded text-[10px] bg-[#1e2d42] text-[#7d92b0]">{tag}</span>
                  ))}
                </div>
                <button
                  onClick={() => setCreatingNew(true)}

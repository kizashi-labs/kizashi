        ...
        <div className="flex items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              <Network className="w-7 h-7 text-falcon-red" />
              Network Topology
            </h1>
            <p className="text-falcon-muted text-sm mt-0.5">Visual map of your network infrastructure</p>
          </div>
          <div className="flex gap-2 ml-2">
            <span className="px-3 py-1 rounded-full bg-falcon-surface border border-falcon-border text-falcon-muted text-sm">
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-falcon-muted text-sm">Filter:</span>
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-medium capitalize transition-colors border ${
                  selectedNodeType === type
                    ...
                    : 'bg-falcon-surface border-falcon-border text-falcon-muted hover:border-falcon-red hover:text-white'
                }`}
              >
          ...
          {topoLoading ? (
            <div className="grid grid-cols-4 gap-3">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="bg-falcon-surface border border-falcon-border rounded-xl p-4 animate-pulse h-40" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-4 gap-3">
              {filteredNodes.map((node) => (
                <button
                  ...
                  className={`bg-falcon-surface border-2 ${CRITICALITY_BORDER[node.criticality]} rounded-xl p-4 text-left hover:bg-falcon-card transition-all ${
                    selectedNode?.id === node.id ? 'ring-2 ring-falcon-red' : ''
                  }`}
                >
                  ...
                  <div className="space-y-0.5 mb-2">
                    {node.ip_addresses.map((ip) => (
                      <p key={ip} className="text-falcon-muted text-xs font-mono">{ip}</p>
                    ))}
                  </div>
                  <div className="flex items-center justify-between">
                    <span className={`px-1.5 py-0.5 rounded-sm text-xs font-medium capitalize ${CRITICALITY_BADGE[node.criticality]}`}>
                      {node.criticality}
                    </span>
                  </div>
                  <p className="text-falcon-muted text-xs mt-1 truncate">{node.os}</p>
                  <p className="text-falcon-muted text-xs truncate">{node.department}</p>
                </button>
              ))}
            </div>
          )}
          ...
          {/* Connections Table */}
          <div className="mt-6 bg-falcon-surface border border-falcon-border rounded-xl overflow-hidden">
                ...
                <thead>
                  <tr className="border-b border-falcon-border">
                    {['Source', '', 'Target', 'Protocol', 'Port', 'Bytes Transferred'].map((h) => (
                      <th key={h} className="text-left text-falcon-muted font-medium px-4 py-2 text-xs">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {(topology?.edges ?? []).map((edge) => (
                    <tr key={edge.id} className="border-b border-falcon-border last:border-0 hover:bg-falcon-card">
                      <td className="px-4 py-2.5 text-white font-mono text-xs">{edge.source_name}</td>
                      <td className="px-2 py-2.5 text-falcon-muted"><ChevronRight className="w-4 h-4" /></td>
                      <td className="px-4 py-2.5 text-white font-mono text-xs">{edge.target_name}</td>
                      <td className="px-4 py-2.5">
                        <span className="px-2 py-0.5 rounded-sm bg-blue-500/20 text-blue-400 text-xs">{edge.protocol}</span>
                      </td>
                      <td className="px-4 py-2.5 text-falcon-muted text-xs">{edge.port}</td>
                      <td className="px-4 py-2.5 text-falcon-muted text-xs">{formatBytes(edge.bytes_transferred)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {selectedNode && (
          <div className="w-72 shrink-0">
            <div className="bg-falcon-surface border border-falcon-border rounded-xl p-4 sticky top-6">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-white font-semibold text-sm">Node Details</h2>
                <button onClick={() => setSelectedNode(null)} className="text-falcon-muted hover:text-white">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <span className={`inline-block px-2 py-0.5 rounded-sm text-xs font-medium capitalize mb-3 ${CRITICALITY_BADGE[selectedNode.criticality]}`}>
                {selectedNode.criticality}
              </span>

              <div className="space-y-2 mb-4">
                <div>
                  <p className="text-falcon-muted text-xs">Type</p>
                  <p className="text-white text-sm capitalize">{selectedNode.type}</p>
                </div>
                <div>
                  <p className="text-falcon-muted text-xs">IP Addresses</p>
                  {selectedNode.ip_addresses.map((ip) => (
                    <p key={ip} className="text-white text-sm font-mono">{ip}</p>
                  ))}
                </div>
                <div>
                  <p className="text-falcon-muted text-xs">OS</p>
                  <p className="text-white text-sm">{selectedNode.os}</p>
                </div>
                <div>
                  <p className="text-falcon-muted text-xs">Department</p>
                  <p className="text-white text-sm">{selectedNode.department}</p>
                </div>
              </div>

              <div className="border-t border-falcon-border pt-3">
                <p className="text-falcon-muted text-xs font-medium mb-2">Connected via</p>
                {selectedNodeEdges.length === 0 ? (
                  <p className="text-falcon-muted text-xs">No connections</p>
                ) : (
                    ...
                    {selectedNodeEdges.map((edge) => {
                      ...
                      return (
                        <div key={edge.id} className="bg-[#0a1020] rounded-lg p-2">
                          <div className="flex items-center justify-between">
                            <span className="text-falcon-muted text-xs">{direction}</span>
                            <span className="px-1.5 py-0.5 rounded-sm bg-blue-500/20 text-blue-400 text-xs">{edge.protocol}</span>
                          </div>
                          <p className="text-white text-xs font-mono truncate">{peer}</p>
                          <p className="text-falcon-muted text-xs">:{edge.port} · {formatBytes(edge.bytes_transferred)}</p>
                        </div>
                      )
                    })}
                  </div>
          ...
          <div className="bg-falcon-surface border border-falcon-border rounded-2xl w-full max-w-md">
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-falcon-muted text-xs mb-1">Criticality</label>
                  <select
              ...
              <div>
                <label className="block text-falcon-muted text-xs mb-1">IP Address</label>
                <input
                  value={addForm.ip_address}
                  onChange={(e) => setAddForm((f) => ({ ...f, ip_address: e.target.value }))}
                  placeholder="e.g. 10.0.1.50"
                  className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm font-mono focus:outline-hidden focus:border-falcon-red"
                />
              </div>
              <div>
                <label className="block text-falcon-muted text-xs mb-1">OS</label>
                <input
                  value={addForm.os}
              ...
              <div>
                <label className="block text-falcon-muted text-xs mb-1">Department</label>
                <input

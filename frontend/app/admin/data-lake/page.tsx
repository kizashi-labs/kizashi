...
export default function DataLakePage() {
  ...
  const { data: sources = [] } = useQuery<DataSource[]>({
    ...
    queryFn: async () => {
      try { return await apiFetchList<DataSource>('/api/v1/admin/data-lake/sources') }
      catch { return [] }
    },
  })
  ...
  const { data: policies = [] } = useQuery<RetentionPolicy[]>({
    ...
    queryFn: async () => {
      try { return await apiFetchList<RetentionPolicy>('/api/v1/admin/data-lake/policies') }
      catch { return [] }
    },
  })
  ...
  const { data: archivalJobs = [] } = useQuery<ArchivalJob[]>({
    ...
    queryFn: async () => {
      try { return await apiFetchList<ArchivalJob>('/api/v1/admin/data-lake/archival-jobs') }
      catch { return [] }
    },
  })
  ...
  const saveSourceMutation = useMutation({
    mutationFn: async (data: Partial<DataSource>) => {
      ...
      } catch { return { ...data, id: `src-${Date.now()}` } }
    },
  ...
  const deleteSourceMutation = useMutation({
    mutationFn: async (id: string) => {
      try { return await apiFetch(`/api/v1/admin/data-lake/sources/${id}`, { method: 'DELETE' }) }
      catch { return null }
    },
  ...
  const savePolicyMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<RetentionPolicy> }) => {
      try { return await apiFetch(`/api/v1/admin/data-lake/policies/${id}`, { method: 'PUT', body: JSON.stringify(data) }) }
      catch { return null }
    },

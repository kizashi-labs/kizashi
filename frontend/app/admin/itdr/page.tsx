      ...
      {tab === 'incidents' && (
        ...
        <div className="flex gap-6">
            ...
            <div className="flex gap-2 mb-4">
              ...
              {['全て','open','investigating','resolved'].map(s => (
                ...
                <button key={s} onClick={() => setStatusFilter(s)}
                  ...
                  className={`px-3 py-1.5 text-xs rounded transition-colors ${statusFilter === s ? 'bg-[#1e2d42] text-white' : 'text-[#7d92b0] hover:text-white border border-[#1e2d42]'}`}>{s}</button>
                    ...
                    return (
                      ...
                      <tr key={inc.id} onClick={() => setSelectedIncident(selectedIncident === inc.id ? null : inc.id)}
                        ...
                        <td className="px-3 py-3">
                          ...
                          <span className={`px-2 py-0.5 rounded text-xs font-medium ${cm?.badge || 'bg-gray-700 text-gray-300'}`}>
                        ...
                        <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded text-xs ${SEV_BADGE[inc.severity]}`}>{inc.severity}</span></td>

                        <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded text-xs ${STATUS_BADGE[inc.status]}`}>{inc.status}</span></td>
                        ...
                        <td className="px-3 py-3">
                          ...
                          <div className="flex flex-wrap gap-1">
                            ...
                            {inc.indicators.slice(0,2).map((ind: any) => (
                              ...
                              <span key={ind} className="bg-[#1e2d42] text-[#7d92b0] text-xs px-1.5 py-0.5 rounded">{ind}</span>
          ...
          {selectedInc && (
            ...
            <div className="w-72 bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4 flex-shrink-0 self-start">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => investigateMutation.mutate(selectedInc.id)}
                  ...
                  className="flex-1 py-1.5 bg-[#e8002d] text-white text-xs rounded hover:bg-[#c0001f]">調査開始</button>

                <button onClick={() => fpMutation.mutate(selectedInc.id)}

                  className="flex-1 py-1.5 border border-[#1e2d42] text-[#7d92b0] text-xs rounded hover:text-white">偽陽性</button>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            {[['risk','リスクスコア順'],['privileged','特権アカウント'],['recent','最近の活動']].map(([k,l]) => (
              ...
              <button key={k} onClick={() => setUserSort(k)}
                ...
                className={`px-3 py-1.5 text-xs rounded ${userSort === k ? 'bg-[#1e2d42] text-white' : 'text-[#7d92b0] hover:text-white'}`}>{l}</button>
              ...
              <div key={u.id} className={`bg-[#0d1220] border-2 ${RISK_BORDER[u.risk_level]} rounded-lg p-5`}>
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div>
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded mt-1 inline-block ${ACCOUNT_BADGE[u.account_type]}`}>{u.account_type}</span>
                ...
                <div className="flex flex-wrap gap-1 mb-4">
                  ...
                  {u.risk_factors.map((f: any) => (
                    ...
                    <span key={f} className="bg-[#1e2d42] text-[#7d92b0] text-xs px-2 py-0.5 rounded">{f}</span>
                ...
                <div className="flex gap-2">
                  ...
                  <button className="flex-1 py-1.5 border border-[#1e2d42] text-[#7d92b0] text-xs rounded hover:text-white">詳細</button>

                  <button onClick={() => lockMutation.mutate(u.id)}

                    className="flex-1 py-1.5 bg-red-900/40 border border-red-700 text-red-300 text-xs rounded hover:bg-red-900/60">
      ...
      {tab === 'rules' && (
        ...
        <div>
          ...
          <div className="flex justify-end mb-4">
            ...
            <button className="px-3 py-1.5 bg-[#e8002d] text-white text-xs rounded hover:bg-[#c0001f]">+ 新規ルール</button>
                  ...
                  return (
                    ...
                    <tr key={r.id} className="border-b border-[#1e2d42] hover:bg-[#1e2d42]/20">
                      ...
                      <td className="px-3 py-3">
                        ...
                        <span className={`px-2 py-0.5 rounded text-xs ${cm?.badge || 'bg-gray-700 text-gray-300'}`}>
                      ...
                      <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded text-xs ${SEV_BADGE[r.severity]}`}>{r.severity}</span></td>

                      <td className="px-3 py-3">

                        <div className="flex gap-1 flex-wrap">

                          {r.mitre.map((t: any) => <span key={t} className="bg-blue-900/40 text-blue-300 text-xs px-1.5 py-0.5 rounded font-mono">{t}</span>)}

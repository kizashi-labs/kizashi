                    ...
                    <tr key={src.id} className="border-b border-[#1e2d42]/50 hover:bg-[#1e2d42]/20 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button onClick={() => handleTestConnection(src)} className="px-2 py-1 rounded bg-[#1e2d42] hover:bg-[#2a3f5a] text-xs transition-colors">テスト</button>
                          <button onClick={() => setSelectedSource(src)} className="p-1 rounded hover:bg-[#1e2d42] transition-colors"><Eye className="w-3.5 h-3.5" /></button>
                        </div>
                      </td>
                    </tr>
                  )
                  ...
                  {selectedSource.ingestion_trend.map((v, i) => {
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1">
                        <div className="w-full bg-blue-500/60 rounded-sm" style={{ height: `${(v / maxV) * 52}px` }} />
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {selectedSource.errors.map((e, i) => (
                      <p key={i} className="text-xs font-mono text-red-400 bg-red-900/10 border border-red-500/20 rounded px-2 py-1">{e}</p>
                    ))}
                  </div>
                </div>
              )}
              ...
              <tbody>
                {rules.map(rule => (
                  <tr key={rule.id} className="border-b border-[#1e2d42]/50 hover:bg-[#1e2d42]/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleTestRule(rule)} className="px-2 py-1 rounded bg-[#1e2d42] hover:bg-[#2a3f5a] text-xs transition-colors">テスト</button>
                        <button onClick={() => { setEditRule(rule); setShowAddRule(true) }} className="p-1 rounded hover:bg-[#1e2d42] transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
              ...
              {topRules.map(rule => (
                <div key={rule.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-5 bg-[#1e2d42] rounded overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-[#e8002d] to-[#ff4060] rounded transition-all"
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-4" style={{ flex: '0 0 40%' }}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <select value={timeRange} onChange={e => setTimeRange(e.target.value)} className="text-xs bg-[#1e2d42] border border-[#1e2d42] rounded px-2 py-1 text-[#7d92b0] outline-none">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setShowSaveQuery(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-[#1e2d42] hover:bg-[#2a3f5a] text-sm transition-colors">
                    <Save className="w-3.5 h-3.5" />
                    保存
                  </button>
                  <button onClick={handleExecuteQuery} className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-[#e8002d] text-white text-sm hover:bg-[#c8001e] transition-colors">
              ...
              <textarea
                ...
                className="w-full h-32 bg-[#070d19] border border-[#1e2d42] rounded-lg p-3 text-sm font-mono text-white placeholder-[#3d5068] outline-none resize-none focus:border-[#7d92b0]/40"
              />
              {showSaveQuery && (
                <div className="mt-2 flex items-center gap-2">
                  <input
                    ...
                    placeholder="クエリ名を入力"
                    className="flex-1 bg-[#1e2d42] border border-[#1e2d42] rounded px-3 py-1.5 text-sm text-white outline-none"
                  />
                  <button onClick={handleSaveQuery} className="px-3 py-1.5 rounded bg-green-600 text-white text-sm hover:bg-green-500 transition-colors">保存</button>
                  <button onClick={() => setShowSaveQuery(false)} className="px-3 py-1.5 rounded bg-[#1e2d42] text-[#7d92b0] text-sm hover:text-white transition-colors">キャンセル</button>
                </div>
              )}
            </div>
              ...
              {savedQueries.map(q => (
                <div key={q.id} className="bg-[#1e2d42]/40 border border-[#1e2d42] rounded-lg p-3">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    ...
                    <button onClick={() => handleDeleteSavedQuery(q.id)} className="text-[#3d5068] hover:text-red-400 transition-colors flex-shrink-0">
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={newSource.name} onChange={e => setNewSource(p => ({ ...p, name: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#7d92b0]/40" placeholder="本番環境CloudWatch" />
              </div>
              <div>
                <label className="block text-xs text-[#7d92b0] mb-1">ソースタイプ</label>
                <select value={newSource.source_type} onChange={e => setNewSource(p => ({ ...p, source_type: e.target.value as SourceType, config: {} }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none">
              ...
              {configFieldLabels[newSource.source_type].map(f => (
                <div key={f.key}>
                  ...
                  <input value={newSource.config[f.key] ?? ''} onChange={e => setNewSource(p => ({ ...p, config: { ...p.config, [f.key]: e.target.value } }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#7d92b0]/40" placeholder={f.placeholder} />
                </div>
              ))}
              <div>
                <label className="block text-xs text-[#7d92b0] mb-1">保持日数</label>
                <input type="number" value={newSource.retention_days} onChange={e => setNewSource(p => ({ ...p, retention_days: Number(e.target.value) }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#7d92b0]/40" />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#7d92b0]/40"
                />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={editRule?.rule_type ?? newRule.rule_type} onChange={e => editRule ? setEditRule(p => p ? { ...p, rule_type: e.target.value as RuleType } : null) : setNewRule(p => ({ ...p, rule_type: e.target.value as RuleType }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none">
                ...
                <div>
                  ...
                  <select value={editRule?.severity ?? newRule.severity} onChange={e => editRule ? setEditRule(p => p ? { ...p, severity: e.target.value as Severity } : null) : setNewRule(p => ({ ...p, severity: e.target.value as Severity }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full h-24 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm font-mono text-white placeholder-[#3d5068] outline-none resize-none focus:border-[#7d92b0]/40"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-[#7d92b0] mb-1">時間窓 (秒)</label>
                  <input type="number" value={editRule?.time_window ?? newRule.time_window} onChange={e => editRule ? setEditRule(p => p ? { ...p, time_window: Number(e.target.value) } : null) : setNewRule(p => ({ ...p, time_window: Number(e.target.value) }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#7d92b0]/40" />
                </div>
                <div>
                  <label className="block text-xs text-[#7d92b0] mb-1">閾値 (任意)</label>
                  <input type="number" value={editRule?.threshold ?? newRule.threshold} onChange={e => editRule ? setEditRule(p => p ? { ...p, threshold: Number(e.target.value) } : null) : setNewRule(p => ({ ...p, threshold: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#7d92b0]/40" />
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-3 mt-6">

...
function CategoryBadge({ category }: { category: string }) {
  ...
  return <span className={`px-2 py-0.5 rounded text-[11px] font-medium ${cls}`}>{category}</span>
}
...
function ExecStatusBadge({ status }: { status: PlaybookExecution['status'] }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded text-[11px] font-medium flex items-center gap-1 w-fit ${map[status]}`}>
      {icons[status]}
...
function Toggle({ enabled, onChange }: { enabled: boolean; onChange: () => void }) {
  return (
    <button
      onClick={onChange}
      className={`relative w-9 h-5 rounded-full transition-colors flex-shrink-0 ${enabled ? 'bg-[#1a6bff]' : 'bg-[#1e2d42]'}`}
    >

            ...
            {([] as ForensicCase[]).map(c => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded border capitalize ${priorityBadge(c.priority)}`}>
                    {c.priority}
                  </span>
                </div>
                ...
                <div className="space-y-2 mb-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded border ${caseStatusBadge(c.status)}`}>
                    ...
                    <div key={agent} className="flex items-center justify-between py-1.5 px-2 rounded-lg bg-[#070d19] border border-[#1e2d42]/60">
                      ...
                      {state === 'idle' && (
                        <button
                          ...
                          className="flex items-center gap-1 px-2 py-1 text-xs bg-purple-900/40 hover:bg-purple-900/60 text-purple-300 border border-purple-700/50 rounded transition-colors whitespace-nowrap"
                        >
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      type="date"
                      className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-2 py-2 text-xs text-white focus:outline-none focus:border-[#e8002d]/60"
                      value={logsFrom}
                ...
                {([] as Evidence[]).map(evd => {
                  ...
                  return (
                    <tr key={evd.id} className="border-b border-[#1e2d42]/50 hover:bg-[#131d31]/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1.5 text-xs px-2 py-0.5 rounded border ${evidenceTypeBadge(evd.type)}`}>
                      ...
                      <td className="px-4 py-3">
                        <code className="text-[#7d92b0] text-xs font-mono bg-[#070d19] px-2 py-0.5 rounded">
                          {evd.hash}
                        </code>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1.5 text-xs px-2 py-0.5 rounded border capitalize ${evidenceStatusBadge(evd.status)}`}>
              ...
              <div className="space-y-6">
                {([] as TimelineEvent[]).map((evt, idx) => (
                  <div key={idx} className="flex gap-4 relative">
                    {/* Dot */}
                    <div className={`w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center z-10 ${timelineColor(evt.type)}`}>

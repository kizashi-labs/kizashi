          ...
          <div>
            ...
            <div className="relative">
              <input type={showKey ? 'text' : 'password'} value={form.api_key} onChange={e => setForm(p => ({ ...p, api_key: e.target.value }))}
                ...
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded px-3 py-2 pr-10 text-sm text-white focus:outline-none focus:border-[#e8002d]/50 font-mono" />
    ...
    <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 mt-4">
      <div className="flex items-start justify-between mb-4">
        ...
        <button onClick={onAddToIOC}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-[#e8002d]/20 border border-[#e8002d]/40 text-[#e8002d] rounded text-xs font-medium hover:bg-[#e8002d]/30 transition-colors flex-shrink-0">
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {result.sources_used.map(s => (
          <span key={s} className="text-xs px-2 py-0.5 rounded bg-[#1e2d42] text-[#7d92b0] border border-[#2a3f5a]">{s}</span>
        ))}
      </div>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 mb-6">
        ...
        <div className="flex gap-3">
          <select value={iocType} onChange={e => setIocType(e.target.value as IOCType)}
            className="bg-[#070d19] border border-[#1e2d42] rounded px-3 py-2 text-sm text-white focus:outline-none w-28">
          ...
          <input value={iocValue} onChange={e => setIocValue(e.target.value)}
            ...
            className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded px-3 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50 font-mono" />
          <button onClick={handleEnrich} disabled={enriching || !iocValue.trim()}
            className="flex items-center gap-2 px-5 py-2 bg-[#e8002d] text-white rounded text-sm font-medium hover:bg-[#c8001e] transition-colors disabled:opacity-40">
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-sm p-6">
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowClearConfirm(false)} className="flex-1 py-2 rounded border border-[#1e2d42] text-[#7d92b0] hover:text-white text-sm transition-colors">キャンセル</button>
              <button onClick={handleClearCache} className="flex-1 py-2 rounded bg-red-600 text-white text-sm font-medium hover:bg-red-700 transition-colors">クリア実行</button>
            </div>
          </div>
        </div>
      )}

    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-md p-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-xs text-[#7d92b0] mb-1 block">名前</label>
          ...
          <div>
            <label className="text-xs text-[#7d92b0] mb-1 block">ソースタイプ</label>
          ...
          <div>
            <label className="text-xs text-[#7d92b0] mb-1 block">APIキー</label>
            <div className="relative">
              <input type={showKey ? 'text' : 'password'} value={form.api_key} onChange={e => setForm(p => ({ ...p, api_key: e.target.value }))}
                ...
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 pr-10 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50 font-mono" />
              <button onClick={() => setShowKey(!showKey)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#7d92b0] hover:text-white">
                {showKey ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              </button>
            </div>
          </div>
          <div>
            <label className="text-xs text-[#7d92b0] mb-1 block">1日あたりリクエスト上限</label>
...
function EnrichmentResultCard({ result, onAddToIOC }: { result: EnrichmentResult; onAddToIOC: () => void }) {
  ...
  return (
    <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 mt-4">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <code className="text-sm text-[#e2e8f4] font-mono">{result.indicator}</code>
        </div>
        <button onClick={onAddToIOC}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-[#e8002d]/20 border border-[#e8002d]/40 text-[#e8002d] rounded-sm text-xs font-medium hover:bg-[#e8002d]/30 transition-colors shrink-0">
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {result.sources_used.map(s => (
          <span key={s} className="text-xs px-2 py-0.5 rounded-sm bg-[#1e2d42] text-[#7d92b0] border border-[#2a3f5a]">{s}</span>
        ))}
      </div>

      <div className="grid grid-cols-2 gap-3 text-sm">
        {/* IP-specific */}
        {result.country && <div><p className="text-xs text-[#7d92b0] mb-0.5">国 / 都市</p><p className="text-[#e2e8f4]">{result.country} {result.city && `/ ${result.city}`}</p></div>}
        {result.asn && <div><p className="text-xs text-[#7d92b0] mb-0.5">ASN</p><p className="text-[#e2e8f4] font-mono text-xs">{result.asn}</p></div>}
        {result.abuse_score !== undefined && <div><p className="text-xs text-[#7d92b0] mb-0.5">不正使用スコア</p><p className={`font-bold ${result.abuse_score >= 80 ? 'text-red-400' : result.abuse_score >= 50 ? 'text-orange-400' : 'text-green-400'}`}>{result.abuse_score}/100</p></div>}
        {result.open_ports && result.open_ports.length > 0 && <div><p className="text-xs text-[#7d92b0] mb-0.5">オープンポート (Shodan)</p><p className="text-[#e2e8f4] font-mono text-xs">{result.open_ports.join(', ')}</p></div>}
        {/* Domain-specific */}
        {result.registrar && <div><p className="text-xs text-[#7d92b0] mb-0.5">レジストラ</p><p className="text-[#e2e8f4]">{result.registrar}</p></div>}
        {result.registered_at && <div><p className="text-xs text-[#7d92b0] mb-0.5">登録日</p><p className="text-[#e2e8f4]">{result.registered_at}</p></div>}
        {result.category && <div><p className="text-xs text-[#7d92b0] mb-0.5">カテゴリ</p><p className="text-[#e2e8f4]">{result.category}</p></div>}
        {result.dns_records && result.dns_records.length > 0 && <div><p className="text-xs text-[#7d92b0] mb-0.5">DNSレコード</p><div className="space-y-0.5">{result.dns_records.map(r => <p key={r} className="text-[#e2e8f4] font-mono text-xs">{r}</p>)}</div></div>}
        {/* Hash-specific */}
        {result.family && <div><p className="text-xs text-[#7d92b0] mb-0.5">マルウェアファミリ</p><p className="text-red-300 font-medium">{result.family}</p></div>}
        {result.file_type && <div><p className="text-xs text-[#7d92b0] mb-0.5">ファイルタイプ</p><p className="text-[#e2e8f4] font-mono text-xs">{result.file_type}</p></div>}
        {result.first_seen && <div><p className="text-xs text-[#7d92b0] mb-0.5">初回検知</p><p className="text-[#e2e8f4]">{result.first_seen}</p></div>}
        {result.last_seen && <div><p className="text-xs text-[#7d92b0] mb-0.5">最終検知</p><p className="text-[#e2e8f4]">{result.last_seen}</p></div>}
        {/* Common */}
        {result.vt_detections !== undefined && <div><p className="text-xs text-[#7d92b0] mb-0.5">VirusTotal 検知</p><p className={`font-bold ${result.vt_detections > 20 ? 'text-red-400' : result.vt_detections > 5 ? 'text-orange-400' : 'text-green-400'}`}>{result.vt_detections}/{result.vt_total}</p></div>}
        {result.threat_intel_matches !== undefined && <div><p className="text-xs text-[#7d92b0] mb-0.5">TIマッチ数</p><p className={`font-bold ${result.threat_intel_matches > 5 ? 'text-red-400' : result.threat_intel_matches > 0 ? 'text-orange-400' : 'text-green-400'}`}>{result.threat_intel_matches}</p></div>}
      </div>
    </div>
  )
}
...
export default function EnrichmentPage() {
  ...
  const handleToggleSource = async (s: EnrichmentSource) => {
    if (await persist('エンリッチメントソースの有効/無効', `/api/v1/admin/enrichment/sources/${s.id}/toggle`, { method: 'POST' })) {
      setLocalSources(prev => prev.map(x => x.id === s.id ? { ...x, is_active: !x.is_active } : x))
    }
  }

  // ヘルスチェックは1.5秒後に無条件で「ステータス正常」と出していました。
  // 疎通を確かめるための機能が、確かめられなかったときに正常と答えます。
  const handleHealthCheck = async (s: EnrichmentSource) => {
    showToast(`${s.name} のヘルスチェック中...`)
    if (await persist(`${s.name} のヘルスチェック`, `/api/v1/admin/enrichment/sources/${s.id}/health`, { method: 'POST' })) {
      showToast(`${s.name}: ステータス正常`)
    }
  }

  const handleDeleteSource = async (s: EnrichmentSource) => {
    if (await persist(`${s.name} の削除`, `/api/v1/admin/enrichment/sources/${s.id}`, { method: 'DELETE' })) {
      setLocalSources(prev => prev.filter(x => x.id !== s.id))
      showToast(`${s.name} を削除しました`)
    }
  }

  const handleSaveSource = async (form: Partial<EnrichmentSource>) => {
    if (editSource) {
    ...
    if (await persist(`${form.name} の追加`, '/api/v1/admin/enrichment/sources', { method: 'POST', body: JSON.stringify(form) })) {
  ...
  const handleClearCache = async () => {
    if (await persist('キャッシュのクリア', '/api/v1/admin/enrichment/cache', { method: 'DELETE' })) {
      setLocalCache([])
    ...
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-[#7d92b0] text-sm">外部ソースによるIOCコンテキスト強化</p>
        </div>
      </div>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 mb-6">
        ...
        <div className="flex gap-3">
          <select value={iocType} onChange={e => setIocType(e.target.value as IOCType)}
            className="bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden w-28">
          ...
          <input value={iocValue} onChange={e => setIocValue(e.target.value)}
            ...
            className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50 font-mono" />
          <button onClick={handleEnrich} disabled={enriching || !iocValue.trim()}
            className="flex items-center gap-2 px-5 py-2 bg-[#e8002d] text-white rounded-sm text-sm font-medium hover:bg-[#c8001e] transition-colors disabled:opacity-40">
      ...
      {tab === 'sources' && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <p className="text-[#7d92b0] text-sm">{localSources.length} ソース</p>
            ...
            <table className="w-full">
              ...
              <tbody>
                {localSources.map(s => {
                  ...
                  return (
                    <tr key={s.id} className={`border-b border-[#1e2d42]/50 hover:bg-[#070d19]/50 transition-colors ${!s.is_active ? 'opacity-50' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        <span className="text-xs font-mono text-[#7d92b0]">{s.api_key_masked}</span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 min-w-[120px]">
                        {s.daily_limit < 999999 ? (
                          ...
                          <span className="text-xs text-[#7d92b0]">無制限</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button onClick={() => handleHealthCheck(s)} className="text-[#7d92b0] hover:text-blue-400 transition-colors" title="ヘルスチェック">
                            <Activity className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => { setEditSource(s); setShowAddSource(true) }} className="text-[#7d92b0] hover:text-white transition-colors">
                            <Edit2 className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => handleDeleteSource(s)} className="text-[#7d92b0] hover:text-red-400 transition-colors">
      ...
      {tab === 'cache' && (
        <div>
          ...
          <div className="flex items-center justify-between mb-4">
            <p className="text-[#7d92b0] text-sm">{localCache.length} キャッシュエントリ</p>
          ...
          {localCache.length > 0 ? (
              ...
              <table className="w-full">
                ...
                <tbody>
                  {localCache.map(c => {
                    ...
                    return (
                      <tr key={c.id} className="border-b border-[#1e2d42]/50 hover:bg-[#070d19]/50 transition-colors">
                        <td className="px-4 py-3 max-w-[150px]">
                          <code className="text-xs font-mono text-[#e2e8f4] truncate block" title={c.indicator_value}>
                        ...
                        <td className="px-4 py-3 text-xs text-[#7d92b0]">{c.source}</td>
                        <td className="px-4 py-3 max-w-[180px]">
                          <code className="text-xs font-mono text-[#3d5068] truncate block" title={c.result_preview}>{c.result_preview.slice(0, 36)}…</code>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`text-xs ${expiring ? 'text-orange-400 font-medium' : 'text-[#7d92b0]'}`}>
                            {expiring && '⚠ '}{fmt(c.expires_at)}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-xs text-[#7d92b0] whitespace-nowrap">{fmt(c.created_at)}</td>
                      </tr>
                    )
                  })}
                </tbody>
            ...
            <div className="text-center py-16 text-[#7d92b0] text-sm bg-[#0d1220] border border-[#1e2d42] rounded-xl">
              キャッシュは空です
            </div>
          )}
        </div>
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-sm p-6">
            <div className="flex items-center gap-3 mb-4">
              <AlertTriangle className="w-6 h-6 text-red-400 shrink-0" />
              <h2 className="text-white font-semibold">キャッシュをクリアしますか?</h2>
            </div>
            <p className="text-[#7d92b0] text-sm mb-5">全{localCache.length}件のキャッシュエントリが削除されます。この操作は元に戻せません。</p>
            <div className="flex gap-3">
              <button onClick={() => setShowClearConfirm(false)} className="flex-1 py-2 rounded-sm border border-[#1e2d42] text-[#7d92b0] hover:text-white text-sm transition-colors">キャンセル</button>

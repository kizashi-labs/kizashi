...
export default function TrainingMgmtPage() {
  ...
  const { data: programs = [] } = useQuery<TrainingProgram[]>({
    ...
    queryFn: () => apiFetchList<TrainingProgram>('/api/v1/admin/training/programs'),
  })

  const { data: enrollments = [] } = useQuery<Enrollment[]>({
    queryKey: ['training-enrollments'],
    queryFn: () => apiFetchList<Enrollment>('/api/training/enrollments'),
  })

  const toggleMutation = useMutation({
    mutationFn: ({ id, active }: { id: string; active: boolean }) =>
      // .catch(() => null) で失敗が成功になっていました。
      apiFetch(`/api/v1/admin/training/programs/${id}`, { method: 'PATCH', body: JSON.stringify({ active }) }),
  ...
  return (
    <div className="min-h-screen bg-[#070d19] text-white p-6">
      <PageDataUnavailable />
      <SaveFailed error={saveErrorOf('研修プログラム', toggleMutation)} />
      {/* ヘッダー */}
                      ...
                      <td colSpan={7} className="px-6 py-4">
                        <div className="grid grid-cols-2 gap-6">
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1">{p.required_for_roles.map(r => <span key={r} className="px-2 py-0.5 bg-[#1e2d42] rounded-sm text-xs">{r}</span>)}</div>
                          </div>
                          <div>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-6 w-[420px]">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select className="w-full bg-[#1e2d42] border border-[#2a3f5f] rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#e8002d]">

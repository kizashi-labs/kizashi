        ...
        <div>
          <div className="flex items-center gap-3 mb-1">
            <div className="w-8 h-8 rounded-sm bg-falcon-red/10 border border-falcon-red/30 flex items-center justify-center">
              <GitMerge className="w-4 h-4 text-falcon-red" />
            </div>
            <h1 className="text-xl font-bold text-white">アラート自動割り当てルール</h1>
          </div>
          <p className="text-falcon-muted text-sm ml-11">アラートの自動アサインルールを管理します</p>
        </div>
        <button
          onClick={openCreate}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-falcon-border">
                  <th className="px-4 py-3 text-left text-xs font-medium text-falcon-muted uppercase tracking-wider">優先度</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-falcon-muted uppercase tracking-wider">ルール名</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-falcon-muted uppercase tracking-wider">条件</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-falcon-muted uppercase tracking-wider">割り当て先</th>
                ...
                {[...rules].sort((a, b) => a.priority - b.priority).map(rule => (
                  <tr key={rule.id} className="hover:bg-[#0a1628] transition-colors">
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-falcon-border text-xs font-bold text-white">
                        {rule.priority}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {(rule.conditions.severity_match ?? []).map(sev => (
                          <span
                            key={sev}
                            className={`px-1.5 py-0.5 rounded-sm text-[10px] font-medium border ${severityColor[sev] ?? 'text-falcon-muted border-falcon-border bg-falcon-border'}`}
                          >
                            {sev}
                          </span>
                        ))}
                        {(rule.conditions.rule_id_match ?? []).map(rid => (
                          <span key={rid} className="px-1.5 py-0.5 rounded-sm text-[10px] font-medium border border-falcon-border bg-falcon-border text-falcon-muted">
                            {rid}
                          </span>
                        ))}
                        {!(rule.conditions.severity_match?.length) && !(rule.conditions.rule_id_match?.length) && (
                          <span className="text-xs text-falcon-muted">全アラート</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-xs font-mono text-falcon-muted max-w-[140px] truncate">
                      {rule.assignee_id || '—'}
                    </td>
                    <td className="px-4 py-3">
                      {rule.enabled ? (
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-28 bg-[#070d19] border border-falcon-border rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden focus:border-falcon-red/50"
                />
              </div>

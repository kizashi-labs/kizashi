    ...
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          {/* View toggle */}
          <div className="flex bg-[#0d1220] border border-[#1e2d42] rounded-sm overflow-hidden">
            <button
      ...
      {!statusLoading && (
        <div className={`mb-6 rounded-lg border p-4 flex items-center gap-4 ${
          ...
          {isActive ? (
            <AlertTriangle className="w-6 h-6 text-[#e8002d] shrink-0" />
          ) : (
            <CheckCircle className="w-6 h-6 text-[#00c853] shrink-0" />
          )}
                      ...
                      <div className="mt-1 space-y-0.5">
                        {dayWindows.slice(0, 3).map(w => (
                          <div
                            key={w.id}
                            title={w.name}
                            className={`text-[9px] rounded-sm px-1 py-0.5 truncate cursor-pointer ${
                              w.enabled
        ...
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg overflow-hidden">
          ...
          {windowsLoading ? (
            ...
            <div className="p-10 text-center">
              ...
              <button
                onClick={openCreate}
                className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#e8002d]/10 hover:bg-[#e8002d]/20 text-[#e8002d] text-sm rounded-sm border border-[#e8002d]/30 transition-colors"
              >
                      ...
                      <tr key={w.id} className={`hover:bg-[#0a1628] transition-colors ${activeNow ? 'bg-[#e8002d]/5' : ''}`}>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            {activeNow && (
                              <span className="w-1.5 h-1.5 rounded-full bg-[#e8002d] shrink-0 animate-pulse" />
                            )}
                            <div>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl mx-4 shadow-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="px-5 py-4 space-y-5 overflow-y-auto flex-1">
                  ...
                  {form.recurrence_type === 'monthly' && (
                    <div>
                      ...
                      <input
                        ...
                        className="w-24 bg-[#0d1220] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50"
                      />
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-3">
                ...
                {form.scope === 'agents' && (
                  <div className="border border-[#1e2d42] rounded-sm p-2 max-h-40 overflow-y-auto bg-[#070d19] space-y-1">
                    {agentsList.length === 0 ? (
                      <p className="text-xs text-[#3d5068] px-1">エージェントなし</p>
                    ) : agentsList.map(a => {
                      const selected = form.affected_agents_text.split(',').map(s => s.trim()).includes(a.id)
                      return (
                        <label key={a.id} className="flex items-center gap-2 px-1 py-0.5 rounded-sm hover:bg-[#1e2d42] cursor-pointer">
                          <input
                            type="checkbox"
                            checked={selected}
                            onChange={e => {
              ...
              {formError && (
                <div className="flex items-center gap-2 bg-[#e8002d]/10 border border-[#e8002d]/30 rounded-sm px-3 py-2">
                  <AlertTriangle className="w-3.5 h-3.5 text-[#e8002d] shrink-0" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-sm mx-4 shadow-2xl p-5">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-8 h-8 rounded-full bg-[#e8002d]/10 flex items-center justify-center shrink-0">

...
const campaignTypeColor: Record<CampaignType, string> = {
  ...
  all_channel: 'bg-[#e8002d]/20 text-[#e8002d] border-[#e8002d]/30',
}
...
const statusColor: Record<CampaignStatus, string> = {
  ...
  completed: 'bg-[#7d92b0]/20 text-[#7d92b0] border-[#7d92b0]/30',
  ...
  return (
    <span className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <span key={i} className={i < full ? 'text-amber-400' : i === full && half ? 'text-amber-400/60' : 'text-[#1e2d42]'}>
          ★
        </span>
      ))}
      <span className="text-[#7d92b0] text-xs ml-1">{rating.toFixed(1)}</span>
    </span>
  )
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg shadow-2xl">
        ...
        <div className="px-6 py-4 space-y-4 max-h-[70vh] overflow-y-auto">
          <div>
            <label className="text-[#7d92b0] text-sm mb-1 block">キャンペーン名 *</label>
            <input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[#7d92b0] text-sm mb-1 block">種別</label>
              <select
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/60"
            ...
            <div>
              <label className="text-[#7d92b0] text-sm mb-1 block">対象</label>
              <select
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/60"
          ...
          <div>
            <label className="text-[#7d92b0] text-sm mb-1 block">目的</label>
            <textarea
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/60 resize-none"
              rows={2}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[#7d92b0] text-sm mb-1 block">開始日</label>
              <input
                type="date"
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/60"
            ...
            <div>
              <label className="text-[#7d92b0] text-sm mb-1 block">終了日</label>
              <input
                type="date"
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/60"
                value={form.end_date}
          ...
          <div>
            <label className="text-[#7d92b0] text-sm mb-2 block">コンテンツモジュール</label>
            <div className="grid grid-cols-2 gap-2">
              {ALL_MODULES.map(m => (
                ...
                >
                  {form.content_modules.includes(m) && <CheckCircle2 className="w-3.5 h-3.5 text-[#e8002d] flex-shrink-0" />}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-[#1e2d42]">
          ...
          <button
            ...
            className="px-5 py-2 bg-[#e8002d] hover:bg-[#c0001f] disabled:opacity-50 text-white text-sm font-medium rounded-lg transition-colors"
          >
            {isPending ? '作成中...' : '作成'}
          </button>
        </div>
...
function DetailModal({ campaign, onClose }: { campaign: Campaign; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1e2d42]">
          <div>
            <h2 className="text-white font-semibold text-lg">{detail.name}</h2>
            <p className="text-[#7d92b0] text-sm mt-0.5">{detail.objective}</p>
          </div>
          <button onClick={onClose} className="text-[#7d92b0] hover:text-white transition-colors"><X className="w-5 h-5" /></button>
        </div>
            ...
            {[
              ...
              <div key={label} className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-3">
                <div className="flex items-center gap-2 mb-1">
                  <Icon className="w-3.5 h-3.5 text-[#e8002d]" />
          ...
          <div>
            ...
            <div className="space-y-2">
              {detail.module_completions.map(mc => (
                <div key={mc.module} className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-3">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[#7d92b0] text-sm">{moduleLabel[mc.module]}</span>
                    <div className="flex items-center gap-4 text-xs">
                      <span className="text-[#7d92b0]">平均スコア: <span className="text-white">{mc.avg_score}点</span></span>
                      <span className="text-white font-medium">{mc.completion_rate}%</span>
                    </div>
                  </div>
                  <div className="h-1.5 bg-[#1e2d42] rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-[#e8002d] to-[#ff4d6d] rounded-full"
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={label} className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-3 flex items-center justify-between">
            ...
            <div className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-4">
              <div className="flex items-end gap-4">
                <div className="flex-1">
                  <p className="text-[#7d92b0] text-xs mb-1">事前スコア</p>
                  <div className="h-16 bg-[#1e2d42] rounded relative overflow-hidden">
                    <div
                      className="absolute bottom-0 left-0 right-0 bg-[#7d92b0]/40 rounded"
                ...
                <div className="flex-1">
                  <p className="text-[#7d92b0] text-xs mb-1">事後スコア</p>
                  <div className="h-16 bg-[#1e2d42] rounded relative overflow-hidden">
                    <div
                      className="absolute bottom-0 left-0 right-0 bg-[#e8002d]/40 rounded"
  ...
  return (
    <div className="min-h-screen bg-[#070d19] p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-[#e8002d]/20 border border-[#e8002d]/30 flex items-center justify-center">
            <Megaphone className="w-5 h-5 text-[#e8002d]" />
          </div>
          <div>
            <h1 className="text-white font-bold text-xl">セキュリティ意識向上キャンペーン</h1>
            <p className="text-[#7d92b0] text-sm">Security Awareness Campaign Management</p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: '総キャンペーン数', value: campaigns.length, sub: `実施中: ${campaigns.filter(c => c.status === 'active').length}`, icon: Megaphone, color: 'text-[#e8002d]' },
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-[#1e2d42] flex items-center justify-between">
          ...
          <span className="text-[#7d92b0] text-sm">{campaigns.length}件</span>
        </div>
              ...
              {campaigns.map(c => {
                ...
                return (
                  <tr key={c.id} className="hover:bg-[#0a1020] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded border text-xs font-medium ${campaignTypeColor[c.campaign_type]}`}>
                        {campaignTypeLabel[c.campaign_type]}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded border text-xs font-medium ${statusColor[c.status]}`}>
                        {c.status === 'active' && <span className="w-1.5 h-1.5 rounded-full bg-green-400 mr-1 animate-pulse" />}
                        {statusLabel[c.status]}
                      </span>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className="text-[#7d92b0] text-xs">{c.start_date} 〜 {c.end_date}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-[#7d92b0] text-xs">{audienceLabel[c.target_audience]}</span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-1.5 bg-[#1e2d42] rounded-full overflow-hidden">
                          <div className="h-full bg-[#e8002d] rounded-full" style={{ width: `${c.completion_rate}%` }} />
                        </div>
                        <span className="text-white text-xs">{c.completion_rate}%</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-[#7d92b0] text-sm">{c.pre_assessment_score > 0 ? c.pre_assessment_score : '—'}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-[#7d92b0] text-sm">{c.post_assessment_score > 0 ? c.post_assessment_score : '—'}</span>
                    </td>
                    <td className="px-4 py-3">
                      {improvement > 0 ? (
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-[#1e2d42] flex items-center justify-between">
          ...
          <span className="text-[#7d92b0] text-sm">0件</span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-5">
          {([] as ContentLibraryItem[]).map(item => (
            <div key={item.id} className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-4 hover:border-[#7d92b0]/40 transition-colors">
              <div className="flex items-start justify-between mb-2">
                <span className={`inline-flex px-2 py-0.5 rounded border text-xs font-medium ${campaignTypeColor[item.type]}`}>
              ...
              <p className="text-[#7d92b0] text-xs mb-2">{item.language}</p>
              <div className="flex items-center justify-between">
                <StarRating rating={item.rating} />
                <span className="text-[#3d5068] text-xs">更新: {item.last_updated}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          {scored.map(c => {
            ...
            return (
              <div key={c.id} className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-white text-sm font-medium">{c.name}</span>
                  <span className={`text-sm font-bold ${imp > 0 ? 'text-green-400' : 'text-[#7d92b0]'}`}>
                    {imp > 0 ? `+${imp}pt 向上` : '—'}
                  </span>
                </div>
          ...
          {scored.length === 0 && (
            <p className="text-center text-[#7d92b0] py-8">評価データがありません</p>
          )}
        </div>
      </div>

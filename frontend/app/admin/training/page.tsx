...
function TypeBadge({ type }: { type: Campaign['campaign_type'] }) {
  ...
  return <span className={`px-2 py-0.5 rounded text-[11px] font-medium border ${cls}`}>{label}</span>
}

function StatusBadge({ status }: { status: Campaign['status'] }) {
  if (status === 'running') return (
    <span className="flex items-center gap-1.5 px-2 py-0.5 rounded text-[11px] font-medium border bg-blue-900/40 text-blue-300 border-blue-700/40">
      <span className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" />実行中
    </span>
  )
  if (status === 'completed') return (
    <span className="flex items-center gap-1.5 px-2 py-0.5 rounded text-[11px] font-medium border bg-green-900/40 text-green-300 border-green-700/40">
      <CheckCircle2 className="w-3 h-3" />完了
    </span>
  )
  return <span className="px-2 py-0.5 rounded text-[11px] font-medium border bg-[#1e2d42]/60 text-[#7d92b0] border-[#1e2d42]">下書き</span>
}
      ...
      {steps.map((step, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-3">
            <div className="w-20 text-right text-[12px] text-[#7d92b0] flex-shrink-0">{step.label}</div>
            <div className="flex-1 h-7 bg-[#0d1220] rounded border border-[#1e2d42] relative overflow-hidden">
              <div
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-md p-6 shadow-2xl">
        ...
        <div className="flex gap-3 mt-6">
          <button
            onClick={onClose}
            className="flex-1 px-4 py-2 rounded border border-[#1e2d42] text-[#7d92b0] text-sm hover:text-[#e2e8f4] hover:border-[#7d92b0]/40"
          >
            キャンセル
          </button>
          <button
            ...
            className="flex-1 px-4 py-2 rounded bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium disabled:opacity-50"
          >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-sm p-6 shadow-2xl">
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 px-4 py-2 rounded border border-[#1e2d42] text-[#7d92b0] text-sm hover:text-[#e2e8f4]">
            キャンセル
          </button>
          <button
            onClick={() => mut.mutate()}
    ...
    <div className="p-6 space-y-6 bg-[#070d19] min-h-screen">
      ...
      <div className="grid grid-cols-4 gap-4">
        {statCards.map(({ label, value, icon: Icon, color }) => (
          <div key={label} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4 flex items-center gap-3">
            <div className={`w-9 h-9 rounded-lg bg-[#0d1220] border border-[#1e2d42] flex items-center justify-center flex-shrink-0`}>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {c.status === 'draft' && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-medium"
                      >
                    ...
                    {c.status !== 'draft' && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 border border-[#1e2d42] text-[#7d92b0] hover:text-[#e2e8f4] rounded text-xs font-medium"
                      >
      ...
      {tab === 'results' && (
        <div className="space-y-6">
          {/* Campaign Select */}
          <div className="flex items-center gap-3">
            <label className="text-sm text-[#7d92b0] flex-shrink-0">キャンペーン選択:</label>
            <div className="relative">
              <select
                className="bg-[#0d1220] border border-[#1e2d42] rounded px-3 py-2 text-sm text-[#e2e8f4] pr-8 focus:outline-none focus:border-blue-500 appearance-none"
            ...
            <div className="bg-[#0d1220] border border-[#e8002d]/30 rounded-lg p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {vulnerableUsers.map((u, i) => (
                  <span key={i} className="px-2.5 py-1 bg-[#e8002d]/10 border border-[#e8002d]/30 rounded text-xs text-[#e8002d] font-mono">
                    {u.email}
                  </span>
                ))}
              </div>

...
const STATUS_STYLE: Record<IntegrationStatus, { badge: string; dot: string; label: string }> = {
  ...
  disconnected: { badge: 'bg-falcon-border text-falcon-muted', dot: 'bg-falcon-subtle', label: '未接続' },
  error: { badge: 'bg-falcon-red/20 text-[#ff4d6d] border border-falcon-red/40', dot: 'bg-falcon-red', label: 'エラー' },
...
const CATEGORY_COLOR: Record<IntegrationCategory, string> = {
  ...
  EDR: 'text-falcon-red bg-falcon-red/10 border-falcon-red/30',
      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-lg shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-falcon-border">
          <div>
            ...
            <p className="text-falcon-muted text-xs mt-0.5">{integration.description}</p>
          </div>
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-falcon-muted text-xs">最終確認: {fmt(integration.last_ping)}</span>
            <span className="text-falcon-muted text-xs ml-auto">{fmtNum(integration.events_per_day)} events/day</span>
          </div>
          {Object.entries(fields).map(([key, value]) => (
            <div key={key}>
              <label className="text-falcon-muted text-xs font-medium block mb-1">{key}</label>
              <input
                value={value}
                onChange={e => setFields(prev => ({ ...prev, [key]: e.target.value }))}
                className="w-full bg-falcon-raised border border-falcon-border rounded-sm px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 font-mono"
              />
            </div>
          ))}
        </div>
        <div className="flex justify-end gap-2 px-5 pb-4">
          ...
          <button
            onClick={handleSave}
            className={`px-4 py-2 text-sm rounded-sm font-medium transition-colors ${saved ? 'bg-green-700 text-white' : 'bg-falcon-red text-white hover:bg-[#c4001f]'}`}
          >
            {saved ? '保存しました' : '設定を保存'}
          </button>
        </div>
      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-md shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          {[
            ...
            <div key={f.key}>
              <label className="text-falcon-muted text-xs font-medium block mb-1">{f.label}</label>
              {f.type === 'select' ? (
                <select
                  value={form[f.key as keyof typeof form] as string}
                  onChange={e => setForm(prev => ({ ...prev, [f.key]: e.target.value }))}
                  className="w-full bg-falcon-raised border border-falcon-border rounded-sm px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50"
                >
        ...
        <div className="flex justify-end gap-2 px-5 pb-4">
          ...
          <button
            ...
            className="px-4 py-2 text-sm rounded-sm font-medium bg-falcon-red text-white hover:bg-[#c4001f] transition-colors"
          >
            追加
          </button>
        </div>
    ...
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-falcon-muted text-sm mt-0.5">Security Orchestration Hub — Integration Management</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={testing}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-falcon-surface border border-falcon-border text-falcon-muted text-xs rounded-sm hover:border-[#2a3f5c] hover:text-white transition-colors disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-falcon-surface border border-falcon-border rounded-lg p-4">
          <p className="text-falcon-muted text-xs">接続中の統合</p>
          <div className="flex items-baseline gap-1 mt-1">
            <span className="text-2xl font-bold text-green-400">{connectedCount}</span>
            <span className="text-falcon-muted text-xs">/ {integrations.length}</span>
          </div>
        </div>
        <div className="bg-falcon-surface border border-falcon-border rounded-lg p-4">
          <p className="text-falcon-muted text-xs">NATS メッセージ/秒</p>
          <p className="text-2xl font-bold text-blue-400 mt-1">{fmtNum(nats.messages_per_sec)}</p>
        </div>
        <div className="bg-falcon-surface border border-falcon-border rounded-lg p-4">
          <p className="text-falcon-muted text-xs">保留中メッセージ</p>
          <p className={`text-2xl font-bold mt-1 ${nats.pending_messages > 1000 ? 'text-orange-400' : 'text-white'}`}>{fmtNum(nats.pending_messages)}</p>
        </div>
        <div className="bg-falcon-surface border border-falcon-border rounded-lg p-4">
          <p className="text-falcon-muted text-xs">ルーティングルール</p>
          <p className="text-2xl font-bold text-white mt-1">{rules.filter(r => r.enabled).length}<span className="text-falcon-muted text-sm font-normal"> 有効</span></p>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-1 flex-wrap">
                <button
                  ...
                  className={`px-2 py-1 text-xs rounded-sm transition-colors ${filterCategory === 'ALL' ? 'bg-falcon-active text-white' : 'text-falcon-muted hover:text-white'}`}
                >
                  ALL
                </button>
                {categories.map(cat => (
                  <button
                    ...
                    className={`px-2 py-1 text-xs rounded-sm transition-colors ${filterCategory === cat ? 'bg-falcon-active text-white' : 'text-falcon-muted hover:text-white'}`}
                  >
                    {cat}
                  </button>
                ))}
              ...
              {filtered.map(intg => {
                ...
                return (
                  <button
                    ...
                    className="bg-falcon-raised border border-falcon-border rounded-lg p-3 text-left hover:border-[#2a3f5c] transition-all relative"
                  >
                    {/* Category badge */}
                    ...
                    <div className="w-8 h-8 rounded-sm bg-falcon-border flex items-center justify-center mb-2 text-[10px] font-bold text-falcon-muted">
                    ...
                    <p className="text-falcon-subtle text-[10px] mt-1">{fmt(intg.last_ping)}</p>
                    <p className="text-falcon-muted text-[10px]">{fmtNum(intg.events_per_day)}/day</p>
                    {/* Test result overlay */}
                    {testResult && (
                      <div className={`absolute top-2 right-2 w-4 h-4 rounded-full flex items-center justify-center ${testResult === 'pending' ? 'bg-yellow-900/50' : testResult === 'success' ? 'bg-green-900/50' : 'bg-falcon-red/20'}`}>
          ...
          <div className="bg-falcon-surface border border-falcon-border rounded-lg p-4">
            ...
            <div className="relative min-h-[280px] flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-32 h-16 bg-falcon-active border-2 border-falcon-red/60 rounded-xl flex flex-col items-center justify-center shadow-lg">
                  <Network className="w-5 h-5 text-falcon-red mb-1" />
              ...
              <div className="absolute left-0 top-1/2 -translate-y-1/2">
                <div className="w-24 bg-falcon-raised border border-falcon-border rounded-lg p-2 text-center">
                  <Database className="w-4 h-4 text-blue-400 mx-auto mb-1" />
                  <p className="text-white text-[10px] font-medium">Data Sources</p>
                  <p className="text-falcon-muted text-[9px]">Endpoints, Logs, Cloud</p>
                </div>
              </div>
              {/* Left arrow */}
              <div className="absolute left-[100px] top-1/2 -translate-y-1/2 flex items-center">
                <div className="w-16 h-0.5 bg-linear-to-r from-blue-400/50 to-falcon-border relative overflow-hidden">
              ...
              <div className="absolute top-0 left-1/2 -translate-x-1/2">
                <div className="w-24 bg-falcon-raised border border-falcon-border rounded-lg p-2 text-center">
                  ...
                  <p className="text-falcon-muted text-[9px]">SIEM, Rules, ML</p>
                </div>
              </div>
              {/* Top arrow */}
              <div className="absolute left-1/2 -translate-x-1/2 top-[72px] flex flex-col items-center">
                <div className="w-0.5 h-12 bg-linear-to-b from-yellow-400/50 to-falcon-border relative">
              ...
              <div className="absolute right-0 top-1/2 -translate-y-1/2">
                <div className="w-24 bg-falcon-raised border border-falcon-border rounded-lg p-2 text-center">
                  ...
                  <p className="text-falcon-muted text-[9px]">SOAR, Playbooks</p>
                </div>
              </div>
              {/* Right arrow */}
              <div className="absolute right-[100px] top-1/2 -translate-y-1/2 flex items-center">
                ...
                <div className="w-16 h-0.5 bg-linear-to-r from-falcon-border to-orange-400/50 relative overflow-hidden">
              ...
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2">
                <div className="w-24 bg-falcon-raised border border-falcon-border rounded-lg p-2 text-center">
                  ...
                  <p className="text-falcon-muted text-[9px]">Dashboards, API</p>
                </div>
              </div>
              {/* Bottom arrow */}
              <div className="absolute left-1/2 -translate-x-1/2 bottom-[72px] flex flex-col items-center">
                ...
                <div className="w-0.5 h-12 bg-linear-to-t from-green-400/50 to-falcon-border" />
              </div>
            </div>
            <style>{`
          ...
          <div className="bg-falcon-surface border border-falcon-border rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                <div key={s.label} className="bg-falcon-raised border border-falcon-border rounded-lg p-3">
          ...
          <div className="bg-falcon-surface border border-falcon-border rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 bg-falcon-red text-white text-xs rounded-sm hover:bg-[#c4001f] transition-colors"
              >
                ...
                <tbody>
                  {rules.map(rule => (
                    <tr key={rule.id} className="border-b border-falcon-border/50 hover:bg-falcon-raised transition-colors">
                      <td className="py-2 px-3 font-mono text-falcon-red">{rule.event_type}</td>
                      <td className="py-2 px-3 font-mono text-falcon-muted max-w-[120px] truncate">{rule.condition}</td>
                      <td className="py-2 px-3 text-white">{rule.target_integration}</td>
                      <td className="py-2 px-3 text-falcon-muted font-mono">{rule.action}</td>
                      <td className="py-2 px-3">
                        <button
                          onClick={() => setRules(prev => prev.map(r => r.id === rule.id ? { ...r, enabled: !r.enabled } : r))}
                          className={`px-2 py-0.5 rounded-sm text-xs ${rule.enabled ? 'bg-green-900/30 text-green-400 border border-green-700/40' : 'bg-falcon-border text-falcon-muted'}`}
                        >
                          {rule.enabled ? '有効' : '無効'}
                        </button>
                      </td>
                      <td className="py-2 px-3">
                        <button
                          ...
                          className="text-falcon-subtle hover:text-[#ff4d6d] transition-colors"
                        >
          ...
          <div className="bg-falcon-surface border border-falcon-border rounded-lg p-4">
            ...
            <div className="space-y-2">
              {([] as { id: string; status: string; integration_name: string; action: string; timestamp: string; message: string; event_type: string }[]).map(log => (
                <div key={log.id} className="border border-falcon-border rounded-lg p-2.5 hover:border-[#2a3f5c] transition-colors">
                  <div className="flex items-center gap-2">
                    {log.status === 'success'
                    ...
                    <span className="text-falcon-subtle text-[10px] ml-auto font-mono">{fmt(log.timestamp)}</span>
                  </div>
                  <p className="text-falcon-muted text-[11px] mt-1 ml-5">{log.message}</p>
                  <p className="text-falcon-subtle text-[10px] ml-5 mt-0.5 font-mono">{log.event_type}</p>
                </div>
              ))}
            </div>
          </div>

      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-center gap-3">
            <span className={`text-xs px-2 py-0.5 rounded ${STATUS_STYLE[integration.status].badge}`}>
          ...
          {Object.entries(fields).map(([key, value]) => (
            <div key={key}>
              ...
              <input
                ...
                className="w-full bg-[#161f33] border border-[#1e2d42] rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/50 font-mono"
              />
            </div>
          ))}
        </div>
        <div className="flex justify-end gap-2 px-5 pb-4">
          ...
          <button
            onClick={handleSave}
            className={`px-4 py-2 text-sm rounded font-medium transition-colors ${saved ? 'bg-green-700 text-white' : 'bg-[#e8002d] text-white hover:bg-[#c4001f]'}`}
          >
            {saved ? '保存しました' : '設定を保存'}
          </button>
        </div>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-md shadow-2xl">
            ...
            <div key={f.key}>
              ...
              {f.type === 'select' ? (
                <select
                  ...
                  className="w-full bg-[#161f33] border border-[#1e2d42] rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/50"
                >
        ...
        <div className="flex justify-end gap-2 px-5 pb-4">
          ...
          <button
            ...
            className="px-4 py-2 text-sm rounded font-medium bg-[#e8002d] text-white hover:bg-[#c4001f] transition-colors"
          >
            追加
          </button>
        </div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={testing}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0d1220] border border-[#1e2d42] text-[#7d92b0] text-xs rounded hover:border-[#2a3f5c] hover:text-white transition-colors disabled:opacity-50"
          >
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-1 flex-wrap">
                <button
                  ...
                  className={`px-2 py-1 text-xs rounded transition-colors ${filterCategory === 'ALL' ? 'bg-[#1d2f4a] text-white' : 'text-[#7d92b0] hover:text-white'}`}
                >
                  ALL
                </button>
                {categories.map(cat => (
                  <button
                    ...
                    className={`px-2 py-1 text-xs rounded transition-colors ${filterCategory === cat ? 'bg-[#1d2f4a] text-white' : 'text-[#7d92b0] hover:text-white'}`}
                  >
                    {cat}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {filtered.map(intg => {
                ...
                return (
                  ...
                  >
                    {/* Category badge */}
                    <div className={`inline-block text-[10px] px-1.5 py-0.5 rounded border mb-2 font-medium ${CATEGORY_COLOR[intg.category]}`}>
                      {intg.category}
                    </div>
                    {/* Service name logo placeholder */}
                    <div className="w-8 h-8 rounded bg-[#1e2d42] flex items-center justify-center mb-2 text-[10px] font-bold text-[#7d92b0]">
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4">
            ...
            <div className="relative min-h-[280px] flex items-center justify-center">
              ...
              <div className="absolute left-[100px] top-1/2 -translate-y-1/2 flex items-center">
                <div className="w-16 h-0.5 bg-gradient-to-r from-blue-400/50 to-[#1e2d42] relative overflow-hidden">
              ...
              <div className="absolute left-1/2 -translate-x-1/2 top-[72px] flex flex-col items-center">
                <div className="w-0.5 h-12 bg-gradient-to-b from-yellow-400/50 to-[#1e2d42] relative">
              ...
              <div className="absolute right-[100px] top-1/2 -translate-y-1/2 flex items-center">
                ...
                <div className="w-16 h-0.5 bg-gradient-to-r from-[#1e2d42] to-orange-400/50 relative overflow-hidden">
              ...
              <div className="absolute left-1/2 -translate-x-1/2 bottom-[72px] flex flex-col items-center">
                ...
                <div className="w-0.5 h-12 bg-gradient-to-t from-green-400/50 to-[#1e2d42]" />
              </div>
            </div>
            <style>{`
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 bg-[#e8002d] text-white text-xs rounded hover:bg-[#c4001f] transition-colors"
              >
                    ...
                    <tr key={rule.id} className="border-b border-[#1e2d42]/50 hover:bg-[#161f33] transition-colors">
                      ...
                      <td className="py-2 px-3">
                        <button
                          ...
                          className={`px-2 py-0.5 rounded text-xs ${rule.enabled ? 'bg-green-900/30 text-green-400 border border-green-700/40' : 'bg-[#1e2d42] text-[#7d92b0]'}`}
                        >
                          {rule.enabled ? '有効' : '無効'}
                        </button>
                      </td>
                ...
                <div key={log.id} className="border border-[#1e2d42] rounded-lg p-2.5 hover:border-[#2a3f5c] transition-colors">
                  <div className="flex items-center gap-2">
                    {log.status === 'success'
                      ...
                      : <XCircle className="w-3.5 h-3.5 text-[#ff4d6d] flex-shrink-0" />}

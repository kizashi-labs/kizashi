  ...
  const AgentSelect = ({
    ...
    <div className="relative">
      <select
        ...
        className={`appearance-none bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50 pr-8 ${className ?? ''}`}
      >
  ...
  return (
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-[#7d92b0] text-sm ml-11">エージェントへのタグ付けと管理</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 gap-4 mb-6 max-w-sm">
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4">
          <p className="text-[#7d92b0] text-xs mb-1">総タグ数</p>
          <p className="text-2xl font-bold text-white">{tags.length}</p>
        </div>
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4">
          <p className="text-[#7d92b0] text-xs mb-1">登録エージェント数</p>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4 mb-6">
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex flex-col gap-1">
            <label className="text-xs text-[#7d92b0]">エージェント</label>
            <AgentSelect value={quickAgent} onChange={setQuickAgent} className="w-64" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs text-[#7d92b0]">タグ名</label>
            <input
              type="text"
              value={quickTag}
              onChange={e => setQuickTag(e.target.value)}
              placeholder="production"
              className="bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white placeholder-[#3d5068] focus:outline-hidden focus:border-[#e8002d]/50 w-48"
            />
          </div>
          <button
            onClick={() => handleAddTag(quickAgent, quickTag)}
            disabled={addTagMutation.isPending}
            className="px-4 py-2 bg-[#e8002d] hover:bg-[#c8001f] text-white text-sm font-medium rounded-sm transition-colors disabled:opacity-50"
          >
            {addTagMutation.isPending ? '追加中...' : '追加'}
          </button>
        </div>
        {successMsg && <p className="mt-2 text-xs text-green-400">{successMsg}</p>}
        {errorMsg && <p className="mt-2 text-xs text-[#e8002d]">{errorMsg}</p>}
      </div>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="p-8 text-center text-[#7d92b0] text-sm">タグが登録されていません</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#1e2d42]">
                <th className="px-4 py-3 text-left text-xs font-medium text-[#7d92b0] uppercase tracking-wider">タグ名</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-[#7d92b0] uppercase tracking-wider">エージェント数</th>
                  ...
                  <>
                    <tr key={tag} className="hover:bg-[#0a1628] transition-colors">
                      <td className="px-4 py-3">
                        <span className="inline-block px-2 py-0.5 rounded-sm text-xs font-medium bg-[#e8002d]/10 text-[#e8002d] border border-[#e8002d]/20">
                          {tag}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <button
                          ...
                          className="flex items-center gap-1.5 text-xs text-[#7d92b0] hover:text-white transition-colors px-2 py-1 rounded-sm hover:bg-[#1e2d42]"
                        >
                    ...
                    {isExpanded && (
                      <tr key={`${tag}-expanded`} className="bg-[#070d19]">
                        <td colSpan={3} className="px-6 py-4">
                          <div className="border border-[#1e2d42] rounded-lg overflow-hidden mb-3">
                            <div className="px-3 py-2 border-b border-[#1e2d42]">
                              <span className="text-xs font-medium text-[#7d92b0]">タグ「{tag}」のエージェント</span>
                            </div>
                            {!agentData ? (
                              <div className="p-4 text-center text-xs text-[#7d92b0]">読み込み中...</div>
                            ) : agentData.agent_ids.length === 0 ? (
                              <div className="p-4 text-center text-xs text-[#7d92b0]">エージェントなし</div>
                            ) : (
                              <ul className="divide-y divide-[#1e2d42]">
                                {agentData.agent_ids.map(id => (
                                  <li key={id} className="px-4 py-2.5 flex items-center justify-between hover:bg-[#0d1220]">
                                    <span className="text-xs text-[#e2e8f4] font-medium">
                                      {agentMap.get(id) ?? id}
                                    </span>
                                    <a
                                      href={`/endpoints/${id}`}
                                      className="flex items-center gap-1 text-xs text-[#7d92b0] hover:text-[#e8002d] transition-colors"
                                    >
                          ...
                          <div className="flex flex-wrap gap-2 items-end">
                            <div className="flex flex-col gap-1">
                              <label className="text-xs text-[#7d92b0]">別エージェントにこのタグを追加</label>
                              <AgentSelect value={inlineAgent} onChange={setInlineAgent} className="w-56" />
                            </div>
                            <button
                              onClick={() => handleAddTag(inlineAgent, tag)}
                              disabled={addTagMutation.isPending}
                              className="px-3 py-2 bg-[#1e2d42] hover:bg-[#e8002d] text-white text-xs font-medium rounded-sm transition-colors disabled:opacity-50"
                            >
                              追加
                            </button>
                          </div>

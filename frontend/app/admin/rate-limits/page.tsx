...
function ToggleSwitch({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    ...
    >
      {checked
        ? <ToggleRight className="w-6 h-6 text-falcon-red" />
        : <ToggleLeft className="w-6 h-6 text-falcon-subtle" />
      }
      <span className={`text-xs font-medium ${checked ? 'text-falcon-red' : 'text-falcon-muted'}`}>
        {checked ? '有効' : '無効'}
      </span>
    </button>
  )
      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-lg mx-4 shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-falcon-border">
          <div className="flex items-center gap-3">
            <Gauge className="w-5 h-5 text-falcon-red" />
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm placeholder-falcon-subtle focus:outline-hidden focus:border-falcon-red/50"
            />
          </div>

          {/* Endpoint pattern */}
          <div>
            <label className="block text-xs text-falcon-muted mb-1.5">エンドポイントパターン</label>
            <input
              value={form.endpoint_pattern}
              onChange={e => set('endpoint_pattern', e.target.value)}
              placeholder="例: /api/v1/*"
              className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm font-mono placeholder-falcon-subtle focus:outline-hidden focus:border-falcon-red/50"
            />
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs text-falcon-muted mb-1.5">リクエスト数/窓</label>
              <input
                type="number"
                min={1}
            ...
            <div>
              <label className="block text-xs text-falcon-muted mb-1.5">窓サイズ (秒)</label>
              <input
                type="number"
                min={1}
            ...
            <div>
              <label className="block text-xs text-falcon-muted mb-1.5">バースト上限</label>
              <input
                type="number"
                min={1}
          ...
          {/* Enabled */}
          <div className="flex items-center justify-between py-2 border-t border-falcon-border">
            <span className="text-sm text-falcon-muted">ルールを有効にする</span>
...
function DeleteConfirm({ rule, onConfirm, onCancel, isDeleting }: { rule: RateLimitRule; onConfirm: () => void; onCancel: () => void; isDeleting: boolean }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs">
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-sm mx-4 shadow-2xl p-6 text-center">
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 py-2 text-sm border border-falcon-border text-falcon-muted hover:text-white rounded-lg transition-colors">
            キャンセル
          </button>
          <button
            onClick={onConfirm}
            disabled={isDeleting}
            className="flex-1 py-2 text-sm bg-falcon-red hover:bg-[#c5001f] text-white rounded-lg transition-colors disabled:opacity-50"
          >
            {isDeleting ? '削除中...' : '削除する'}
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <Gauge className="w-6 h-6 text-falcon-red" />
            <h1 className="text-2xl font-bold text-white">レートリミット設定</h1>
          </div>
          <p className="text-falcon-muted text-sm">APIエンドポイントのリクエストレートを制御するルールを管理します。</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-sm bg-falcon-red hover:bg-[#c5001f] text-white rounded-lg transition-colors"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <StatCard label="総ルール数" value={rules.length} icon={Gauge} color="bg-falcon-border text-falcon-muted" />
        <StatCard label="有効なルール" value={enabledCount} icon={ShieldAlert} color="bg-falcon-red/10 text-falcon-red" />
        <StatCard label="無効なルール" value={disabledCount} icon={ToggleLeft} color="bg-falcon-border text-falcon-subtle" />
        <StatCard label="監視中のIP" value={0} icon={Zap} color="bg-orange-900/30 text-orange-400" />
      </div>

      {/* ── Rules table */}
      <div className="bg-falcon-surface border border-falcon-border rounded-xl mb-6">
        <div className="px-5 py-4 border-b border-falcon-border flex items-center justify-between">
          ...
          <span className="text-falcon-subtle text-xs">{rules.length} ルール</span>
        </div>

        {isLoading ? (
          <div className="flex items-center justify-center py-16 text-falcon-muted">
            <RefreshCw className="w-5 h-5 animate-spin mr-2" />
            読み込み中...
          </div>
        ) : rules.length === 0 ? (
          <div className="py-16 text-center text-falcon-muted">
            <Gauge className="w-10 h-10 mx-auto mb-3 text-falcon-subtle" />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-falcon-border">
                  <th className="text-left px-5 py-3 text-xs font-medium text-falcon-muted uppercase tracking-wider">ルール名</th>
                  <th className="text-left px-5 py-3 text-xs font-medium text-falcon-muted uppercase tracking-wider">エンドポイントパターン</th>
                  <th className="text-left px-5 py-3 text-xs font-medium text-falcon-muted uppercase tracking-wider">リクエスト/窓</th>
                  <th className="text-left px-5 py-3 text-xs font-medium text-falcon-muted uppercase tracking-wider">窓サイズ</th>
                  <th className="text-left px-5 py-3 text-xs font-medium text-falcon-muted uppercase tracking-wider">バースト</th>
                  <th className="text-left px-5 py-3 text-xs font-medium text-falcon-muted uppercase tracking-wider">状態</th>
                  <th className="text-right px-5 py-3 text-xs font-medium text-falcon-muted uppercase tracking-wider">操作</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-falcon-border/50">
                {rules.map(rule => (
                  <tr key={rule.id} className="hover:bg-falcon-hover/40 transition-colors">
                    <td className="px-5 py-3.5 text-white font-medium">{rule.name}</td>
                    <td className="px-5 py-3.5">
                      <code className="text-falcon-muted text-xs bg-[#070d19] px-2 py-0.5 rounded-sm font-mono">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-falcon-text font-mono text-xs">{(rule.requests_per_window ?? 0).toLocaleString()}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3.5 text-falcon-muted font-mono text-xs">{rule.window_size}s</td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1.5">
                        <Zap className="w-3.5 h-3.5 text-orange-400" />
                        <span className="text-falcon-text font-mono text-xs">{(rule.burst_limit ?? 0).toLocaleString()}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3.5">
                      {rule.enabled ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-falcon-red/10 text-falcon-red border border-falcon-red/20">
                          有効
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-falcon-border text-falcon-muted border border-falcon-border">
                          無効
                        </span>
                      )}
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="p-1.5 text-falcon-muted hover:text-white hover:bg-falcon-border rounded-sm transition-colors"
                          title="編集"
                        >
      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-xl">
        <div className="px-5 py-4 border-b border-falcon-border flex items-center gap-3">
          <ShieldAlert className="w-4 h-4 text-falcon-red" />
          <h2 className="text-white font-semibold text-sm">レートリミットに引っかかったIP (Top 5)</h2>
          <span className="ml-auto text-falcon-subtle text-xs">過去1時間</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-falcon-border">
                <th className="text-left px-5 py-3 text-xs font-medium text-falcon-muted uppercase tracking-wider">IPアドレス</th>
                <th className="text-left px-5 py-3 text-xs font-medium text-falcon-muted uppercase tracking-wider">リクエスト数</th>
                <th className="text-left px-5 py-3 text-xs font-medium text-falcon-muted uppercase tracking-wider">適用ルール</th>
                <th className="text-left px-5 py-3 text-xs font-medium text-falcon-muted uppercase tracking-wider">最終ヒット</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-falcon-border/50">
              {([] as TopIP[]).map((ip, i) => (
                <tr key={i} className="hover:bg-falcon-hover/40 transition-colors">
                  <td className="px-5 py-3.5">
                    <code className="text-falcon-text font-mono text-sm">{ip.ip}</code>
                  </td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 max-w-[100px] h-1.5 bg-falcon-border rounded-full overflow-hidden">
                        <div
                  ...
                  <td className="px-5 py-3.5">
                    <span className="text-falcon-muted text-xs">{ip.rule}</span>
                  </td>
                  <td className="px-5 py-3.5 text-falcon-muted text-xs">

                  ...
                  return (
                    <tr key={c.id} className="border-b border-[#1e2d42]/50 hover:bg-[#1e2d42]/20 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => setSelectedCampaign(c)} className="flex items-center gap-1.5 px-2 py-1 rounded bg-[#1e2d42] hover:bg-[#2a3f5a] text-xs transition-colors">
              ...
              <div key={tpl.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-4 hover:border-[#7d92b0]/30 transition-colors">
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  {tpl.industry_tags.map(tag => (
                    <span key={tag} className="text-[10px] px-1.5 py-0.5 bg-[#1e2d42] rounded text-[#3d5068]">{tag}</span>
                  ))}
                </div>
                <button onClick={() => setSelectedTemplate(tpl)} className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded bg-[#1e2d42] hover:bg-[#2a3f5a] text-sm text-[#7d92b0] transition-colors">
              ...
              {[
                ...
                return (
                  <div key={stat.label} className="bg-[#1e2d42]/30 rounded-lg p-3">
                    ...
                    <div className="mt-2 h-1.5 bg-[#1e2d42] rounded overflow-hidden">
                      <div className={`h-full ${stat.color} rounded`} style={{ width: `${pct}%` }} />
                    </div>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={newCampaign.name} onChange={e => setNewCampaign(p => ({ ...p, name: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none" placeholder="Q2 2026 フィッシングテスト" />
              </div>
              <div>
                <label className="block text-xs text-[#7d92b0] mb-1">テンプレート</label>
                <select value={newCampaign.template_id} onChange={e => setNewCampaign(p => ({ ...p, template_id: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none">
              ...
              <div>
                ...
                <select value={newCampaign.target_type} onChange={e => setNewCampaign(p => ({ ...p, target_type: e.target.value as typeof p.target_type }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none">
              ...
              {newCampaign.target_type === 'custom_list' && (
                <div>
                  ...
                  <textarea value={newCampaign.custom_emails} onChange={e => setNewCampaign(p => ({ ...p, custom_emails: e.target.value }))} className="w-full h-24 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm font-mono text-white outline-none resize-none" placeholder="user1@corp.local&#10;user2@corp.local" />
                </div>
              )}
              <div>
                <label className="block text-xs text-[#7d92b0] mb-1">開始日時</label>
                <input type="datetime-local" value={newCampaign.scheduled_at} onChange={e => setNewCampaign(p => ({ ...p, scheduled_at: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none" />
              </div>
              <div>
                <label className="block text-xs text-[#7d92b0] mb-1">ランディングページ (任意)</label>
                <input value={newCampaign.landing_page} onChange={e => setNewCampaign(p => ({ ...p, landing_page: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none" placeholder="https://example.com/awareness (空白=デフォルト)" />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={newTemplate.name} onChange={e => setNewTemplate(p => ({ ...p, name: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none" />
                </div>
                <div>
                  <label className="block text-xs text-[#7d92b0] mb-1">カテゴリー</label>
                  <select value={newTemplate.category} onChange={e => setNewTemplate(p => ({ ...p, category: e.target.value as TemplateCategory }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={newTemplate.difficulty} onChange={e => setNewTemplate(p => ({ ...p, difficulty: e.target.value as TemplateDifficulty }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none">
                ...
                <div>
                  ...
                  <input value={newTemplate.industry_tags} onChange={e => setNewTemplate(p => ({ ...p, industry_tags: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none" placeholder="finance, enterprise" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-[#7d92b0] mb-1">送信者名</label>
                  <input value={newTemplate.from_name} onChange={e => setNewTemplate(p => ({ ...p, from_name: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none" />
                </div>
                <div>
                  <label className="block text-xs text-[#7d92b0] mb-1">送信元メール</label>
                  <input value={newTemplate.from_email} onChange={e => setNewTemplate(p => ({ ...p, from_email: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-[#7d92b0] mb-1">件名</label>
                <input value={newTemplate.subject} onChange={e => setNewTemplate(p => ({ ...p, subject: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-none" />
              </div>
              <div>
                <label className="block text-xs text-[#7d92b0] mb-1">本文 (HTML)</label>
                <textarea value={newTemplate.body} onChange={e => setNewTemplate(p => ({ ...p, body: e.target.value }))} className="w-full h-36 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm font-mono text-white outline-none resize-none" placeholder="<p>本文を入力...</p>" />
              </div>
            </div>
            <div className="flex justify-end gap-3 mt-6">

    ...
    <div className="border border-[#1e2d42] rounded-lg overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className="text-xs text-[#7d92b0]">({section.criteria.length}項目)</span>
          <span className="text-xs text-[#7d92b0]">重み: {(section.weight * 100).toFixed(0)}%</span>
        </div>
      ...
      {open && (
        <div className="border-t border-[#1e2d42] divide-y divide-[#1e2d42]/50">
          {section.criteria.map(c => (
            <div key={c.id} className="flex items-center justify-between px-4 py-2.5 bg-[#070d19]/50">
              <span className="text-sm text-[#7d92b0]">{c.label}</span>
              <div className="flex items-center gap-2">
                {[1, 2, 3, 4, 5].map(n => (
                  <span key={n} className={`w-5 h-5 rounded text-[10px] font-bold flex items-center justify-center border ${n <= c.score ? 'bg-[#e8002d] border-[#e8002d] text-white' : 'border-[#1e2d42] text-[#3d5068]'}`}>{n}</span>
                ))}
              </div>
            </div>
          ))}
...
function AssessmentDetailPanel({ a, onClose, allAssessments }: { a: VendorAssessment; onClose: () => void; allAssessments: VendorAssessment[] }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-2xl h-full bg-[#0d1220] border-l border-[#1e2d42] overflow-y-auto"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="sticky top-0 z-10 bg-[#0d1220] border-b border-[#1e2d42] px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-white font-semibold text-lg">{a.vendor_name}</h2>
            <p className="text-[#7d92b0] text-sm">{a.product_name} v{a.version}</p>
          </div>
          <div className="flex items-center gap-2">
            {comparable.length > 0 && (
              <button
                onClick={() => setCompareMode(v => !v)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-sm border transition-colors ${compareMode ? 'bg-[#e8002d]/20 border-[#e8002d]/50 text-[#e8002d]' : 'border-[#1e2d42] text-[#7d92b0] hover:border-[#7d92b0]/40'}`}
              >
                <GitCompare className="w-3.5 h-3.5" />
                他のベンダーと比較
              </button>
            )}
            <button onClick={onClose} className="p-1.5 rounded hover:bg-[#1e2d42] text-[#7d92b0] hover:text-white">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-1 bg-[#070d19] border border-[#1e2d42] rounded-lg p-4 text-center">
              <p className="text-[#7d92b0] text-xs mb-1">総合スコア</p>
              <p className={`text-4xl font-black ${score >= 70 ? 'text-green-400' : score >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>{score}</p>
              <p className="text-[#7d92b0] text-[10px] mt-1">/ 100</p>
            </div>
            <div className="col-span-2 bg-[#070d19] border border-[#1e2d42] rounded-lg p-4 space-y-2">
              {a.scoring_sections.slice(0, 3).map(sec => (
                <div key={sec.id}>
                  <div className="flex justify-between text-xs mb-0.5">
                    <span className="text-[#7d92b0]">{sec.label}</span>
                ...
                {[a, ...comparable].map(v => {
                  ...
                  return (
                    <div key={v.id} className="flex items-center gap-3">
                      <span className={`text-sm w-36 truncate ${v.id === a.id ? 'text-white font-medium' : 'text-[#7d92b0]'}`}>{v.vendor_name}</span>
          ...
          {/* Vendor info */}
          <div className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-4">
            <h3 className="text-white font-medium text-sm mb-3 flex items-center gap-2"><Building2 className="w-4 h-4 text-[#e8002d]" />ベンダー情報</h3>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-[#7d92b0]">設立:</span> <span className="text-white ml-2">{a.founding_year}年</span></div>
              <div><span className="text-[#7d92b0]">本社:</span> <span className="text-white ml-2">{a.headquarters}</span></div>
              <div><span className="text-[#7d92b0]">売上推定:</span> <span className="text-white ml-2">{a.annual_revenue_estimate}</span></div>
              <div><span className="text-[#7d92b0]">上場:</span> <span className={`ml-2 font-medium ${a.public_company ? 'text-green-400' : 'text-[#7d92b0]'}`}>{a.public_company ? '上場企業' : '非上場'}</span></div>
              <div className="col-span-2 flex items-center gap-2">
                <span className="text-[#7d92b0]">Webサイト:</span>
            ...
            {a.certifications.length > 0 && (
              <div className="mt-3">
                <span className="text-[#7d92b0] text-xs">認証・認定:</span>
          ...
          <div className="bg-[#070d19] border border-[#1e2d42] rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <span className="text-[#7d92b0]">推奨事項:</span>
                {a.recommendation ? (
                  <span className={`ml-2 px-2 py-0.5 rounded text-xs border ${RECOM_COLORS[a.recommendation]}`}>{RECOM_LABELS[a.recommendation]}</span>
                ) : <span className="text-[#7d92b0] ml-2">未決定</span>}
              </div>
              <div><span className="text-[#7d92b0]">最終承認者:</span> <span className="text-white ml-2">{a.final_approver || '—'}</span></div>
              {a.conditions && (
                <div className="col-span-2">
                  <span className="text-[#7d92b0]">条件:</span>
                  <p className="text-white text-xs mt-1 bg-yellow-500/5 border border-yellow-500/20 rounded p-2">{a.conditions}</p>
                </div>
              )}
            </div>
          </div>
...
function CreateAssessmentModal({ templates, onClose, onCreated }: { templates: AssessmentTemplate[]; onClose: () => void; onCreated: (a: Partial<VendorAssessment>) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg p-6 shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-[#7d92b0] mb-1.5 block">ベンダー名 *</label>
              <input value={form.vendor_name} onChange={e => set('vendor_name', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50" placeholder="例: CrowdStrike" />
            </div>
            <div>
              <label className="text-xs text-[#7d92b0] mb-1.5 block">製品名 *</label>
              <input value={form.product_name} onChange={e => set('product_name', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50" placeholder="例: Falcon Insight XDR" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-[#7d92b0] mb-1.5 block">カテゴリ</label>
              <select value={form.category} onChange={e => set('category', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50">
                {(['EDR', 'SIEM', 'SOAR', 'IAM', 'Network', 'Cloud', 'GRC', 'Other'] as AssessmentCategory[]).map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs text-[#7d92b0] mb-1.5 block">バージョン</label>
              <input value={form.version} onChange={e => set('version', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50" placeholder="例: 7.2" />
            </div>
          </div>
          <div>
            <label className="text-xs text-[#7d92b0] mb-1.5 block">導入目的</label>
            <input value={form.use_case} onChange={e => set('use_case', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50" placeholder="例: 次世代EDR/XDR導入" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-[#7d92b0] mb-1.5 block">テンプレート</label>
              <select value={form.template_id} onChange={e => set('template_id', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50">
                <option value="">テンプレートなし</option>
                {templates.map(t => <option key={t.id} value={t.id}>{t.template_name}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs text-[#7d92b0] mb-1.5 block">評価担当者</label>
              <input value={form.assessor} onChange={e => set('assessor', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50" placeholder="担当者名" />
            </div>
          </div>
        </div>
        <div className="flex justify-end gap-3 mt-6">
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-[#1e2d42] text-[#7d92b0] hover:text-white hover:border-[#7d92b0]/40 text-sm transition-colors">キャンセル</button>
          <button
            onClick={() => { if (form.vendor_name && form.product_name) { onCreated(form); onClose() } }}
            className="px-4 py-2 rounded-lg bg-[#e8002d] hover:bg-[#c0001f] text-white text-sm font-medium transition-colors"
          >
            評価案件を作成
          </button>
        </div>
      ...
      <div className="border-b border-[#1e2d42] bg-[#0d1220] px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-[#e8002d]/10 border border-[#e8002d]/20 rounded-lg">
              <ClipboardList className="w-5 h-5 text-[#e8002d]" />
            </div>
            <div>
              <h1 className="text-white font-semibold text-xl">セキュリティベンダー評価</h1>
              <p className="text-[#7d92b0] text-sm">調達前・更新時のセキュリティ能力評価</p>
            </div>
          </div>
          {tab === 'cases' && (
            <button
        ...
        <div className="grid grid-cols-4 gap-4 mt-4">
          {[
            ...
            { label: '高リスク', value: stats.high_risk, color: 'text-[#e8002d]' },
          ].map(s => (
        ...
        {tab === 'cases' && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <select value={filterCategory} onChange={e => setFilterCategory(e.target.value as AssessmentCategory | 'all')} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-[#7d92b0] focus:outline-none">
                <option value="all">全カテゴリ</option>
                {(['EDR', 'SIEM', 'SOAR', 'IAM', 'Network', 'Cloud', 'GRC', 'Other'] as AssessmentCategory[]).map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as AssessmentStatus | 'all')} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-[#7d92b0] focus:outline-none">
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#1e2d42]/50">
                    ...
                    return (
                      <tr key={a.id} className="hover:bg-[#070d19]/50 transition-colors">
                        <td className="px-4 py-3">
                          ...
                          <p className="text-[#7d92b0] text-xs">{a.product_name}</p>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded text-xs font-medium border ${CATEGORY_COLORS[a.category]}`}>{a.category}</span>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded text-xs border flex items-center gap-1 w-fit ${status.color}`}>{status.icon}{status.label}</span>
                        </td>
                        <td className="px-4 py-3 text-[#7d92b0] text-xs">{a.assessor || '—'}</td>
                        <td className="px-4 py-3 text-[#7d92b0] text-xs">{a.submitted_date ?? '—'}</td>
                        <td className="px-4 py-3">
                          {sc > 0 ? (
                            <div className="w-20">
                              <ScoreBar value={sc} />
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded text-xs border ${RISK_COLORS[a.risk_rating]}`}>{a.risk_rating}</span>
                        </td>
                        <td className="px-4 py-3">
                          {a.recommendation ? (
                            <span className={`px-2 py-0.5 rounded text-xs border ${RECOM_COLORS[a.recommendation]}`}>{RECOM_LABELS[a.recommendation]}</span>
                          ) : <span className="text-[#3d5068]">—</span>}
                        </td>
                        <td className="px-4 py-3">
                          <button
                  ...
                  {filtered.length === 0 && (
                    <tr><td colSpan={9} className="px-4 py-12 text-center text-[#3d5068]">条件に一致する評価案件が見つかりません</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        {tab === 'templates' && (
          <div className="space-y-4">
            <div className="flex justify-end">
              <button className="flex items-center gap-2 px-4 py-2 border border-[#1e2d42] hover:border-[#7d92b0]/40 text-[#7d92b0] hover:text-white text-sm rounded-lg transition-colors">
                <Plus className="w-4 h-4" />
                テンプレートを作成
              </button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {templates.map(t => (
                <div key={t.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 hover:border-[#e8002d]/30 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    <div className="p-2 bg-[#e8002d]/10 border border-[#e8002d]/20 rounded-lg">
                      <FileText className="w-4 h-4 text-[#e8002d]" />
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 rounded text-xs border ${CATEGORY_COLORS[t.category]}`}>{t.category}</span>
                      {t.built_in && <span className="px-2 py-0.5 bg-blue-500/10 border border-blue-500/20 text-blue-300 text-xs rounded">組み込み</span>}
                    </div>
                  </div>
                  <h3 className="text-white font-medium mb-1">{t.template_name}</h3>
                  <p className="text-[#7d92b0] text-sm">{t.question_count} 質問項目</p>
                  <p className="text-[#3d5068] text-xs mt-1">最終更新: {t.last_updated}</p>
                  <button className="mt-4 w-full py-2 bg-[#e8002d]/10 hover:bg-[#e8002d]/20 border border-[#e8002d]/30 text-[#e8002d] text-sm rounded-lg transition-colors">
                    このテンプレートを使用
                  </button>
                </div>
              ))}

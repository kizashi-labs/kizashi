        ...
        <div className="border-t border-[#1e2d42] divide-y divide-[#1e2d42]/50">
          {section.criteria.map(c => (
            <div key={c.id} className="flex items-center justify-between px-4 py-2.5 bg-[#070d19]/50">
              ...
              <div className="flex items-center gap-2">
                {[1, 2, 3, 4, 5].map(n => (
                  <span key={n} className={`w-5 h-5 rounded-sm text-[10px] font-bold flex items-center justify-center border ${n <= c.score ? 'bg-[#e8002d] border-[#e8002d] text-white' : 'border-[#1e2d42] text-[#3d5068]'}`}>{n}</span>
                ))}
              </div>
            </div>
          ))}
          ...
          <div className="flex items-center gap-2">
            {comparable.length > 0 && (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-sm text-sm border transition-colors ${compareMode ? 'bg-[#e8002d]/20 border-[#e8002d]/50 text-[#e8002d]' : 'border-[#1e2d42] text-[#7d92b0] hover:border-[#7d92b0]/40'}`}
              >
                <GitCompare className="w-3.5 h-3.5" />
                他のベンダーと比較
              </button>
            )}
            <button onClick={onClose} className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-white">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.vendor_name} onChange={e => set('vendor_name', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50" placeholder="例: CrowdStrike" />
            </div>
            <div>
              <label className="text-xs text-[#7d92b0] mb-1.5 block">製品名 *</label>
              <input value={form.product_name} onChange={e => set('product_name', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50" placeholder="例: Falcon Insight XDR" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-[#7d92b0] mb-1.5 block">カテゴリ</label>
              <select value={form.category} onChange={e => set('category', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50">
            ...
            <div>
              ...
              <input value={form.version} onChange={e => set('version', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50" placeholder="例: 7.2" />
            </div>
          </div>
          <div>
            <label className="text-xs text-[#7d92b0] mb-1.5 block">導入目的</label>
            <input value={form.use_case} onChange={e => set('use_case', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50" placeholder="例: 次世代EDR/XDR導入" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-[#7d92b0] mb-1.5 block">テンプレート</label>
              <select value={form.template_id} onChange={e => set('template_id', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50">
            ...
            <div>
              ...
              <input value={form.assessor} onChange={e => set('assessor', e.target.value)} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50" placeholder="担当者名" />
            </div>
          </div>
        </div>
        ...
        {tab === 'cases' && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <select value={filterCategory} onChange={e => setFilterCategory(e.target.value as AssessmentCategory | 'all')} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-[#7d92b0] focus:outline-hidden">
                <option value="all">全カテゴリ</option>
                {(['EDR', 'SIEM', 'SOAR', 'IAM', 'Network', 'Cloud', 'GRC', 'Other'] as AssessmentCategory[]).map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as AssessmentStatus | 'all')} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-[#7d92b0] focus:outline-hidden">

            ...
            <div className="space-y-2">
              {course.score_buckets.map((v, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-4 bg-[#1e2d42] rounded-sm overflow-hidden">
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={onClose} className="flex-1 py-2 rounded-sm border border-[#1e2d42] text-[#7d92b0] text-sm">キャンセル</button>
            <button
              ...
              className="flex-1 py-2 rounded-sm bg-[#e8002d] text-white text-sm font-medium hover:bg-[#c8001e] disabled:opacity-50 transition-colors">
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-md p-6">
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button onClick={() => { if (name) { onCreate(name); onClose() } }}
            className="flex-1 py-2 rounded-sm bg-[#e8002d] text-white text-sm font-medium hover:bg-[#c8001e]">作成</button>
        </div>
      </div>
    </div>
  )
...
export default function TrainingAnalyticsPage() {
  ...
  // 受講者への割り当て。失敗を捨てて何も言わずに終わっていました。
  // /api/v1/admin/training/assign にはサーバ側のルートがありません。
  const handleAssign = async (data: { userIds: string[]; courseId: string; dueDate: string }) => {
    await persist('研修の割り当て', '/api/v1/admin/training/assign', {
      method: 'POST',
            ...
            return (
              <div key={course.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 hover:border-[#2a3f5a] transition-colors">
                ...
                <button onClick={() => setSelectedCourse(course)}
                  className="w-full py-2 bg-[#070d19] border border-[#1e2d42] text-[#7d92b0] hover:text-white hover:border-[#7d92b0]/40 rounded-sm text-sm transition-colors">
                  詳細を見る
                </button>
              </div>
            )
                  ...
                  <tr key={u.id} className="border-b border-[#1e2d42]/50 hover:bg-[#070d19]/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => setShowAssign(true)}
                          className="p-1.5 rounded-sm text-[#7d92b0] hover:text-white hover:bg-[#1e2d42] transition-colors" title="コース割り当て">
                          <GraduationCap className="w-3.5 h-3.5" />
                        </button>
                        <button className="p-1.5 rounded-sm text-[#7d92b0] hover:text-white hover:bg-[#1e2d42] transition-colors" title="リマインダー送信">
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowNewCampaign(true)}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-[#e8002d]/20 border border-[#e8002d]/40 text-[#e8002d] rounded-sm text-xs hover:bg-[#e8002d]/30 transition-colors">

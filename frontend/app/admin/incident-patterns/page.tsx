        ...
        <div key={c.id} className="flex items-center gap-2 mb-2">
          {i > 0 && (
            <select value={c.logic} onChange={e => updateCondition(c.id, 'logic', e.target.value)}
              className="w-14 bg-[#070d19] border border-[#1e2d42] rounded-sm px-1 py-1.5 text-xs text-[#7d92b0] focus:outline-hidden">
          ...
          <select value={c.operator} onChange={e => updateCondition(c.id, 'operator', e.target.value)}
            className="w-28 bg-[#070d19] border border-[#1e2d42] rounded-sm px-1 py-1.5 text-xs text-[#7d92b0] focus:outline-hidden">
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-3">
          ...
          <div>
            ...
            <textarea rows={2} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 resize-none"
        ...
        <div className="flex justify-end gap-2 p-4 border-t border-[#1e2d42]">
          ...
          <button onClick={() => onSave(form)} className="px-4 py-2 bg-[#e8002d] rounded-sm text-white text-sm hover:bg-[#e8002d]/80">保存</button>
        </div>
      </div>
    </div>
  )
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            ...
            <p className="text-[#7d92b0] text-sm leading-relaxed bg-[#070d19] rounded-sm p-3">{match.summary}</p>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {match.matched_incident_ids.map(id => (
                <span key={id} className="flex items-center gap-1 px-2 py-1 bg-[#1e2d42] rounded-sm text-xs text-blue-300 font-mono">
            ...
            <div className="space-y-1">
              {match.recommended_actions.map((action, i) => (
                <div key={i} className="flex items-start gap-2 bg-[#070d19] rounded-sm px-3 py-1.5">
                  <ChevronRight className="w-3 h-3 text-[#e8002d] shrink-0 mt-0.5" />
            ...
            <div className="flex flex-wrap gap-2">
              {(Object.keys(MATCH_STATUS_STYLES) as MatchStatus[]).map(status => (
                <button key={status} onClick={() => { onStatusChange(match.id, status); onClose() }}
                  className={`px-3 py-1.5 rounded-sm text-xs font-medium transition-all ${match.status === status ? `${MATCH_STATUS_STYLES[status].bg} ${MATCH_STATUS_STYLES[status].text} ring-1 ring-white/20` : 'bg-[#1e2d42] text-[#7d92b0] hover:text-white'}`}>

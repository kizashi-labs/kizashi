...
import { toAnomalyItem, type AnomalyType, type UBAAnomalyItem, type UEBAAnomalyRow } from '@/lib/ueba'
...
export default function UserBehaviorAnalyticsPage() {
  ...
  const { data: users = [], isLoading: usersLoading } = useQuery<UBAUser[]>({
    queryKey: ['uba-users'],
    queryFn: () => apiFetchList<UBAUser>('/api/v1/admin/uba/users'),
  })

  const { data: anomalies = [], isLoading: anomaliesLoading } = useQuery<UBAAnomalyItem[]>({
    queryKey: ['uba-anomalies'],
    queryFn: async () =>
      (await apiFetchList<UEBAAnomalyRow>('/api/v1/admin/ueba/anomalies')).map(toAnomalyItem),
  })
    ...
    <div className="min-h-screen bg-[#070d19] text-white p-6">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        ...
        <select
          ...
          className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-[#7d92b0] text-sm focus:outline-hidden focus:border-[#e8002d]"
        >
          {departments.map((d) => (
                    ...
                    <div className="flex items-start gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          ...
                          <span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${ANOMALY_BADGE[anomaly.type] ?? 'bg-[#1e2d42] text-[#7d92b0]'}`}>
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden sticky top-6">
              ...
              <div className="p-4 border-b border-[#1e2d42]">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-xs bg-[#1e2d42]" /><span className="text-[#7d92b0] text-xs">低</span></div>
                  <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-xs bg-orange-500/60" /><span className="text-[#7d92b0] text-xs">中</span></div>
                  <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-xs bg-[#e8002d]" /><span className="text-[#7d92b0] text-xs">高</span></div>
                </div>
              </div>
              ...
              <div className="p-4 border-b border-[#1e2d42]">
                ...
                <div className="space-y-1">
                  {([] as string[]).map((r) => (
                    <div key={r} className="text-xs text-[#7d92b0] font-mono truncate bg-[#070d19] rounded-sm px-2 py-1">{r}</div>
                  ))}
                </div>
              </div>

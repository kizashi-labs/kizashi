...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={doCopy}
      className="flex items-center gap-1 px-2 py-1 text-xs text-[#7d92b0] hover:text-white bg-[#0d1220] border border-[#1e2d42] rounded-sm transition-colors"
    >
        ...
        <div className="space-y-5">
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5 space-y-5">
            ...
            {sendMutation.isError && (
              <div className="flex items-center gap-2 px-3 py-2.5 bg-[#e8002d]/10 border border-[#e8002d]/30 rounded-lg">
                <XCircle className="w-4 h-4 text-[#e8002d] shrink-0" />
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-[#e8002d] hover:bg-[#c5001f] disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-lg transition-colors"
            >
              {isLoading ? (
                <>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-medium text-[#7d92b0] uppercase tracking-wide">
                      レスポンスボディ
                      {responseBody?.isJson && (
                        <span className="ml-2 px-1.5 py-0.5 bg-blue-900/30 border border-blue-800/40 text-blue-300 text-[10px] rounded-sm normal-case tracking-normal">
                          JSON
                        </span>
                      )}
                    </p>
                ...
                {history.map((entry) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-3 py-2.5 bg-[#070d19] border border-[#1e2d42] rounded-lg text-left hover:border-[#e8002d]/40 hover:bg-[#0d1220] transition-colors group"
                    >
                      <span
                        className={`shrink-0 text-xs font-bold tabular-nums px-1.5 py-0.5 rounded-sm
                          ${is2xx ? 'bg-green-900/40 text-green-300' : 'bg-[#e8002d]/20 text-[#ff6b7a]'}`}
                      >
                        {entry.status}
                      </span>
                      <span className="shrink-0 text-[10px] font-mono text-[#7d92b0] uppercase">
                        {entry.method}
                      </span>
                      <span className="flex-1 text-xs text-[#7d92b0] truncate font-mono group-hover:text-[#e2e8f4] transition-colors">
                        {entry.url}
                      </span>
                      <span className="shrink-0 flex items-center gap-1 text-xs text-[#3d5068]">
                        <Clock className="w-3 h-3" />
                        {entry.latency}ms
                      </span>
                      <span className="shrink-0 text-[10px] text-[#3d5068]">

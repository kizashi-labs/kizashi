      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50" placeholder="レポート名を入力..." />
          </div>
          {/* Type & Format */}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[#7d92b0] text-sm mb-1.5">レポートタイプ</label>
              <select value={form.report_type} onChange={e => setForm(f => ({ ...f, report_type: e.target.value as ReportType }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50">
            ...
            <div>
              ...
              <select value={form.output_format} onChange={e => setForm(f => ({ ...f, output_format: e.target.value as OutputFormat }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50">
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={2} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 resize-none" />
          </div>
          {/* Schedule */}
          <div>
            ...
            <div className="space-y-2">
              ...
              {!form.use_custom ? (
                <div className="flex gap-3">
                  <select value={form.schedule_preset} onChange={e => setForm(f => ({ ...f, schedule_preset: e.target.value }))} className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50">
                    <option value="daily">毎日</option>
                    <option value="weekly">毎週</option>
                    <option value="monthly">毎月</option>
                  </select>
                  <input type="time" value={form.schedule_time} onChange={e => setForm(f => ({ ...f, schedule_time: e.target.value }))} className="bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50" />
                </div>
              ) : (
                <input value={form.custom_cron} onChange={e => setForm(f => ({ ...f, custom_cron: e.target.value }))} placeholder="0 9 * * 1" className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50 font-mono" />
              )}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {form.recipients.map(r => (
                <span key={r} className="flex items-center gap-1 px-2 py-1 bg-[#1e2d42] rounded-sm text-xs text-white">
                  {r}
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              <input value={form.param_key} onChange={e => setForm(f => ({ ...f, param_key: e.target.value }))} placeholder="キー" className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50" />
              <input value={form.param_value} onChange={e => setForm(f => ({ ...f, param_value: e.target.value }))} placeholder="値" className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50" />
            ...
            {Object.entries(form.parameters).length > 0 && (
              <div className="space-y-1">
                {Object.entries(form.parameters).map(([k, v]) => (
                  <div key={k} className="flex items-center gap-2 px-2 py-1 bg-[#070d19] border border-[#1e2d42] rounded-sm text-xs">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" value={form.period_start} onChange={e => setForm(f => ({ ...f, period_start: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50" />
            </div>
            <div>
              <label className="block text-[#7d92b0] text-sm mb-1.5">終了日</label>
              <input type="date" value={form.period_end} onChange={e => setForm(f => ({ ...f, period_end: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50" />
            </div>
          </div>
        </div>
        <div className="flex justify-end gap-3 p-5 border-t border-[#1e2d42]">
      ...
      <div className="space-y-3">
        {types.map((t, i) => (
          <div key={t} className="flex items-center gap-3">
            <span className="text-[#7d92b0] text-xs w-28 shrink-0">{TYPE_LABEL[t]}</span>
...
export default function ReportsEnginePage() {
  ...
  // 以前はここでサーバを呼ばずに「生成しました」と言い、completed の行を
  // ファイルサイズ付き（Math.random()）で一覧に足していました。開ける
  // ファイルはどこにもありません。
  const handleRunNow = (s: Schedule) => {
    showToast(`「${s.name}」の即時生成はサーバ側が未実装です`, 'error')
    if (!USE_MOCK) return
  ...
  const handleGenerate = (data: { name: string; report_type: ReportType; period_start: string; period_end: string; output_format: OutputFormat }) => {
    ...
    // handleRunNow と同じく、サーバを呼ばずに3秒後 completed にして
    // ファイルサイズを Math.random() で付けていました。
    if (!USE_MOCK) {
      showToast('レポートの生成はサーバ側が未実装です', 'error')
      return
    }
                ...
                <tr key={s.id} className="hover:bg-[#0d1826] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => handleRunNow(s)} title="今すぐ生成" className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-green-400 transition-colors"><Play className="w-3.5 h-3.5" /></button>
                      <button onClick={() => { setEditingSchedule(s); setShowScheduleModal(true) }} title="編集" className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-white transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => handleDeleteSchedule(s.id)} title="削除" className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-[#e8002d] transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {activeTab === 'reports' && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select value={filterType} onChange={e => setFilterType(e.target.value as ReportType | 'all')} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50">
              <option value="all">全タイプ</option>
              {(Object.keys(TYPE_LABEL) as ReportType[]).map(t => <option key={t} value={t}>{TYPE_LABEL[t]}</option>)}
            </select>
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as ReportStatus | 'all')} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50">
              <option value="all">全ステータス</option>
              {(Object.keys(STATUS_LABEL) as ReportStatus[]).map(s => <option key={s} value={s}>{STATUS_LABEL[s]}</option>)}
            </select>
            <input type="date" value={filterFrom} onChange={e => setFilterFrom(e.target.value)} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50" />
            <span className="text-[#7d92b0] text-sm">〜</span>
            <input type="date" value={filterTo} onChange={e => setFilterTo(e.target.value)} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-[#e8002d]/50" />
            {(filterType !== 'all' || filterStatus !== 'all' || filterFrom || filterTo) && (
              <button onClick={() => { setFilterType('all'); setFilterStatus('all'); setFilterFrom(''); setFilterTo('') }} className="flex items-center gap-1 px-2 py-1 text-xs text-[#7d92b0] hover:text-white rounded-sm hover:bg-[#1e2d42] transition-colors"><X className="w-3 h-3" />クリア</button>
            )}
          </div>
                ...
                {filteredReports.map(r => (
                  <tr key={r.id} className="hover:bg-[#0d1826] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        {r.status === 'completed' && (
                          <button title="ダウンロード" onClick={() => showToast('ダウンロードを開始しました')} className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-blue-400 transition-colors"><Download className="w-3.5 h-3.5" /></button>
                        )}
                        <button onClick={() => handleDeleteReport(r.id)} title="削除" className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-[#e8002d] transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}

    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          {/* Name */}
          <div>
            <label className="block text-falcon-muted text-sm mb-1.5">レポート名 *</label>
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50" placeholder="レポート名を入力..." />
          </div>
          {/* Type & Format */}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-falcon-muted text-sm mb-1.5">レポートタイプ</label>
              <select value={form.report_type} onChange={e => setForm(f => ({ ...f, report_type: e.target.value as ReportType }))} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50">
                {(Object.keys(TYPE_LABEL) as ReportType[]).map(t => <option key={t} value={t}>{TYPE_LABEL[t]}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-falcon-muted text-sm mb-1.5">出力形式</label>
              <select value={form.output_format} onChange={e => setForm(f => ({ ...f, output_format: e.target.value as OutputFormat }))} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50">
          ...
          {/* Description */}
          <div>
            <label className="block text-falcon-muted text-sm mb-1.5">説明</label>
            <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={2} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 resize-none" />
          </div>
          {/* Schedule */}
          <div>
            <label className="block text-falcon-muted text-sm mb-1.5">スケジュール</label>
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 text-sm text-falcon-muted cursor-pointer">
                  <input type="radio" checked={!form.use_custom} onChange={() => setForm(f => ({ ...f, use_custom: false }))} className="accent-falcon-red" />
                  プリセット
                </label>
                <label className="flex items-center gap-2 text-sm text-falcon-muted cursor-pointer">
                  <input type="radio" checked={form.use_custom} onChange={() => setForm(f => ({ ...f, use_custom: true }))} className="accent-falcon-red" />
                  カスタムCron
                </label>
              </div>
              {!form.use_custom ? (
                <div className="flex gap-3">
                  <select value={form.schedule_preset} onChange={e => setForm(f => ({ ...f, schedule_preset: e.target.value }))} className="flex-1 bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50">
                    <option value="daily">毎日</option>
                    <option value="weekly">毎週</option>
                    <option value="monthly">毎月</option>
                  </select>
                  <input type="time" value={form.schedule_time} onChange={e => setForm(f => ({ ...f, schedule_time: e.target.value }))} className="bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50" />
                </div>
              ) : (
                <input value={form.custom_cron} onChange={e => setForm(f => ({ ...f, custom_cron: e.target.value }))} placeholder="0 9 * * 1" className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50 font-mono" />
              )}
              <p className="text-xs text-falcon-muted">スケジュール: <span className="text-white">{scheduleLabel}</span></p>
            </div>
          </div>
          {/* Recipients */}
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {form.recipients.map(r => (
                <span key={r} className="flex items-center gap-1 px-2 py-1 bg-falcon-border rounded-sm text-xs text-white">
                  {r}
                  <button onClick={() => removeRecipient(r)} className="text-falcon-muted hover:text-falcon-red ml-1"><X className="w-3 h-3" /></button>
                </span>
              ))}
            </div>
          </div>
          {/* Parameters */}
          <div>
            <label className="block text-falcon-muted text-sm mb-1.5">パラメータ</label>
            <div className="flex gap-2 mb-2">
              <input value={form.param_key} onChange={e => setForm(f => ({ ...f, param_key: e.target.value }))} placeholder="キー" className="flex-1 bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50" />
              <input value={form.param_value} onChange={e => setForm(f => ({ ...f, param_value: e.target.value }))} placeholder="値" className="flex-1 bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50" />
              <button onClick={addParam} className="px-3 py-2 bg-falcon-border text-white rounded-lg text-sm hover:bg-[#243347] transition-colors">追加</button>
            </div>
            {Object.entries(form.parameters).length > 0 && (
              <div className="space-y-1">
                {Object.entries(form.parameters).map(([k, v]) => (
                  <div key={k} className="flex items-center gap-2 px-2 py-1 bg-[#070d19] border border-falcon-border rounded-sm text-xs">
                    <span className="text-falcon-muted font-mono">{k}</span>
                    <span className="text-falcon-subtle">=</span>
                    <span className="text-white font-mono flex-1">{v}</span>
                    <button onClick={() => removeParam(k)} className="text-falcon-muted hover:text-falcon-red"><X className="w-3 h-3" /></button>
                  </div>
                ))}
              </div>
            )}
          ...
          {/* Active */}
          <div className="flex items-center gap-3">
            <label className="text-falcon-muted text-sm">有効</label>
            <button onClick={() => setForm(f => ({ ...f, is_active: !f.is_active }))} className={`text-2xl transition-colors ${form.is_active ? 'text-green-400' : 'text-falcon-subtle'}`}>
        ...
        <div className="flex justify-end gap-3 p-5 border-t border-falcon-border">
          ...
          <button onClick={() => onSave({ ...form, schedule: scheduleValue, schedule_label: scheduleLabel })} className="px-4 py-2 bg-falcon-red text-white text-sm font-medium rounded-lg hover:bg-[#c0001f] transition-colors">保存</button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-md shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-falcon-muted text-sm mb-1.5">開始日</label>
              <input type="date" value={form.period_start} onChange={e => setForm(f => ({ ...f, period_start: e.target.value }))} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50" />
            </div>
            <div>
              <label className="block text-falcon-muted text-sm mb-1.5">終了日</label>
              <input type="date" value={form.period_end} onChange={e => setForm(f => ({ ...f, period_end: e.target.value }))} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50" />
            </div>
          </div>
        </div>
        <div className="flex justify-end gap-3 p-5 border-t border-falcon-border">
          <button onClick={onClose} className="px-4 py-2 text-falcon-muted text-sm hover:text-white rounded-lg hover:bg-falcon-border transition-colors">キャンセル</button>
          <button onClick={() => onGenerate(form)} className="px-4 py-2 bg-falcon-red text-white text-sm font-medium rounded-lg hover:bg-[#c0001f] transition-colors flex items-center gap-2"><Play className="w-4 h-4" />生成開始</button>
        </div>
      </div>
    </div>
  )
      ...
      <div className="space-y-3">
        {types.map((t, i) => (
          <div key={t} className="flex items-center gap-3">
            <span className="text-falcon-muted text-xs w-28 shrink-0">{TYPE_LABEL[t]}</span>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-falcon-surface border border-falcon-border flex items-center justify-center">
            <Printer className="w-5 h-5 text-falcon-red" />
          </div>
          <div>
            <h1 className="text-white font-bold text-xl">レポーティングエンジン</h1>
            <p className="text-falcon-muted text-sm">セキュリティメトリクスレポートの自動生成・管理</p>
          </div>
        </div>
        <button onClick={activeTab === 'schedules' ? () => { setEditingSchedule(undefined); setShowScheduleModal(true) } : () => setShowGenerateModal(true)} className="flex items-center gap-2 px-4 py-2 bg-falcon-red text-white text-sm font-medium rounded-lg hover:bg-[#c0001f] transition-colors">
              ...
              {displaySchedules.map(s => (
                <tr key={s.id} className="hover:bg-[#0d1826] transition-colors">
                  <td className="px-4 py-3">
                    ...
                    {s.description && <p className="text-falcon-muted text-xs mt-0.5">{s.description}</p>}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${TYPE_BADGE[s.report_type]}`}>{TYPE_LABEL[s.report_type]}</span>
                  </td>
                  <td className="px-4 py-3 text-falcon-muted text-sm">{s.schedule_label}</td>
                  <td className="px-4 py-3 text-falcon-muted text-sm">{s.recipients.length}人</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${FORMAT_BADGE[s.output_format]}`}>{s.output_format}</span>
                  </td>
                  <td className="px-4 py-3 text-falcon-muted text-sm">{formatDate(s.next_run)}</td>
                  <td className="px-4 py-3">
                    <button onClick={() => handleToggleSchedule(s.id)} className={`text-2xl transition-colors ${s.is_active ? 'text-green-400' : 'text-falcon-subtle'}`}>
                      {s.is_active ? <ToggleRight className="w-8 h-5" /> : <ToggleLeft className="w-8 h-5" />}
                    </button>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => handleRunNow(s)} title="今すぐ生成" className="p-1.5 rounded-sm hover:bg-falcon-border text-falcon-muted hover:text-green-400 transition-colors"><Play className="w-3.5 h-3.5" /></button>
                      <button onClick={() => { setEditingSchedule(s); setShowScheduleModal(true) }} title="編集" className="p-1.5 rounded-sm hover:bg-falcon-border text-falcon-muted hover:text-white transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => handleDeleteSchedule(s.id)} title="削除" className="p-1.5 rounded-sm hover:bg-falcon-border text-falcon-muted hover:text-falcon-red transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {activeTab === 'reports' && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 text-falcon-muted"><Filter className="w-4 h-4" /></div>
            <select value={filterType} onChange={e => setFilterType(e.target.value as ReportType | 'all')} className="bg-falcon-surface border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50">
              <option value="all">全タイプ</option>
              {(Object.keys(TYPE_LABEL) as ReportType[]).map(t => <option key={t} value={t}>{TYPE_LABEL[t]}</option>)}
            </select>
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as ReportStatus | 'all')} className="bg-falcon-surface border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50">
              <option value="all">全ステータス</option>
              {(Object.keys(STATUS_LABEL) as ReportStatus[]).map(s => <option key={s} value={s}>{STATUS_LABEL[s]}</option>)}
            </select>
            <input type="date" value={filterFrom} onChange={e => setFilterFrom(e.target.value)} className="bg-falcon-surface border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50" />
            <span className="text-falcon-muted text-sm">〜</span>
            <input type="date" value={filterTo} onChange={e => setFilterTo(e.target.value)} className="bg-falcon-surface border border-falcon-border rounded-lg px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50" />
            {(filterType !== 'all' || filterStatus !== 'all' || filterFrom || filterTo) && (
              <button onClick={() => { setFilterType('all'); setFilterStatus('all'); setFilterFrom(''); setFilterTo('') }} className="flex items-center gap-1 px-2 py-1 text-xs text-falcon-muted hover:text-white rounded-sm hover:bg-falcon-border transition-colors"><X className="w-3 h-3" />クリア</button>
            )}
          </div>
              ...
              <tbody className="divide-y divide-falcon-border">
                {filteredReports.map(r => (
                  <tr key={r.id} className="hover:bg-[#0d1826] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${TYPE_BADGE[r.report_type]}`}>{TYPE_LABEL[r.report_type]}</span>
                    </td>
                    <td className="px-4 py-3 text-falcon-muted text-xs">{r.period_start} 〜 {r.period_end}</td>
                    <td className="px-4 py-3">
                      <span className={`flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs font-medium w-fit ${STATUS_BADGE[r.status]}`}>
                        {r.status === 'generating' && <RefreshCw className="w-3 h-3 animate-spin" />}
                        {STATUS_LABEL[r.status]}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${FORMAT_BADGE[r.output_format]}`}>{r.output_format}</span>
                    </td>
                    <td className="px-4 py-3 text-falcon-muted text-sm">{formatBytes(r.file_size_kb)}</td>
                    <td className="px-4 py-3 text-falcon-muted text-sm">{formatDate(r.generated_at)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        {r.status === 'completed' && (
                          <button title="ダウンロード" onClick={() => showToast('ダウンロードを開始しました')} className="p-1.5 rounded-sm hover:bg-falcon-border text-falcon-muted hover:text-blue-400 transition-colors"><Download className="w-3.5 h-3.5" /></button>
                        )}
                        <button onClick={() => handleDeleteReport(r.id)} title="削除" className="p-1.5 rounded-sm hover:bg-falcon-border text-falcon-muted hover:text-falcon-red transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
                {filteredReports.length === 0 && (
                  <tr><td colSpan={8} className="px-4 py-12 text-center text-falcon-muted text-sm">条件に一致するレポートがありません</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            ...
            <div className="bg-falcon-surface border border-falcon-border rounded-xl p-5">
                ...
                {(Object.keys(STATUS_LABEL) as ReportStatus[]).map(s => {
                  ...
                  return (
                    <div key={s} className="flex items-center gap-3">
                      <span className={`px-2 py-0.5 rounded-sm text-xs font-medium w-20 text-center ${STATUS_BADGE[s]}`}>{STATUS_LABEL[s]}</span>
                      <div className="flex-1 bg-[#070d19] rounded-full h-2">
                        <div className="bg-falcon-red h-2 rounded-full transition-all duration-500" style={{ width: `${pct}%` }} />
                      </div>
              ...
              <div className="mt-4 pt-4 border-t border-falcon-border flex items-center justify-between">
                <span className="text-falcon-muted text-sm">合計レポート数</span>

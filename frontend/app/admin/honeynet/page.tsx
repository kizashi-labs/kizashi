              ...
              {seg.nodes.map(node => {
                ...
                return (
                  <div key={node.id} className="flex items-center gap-2">
                    <div className={`relative flex-shrink-0 w-6 h-6 rounded flex items-center justify-center
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-[#1e2d42]">
          <div className="flex items-center gap-2">
            ...
            <span className={`px-2 py-0.5 rounded text-xs font-medium ${NODE_TYPE_STYLES[node.node_type].bg} ${NODE_TYPE_STYLES[node.node_type].text}`}>
            ...
            <div>
              ...
              <div className="space-y-1">
                {topIPs.map(ip => {
                  ...
                  return (
                    <div key={ip} className="flex items-center justify-between bg-[#070d19] rounded px-3 py-2">
                      <div className="flex items-center gap-2">
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-[#1e2d42]">
          <div className="flex items-center gap-2">
            ...
            <span className={`px-2 py-0.5 rounded text-xs font-medium ${THREAT_STYLES[interaction.threat_level].bg} ${THREAT_STYLES[interaction.threat_level].text}`}>
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-[#070d19] rounded p-3 col-span-2">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {interaction.techniques.map(t => (
                  <span key={t} className="px-2 py-0.5 bg-[#1e2d42] rounded text-xs text-[#7d92b0] font-mono">{t}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {interaction.commands.length > 0 && (
            <div>
              ...
              <div className="bg-[#070d19] border border-[#1e2d42] rounded p-3 max-h-40 overflow-y-auto">
            ...
            <div>
              ...
              <div className="space-y-1">
                {interaction.files_accessed.map((f, i) => (
                  <p key={i} className="text-[#7d92b0] font-mono text-xs bg-[#070d19] rounded px-3 py-1">{f}</p>
                ))}
              </div>
            </div>
          )}
          ...
          {interaction.payload && (
            <div>
              ...
              <div className="bg-[#070d19] border border-[#e8002d]/30 rounded p-3 max-h-40 overflow-y-auto">
          ...
          <div>
            ...
            <div className="space-y-2">
              {ipInteractions.map(ix => (
                <div key={ix.id} className="flex items-center gap-3 bg-[#070d19] rounded px-3 py-2">
                  <Clock className="w-3 h-3 text-[#3d5068] flex-shrink-0" />
                  <span className="text-[#7d92b0] text-xs w-24 flex-shrink-0">{fmt(ix.timestamp)}</span>
                  <span className="text-white text-xs flex-1">{ix.node_name}</span>
                  <span className={`px-1.5 py-0.5 rounded text-[10px] ${THREAT_STYLES[ix.threat_level].bg} ${THREAT_STYLES[ix.threat_level].text}`}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select className="w-full bg-[#070d19] border border-[#1e2d42] rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/50"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className="w-full bg-[#070d19] border border-[#1e2d42] rounded px-3 py-2 text-white text-sm font-mono focus:outline-none focus:border-[#e8002d]/50"
          ...
          <div>
            ...
            {services.map((svc, i) => (
              <div key={i} className="flex items-center gap-2 mb-2">
                <input type="number" className="w-20 bg-[#070d19] border border-[#1e2d42] rounded px-2 py-1.5 text-white text-xs focus:outline-none"
                  value={svc.port} onChange={e => updateService(i, 'port', Number(e.target.value))} placeholder="Port" />
                <input className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded px-2 py-1.5 text-white text-xs focus:outline-none"
        ...
        <div className="flex justify-end gap-2 p-4 border-t border-[#1e2d42]">
          <button onClick={onClose} className="px-4 py-2 border border-[#1e2d42] rounded text-[#7d92b0] hover:text-white text-sm transition-colors">キャンセル</button>
          <button onClick={() => onSave({ ...form, services })} className="px-4 py-2 bg-[#e8002d] rounded text-white text-sm hover:bg-[#e8002d]/80 transition-colors">保存</button>
        </div>
      </div>
    </div>
  )
                  ...
                  {nodes.map(node => {
                    ...
                    return (
                      <tr key={node.id} className="border-t border-[#1e2d42] hover:bg-[#070d19]/50">
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded text-xs font-medium ${style.bg} ${style.text}`}>{style.label}</span>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span className="px-2 py-0.5 bg-[#1e2d42] rounded text-xs text-[#7d92b0]">{OS_LABELS[node.os_profile]}</span>
                        </td>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3 bg-[#0d1220] border border-[#1e2d42] rounded-lg p-3">
              ...
              <input value={filterCountry} onChange={e => setFilterCountry(e.target.value)}
                placeholder="国名フィルター" className="bg-[#070d19] border border-[#1e2d42] rounded px-2 py-1.5 text-[#7d92b0] text-xs focus:outline-none w-32" />
                    ...
                    {filteredInteractions.map(ix => {
                      ...
                      return (
                        <tr key={ix.id} className="border-t border-[#1e2d42] hover:bg-[#070d19]/50">
                          ...
                          <td className="px-3 py-3">
                            <span className="px-1.5 py-0.5 bg-blue-900/40 text-blue-300 rounded text-xs">{ix.protocol}</span>
                          </td>
                          <td className="px-3 py-3 text-[#7d92b0] text-xs">{ix.session_duration_s}</td>
                          <td className="px-3 py-3">
                            <span className={`px-2 py-0.5 rounded text-xs font-medium ${threat.bg} ${threat.text}`}>{threat.label}</span>
                          </td>
                          <td className="px-3 py-3">

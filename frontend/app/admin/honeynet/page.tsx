  ...
  return (
    <div className="bg-falcon-surface border border-falcon-border rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-falcon-muted text-xs">{nodes.filter(n => n.is_active).length} アクティブノード</span>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {segments.map(seg => (
          <div key={seg.name} className="border border-falcon-border rounded-lg p-3 bg-[#070d19]">
            <p className="text-falcon-muted text-[10px] uppercase tracking-wider mb-2 font-medium">{seg.name}</p>
            <div className="space-y-1.5">
              {seg.nodes.map(node => {
                const hasRecentActivity = isRecent(node.last_interaction)
                return (
                  <div key={node.id} className="flex items-center gap-2">
                    <div className={`relative shrink-0 w-6 h-6 rounded flex items-center justify-center
                      ...
                      {hasRecentActivity && (
                        <span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-falcon-red rounded-full animate-pulse" />
                      )}
                    </div>
                    <span className="text-[10px] text-falcon-muted truncate">{node.name}</span>
                  </div>
                )
              })}
            </div>
      ...
      <div className="flex items-center gap-4 mt-3 pt-3 border-t border-falcon-border">
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 bg-green-400 rounded-full" />
          <span className="text-falcon-muted text-[10px]">アクティブ</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 bg-gray-500 rounded-full" />
          <span className="text-falcon-muted text-[10px]">非アクティブ</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 bg-falcon-red rounded-full animate-pulse" />
          <span className="text-falcon-muted text-[10px]">最近のインタラクション</span>
        </div>
      </div>
    </div>
  )
...
function NodeDetail({ node, interactions, onClose }: { node: HoneyNode; interactions: HoneyInteraction[]; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-falcon-surface border border-falcon-border rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#070d19] rounded-sm p-3">
              <p className="text-falcon-muted text-xs mb-1">IPアドレス</p>
              <p className="text-white font-mono text-sm">{node.ip_address}</p>
            </div>
            <div className="bg-[#070d19] rounded-sm p-3">
              <p className="text-falcon-muted text-xs mb-1">ホスト名</p>
              <p className="text-white font-mono text-sm">{node.hostname}</p>
            </div>
            <div className="bg-[#070d19] rounded-sm p-3">
              <p className="text-falcon-muted text-xs mb-1">OSプロファイル</p>
              <p className="text-white text-sm">{OS_LABELS[node.os_profile]}</p>
            </div>
            <div className="bg-[#070d19] rounded-sm p-3">
              <p className="text-falcon-muted text-xs mb-1">ネットワークセグメント</p>
          ...
          {node.services.length > 0 && (
            <div>
              <p className="text-falcon-muted text-xs uppercase tracking-wider mb-2">サービス</p>
              <div className="border border-falcon-border rounded-sm overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-[#070d19]">
                    <tr>
                      <th className="text-left px-3 py-2 text-falcon-muted text-xs">ポート</th>
                      <th className="text-left px-3 py-2 text-falcon-muted text-xs">プロトコル</th>
                      <th className="text-left px-3 py-2 text-falcon-muted text-xs">バナー</th>
                    </tr>
                  </thead>
                  <tbody>
                    {node.services.map((svc, idx) => (
                      <tr key={idx} className="border-t border-falcon-border">
                        <td className="px-3 py-2 text-white font-mono">{svc.port}</td>
                        <td className="px-3 py-2 text-falcon-muted">{svc.protocol}</td>
                        <td className="px-3 py-2 text-falcon-muted">{svc.banner || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {/* 7-day chart */}
          <div>
            <p className="text-falcon-muted text-xs uppercase tracking-wider mb-2">インタラクション履歴 (7日間)</p>
            <div className="flex items-end gap-1 h-20">
              {days.map(d => (
                <div key={d.label} className="flex-1 flex flex-col items-center gap-1">
                  <div
                    className="w-full bg-falcon-red/60 rounded-t transition-all"
          ...
          {topIPs.length > 0 && (
            <div>
              <p className="text-falcon-muted text-xs uppercase tracking-wider mb-2">上位攻撃者IP</p>
                ...
                {topIPs.map(ip => {
                  ...
                  return (
                    <div key={ip} className="flex items-center justify-between bg-[#070d19] rounded-sm px-3 py-2">
                      <div className="flex items-center gap-2">
                        <span className="text-falcon-muted text-xs">{interaction?.geo_flag}</span>
                        <span className="text-white font-mono text-sm">{ip}</span>
                        <span className="text-falcon-muted text-xs">{interaction?.geo_country}</span>
                      </div>
                      <span className="text-falcon-red text-sm font-semibold">{count}回</span>
                    </div>
                  )
                })}
              </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-falcon-surface border border-falcon-border rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#070d19] rounded-sm p-3">
              <p className="text-falcon-muted text-xs mb-1">攻撃者IP</p>
              <p className="text-white font-mono">{interaction.attacker_ip}</p>
            </div>
            <div className="bg-[#070d19] rounded-sm p-3">
              <p className="text-falcon-muted text-xs mb-1">地域</p>
              <p className="text-white">{interaction.geo_flag} {interaction.geo_country}</p>
            </div>
            <div className="bg-[#070d19] rounded-sm p-3">
              <p className="text-falcon-muted text-xs mb-1">ノード</p>
              <p className="text-white">{interaction.node_name}</p>
            </div>
            <div className="bg-[#070d19] rounded-sm p-3">
              <p className="text-falcon-muted text-xs mb-1">プロトコル</p>
              <p className="text-white">{interaction.protocol}</p>
            </div>
            <div className="bg-[#070d19] rounded-sm p-3">
              <p className="text-falcon-muted text-xs mb-1">セッション時間</p>
              <p className="text-white">{interaction.session_duration_s}秒</p>
            </div>
            <div className="bg-[#070d19] rounded-sm p-3">
              <p className="text-falcon-muted text-xs mb-1">自動化</p>
            ...
            <div className="bg-[#070d19] rounded-sm p-3 col-span-2">
              <p className="text-falcon-muted text-xs mb-1">帰属確信度</p>
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-falcon-border rounded-full h-2">
                  <div className="bg-falcon-red rounded-full h-2 transition-all" style={{ width: `${interaction.attribution_confidence}%` }} />
                </div>
          ...
          {interaction.techniques.length > 0 && (
            <div>
              <p className="text-falcon-muted text-xs uppercase tracking-wider mb-2">MITRE ATT&CK テクニック</p>
              <div className="flex flex-wrap gap-2">
                {interaction.techniques.map(t => (
                  <span key={t} className="px-2 py-0.5 bg-falcon-border rounded-sm text-xs text-falcon-muted font-mono">{t}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {interaction.commands.length > 0 && (
            <div>
              <p className="text-falcon-muted text-xs uppercase tracking-wider mb-2">コマンド ({interaction.commands.length}件)</p>
              <div className="bg-[#070d19] border border-falcon-border rounded-sm p-3 max-h-40 overflow-y-auto">
          ...
          {interaction.files_accessed.length > 0 && (
            <div>
              <p className="text-falcon-muted text-xs uppercase tracking-wider mb-2">アクセスファイル ({interaction.files_accessed.length}件)</p>
              <div className="space-y-1">
                {interaction.files_accessed.map((f, i) => (
                  <p key={i} className="text-falcon-muted font-mono text-xs bg-[#070d19] rounded-sm px-3 py-1">{f}</p>
                ))}
              </div>
            </div>
          )}
          ...
          {interaction.payload && (
            <div>
              <p className="text-falcon-muted text-xs uppercase tracking-wider mb-2">ペイロード</p>
              <div className="bg-[#070d19] border border-falcon-red/30 rounded-sm p-3 max-h-40 overflow-y-auto">
      ...
      <div className="bg-falcon-surface border border-falcon-border rounded-lg w-full max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-[#070d19] rounded-sm p-3">
              <p className="text-falcon-muted text-xs mb-1">セッション数</p>
              <p className="text-white font-bold text-lg">{ipInteractions.length}</p>
            </div>
          </div>
          {techniques.length > 0 && (
            <div>
              <p className="text-falcon-muted text-xs uppercase tracking-wider mb-2">使用テクニック</p>
              <div className="flex flex-wrap gap-2">
                {techniques.map(t => (
                  <span key={t} className="px-2 py-0.5 bg-falcon-border rounded-sm text-xs text-falcon-muted font-mono">{t}</span>
                ))}
          ...
          <div>
            <p className="text-falcon-muted text-xs uppercase tracking-wider mb-2">インタラクションタイムライン</p>
            <div className="space-y-2">
              {ipInteractions.map(ix => (
                <div key={ix.id} className="flex items-center gap-3 bg-[#070d19] rounded-sm px-3 py-2">
                  ...
                  <span className="text-falcon-muted text-xs w-24 shrink-0">{fmt(ix.timestamp)}</span>
...
function AddNodeModal({ onClose, onSave }: { onClose: () => void; onSave: (data: Partial<HoneyNode>) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-falcon-surface border border-falcon-border rounded-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-falcon-border">
        ...
        <div className="p-4 space-y-3">
          <div>
            <label className="text-falcon-muted text-xs block mb-1">ノード名</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-falcon-muted text-xs block mb-1">ノードタイプ</label>
              <select className="w-full bg-[#070d19] border border-falcon-border rounded-sm px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50"
            ...
            <div>
              <label className="text-falcon-muted text-xs block mb-1">OSプロファイル</label>
              <select className="w-full bg-[#070d19] border border-falcon-border rounded-sm px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-falcon-muted text-xs block mb-1">IPアドレス</label>
              <input className="w-full bg-[#070d19] border border-falcon-border rounded-sm px-3 py-2 text-white text-sm font-mono focus:outline-hidden focus:border-falcon-red/50"
                value={form.ip_address} onChange={e => setForm(f => ({ ...f, ip_address: e.target.value }))} placeholder="10.0.0.250" />
            </div>
            <div>
              <label className="text-falcon-muted text-xs block mb-1">ホスト名</label>
              <input className="w-full bg-[#070d19] border border-falcon-border rounded-sm px-3 py-2 text-white text-sm font-mono focus:outline-hidden focus:border-falcon-red/50"
                value={form.hostname} onChange={e => setForm(f => ({ ...f, hostname: e.target.value }))} placeholder="fake-host.corp.local" />
            </div>
          </div>
          <div>
            <label className="text-falcon-muted text-xs block mb-1">ネットワークセグメント</label>
            <input className="w-full bg-[#070d19] border border-falcon-border rounded-sm px-3 py-2 text-white text-sm focus:outline-hidden focus:border-falcon-red/50"
              value={form.network_segment} onChange={e => setForm(f => ({ ...f, network_segment: e.target.value }))} placeholder="CORP-CORE" />
          </div>
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-falcon-muted text-xs">サービス</label>
              <button onClick={addService} className="text-falcon-red text-xs hover:text-falcon-red/80 flex items-center gap-1">
                <Plus className="w-3 h-3" /> 追加
              </button>
            </div>
            {services.map((svc, i) => (
              <div key={i} className="flex items-center gap-2 mb-2">
                <input type="number" className="w-20 bg-[#070d19] border border-falcon-border rounded-sm px-2 py-1.5 text-white text-xs focus:outline-hidden"
                  value={svc.port} onChange={e => updateService(i, 'port', Number(e.target.value))} placeholder="Port" />
                <input className="flex-1 bg-[#070d19] border border-falcon-border rounded-sm px-2 py-1.5 text-white text-xs focus:outline-hidden"
                ...
                <button onClick={() => removeService(i)} className="text-falcon-muted hover:text-falcon-red"><X className="w-3 h-3" /></button>
              </div>
            ))}
          </div>
        ...
        <div className="flex justify-end gap-2 p-4 border-t border-falcon-border">
          <button onClick={onClose} className="px-4 py-2 border border-falcon-border rounded-sm text-falcon-muted hover:text-white text-sm transition-colors">キャンセル</button>
          <button onClick={() => onSave({ ...form, services })} className="px-4 py-2 bg-falcon-red rounded-sm text-white text-sm hover:bg-falcon-red/80 transition-colors">保存</button>
        </div>
      </div>
    </div>
  )
...
export default function HoneynetPage() {
  ...
  const { data: nodesData } = useQuery<HoneyNode[]>({
    ...
    queryFn: () => apiFetchList<HoneyNode>('/api/v1/admin/honeynet/nodes').catch(() => []),
  })

  const { data: interactionsData } = useQuery<HoneyInteraction[]>({
    queryKey: ['honeynet-interactions'],
    queryFn: () => apiFetchList<HoneyInteraction>('/api/v1/admin/honeynet/interactions').catch(() => []),
  })
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-falcon-muted text-sm">デセプションネットワーク – 欺瞞によるサイバー脅威の検知</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'ユニーク攻撃者', value: uniqueAttackers, icon: Globe, color: 'text-falcon-red' },
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {nodes.map(node => {
                    ...
                    return (
                      <tr key={node.id} className="border-t border-falcon-border hover:bg-[#070d19]/50">
                        <td className="px-4 py-3">
                          <button onClick={() => setSelectedNode(node)} className="text-white font-medium hover:text-falcon-red transition-colors text-left">
                            {node.name}
                          </button>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 font-mono text-falcon-muted text-xs">{node.ip_address}</td>
                        <td className="px-4 py-3 font-mono text-falcon-muted text-xs truncate max-w-[160px]">{node.hostname}</td>
                        <td className="px-4 py-3">
                          <span className="px-2 py-0.5 bg-falcon-border rounded-sm text-xs text-falcon-muted">{OS_LABELS[node.os_profile]}</span>
                        </td>
                        <td className="px-4 py-3 text-falcon-muted text-xs">{node.network_segment}</td>
                        <td className="px-4 py-3">
                          <button onClick={() => toggleMutation.mutate(node.id)}
                            className={`w-10 h-5 rounded-full transition-colors ${node.is_active ? 'bg-green-500' : 'bg-falcon-border'}`}>
                            <div className={`w-4 h-4 bg-falcon-text rounded-full transition-transform mx-0.5 ${node.is_active ? 'translate-x-5' : 'translate-x-0'}`} />
                          </button>
                        </td>
                        <td className="px-4 py-3 text-white font-semibold">{node.interaction_count}</td>
                        <td className={`px-4 py-3 text-xs ${recent ? 'text-green-400' : 'text-falcon-muted'}`}>{fmtAgo(node.last_interaction)}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <button onClick={() => setSelectedNode(node)} className="text-falcon-muted hover:text-white transition-colors"><Eye className="w-4 h-4" /></button>
                            <button onClick={() => deleteMutation.mutate(node.id)} className="text-falcon-muted hover:text-falcon-red transition-colors"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                    )
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3 bg-falcon-surface border border-falcon-border rounded-lg p-3">
              ...
              <input value={filterCountry} onChange={e => setFilterCountry(e.target.value)}
                placeholder="国名フィルター" className="bg-[#070d19] border border-falcon-border rounded-sm px-2 py-1.5 text-falcon-muted text-xs focus:outline-hidden w-32" />
              <span className="text-falcon-muted text-xs ml-auto">{filteredInteractions.length}件</span>
            </div>
                  ...
                  <thead className="bg-[#070d19] border-b border-falcon-border">
                    <tr>
                      {['時刻', 'ノード', '攻撃者IP', 'プロトコル', '時間(秒)', '脅威', '自動', '地域', 'コマンド', 'ファイル', '詳細'].map(h => (
                        <th key={h} className="text-left px-3 py-3 text-falcon-muted text-xs font-medium">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {filteredInteractions.map(ix => {
                      ...
                      return (
                        <tr key={ix.id} className="border-t border-falcon-border hover:bg-[#070d19]/50">
                          <td className="px-3 py-3 text-falcon-muted text-xs whitespace-nowrap">{fmt(ix.timestamp)}</td>
                          <td className="px-3 py-3 text-white text-xs">{ix.node_name}</td>
                          <td className="px-3 py-3">
                            <button onClick={() => setAttackerIP(ix.attacker_ip)}
                              className="text-falcon-red font-mono text-xs hover:underline">{ix.attacker_ip}</button>
                          </td>
                          <td className="px-3 py-3">
                            <span className="px-1.5 py-0.5 bg-blue-900/40 text-blue-300 rounded-sm text-xs">{ix.protocol}</span>
                          </td>
                          <td className="px-3 py-3 text-falcon-muted text-xs">{ix.session_duration_s}</td>
                          <td className="px-3 py-3">
                            <span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${threat.bg} ${threat.text}`}>{threat.label}</span>
                          </td>
                          <td className="px-3 py-3">
                            <span className={`text-xs ${ix.is_automated ? 'text-yellow-400' : 'text-falcon-muted'}`}>
                              {ix.is_automated ? '自動' : '手動'}
                            </span>
                          </td>
                          <td className="px-3 py-3 text-xs text-falcon-muted">{ix.geo_flag} {ix.geo_country}</td>
                          <td className="px-3 py-3 text-falcon-muted text-xs">{ix.commands.length}</td>
                          <td className="px-3 py-3 text-falcon-muted text-xs">{ix.files_accessed.length}</td>
                          <td className="px-3 py-3">

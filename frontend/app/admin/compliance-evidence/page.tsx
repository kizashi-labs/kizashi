...
export default function ComplianceEvidencePage() {
  ...
  const { data: tasksData = [] } = useQuery<ComplianceTask[]>({
    ...
    queryFn: () => apiFetchList<ComplianceTask>('/api/v1/admin/compliance-evidence/tasks'),
    staleTime: 30_000,
  })
  ...
  const { data: evidenceData = [] } = useQuery<Evidence[]>({
    ...
    queryFn: () => apiFetchList<Evidence>('/api/v1/admin/compliance-evidence/evidence'),
    staleTime: 30_000,
  })
    ...
    <div className="min-h-screen bg-[#070d19] text-[#7d92b0] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 bg-[#0d1220] border border-[#1e2d42] rounded-lg p-1">
            <button onClick={() => setSelectedFramework('all')} className={`px-3 py-1.5 rounded-sm text-xs font-medium transition-colors ${selectedFramework === 'all' ? 'bg-[#1e2d42] text-white' : 'text-[#7d92b0] hover:text-white'}`}>全て</button>
            {FRAMEWORKS.map(f => (
              <button key={f} onClick={() => setSelectedFramework(f)} className={`px-3 py-1.5 rounded-sm text-xs font-medium transition-colors ${selectedFramework === f ? 'bg-[#1e2d42] text-white' : 'text-[#7d92b0] hover:text-white'}`}>{f}</button>
            ))}
          </div>
        </div>
      </div>
              ...
              <tbody>
                {(selectedFramework === 'all' ? tasks : tasks.filter(t => t.framework === selectedFramework)).map(task => (
                  <tr key={task.id} className="border-b border-[#1e2d42]/50 hover:bg-[#1e2d42]/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => handleCollect(task)} className="flex items-center gap-1.5 px-2 py-1 rounded-sm bg-[#e8002d]/20 text-[#e8002d] border border-[#e8002d]/30 hover:bg-[#e8002d]/30 text-xs transition-colors">
      ...
      {tab === 'evidence' && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as EvidenceStatus | 'all')} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-[#7d92b0] outline-hidden">
                ...
                {filteredEvidence.map(ev => {
                  ...
                  return (
                    <tr key={ev.id} className="border-b border-[#1e2d42]/50 hover:bg-[#1e2d42]/20 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => { setReviewEvidence(ev); setReviewNotes(ev.reviewer_notes) }} className="flex items-center gap-1.5 px-2 py-1 rounded-sm bg-[#1e2d42] hover:bg-[#2a3f5a] text-xs transition-colors">
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={newTask.name} onChange={e => setNewTask(p => ({ ...p, name: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-hidden" placeholder="アクセスログ収集" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-[#7d92b0] mb-1">フレームワーク</label>
                  <select value={newTask.framework} onChange={e => setNewTask(p => ({ ...p, framework: e.target.value as Framework }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-hidden">
                    {FRAMEWORKS.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs text-[#7d92b0] mb-1">コントロールID</label>
                  <input value={newTask.control_id} onChange={e => setNewTask(p => ({ ...p, control_id: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-hidden" placeholder="CC6.1" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-[#7d92b0] mb-1">説明</label>
                <input value={newTask.description} onChange={e => setNewTask(p => ({ ...p, description: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-hidden" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-[#7d92b0] mb-1">収集方法</label>
                  <select value={newTask.collection_method} onChange={e => setNewTask(p => ({ ...p, collection_method: e.target.value as CollectionMethod }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-hidden">
                ...
                <div>
                  ...
                  <select value={newTask.schedule} onChange={e => setNewTask(p => ({ ...p, schedule: e.target.value }))} className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-hidden">
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={reviewNotes} onChange={e => setReviewNotes(e.target.value)} className="w-full h-20 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white outline-hidden resize-none" placeholder="レビューコメントを入力..." />
              </div>
            </div>
            <div className="flex justify-end gap-3 mt-6">

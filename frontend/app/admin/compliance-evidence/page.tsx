    ...
    <div className="min-h-screen bg-[#070d19] text-falcon-muted p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-falcon-muted mt-0.5">自動証拠収集・レビュー管理</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 bg-falcon-surface border border-falcon-border rounded-lg p-1">
            <button onClick={() => setSelectedFramework('all')} className={`px-3 py-1.5 rounded-sm text-xs font-medium transition-colors ${selectedFramework === 'all' ? 'bg-falcon-border text-white' : 'text-falcon-muted hover:text-white'}`}>全て</button>
            {FRAMEWORKS.map(f => (
              <button key={f} onClick={() => setSelectedFramework(f)} className={`px-3 py-1.5 rounded-sm text-xs font-medium transition-colors ${selectedFramework === f ? 'bg-falcon-border text-white' : 'text-falcon-muted hover:text-white'}`}>{f}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-4 mb-6">
        {[
          ...
          { label: '30日以内失効', value: expiring30d, icon: AlertCircle, color: 'text-falcon-red' },
        ].map(stat => (
      ...
      {tab === 'tasks' && (
        <div>
          <div className="flex justify-end mb-4">
            <button onClick={() => setShowAddTask(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-falcon-red text-white text-sm hover:bg-[#c8001e] transition-colors">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {(selectedFramework === 'all' ? tasks : tasks.filter(t => t.framework === selectedFramework)).map(task => (
                  <tr key={task.id} className="border-b border-falcon-border/50 hover:bg-falcon-border/20 transition-colors">
                    <td className="px-4 py-3">
                      <button onClick={() => setSelectedTask(selectedTask?.id === task.id ? null : task)} className="text-white font-medium hover:text-falcon-red transition-colors text-left">
                        {task.name}
                      </button>
                    </td>
                    ...
                    <td className="px-4 py-3 text-falcon-muted text-xs">{humanizeCron(task.schedule)}</td>
                    <td className="px-4 py-3 text-falcon-muted text-xs">{task.last_collected ? formatDate(task.last_collected) : '—'}</td>
                    <td className="px-4 py-3">
                      <button>
                        {task.is_active
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => handleCollect(task)} className="flex items-center gap-1.5 px-2 py-1 rounded-sm bg-falcon-red/20 text-falcon-red border border-falcon-red/30 hover:bg-falcon-red/30 text-xs transition-colors">
          ...
          {selectedTask && (
            <div className="mt-4 bg-falcon-surface border border-falcon-border rounded-xl p-5">
              ...
              {taskEvidence.length === 0 ? (
                <p className="text-sm text-falcon-subtle">証拠がありません</p>
              ) : (
                <div className="space-y-2">
                  {taskEvidence.map(ev => (
                    <div key={ev.id} className="flex items-center justify-between px-3 py-2 bg-falcon-border/30 border border-falcon-border rounded-lg">
                      <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-falcon-muted">{formatDate(ev.collected_at)}</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {tab === 'evidence' && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as EvidenceStatus | 'all')} className="bg-falcon-surface border border-falcon-border rounded-lg px-3 py-2 text-sm text-falcon-muted outline-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => showToast('ZIP エクスポートを開始しました')} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-falcon-surface border border-falcon-border hover:border-falcon-muted/40 text-sm transition-colors">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filteredEvidence.map(ev => {
                  ...
                  return (
                    <tr key={ev.id} className="border-b border-falcon-border/50 hover:bg-falcon-border/20 transition-colors">
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-falcon-muted text-xs">{ev.task_name}</td>
                      ...
                      <td className="px-4 py-3 text-falcon-muted text-xs">{formatDate(ev.collected_at)}</td>
                      <td className="px-4 py-3 text-falcon-muted text-xs">{ev.collected_by}</td>
                      <td className="px-4 py-3"><Badge className={statusMeta[ev.status].color}>{statusMeta[ev.status].label}</Badge></td>
                      <td className={`px-4 py-3 text-xs font-mono ${isExpiring ? 'text-red-400' : 'text-falcon-muted'}`}>
                        {ev.expires_at ? `${formatDate(ev.expires_at)} (${days}日)` : '—'}
                      </td>
                      <td className="px-4 py-3">
                        <button onClick={() => { setReviewEvidence(ev); setReviewNotes(ev.reviewer_notes) }} className="flex items-center gap-1.5 px-2 py-1 rounded-sm bg-falcon-border hover:bg-[#2a3f5a] text-xs transition-colors">
        ...
        <div className="fixed inset-0 bg-black/60 z-40 flex items-center justify-center p-4">
          <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-lg p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs text-falcon-muted mb-1">タスク名</label>
                <input value={newTask.name} onChange={e => setNewTask(p => ({ ...p, name: e.target.value }))} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-sm text-white outline-hidden" placeholder="アクセスログ収集" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-falcon-muted mb-1">フレームワーク</label>
                  <select value={newTask.framework} onChange={e => setNewTask(p => ({ ...p, framework: e.target.value as Framework }))} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-sm text-white outline-hidden">
                    {FRAMEWORKS.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs text-falcon-muted mb-1">コントロールID</label>
                  <input value={newTask.control_id} onChange={e => setNewTask(p => ({ ...p, control_id: e.target.value }))} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-sm text-white outline-hidden" placeholder="CC6.1" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-falcon-muted mb-1">説明</label>
                <input value={newTask.description} onChange={e => setNewTask(p => ({ ...p, description: e.target.value }))} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-sm text-white outline-hidden" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-falcon-muted mb-1">収集方法</label>
                  <select value={newTask.collection_method} onChange={e => setNewTask(p => ({ ...p, collection_method: e.target.value as CollectionMethod }))} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-sm text-white outline-hidden">
                ...
                <div>
                  ...
                  <select value={newTask.schedule} onChange={e => setNewTask(p => ({ ...p, schedule: e.target.value }))} className="w-full bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-sm text-white outline-hidden">
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowAddTask(false)} className="px-4 py-2 rounded-lg border border-falcon-border text-falcon-muted text-sm hover:text-white transition-colors">キャンセル</button>
              <button onClick={handleAddTask} className="px-4 py-2 rounded-lg bg-falcon-red text-white text-sm hover:bg-[#c8001e] transition-colors">追加</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 z-40 flex items-center justify-center p-4">
          <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs text-falcon-muted mb-2">コンテンツプレビュー</label>
                {reviewEvidence.evidence_type === 'screenshot' ? (
                  <div className="bg-[#070d19] border border-falcon-border rounded-lg p-4 flex items-center justify-center h-24">
                    <p className="text-falcon-subtle text-sm">[スクリーンショット画像]</p>
                  </div>
                ) : (
                  <pre className="bg-[#070d19] border border-falcon-border rounded-lg p-3 text-xs font-mono text-falcon-muted overflow-auto max-h-40 whitespace-pre-wrap">{reviewEvidence.content}</pre>
                )}
              </div>
              <div>
                <label className="block text-xs text-falcon-muted mb-1">レビュアーメモ</label>
                <textarea value={reviewNotes} onChange={e => setReviewNotes(e.target.value)} className="w-full h-20 bg-[#070d19] border border-falcon-border rounded-lg px-3 py-2 text-sm text-white outline-hidden resize-none" placeholder="レビューコメントを入力..." />
              </div>
            </div>
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setReviewEvidence(null)} className="px-4 py-2 rounded-lg border border-falcon-border text-falcon-muted text-sm hover:text-white transition-colors">キャンセル</button>

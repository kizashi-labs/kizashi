...
const simulationStatusBadge = (s: SimulationStatus) => {
  const map: Record<SimulationStatus, string> = {
    ...
    draft:     'bg-[#1e2d42] text-[#7d92b0] border-[#1e2d42]',
  }
  return map[s]
}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[#7d92b0] text-sm">Training courses &amp; phishing simulations</p>
              </div>
            </div>
            <button
              onClick={() => refetch()}
          ...
          {statCards.map(s => (
            <div key={s.label} className={`rounded-xl p-4 border ${s.bg} bg-[#0d1220]`}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-[#7d92b0] leading-tight">{s.label}</span>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {displayCourses.map(c => (
                  <tr key={c.id} className="border-b border-[#1e2d42]/50 hover:bg-[#131d31]/50 transition-colors">
                    <td className="px-4 py-3">
                      <span className="text-white font-medium">{c.title}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded border ${categoryBadge(c.category)}`}>
                        {c.category}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 text-[#7d92b0] text-xs">
                        <Clock className="w-3.5 h-3.5" />
                        {c.duration_min} min
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded border capitalize ${difficultyBadge(c.difficulty)}`}>
                        {c.difficulty}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full font-medium
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${c.enabled ? 'bg-green-400' : 'bg-[#7d92b0]'}`} />
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {displaySimulations.map(sim => {
                  ...
                  return (
                    <tr key={sim.id} className="border-b border-[#1e2d42]/50 hover:bg-[#131d31]/50 transition-colors">
                      <td className="px-4 py-3">
                        <span className="text-white font-medium">{sim.name}</span>
                      </td>
                      <td className="px-4 py-3 max-w-[160px]">
                        <span className="text-[#7d92b0] text-xs block truncate" title={sim.template}>
                          {sim.template}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-bold ${openedPct > 60 ? 'text-orange-400' : 'text-[#7d92b0]'}`}>
                          {sim.sent > 0 ? `${openedPct}%` : '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-xs font-bold ${highClick ? 'text-red-400' : 'text-[#7d92b0]'}`}>
                          {sim.sent > 0 ? `${clickedPct}%` : '—'}
                          {highClick && <AlertTriangle className="inline w-3 h-3 ml-1" />}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-xs font-bold ${reportedPct > 40 ? 'text-green-400' : 'text-[#7d92b0]'}`}>
                      ...
                      <td className="px-4 py-3">
                        {sim.credentials_entered > 0 ? (
                          ...
                          <span className="text-xs text-[#7d92b0]">0</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-0.5 rounded border capitalize ${simulationStatusBadge(sim.status)}`}>
                          {sim.status}
                        </span>
                      </td>
                    </tr>
        ...
        {activeTab === 'leaderboard' && (
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[#7d92b0] text-sm ml-1">— Training Completion Leaderboard</span>
            </div>
            <div className="bg-[#0d1220] rounded-xl border border-[#1e2d42] overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span className="text-lg tracking-wide">
                          {entry.badges.length > 0 ? entry.badges.join(' ') : <span className="text-[#3d5068] text-xs">—</span>}
                        </span>
                      </td>
                    </tr>
                  ))}

  ...
  return (
    ...
    >
      ...
      <div className="absolute z-50 hidden group-hover:block bottom-full mb-2 left-1/2 -translate-x-1/2
                      bg-[#0d1220] border border-[#1e2d42] rounded px-2 py-1.5 text-xs whitespace-nowrap pointer-events-none">
    ...
    <div className="absolute z-50 top-4 right-4 w-72 bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4 shadow-2xl">
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <span className={`inline-block px-2 py-0.5 rounded text-xs border font-medium mt-1 ${severityBg(source.severity)}`}>
      ...
      <div className="space-y-2 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${threatTypeBadge(source.top_threat_type)}`}>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {source.targeted_sectors.map(s => (
              <span key={s} className="px-2 py-0.5 rounded bg-[#1e2d42] text-[#7d92b0] text-xs">{s}</span>
            ))}
          </div>
        </div>
        <div>
    ...
    <div className="min-h-screen bg-[#070d19] p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-[#0d1220] border border-[#1e2d42] rounded text-sm text-[#7d92b0] hover:text-white hover:border-[#7d92b0]/40 transition-colors"
          >
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select
          ...
          className="bg-[#0d1220] border border-[#1e2d42] text-[#7d92b0] text-sm rounded px-3 py-1.5 focus:outline-none focus:border-[#7d92b0]/40"
        >
      ...
      <div className="flex gap-4" style={{ height: 480 }}>
        ...
        <div className="flex-1 relative bg-[#0a1020] border border-[#1e2d42] rounded-lg overflow-hidden">
          ...
          {/* Legend */}
          <div className="absolute bottom-3 left-3 bg-[#0d1220]/90 border border-[#1e2d42] rounded p-2 text-xs space-y-1" style={{ zIndex: 10 }}>
        ...
        <div className="w-72 bg-[#0d1220] border border-[#1e2d42] rounded-lg flex flex-col overflow-hidden">
          <div className="px-3 py-2.5 border-b border-[#1e2d42] flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-2">
            ...
            {liveEvents.map((event, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-1 text-white">
                  ...
                  <ArrowRight className="w-3 h-3 text-[#e8002d] flex-shrink-0" />
                  <span>{event.dst_flag}</span>
                  <span className="text-[#7d92b0] truncate max-w-[60px]">{event.dst_country}</span>
                </div>
                <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] mt-1 ${threatTypeBadge(event.threat_type)}`}>
              ...
              {filteredSources.sort((a, b) => b.threat_count - a.threat_count).map(src => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${threatTypeBadge(src.top_threat_type)}`}>
                      {threatTypeLabel(src.top_threat_type)}
                    </span>
                  </td>
                  <td className="px-4 py-2.5">
                    <span className={`px-2 py-0.5 rounded text-xs border font-medium ${severityBg(src.severity)}`}>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex gap-1 flex-wrap">
                      {src.targeted_sectors.slice(0, 2).map(s => (
                        <span key={s} className="px-1.5 py-0.5 rounded bg-[#1e2d42] text-[#7d92b0] text-xs">{s}</span>
                      ))}

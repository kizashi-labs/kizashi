              ...
              {!selectedEl ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 rounded-sm border border-red-800/40 text-xs text-red-400 hover:bg-red-900/20 transition-colors mt-auto"
                  >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-falcon-surface border border-falcon-border rounded-lg p-5">
            ...
            <div className="flex flex-wrap gap-3 mt-3">
              {STRIDE_CATS.map(c => (
                <span key={c.cat} className={`text-xs px-2 py-0.5 rounded-sm ${CAT_STYLES[c.cat]}`}>
                            ...
                            <td key={c.cat} className="px-3 py-2">
                              <div className="flex flex-col gap-1.5">
                                <select
                                  ...
                                  className={`w-full rounded-sm px-2 py-1 text-xs font-medium focus:outline-hidden border-0 ${RISK_STYLES[cell.risk]}`}
                                >
                  ...
                  {mitigations.map(m => (
                    <tr key={m.id} className="border-b border-falcon-border hover:bg-[#070d19]/60 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-0.5 rounded-sm ${CAT_STYLES[m.category]}`}>
                      ...
                      <td className="px-4 py-3">
                        <select
                          ...
                          className={`rounded-sm px-2 py-1 text-xs font-medium focus:outline-hidden border-0 ${STATUS_STYLES[m.status]}`}
                        >
                          <option value="not_started">未着手</option>
                          <option value="in_progress">進行中</option>
                          <option value="completed">完了</option>

...
function SourceTypeBadge({ type }: { type: SourceType }) {
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-sm text-[10px] font-medium border ${SOURCE_TYPE_COLORS[type]}`}>
...
function VerdictBadge({ verdict }: { verdict: Verdict }) {
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-sm text-xs font-medium border ${VERDICT_COLORS[verdict]}`}>
                  ...
                  <tbody className="divide-y divide-[#1e2d42]/50">
                    {localRules.map(rule => (
                      <tr key={rule.id} className="hover:bg-[#0a1428]/40 transition-colors">
                        ...
                        <td className="px-3 py-3">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs font-medium border ${rule.enabled ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-gray-500/20 text-gray-400 border-gray-500/30'}`}>
                            {rule.enabled ? '有効' : '無効'}
                          </span>
                        </td>
                        <td className="px-3 py-3">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={i} className={`flex items-start gap-2 p-3 rounded-lg border text-xs ${
                      ...
                      {item.type === 'warn' ? <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" /> :
                       item.type === 'success' ? <CheckCircle className="w-3.5 h-3.5 shrink-0 mt-0.5" /> :

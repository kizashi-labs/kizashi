        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div className="bg-[#070d19] rounded-lg p-3 border border-[#1e2d42]">
              ...
              <span className={`px-2 py-0.5 text-xs rounded border ${getTypeStyle(workload.type)}`}>{workload.type}</span>
            </div>
            <div className="bg-[#070d19] rounded-lg p-3 border border-[#1e2d42]">
              <p className="text-[10px] text-[#7d92b0] mb-1">ステータス</p>
              <span className={`px-2 py-0.5 text-xs rounded border ${getStatusStyle(workload.status)}`}>{workload.status}</span>
            </div>
            <div className="bg-[#070d19] rounded-lg p-3 border border-[#1e2d42]">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {Object.entries(workload.labels).map(([k, v]) => (
              <span key={k} className="px-2 py-0.5 text-[10px] font-mono rounded bg-[#1e2d42] text-[#7d92b0] border border-[#2a3f5a]">
                {k}={v}
              </span>
            ))}
          </div>
              ...
              {workload.vulnerabilities.map(v => (
                <div key={v.cve_id} className="bg-[#070d19] rounded-lg p-3 border border-[#1e2d42] space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-0.5 text-[10px] rounded border uppercase font-bold ${getSeverityStyle(v.severity)}`}>
                      {v.severity}
                    </span>
                  </div>
        ...
        {activeTab === 'workloads' && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3 p-4 bg-[#0d1220] border border-[#1e2d42] rounded-xl">
              <select
                className="bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-[#e2e8f4] focus:outline-none focus:border-[#1a6bff]"
                value={filterCluster}
                    ...
                    <tbody className="divide-y divide-[#1e2d42]">
                      {filteredWorkloads.map(w => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span className={`px-2 py-0.5 text-[10px] font-medium rounded border ${getTypeStyle(w.type)}`}>
                              {w.type}
                            </span>
                          </td>
                          ...
                          <td className="px-4 py-3">
                            {w.vulnerability_count > 0 ? (
                              <span className="px-2 py-0.5 text-xs font-bold rounded bg-[#e8002d]/20 text-[#e8002d] border border-[#e8002d]/30">
                                {w.vulnerability_count}
                              </span>
                            ) : (
                              <span className="px-2 py-0.5 text-xs rounded bg-green-500/10 text-green-400 border border-green-500/20">0</span>
                            )}
                          </td>
                          <td className="px-4 py-3">
                            <span className={`px-2 py-0.5 text-[10px] rounded border ${getStatusStyle(w.status)}`}>
                              {w.status}
                            </span>
                          </td>
                        </tr>
        ...
        {activeTab === 'events' && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select
                className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-[#e2e8f4] focus:outline-none focus:border-[#1a6bff]"
                      ...
                      {filteredEvents.map(ev => (
                        <>
                          <tr key={ev.id} className="hover:bg-[#0d1a2e] transition-colors">
                            ...
                            <td className="px-4 py-3">
                              <span className={`px-2 py-0.5 text-[10px] font-medium rounded border ${getEventTypeStyle(ev.event_type)}`}>
                                {ev.event_type}
                              </span>
                            </td>
                            <td className="px-4 py-3">
                              <span className={`px-2 py-0.5 text-[10px] font-bold rounded border uppercase ${getSeverityStyle(ev.severity)}`}>
                                {ev.severity}
                              </span>
                            </td>
                      ...
                      <tbody className="divide-y divide-[#1e2d42]">
                        {workloads.filter(w => w.cluster === selectedCluster).map(w => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              <span className={`px-2 py-0.5 text-[10px] rounded border ${getTypeStyle(w.type)}`}>{w.type}</span>
                            </td>
                            <td className="px-4 py-3 text-xs text-[#7d92b0]">{w.namespace}</td>
                            <td className="px-4 py-3 text-xs font-mono">
                              <span className={w.replicas_ready < w.replicas_total ? 'text-yellow-400' : 'text-green-400'}>
                            ...
                            <td className="px-4 py-3">
                              {w.vulnerability_count > 0 ? (
                                <span className="px-2 py-0.5 text-xs font-bold rounded bg-[#e8002d]/20 text-[#e8002d] border border-[#e8002d]/30">{w.vulnerability_count}</span>
                              ) : (
                                <span className="px-2 py-0.5 text-xs rounded bg-green-500/10 text-green-400 border border-green-500/20">0</span>
                              )}
                            </td>
                            <td className="px-4 py-3">
                              <span className={`px-2 py-0.5 text-[10px] rounded border ${getStatusStyle(w.status)}`}>{w.status}</span>
                            </td>
                          </tr>
                        ))}
                      </tbody>

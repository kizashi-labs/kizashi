              ...
              {group.event_timeline.map((ev, i) => (
                <div key={ev.id} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className="w-2.5 h-2.5 rounded-full bg-[#e8002d] mt-1.5 shrink-0" />
                  ...
                  <div className="flex-1 pb-3">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className="text-[10px] text-[#e8002d] bg-[#e8002d]/10 px-1.5 py-0.5 rounded-sm font-mono">{ev.technique}</span>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {group.involved_hosts.map(h => (
                <span key={h} className="text-xs bg-[#1e2d42] text-[#e2e8f4] px-2 py-1 rounded-sm font-mono">{h}</span>
              ))}
            </div>
          </div>
              ...
              <ul className="space-y-1">
                {group.recommended_response.map((r, i) => (
                  <li key={i} className="flex items-start gap-2 text-xs text-[#e2e8f4]">
                    <ChevronRight className="w-3.5 h-3.5 text-[#e8002d] mt-0.5 shrink-0" />
                    {r}
                  </li>
                ))}
              </ul>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl w-full max-w-lg">
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <input className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden"
              value={name} onChange={e => setName(e.target.value)} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-[#7d92b0] mb-1 block">パターンタイプ</label>
              <select className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden"
            ...
            <div>
              ...
              <input type="number" className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden"
                value={window} onChange={e => setWindow(parseInt(e.target.value) || 30)} />
            </div>
          </div>
          <div>
            <label className="text-xs text-[#7d92b0] mb-1 block">トリガー条件</label>
            <textarea className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-white focus:outline-hidden resize-none"
      ...
      <div className="flex gap-1 bg-[#0d1220] border border-[#1e2d42] rounded-lg p-1 w-fit">
        {(['groups', 'rules'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-1.5 rounded-sm text-sm font-medium transition-colors ${tab === t ? 'bg-[#1d2f4a] text-white' : 'text-[#7d92b0] hover:text-white'}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => setSelectedGroup(g)}
                        className="flex items-center gap-1 text-xs text-[#7d92b0] hover:text-white px-2 py-1 border border-[#1e2d42] rounded-sm transition-colors">
                        <Eye className="w-3 h-3" /> 詳細
                      </button>
                      {g.status !== 'false_positive' && (
                        <button onClick={() => markFP(g.id)}
                          className="text-xs text-[#7d92b0] hover:text-[#e8002d] px-2 py-1 border border-[#1e2d42] rounded-sm transition-colors">
                          誤検知
                        </button>
                      )}
                    </div>

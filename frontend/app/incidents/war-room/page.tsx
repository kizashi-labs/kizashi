  ...
  if (!incident) return (
    <div className="min-h-screen bg-[#070d19] p-4">
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-16 flex flex-col items-center justify-center gap-4">
        ...
        <Link href="/incidents" className="mt-2 px-4 py-2 bg-[#1e2d42] hover:bg-[#2d3f58] border border-[#2d3f58] text-[#e2e8f4] text-sm rounded-lg transition-colors">
          インシデント一覧へ
        </Link>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#070d19] p-4">
      ...
      <div className="flex gap-4 h-[calc(100vh-140px)]">
        ...
        <div className="w-[30%] shrink-0 flex flex-col gap-3 overflow-y-auto">
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button className="flex items-center gap-1 px-2 py-1 bg-[#e8002d]/10 hover:bg-[#e8002d]/20 border border-[#e8002d]/30 rounded-sm text-xs text-[#e8002d]">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden">
          {/* Inner Tabs */}
          <div className="flex border-b border-[#1e2d42] px-1 pt-1 gap-1 shrink-0">
            {([
                  ...
                  <div className="bg-[#070d19] border border-[#e8002d]/30 rounded-xl p-4">
                    <div className="flex gap-3 mb-3">
                      <select value={addEventForm.type} onChange={e => setAddEventForm(p => p ? { ...p, type: e.target.value as EventType } : null)}
                        className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-sm text-white focus:outline-hidden">
                    ...
                    <textarea value={addEventForm.description} onChange={e => setAddEventForm(p => p ? { ...p, description: e.target.value } : null)}
                      ...
                      className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white resize-none focus:outline-hidden mb-3" />
                ...
                <div className="space-y-2">
                  {evidence.map(ev => (
                    <div key={ev.id} className="flex items-center gap-3 bg-[#070d19] border border-[#1e2d42] rounded-xl p-3">
                      <div className="w-9 h-9 bg-[#0d1220] rounded-lg flex items-center justify-center shrink-0">
                        {EV_ICON[ev.type]}
                      </div>
                      ...
                      <span className="px-2 py-0.5 bg-[#1e2d42] rounded-sm text-xs text-[#7d92b0]">{ev.type}</span>
                      <button className="p-1.5 hover:bg-[#1e2d42] rounded-sm text-[#7d92b0] hover:text-white transition-colors">
              ...
              <div className="flex flex-col h-full">
                ...
                <div className="flex gap-2 shrink-0">
                  <input value={chatInput} onChange={e => setChatInput(e.target.value)}
                    ...
                    className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-xl px-4 py-2.5 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50" />
                ...
                {addTaskForm && (
                  <div className="bg-[#070d19] border border-[#e8002d]/30 rounded-xl p-4 space-y-3">
                    <input value={newTask.description} onChange={e => setNewTask(p => ({ ...p, description: e.target.value }))}
                      placeholder="タスク内容..."
                      className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden" />
                    <div className="grid grid-cols-3 gap-2">
                      <input value={newTask.assignee} onChange={e => setNewTask(p => ({ ...p, assignee: e.target.value }))}
                        placeholder="担当者"
                        className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden" />
                          ...
                          <td className="px-4 py-3">
                            <div className="flex gap-1.5 flex-wrap">
                              {['隔離', 'ブロック', 'ファイル検疫', 'プロセス終了'].map(action => (
                                <button key={action}
                                  className="px-2 py-0.5 bg-[#070d19] hover:bg-[#1e2d42] border border-[#1e2d42] rounded-sm text-[10px] text-[#7d92b0] transition-colors">
                                  {action}
                                </button>
                              ))}
                            </div>

...
export default function DarkWebMonitoringPage() {
  ...
  const { data: keywordsData = [] } = useQuery<MonitoredKeyword[]>({
    ...
    queryFn: () => apiFetch<MonitoredKeyword[]>('/api/v1/dark-web/keywords'),
    staleTime: 30_000,
  })
  const { data: integrationsData = [] } = useQuery<ApiIntegration[]>({
    queryKey: ['dark-web-integrations'],
    queryFn: () => apiFetch<ApiIntegration[]>('/api/v1/dark-web/integrations'),
    staleTime: 60_000,
  })
  ...
  const { data: findingsData = [] } = useQuery<DarkWebFinding[]>({
    ...
    queryFn: () => apiFetch<DarkWebFinding[]>('/api/v1/dark-web/findings'),
    staleTime: 30_000,
  })
  ...
  const updateFindingStatus = useMutation({
    mutationFn: ({ id, status }: { id: string; status: FindingStatus }) =>
      // PATCH はどのルートにも当たらず、.catch が失敗を握りつぶして
      // 「更新した」形の値を返していたので、状態は一度も保存されて
      // いませんでした。サーバにあるのは PUT です。
                  ...
                  <div key={finding.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4 space-y-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-[#1e2d42]">
                      ...
                      <button
                        ...
                        className="px-3 py-1 text-xs rounded-sm border border-[#e8002d]/50 text-red-300 hover:bg-red-900/20 transition-colors flex items-center gap-1"
                      >
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm rounded-sm bg-[#e8002d] hover:bg-[#c8001d] text-white transition-colors"
              >
                ...
                {(integrationsData ?? []).map(svc => (
                  <div key={svc.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4 flex items-start justify-between gap-3">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-sm border border-[#1e2d42] text-[#7d92b0] hover:text-white hover:border-[#7d92b0]/40 transition-colors shrink-0"
                    >
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-[#e2e8f4] text-sm focus:outline-hidden focus:border-[#e8002d]/50 pr-10"
                  />
                </div>
              </div>

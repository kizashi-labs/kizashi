        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-1.5 max-h-52 overflow-y-auto">
              {m(MOCK_PACKETS).map(pkt => (
                <div key={pkt.seq} className="flex items-center gap-3 px-3 py-2 rounded bg-[#070d19] border border-[#1e2d42]">
                  <span className="text-[#3d5068] text-xs w-6">{pkt.seq}</span>
                  {pkt.direction === 'sent'
                    ? <ArrowRight className="w-3.5 h-3.5 text-[#e8002d] flex-shrink-0" />
                    : <ArrowLeft className="w-3.5 h-3.5 text-blue-400 flex-shrink-0" />
                  }
                  <span className="text-[#7d92b0] text-xs w-16">{formatBytes(pkt.size_bytes)}</span>
                  <div className="flex gap-1">
                    {pkt.flags.map(f => (
                      <span key={f} className={`text-[10px] px-1.5 py-0.5 rounded font-mono ${flagColor(f)}`}>{f}</span>
                    ))}
                  </div>
          ...
          {flow.payload_preview && (
            <div>
              ...
              <pre className="bg-[#070d19] border border-[#1e2d42] rounded p-3 text-[11px] text-green-300 font-mono overflow-x-auto whitespace-pre-wrap break-all">
                {flow.payload_preview}
              </pre>
            </div>
          )}
          ...
          {relatedFlows.length > 0 && (
            <div>
              ...
              <div className="space-y-1.5">
                {relatedFlows.map(f => (
                  <div key={f.id} className="flex items-center gap-3 px-3 py-2 rounded bg-[#070d19] border border-[#1e2d42]">
                    <span className="text-[#7d92b0] font-mono text-xs">{f.id}</span>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded border font-mono ${protocolColor(f.protocol)}`}>{f.protocol}</span>
                    <span className="text-[#7d92b0] text-xs">{f.dst_ip}:{f.dst_port}</span>
                    <span className={`ml-auto text-[10px] px-1.5 py-0.5 rounded border ${riskColor(f.risk_score)}`}>{f.risk_score}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex gap-3 pt-2 border-t border-[#1e2d42]">
            <button className="flex items-center gap-2 px-4 py-2 rounded bg-[#e8002d]/10 border border-[#e8002d]/30 text-[#e8002d] text-sm hover:bg-[#e8002d]/20 transition-colors">
              <Plus className="w-4 h-4" />
              IOCに追加
            </button>
            <button onClick={onClose} className="flex items-center gap-2 px-4 py-2 rounded bg-[#1e2d42] text-[#7d92b0] text-sm hover:text-white transition-colors">
              閉じる
            </button>
          </div>
        </div>
  ...
  return (
    <div className="grid grid-cols-2 gap-4 mt-4">
      <div>
        ...
        <pre className="bg-[#070d19] border border-[#1e2d42] rounded p-3 text-[11px] text-blue-300 font-mono overflow-x-auto whitespace-pre-wrap break-all h-48">{request}</pre>
      </div>
      <div>
        <p className="text-xs text-[#7d92b0] uppercase tracking-wider mb-2">HTTPレスポンス</p>
        <pre className="bg-[#070d19] border border-[#1e2d42] rounded p-3 text-[11px] text-green-300 font-mono overflow-x-auto whitespace-pre-wrap break-all h-48">{response}</pre>
      </div>
    </div>
  )
}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 bg-[#0d1220] border border-[#1e2d42] rounded-lg p-1">
              {(['1h', '6h', '24h', '7d'] as const).map(r => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${timeRange === r ? 'bg-[#1e2d42] text-white' : 'text-[#7d92b0] hover:text-white'}`}
                >
                  {r}
                </button>
              ))}
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-2 text-sm bg-[#0d1220] border border-[#1e2d42] rounded-lg text-white placeholder-[#3d5068] focus:outline-none focus:border-[#7d92b0] w-44"
              />
            </div>
          </div>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="relative">
                <select
                  ...
                  className="pl-3 pr-8 py-1.5 text-xs bg-[#0d1220] border border-[#1e2d42] rounded text-[#7d92b0] focus:outline-none appearance-none cursor-pointer"
                >
              ...
              {/* Risk filter */}
              <div className="flex items-center gap-1 bg-[#0d1220] border border-[#1e2d42] rounded p-0.5">
                {(['all', 'high', 'medium', 'low'] as const).map(r => (
                  <button
                    key={r}
                    onClick={() => setRiskFilter(r)}
                    className={`px-2.5 py-1 rounded text-xs transition-colors ${riskFilter === r ? 'bg-[#1e2d42] text-white' : 'text-[#7d92b0] hover:text-white'}`}
                  >
                    ...
                    {filteredFlows.map(flow => (
                      <tr key={flow.id} className="hover:bg-[#0a1628] transition-colors">
                        ...
                        <td className="px-3 py-2.5">
                          <span className={`px-2 py-0.5 rounded border text-[10px] font-mono font-bold ${protocolColor(flow.protocol)}`}>{flow.protocol}</span>
                        </td>
                        ...
                        <td className="px-3 py-2.5">
                          <div className="flex gap-1 flex-wrap">
                            {flow.flow_flags.slice(0, 3).map(f => (
                              <span key={f} className={`text-[9px] px-1 py-0.5 rounded font-mono ${flagColor(f)}`}>{f}</span>
                            ))}
                          </div>
                        </td>
                        <td className="px-3 py-2.5">
                          <span className={`px-2 py-0.5 rounded border text-[10px] font-mono font-bold ${riskColor(flow.risk_score)}`}>{flow.risk_score}</span>
                        </td>
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-[#1e2d42] flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 bg-[#070d19] border border-[#1e2d42] rounded p-0.5">
                  {(['bytes', 'count'] as const).map(s => (
                    <button
                      key={s}
                      onClick={() => setTopSortBy(s)}
                      className={`px-2.5 py-1 rounded text-xs transition-colors ${topSortBy === s ? 'bg-[#1e2d42] text-white' : 'text-[#7d92b0]'}`}
                    >
                      {s === 'bytes' ? 'バイト数' : '接続数'}
                    </button>
                  ))}
                  ...
                  return (
                    <div key={pair} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-2 bg-[#070d19] rounded-full overflow-hidden">
                        <div
                          className="h-full bg-gradient-to-r from-[#1a6bff] to-[#0044cc] rounded-full transition-all"
                          style={{ width: `${pct}%` }}
                        />
                      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-[#1e2d42] flex items-center justify-between">
                ...
                <button className="flex items-center gap-2 text-xs text-[#7d92b0] hover:text-white px-3 py-1.5 rounded border border-[#1e2d42] transition-colors">
                  ...
                  <tbody className="divide-y divide-[#1e2d42]/50">
                    {sessions.map(sess => (
                      <tr key={sess.session_id} className="hover:bg-[#0a1628] transition-colors">
                        ...
                        <td className="px-3 py-2.5">
                          <span className={`px-2 py-0.5 rounded border text-[10px] font-mono font-bold ${protocolColor(sess.protocol as Protocol)}`}>{sess.protocol}</span>
                        </td>
                        ...
                        <td className="px-3 py-2.5">
                          <button
                            ...
                            className="flex items-center gap-1.5 text-xs text-[#7d92b0] hover:text-white border border-[#1e2d42] hover:border-[#7d92b0]/40 px-2.5 py-1 rounded transition-colors"
                          >
                        ...
                        <tbody className="divide-y divide-[#1e2d42]/50">
                          {m(MOCK_DNS_QUERIES).map((q, i) => (
                            <tr key={i} className="hover:bg-[#0a1628]">
                              ...
                              <td className="px-3 py-2"><span className="px-1.5 py-0.5 rounded bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-[10px] font-mono">{q.type}</span></td>

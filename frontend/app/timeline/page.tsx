...
function SkeletonCard() {
  return (
    <div className="flex gap-4 p-4 bg-[#0d1220] border border-[#1e2d42] border-l-4 border-l-[#1e2d42] rounded-lg animate-pulse">
      <div className="w-8 h-8 rounded-lg bg-[#1e2d42] flex-shrink-0" />
      <div className="flex-1 space-y-2">
        <div className="h-4 bg-[#1e2d42] rounded w-3/4" />
        <div className="h-3 bg-[#1e2d42] rounded w-full" />
        <div className="h-3 bg-[#1e2d42] rounded w-1/3" />
      </div>
    </div>
  )
}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2 mb-1">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            {event.type === 'alert' && event.severity > 0 && (
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${getSeverityColor(event.severity)}`}>
        ...
        <div className="flex items-center gap-3">
          {/* Type badge */}
          <span className={`text-[10px] font-medium px-2 py-0.5 rounded ${iconColor}`}>
            {TYPE_LABELS[event.type]}
          </span>

          {/* Agent badge */}
          {event.agent_id && (
            <span className="text-[10px] px-2 py-0.5 rounded bg-[#1e2d42] text-[#7d92b0] font-mono truncate max-w-[120px]">
              {event.agent_id.slice(0, 8)}…
            </span>
          )}

          {/* Timestamp */}
          <span className="text-[10px] text-[#3d5068] ml-auto flex-shrink-0">
  ...
  return (
    <div className="flex items-center gap-3 my-4">
      <div className="flex items-center gap-2">
        <div className="w-6 h-6 rounded bg-[#1e2d42] flex items-center justify-center flex-shrink-0">
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                ...
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg
                           pl-9 pr-3 py-2 text-sm text-[#e2e8f4] placeholder-[#3d5068]
                           focus:outline-none focus:border-[#e8002d]/50 transition-all"
              />
            </div>
            ...
            <div className="relative">
              <input
                ...
                className="bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2
                           text-sm text-[#e2e8f4] focus:outline-none focus:border-[#e8002d]/50
            ...
            {(fromDate || toDate) && (
              <button
                ...
                className="px-3 py-2 rounded-lg text-xs text-[#7d92b0] hover:text-white
                           bg-[#1e2d42] hover:bg-[#1e2d42]/80 transition-colors"
              >
                クリア
              </button>
            )}
            ...
            {hasMore && (
              <div className="mt-6 text-center">
                <button
                  ...
                  disabled={isFetching}
                  className="px-6 py-2.5 rounded-lg bg-[#0d1220] border border-[#1e2d42]
                             text-sm text-[#7d92b0] hover:text-white hover:border-[#7d92b0]/40

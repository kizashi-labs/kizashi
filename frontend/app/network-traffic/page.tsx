          ...
          {similarFlows.length > 0 && (
            <div>
              ...
              <div className="space-y-1.5">
                {similarFlows.map(f => (
                  <div key={f.id} className="flex items-center gap-3 p-2 bg-[#070d19] rounded-sm border border-[#1e2d42] text-xs">
              ...
              {([] as CommPair[]).map(p => (
                <tr key={p.id} className="border-b border-[#1e2d42]/40 hover:bg-[#1e2d42]/20">
                  ...
                  <td className="py-2.5 px-3">
                    <span className="px-2 py-0.5 bg-[#1e2d42] text-[#7d92b0] border border-[#2d3f58] rounded-sm text-[10px] font-medium">{p.protocol}</span>
                  </td>
                  <td className="py-2.5 px-3">
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select value={agent} onChange={e => setAgent(e.target.value)}
          className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-xs text-[#e2e8f4] focus:outline-hidden focus:border-[#e8002d]/60">
        ...
        <input
          ...
          className="bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-xs text-[#e2e8f4] focus:outline-hidden focus:border-[#e8002d]/60 w-52"
        />
              ...
              {filtered.map(f => (
                <tr key={f.id} className={`border-b border-[#1e2d42]/40 hover:bg-[#1e2d42]/20 ${f.risk >= 75 ? 'bg-red-500/5' : ''}`}>
                  ...
                  <td className="py-2 px-3">
                    <span className="px-1.5 py-0.5 bg-[#1e2d42] text-[#7d92b0] border border-[#2d3f58] rounded-sm text-[10px] font-medium">{f.protocol}</span>
                  </td>
                  <td className="py-2 px-3">
                  ...
                  <td className="py-2 px-3">
                    <button
                      ...
                      className="flex items-center gap-1 px-2 py-1 text-xs bg-[#1e2d42] hover:bg-[#2a3f5f] text-[#e2e8f4] rounded-sm border border-[#2d3f58] transition-colors whitespace-nowrap"
                    >
...
export default function NetworkTrafficPage() {
  ...
  const { data: stats = EMPTY_TRAFFIC_STATS } = useQuery<TrafficStats>({
    queryKey: ['network-traffic-stats'],
    queryFn: () => apiFetch<TrafficStats>('/api/v1/network-traffic/stats'),
    staleTime: 60_000,

...
function SeverityBadge({ severity }: { severity: CriticalFinding['severity'] }) {
  ...
  return (
    <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${cfg[severity]}`}>
      {label[severity]}
    </span>
  )
}
...
function ComplianceStatusCell({ status, score }: { status: ComplianceCell['status']; score: number }) {
  ...
  return (
    <div className={`rounded p-2 text-center ${cfg[status]}`}>
              ...
              <div className="w-full grid grid-cols-3 gap-2 text-center">
                {(['A', 'B', 'C', 'D', 'F'] as const).map(g => {
                  ...
                  return (
                    <div
                      key={g}
                      className={`rounded px-2 py-1 text-xs border ${summary.grade === g ? 'border-current font-bold' : 'border-[#1e2d42]'}`}
                ...
                <tbody>
                  {summary.critical_findings.map((f, i) => (
                    <tr key={f.id} className={`border-b border-[#1e2d42] last:border-0 hover:bg-[#19253d] transition-colors ${i === 0 ? 'bg-[#e8002d]/5' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        <span className="text-[10px] bg-[#1e2d42] text-[#7d92b0] px-2 py-0.5 rounded">{f.category}</span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right">
                        <button className="text-xs px-3 py-1 rounded bg-[#e8002d]/20 text-[#e8002d] border border-[#e8002d]/40 hover:bg-[#e8002d]/30 transition-colors">
                          対処
                        </button>
                      </td>
                    </tr>
              ...
              <div className="space-y-3">
                {summary.recent_improvements.map(imp => (
                  <div key={imp.id} className="flex items-center gap-3 py-2 border-b border-[#1e2d42] last:border-0">
                    <div className="w-8 h-8 rounded-lg bg-[#00c853]/10 flex items-center justify-center flex-shrink-0">
                    ...
                    <span className="text-xs font-bold px-2 py-0.5 rounded bg-[#00c853]/20 text-[#00c853] border border-[#00c853]/30">
                      +{imp.change}%
                    </span>
                  </div>
                ))}
              ...
              <div className="space-y-2">
                {summary.open_risks.map((risk, i) => (
                  <div key={risk.id} className="flex items-center gap-3">
                    <span className="text-xs text-[#3d5068] w-4 flex-shrink-0">{i + 1}</span>
                    <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[9px] bg-[#1e2d42] text-[#7d92b0] px-1.5 py-0.5 rounded flex-shrink-0">{risk.category}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3 text-xs text-[#7d92b0]">
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-[#00c853]/30 inline-block" />準拠</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-yellow-500/30 inline-block" />一部準拠</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-[#e8002d]/30 inline-block" />非準拠</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-[#1e2d42] inline-block" />未評価</span>
              </div>
            </div>
                        ...
                        return (
                          <td key={d} className="py-2 px-2">
                            {cell ? (
                              ...
                              <div className="rounded p-2 text-center bg-[#1e2d42] text-[#3d5068] text-xs">N/A</div>
                            )}
                          </td>
                        )
                      })}

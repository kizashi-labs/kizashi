...
const motivationBadge = (m: string) => {
  ...
  return (
    <span key={m} className={`inline-flex items-center px-2 py-0.5 rounded-sm text-[10px] border ${configs[m] ?? 'bg-[#1e2d42] text-[#7d92b0] border-[#1e2d42]'}`}>
      {labels[m] ?? m}
    </span>
  )
}
          ...
          >
            ...
            <div className="flex flex-wrap gap-1 mb-3">
              {actor.aliases.slice(0, 3).map(alias => (
                <span key={alias} className="px-1.5 py-0.5 rounded-sm text-[10px] bg-[#1e2d42] text-[#7d92b0] border border-[#1e2d42]">
                  {alias}
                </span>
              ))}
              {actor.aliases.length > 3 && (
                <span className="px-1.5 py-0.5 rounded-sm text-[10px] bg-[#1e2d42] text-[#7d92b0]">
                  +{actor.aliases.length - 3}
                </span>
              )}
            </div>

      ...
      <div className="flex gap-2 min-w-max pb-2">
        {usedTactics.map(tactic => (
          <div key={tactic} className="shrink-0 w-44">
            <div className="bg-[#1e2d42] text-[#e2e8f4] text-[11px] font-semibold px-2 py-1.5 rounded-t text-center">
              {tactic}
            </div>
            <div className="space-y-1 pt-1">
              {(byTactic[tactic] || []).map(t => (
                <div
                  ...
                  className="bg-[#e8002d]/20 border border-[#e8002d]/30 text-[#f87171] text-[10px] px-2 py-1 rounded-sm cursor-default hover:bg-[#e8002d]/30 transition-colors"
                >
    ...
    <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg overflow-hidden">
      ...
      >
        <div className="flex items-start gap-3 flex-1 min-w-0">
          <div className={`mt-0.5 w-2 h-2 rounded-full shrink-0 ${campaign.status === 'active' ? 'bg-green-400 animate-pulse' : 'bg-[#3d5068]'}`} />
          <div className="min-w-0">
            <h4 className="text-[#e2e8f4] font-semibold">{campaign.name}</h4>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className={`text-[11px] px-2 py-0.5 rounded-sm border ${
            campaign.status === 'active'
              ...
              : 'bg-[#1e2d42] text-[#7d92b0] border-[#1e2d42]'
          }`}>
            {campaign.status === 'active' ? 'アクティブ' : '終了'}
          </span>
          {expanded ? <ChevronDown className="w-4 h-4 text-[#7d92b0]" /> : <ChevronRight className="w-4 h-4 text-[#7d92b0]" />}
        </div>
      </div>

      {expanded && (
        <div className="px-4 pb-4 border-t border-[#1e2d42] pt-3">
          <p className="text-[#7d92b0] text-sm mb-3">{campaign.description}</p>
          <div>
            <p className="text-[#7d92b0] text-xs font-medium mb-1.5">標的セクター</p>
...
const iocTypeBadge = (type: string) => {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-sm text-[10px] font-mono font-semibold border uppercase ${cfgs[type] ?? 'bg-[#1e2d42] text-[#7d92b0] border-[#1e2d42]'}`}>
      {type}
    </span>
  )
}
...
export default function ThreatActorDetailPage() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#070d19] flex items-center justify-center">
        <RefreshCw className="w-8 h-8 text-[#7d92b0] animate-spin" />
      </div>
    )
  }
  ...
  if (!actor) {
    return (
      <div className="min-h-screen bg-[#070d19] flex items-center justify-center">
        <p className="text-[#7d92b0]">脅威アクターが見つかりません</p>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#070d19] p-6">
      <PageDataUnavailable />
      {/* Breadcrumb */}
      <div className="flex items-center gap-2 text-sm text-[#7d92b0] mb-4">
        <Link href="/threat-intel" className="hover:text-[#e2e8f4] transition-colors">脅威インテリジェンス</Link>
        <ChevronRight className="w-3 h-3" />
        <Link href="/threat-intel/actors" className="hover:text-[#e2e8f4] transition-colors">脅威アクター</Link>
        <ChevronRight className="w-3 h-3" />
        <span className="text-[#e2e8f4]">{actor.name}</span>
      </div>

      {/* Header */}
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-6 mb-6">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-3xl font-bold text-[#e2e8f4]">{actor.name}</h1>
              {threatLevelBadge(actor.threat_level)}
            </div>
            <div className="flex items-center gap-2 mt-2 text-[#7d92b0]">
            ...
            <div className="flex flex-wrap gap-1.5 mt-3">
              {actor.aliases.map(alias => (
                <span key={alias} className="px-2 py-0.5 rounded-sm text-[11px] bg-[#1e2d42] text-[#7d92b0] border border-[#1e2d42]">
                  {alias}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div className="bg-[#070d19] rounded-lg p-3 border border-[#1e2d42] text-center">
              <p className="text-[#7d92b0] text-xs mb-1">初確認</p>
              <p className="text-[#e2e8f4] font-medium flex items-center gap-1 justify-center">
                <Calendar className="w-3.5 h-3.5 text-[#7d92b0]" />
                {new Date(actor.first_seen).toLocaleDateString('ja-JP')}
              </p>
            </div>
            <div className="bg-[#070d19] rounded-lg p-3 border border-[#1e2d42] text-center">
              <p className="text-[#7d92b0] text-xs mb-1">最終確認</p>
              <p className="text-[#e2e8f4] font-medium flex items-center gap-1 justify-center">
            ...
            <div className="bg-[#070d19] rounded-lg p-3 border border-[#1e2d42] text-center">
              <p className="text-[#7d92b0] text-xs mb-1">キャンペーン</p>
              <p className="text-[#e2e8f4] font-medium">{actor.campaigns.length}</p>
            </div>
            <div className="bg-[#070d19] rounded-lg p-3 border border-[#1e2d42] text-center">
              <p className="text-[#7d92b0] text-xs mb-1">追跡IOC</p>
              <p className="text-[#e2e8f4] font-medium">{actor.iocs.length}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {activeTab === '概要' && (
        <div className="space-y-6">
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {actor.motivation.map(m => (
                  <span key={m} className={`px-3 py-1.5 rounded-full text-sm font-medium border ${motivationColors[m] ?? 'bg-[#1e2d42] text-[#7d92b0] border-[#1e2d42]'}`}>
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {actor.target_industries.map(ind => (
                  <span key={ind} className="px-2.5 py-1 rounded-sm text-sm bg-[#1e2d42] text-[#e2e8f4] border border-[#1e2d42]">
                    {ind}
                  </span>
                ))}
              </div>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
              {actor.target_regions.map(r => (
                <div key={r.country} className="flex items-center gap-2 px-3 py-2 rounded-sm bg-[#070d19] border border-[#1e2d42]">
                  <span className="text-xl">{r.flag}</span>
                  <span className="text-sm text-[#e2e8f4]">{r.country}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {actor.aliases.map(alias => (
                <span key={alias} className="px-3 py-1.5 rounded-sm text-sm bg-[#1e2d42] text-[#e2e8f4] border border-[#1e2d42] font-mono">
                  {alias}
                </span>
              ))}
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[#e2e8f4] font-semibold flex items-center gap-2">
                <Shield className="w-4 h-4 text-[#e8002d]" />
                MITRE ATT&CK マトリクス（使用確認済み）
              </h3>
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <p className="text-xs text-[#7d92b0]">カバレッジ</p>
                  <p className="text-sm font-bold text-[#e2e8f4]">
                    {new Set(actor.ttps.map(t => t.tactic)).size}
                    <span className="text-[#7d92b0] font-normal"> / 14 タクティクス</span>
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-[#7d92b0]">テクニック数</p>
                  <p className="text-sm font-bold text-[#e8002d]">{actor.ttps.length}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-4">
              <div className="h-1.5 bg-[#1e2d42] rounded-full overflow-hidden">
                <div
                  className="h-full bg-linear-to-r from-[#e8002d] to-[#ff6b6b] rounded-full transition-all"
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg overflow-hidden">
            <div className="px-5 py-4 border-b border-[#1e2d42]">
              <h3 className="text-[#e2e8f4] font-semibold">テクニック詳細</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {actor.ttps.map((ttp, i) => (
                    <tr key={ttp.technique_id} className={`border-b border-[#1e2d42] hover:bg-[#111827] transition-colors ${i % 2 === 0 ? '' : 'bg-[#070d19]/30'}`}>
                      <td className="px-4 py-3 font-mono text-[#e8002d] font-semibold whitespace-nowrap">
                        {ttp.technique_id}
                      </td>
                      <td className="px-4 py-3 text-[#e2e8f4] whitespace-nowrap font-medium">
                        {ttp.technique_name}
                      </td>
                      ...
                      <td className="px-4 py-3 text-[#7d92b0] text-xs max-w-xs">{ttp.description}</td>
                      <td className="px-4 py-3 text-[#7d92b0] text-xs max-w-xs">{ttp.detection_notes}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {activeTab === 'キャンペーン' && (
        <div className="space-y-4">
          ...
          <div className="relative pl-6">
            <div className="absolute left-2.5 top-0 bottom-0 w-0.5 bg-[#1e2d42]" />
            <div className="space-y-4">
              {actor.campaigns.map(campaign => (
                <div key={campaign.id} className="relative">
                  <div className={`absolute -left-[19px] top-5 w-3 h-3 rounded-full border-2 border-[#0d1220] ${
                    campaign.status === 'active' ? 'bg-green-400' : 'bg-[#3d5068]'
                  }`} />
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="relative flex-1 min-w-[200px] max-w-sm">
              ...
              <input
                ...
                placeholder="IOC値で検索..."
                className="w-full pl-10 pr-4 py-2 bg-[#0d1220] border border-[#1e2d42] rounded-lg text-sm text-[#e2e8f4] placeholder-[#3d5068] focus:outline-hidden focus:border-[#7d92b0]/50"
              />
            </div>
            ...
            <span className="text-xs text-[#7d92b0]">{filteredIOCs.length} 件</span>
            <button
              onClick={() => {
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-sm bg-[#0d1220] border border-[#1e2d42] text-[#7d92b0] hover:text-[#e2e8f4] hover:border-[#7d92b0]/40 text-xs transition-colors"
            >
          ...
          <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {pagedIOCs.map((ioc, i) => (
                    <tr key={ioc.id} className={`border-b border-[#1e2d42] hover:bg-[#111827] transition-colors ${i % 2 === 0 ? '' : 'bg-[#070d19]/30'}`}>
                      <td className="px-4 py-3 font-mono text-[#e2e8f4] text-xs max-w-xs truncate">
                        {ioc.value}
                      </td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <Link href="/ioc" className="p-1.5 rounded-sm hover:bg-[#1e2d42] text-[#7d92b0] hover:text-[#e2e8f4] transition-colors" title="IOC管理で表示">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  disabled={iocPage === 1}
                  className="px-3 py-1.5 rounded-sm bg-[#0d1220] border border-[#1e2d42] text-[#7d92b0] disabled:opacity-40 hover:border-[#7d92b0]/40 transition-colors"
                >
                  前へ
                </button>
                {Array.from({ length: totalIocPages }, (_, i) => i + 1).map(p => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-sm text-xs transition-colors ${
                      p === iocPage
                        ? 'bg-[#e8002d] text-white'
                        : 'bg-[#0d1220] border border-[#1e2d42] text-[#7d92b0] hover:border-[#7d92b0]/40'
                    }`}
      ...
      {activeTab === 'レポート' && (
        <div className="space-y-4">
          {actor.reports.map(report => (
            <div key={report.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5 hover:border-[#7d92b0]/30 transition-colors">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <h4 className="text-[#e2e8f4] font-semibold mb-1">{report.title}</h4>
                  ...
                  <p className="text-[#7d92b0] text-sm leading-relaxed">{report.summary}</p>
                </div>
                <a
                  href={report.pdf_url}
                  className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-sm bg-[#e8002d]/15 border border-[#e8002d]/30 text-[#f87171] hover:bg-[#e8002d]/25 transition-colors text-sm font-medium"
                >

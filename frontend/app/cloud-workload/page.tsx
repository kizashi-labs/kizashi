        ...
        <div className="flex items-start justify-between p-5 border-b border-[#1e2d42]">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className={`text-xs px-2 py-0.5 rounded border ${prot.cls}`}>{prot.label}</span>
              ...
              : workload.runtime_events.map((e, i) => {
                ...
                return (
                  <div key={i} className="flex gap-3 mb-3 bg-[#070d19] border border-[#1e2d42] rounded-lg p-3">
                    <AlertTriangle className="w-4 h-4 text-[#e8002d] mt-0.5 flex-shrink-0" />
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 overflow-y-auto py-4">
      <div className="w-full max-w-2xl mx-4 bg-[#0d1220] border border-[#1e2d42] rounded-xl shadow-2xl">
        <div className="flex items-start justify-between p-5 border-b border-[#1e2d42]">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className={`text-xs px-2 py-0.5 rounded ${tt.cls}`}>{tt.label}</span>
              <span className={`text-xs px-2 py-0.5 rounded ${sev.cls}`}>{sev.label}</span>
              {threat.auto_blocked && <span className="text-xs px-2 py-0.5 rounded bg-green-900/40 text-green-400">自動ブロック</span>}
            </div>
            ...
            <ol className="space-y-1">
              {threat.recommended_response.map((r, i) => (
                <li key={i} className="text-xs text-[#7d92b0] flex gap-2">
                  <span className="text-[#e8002d] font-bold flex-shrink-0">{i + 1}.</span>
                  {r}
                </li>
              ))}
            </ol>
  ...
  return (
    <div className="min-h-screen bg-[#070d19] p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button className="p-2 border border-[#1e2d42] rounded text-[#7d92b0] hover:text-white hover:border-[#7d92b0]/40 transition-colors">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4">
          ...
          <div className="flex gap-2 mt-1">
            <span className="text-xs px-2 py-0.5 rounded bg-red-900/40 text-red-400">Critical: {stats.critical}</span>
            <span className="text-xs px-2 py-0.5 rounded bg-orange-900/40 text-orange-400">High: {stats.todayThreats - stats.critical}</span>
          </div>
        </div>
      </div>
                        ...
                        <td className="py-2.5 px-3">
                          <div className="flex gap-1 flex-wrap max-w-[120px]">
                            {w.tags.slice(0, 2).map(t => (
                              <span key={t} className="text-xs px-1.5 py-0.5 bg-[#1e2d42] rounded text-[#7d92b0]">{t}</span>
                            ))}
                          </div>
                        </td>
                        <td className="py-2.5 px-3">
                ...
                {threats.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3">
                        {t.auto_blocked
                          ? <span className="text-xs px-2 py-0.5 rounded bg-green-900/40 text-green-400">ブロック済</span>
                          : <span className="text-xs px-2 py-0.5 rounded bg-red-900/40 text-red-400">未ブロック</span>
                        }
                      </td>
                      <td className="py-2.5 px-3">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {isFixed && <span className="text-xs px-2 py-0.5 rounded bg-green-900/40 text-green-400">修正済み</span>}
                      {isSuppressed && <span className="text-xs px-2 py-0.5 rounded bg-slate-700/40 text-slate-400">抑制</span>}
                    </div>
                  ...
                  {m.quick_fixable && !isFixed && !isSuppressed && (
                    <button
                      ...
                      className="flex-shrink-0 px-3 py-1.5 text-xs bg-green-900/30 border border-green-700/40 text-green-400 rounded hover:bg-green-900/50 transition-colors"
                    >
                      クイック修正
                    </button>
                  )}

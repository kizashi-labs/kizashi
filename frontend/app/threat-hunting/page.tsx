    ...
    render: (v) => (
      <span
        className={`text-xs px-1.5 py-0.5 rounded border whitespace-nowrap ${
          EVENT_TYPE_COLORS[v as string] ?? 'bg-falcon-raised text-falcon-muted border-falcon-border'
        }`}
      >
        {v as string}
      </span>
  ...
  network_connection: [
    ...
    { key: 'username', label: 'ユーザー' },
    {
      ...
      render: (v) => {
        ...
        if (!dst) return <span className="font-mono text-xs text-falcon-muted">{JSON.stringify(d).slice(0, 60)}</span>;
        return (
  ...
  return (
    <div className="mb-2">
      <div className="flex justify-between text-xs mb-1">
        <span className="text-falcon-muted truncate max-w-[140px]">{label}</span>
        <span className="text-falcon-text font-mono ml-2">{count}</span>
      </div>
...
export default function ThreatHuntingPage() {
  ...
  return (
    <div className="min-h-screen bg-falcon-bg text-falcon-text flex flex-col">
      {/* ── Header ── */}
      <div className="border-b border-falcon-border px-6 py-4 shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Terminal className="w-5 h-5 text-blue-400" />
            <div>
              <h1 className="text-xl font-semibold text-falcon-text">脅威ハンティング</h1>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                templatesOpen ? 'bg-falcon-border text-falcon-text' : 'bg-falcon-card border border-falcon-border text-falcon-muted hover:text-falcon-text hover:bg-falcon-raised'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                savedSidebarOpen ? 'bg-blue-600 text-white' : 'bg-falcon-card border border-falcon-border text-falcon-muted hover:text-falcon-text hover:bg-falcon-raised'
              }`}
            >
              <Save className="w-4 h-4" />
              保存済み
              {savedHunts.length > 0 && (
                <span className="bg-falcon-border text-falcon-muted text-xs px-1.5 py-0.5 rounded-full">
                  {savedHunts.length}
                </span>
              )}
            </button>
      ...
      {templatesOpen && (
        <div className="border-b border-falcon-border bg-[#0d1524] px-6 py-4 shrink-0">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-medium text-falcon-muted">ハンティングテンプレート</h2>
            <button onClick={() => setTemplatesOpen(false)} className="text-falcon-muted hover:text-falcon-text">
              <X className="w-4 h-4" />
            </button>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            {HUNT_TEMPLATES.map((tmpl) => (
              <button
                ...
                className="text-left bg-falcon-card border border-falcon-border hover:border-[#2d4a6e] rounded-lg p-3 transition-colors group"
              >
                <div className="flex items-center gap-2 mb-1.5">
                  <div className="p-1 bg-falcon-raised rounded-sm group-hover:bg-falcon-border transition-colors">{tmpl.icon}</div>
                  <span className="text-xs font-medium text-falcon-text leading-tight">{tmpl.label}</span>
                </div>
                <p className="text-xs text-falcon-muted line-clamp-2">{tmpl.description}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div className="w-80 shrink-0 border-r border-falcon-border bg-[#0d1524] flex flex-col overflow-y-auto">
          <form onSubmit={handleSearch} className="flex flex-col flex-1 p-4 gap-4">

            {/* Query type selector */}
            <div>
              <label className="block text-xs font-medium text-falcon-muted mb-2 uppercase tracking-wider">クエリタイプ</label>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-2.5 py-2 rounded-lg text-xs font-medium transition-colors ${
                      queryType === opt.value
                        ...
                        : 'bg-falcon-card border border-falcon-border text-falcon-muted hover:text-falcon-text hover:bg-falcon-raised'
                    }`}
                  >
                    {opt.icon}
                    {opt.label}
            ...
            <div>
              <label className="block text-xs font-medium text-falcon-muted mb-2 uppercase tracking-wider flex items-center gap-1.5">
              ...
              <textarea
                ...
                rows={6}
                className="w-full px-3 py-2.5 font-mono text-sm bg-gray-950 border border-falcon-border rounded-lg text-green-300 placeholder-[#2a3a52] focus:outline-hidden focus:border-blue-500 resize-none leading-relaxed"
              />
            </div>

            {/* Time range */}
            <div>
              <label className="block text-xs font-medium text-falcon-muted mb-2 uppercase tracking-wider">時間範囲</label>
              <div className="grid grid-cols-2 gap-1.5">
            ...
            <div>
              <button
                ...
                className="flex items-center gap-1.5 text-xs text-falcon-muted hover:text-falcon-text transition-colors w-full"
              >
              ...
              {advancedOpen && (
                <div className="mt-3 space-y-3">
                  <div>
                    <label className="block text-xs text-falcon-muted mb-1">ホスト名</label>
                    <input
                      type="text"
                      placeholder="hostname"
                      value={hostname}
                      onChange={(e) => setHostname(e.target.value)}
                      className="w-full px-3 py-2 bg-falcon-bg border border-falcon-border rounded-lg text-xs text-falcon-text placeholder-[#4a5568] focus:outline-hidden focus:border-blue-500"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-falcon-muted mb-1">プロセス名</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs text-falcon-muted mb-1">ユーザー名</label>
                    <input
                      type="text"
                  ...
                  {timeRange === 'custom' && (
                    <>
                      <div>
                        <label className="block text-xs text-falcon-muted mb-1">開始時刻</label>
                        <input type="datetime-local" value={startTime} onChange={(e) => setStartTime(e.target.value)}
                          className="w-full px-3 py-2 bg-falcon-bg border border-falcon-border rounded-lg text-xs text-falcon-text focus:outline-hidden focus:border-blue-500" />
                      </div>
                      <div>
                        <label className="block text-xs text-falcon-muted mb-1">終了時刻</label>
            ...
            <div className="flex gap-2 mt-auto pt-2">
              ...
              {canWrite && (
                <button
                  ...
                  onClick={() => setSaveDialogOpen(true)}
                  title="このクエリを保存"
                  className="flex items-center gap-1.5 px-3 py-2.5 bg-falcon-card border border-falcon-border hover:bg-falcon-border disabled:opacity-40 rounded-lg text-sm text-falcon-muted hover:text-falcon-text transition-colors"
                >
                  <Save className="w-4 h-4" />
                  保存
                </button>
            ...
            {hasSearched && results.length > 0 && (
              <div className="border-t border-falcon-border pt-4 space-y-4">
                <div>
                  <div className="flex items-center gap-1.5 mb-2 text-xs font-medium text-falcon-muted uppercase tracking-wider">
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          {/* Results toolbar */}
          {hasSearched && (
            <div className="border-b border-falcon-border bg-[#0d1524] px-4 py-2.5 flex items-center gap-3 shrink-0 flex-wrap">
              <div className="flex items-center gap-2 text-sm">
                {isSearching ? (
                  <Loader2 className="w-4 h-4 animate-spin text-blue-400" />
                ) : (
                  <span className="text-falcon-text font-medium">{totalResults.toLocaleString()}</span>
                )}
                <span className="text-falcon-muted">件</span>
                ...
                {eventTypePills.map(([type, count]) => (
                  <span
                    ...
                    className={`text-xs px-2 py-0.5 rounded-full border font-mono ${
                      EVENT_TYPE_COLORS[type] ?? 'bg-falcon-raised text-falcon-muted border-falcon-border'
                    }`}
                  >
                    {type}: {count}
                  </span>
              ...
              <div className="ml-auto flex items-center gap-2">
                {/* View toggle */}
                <div className="flex items-center gap-0.5 bg-falcon-card border border-falcon-border rounded-lg p-0.5">
                  <button
                    onClick={() => setViewMode('table')}
                    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded text-xs font-medium transition-colors ${
                      viewMode === 'table' ? 'bg-blue-600 text-white' : 'text-falcon-muted hover:text-falcon-text'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded text-xs font-medium transition-colors ${
                      viewMode === 'timeline' ? 'bg-blue-600 text-white' : 'text-falcon-muted hover:text-falcon-text'
                    }`}
                  >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1.5 bg-falcon-card border border-falcon-border hover:bg-falcon-border disabled:opacity-40 rounded-lg text-xs text-falcon-muted hover:text-falcon-text transition-colors"
                >
          ...
          <div className="flex-1 overflow-auto p-4">
            ...
            {isSearching && (
              <div className="flex flex-col items-center justify-center py-20 gap-3">
                ...
                <p className="text-sm text-falcon-muted">クエリを実行中...</p>
              </div>
            )}
            ...
            {!hasSearched && !isSearching && (
              <div className="flex flex-col items-center justify-center py-24 gap-4">
                <div className="w-16 h-16 rounded-full bg-falcon-card border border-falcon-border flex items-center justify-center">
                  <Search className="w-8 h-8 text-falcon-border" />
                </div>
                <div className="text-center">
                  <p className="text-falcon-muted font-medium mb-1">クエリを入力して実行</p>
                ...
                <div className="flex gap-2 mt-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 bg-falcon-card border border-falcon-border hover:bg-falcon-raised rounded-lg text-xs text-falcon-muted hover:text-falcon-text transition-colors"
                  >
            ...
            {hasSearched && !isSearching && results.length === 0 && (
              <div className="flex flex-col items-center justify-center py-24 gap-4">
                ...
                <div className="text-center">
                  <p className="text-falcon-muted font-medium mb-1">結果なし</p>
                  <p className="text-xs text-[#4a5568]">
            ...
            {hasSearched && !isSearching && results.length > 0 && viewMode === 'timeline' && (
              <div className="space-y-4">
                <div className="bg-falcon-card border border-falcon-border rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-4">
                    <TrendingUp className="w-4 h-4 text-blue-400" />
                    <h3 className="text-sm font-medium text-falcon-text">イベント頻度タイムライン</h3>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {eventTypePills.map(([type, count]) => (
                    <div key={type} className="bg-falcon-card border border-falcon-border rounded-lg p-3">
                      <div className={`text-xs px-2 py-0.5 rounded-sm border inline-block mb-2 ${EVENT_TYPE_COLORS[type] ?? 'bg-falcon-raised text-falcon-muted border-falcon-border'}`}>
                        {type}
                      </div>
                      <p className="text-xl font-bold text-falcon-text font-mono">{count}</p>
                      <p className="text-xs text-falcon-muted">{Math.round((count / results.length) * 100)}%</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-falcon-card border border-falcon-border rounded-lg overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-falcon-border bg-[#0d1524]">
                        {columns.map((col) => (
                          <th
                            key={col.key}
                            className="px-3 py-2.5 text-left text-falcon-muted font-medium uppercase tracking-wider whitespace-nowrap"
                          >
                            {col.label}
                          </th>
                        ))}
                    ...
                    <tbody className="divide-y divide-falcon-border">
                      {results.map((row, idx) => (
                        <tr
                          key={row.id ?? idx}
                          className="hover:bg-falcon-raised transition-colors group"
                        >
                          {columns.map((col) => {
                            const val = row[col.key];
                            return (
                              <td key={col.key} className="px-3 py-2 text-falcon-text max-w-[280px]">
                                {col.render
                                  ? col.render(val, row)
                                  : val == null
                                  ? <span className="text-[#4a5568]">-</span>
                                  : typeof val === 'object'
                                  ? <span className="font-mono text-falcon-muted truncate block">{JSON.stringify(val).slice(0, 80)}</span>
                ...
                {totalResults > results.length && (
                  <div className="px-4 py-2.5 border-t border-falcon-border text-xs text-falcon-muted text-center bg-[#0d1524]">
        ...
        {savedSidebarOpen && (
          <div className="w-72 shrink-0 border-l border-falcon-border bg-[#0d1524] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-falcon-border shrink-0">
              <h2 className="text-sm font-medium text-falcon-text flex items-center gap-2">
                <Save className="w-4 h-4 text-blue-400" />
                保存済みハント
                {savedHunts.length > 0 && (
                  <span className="text-[10px] bg-falcon-border text-falcon-muted px-1.5 py-0.5 rounded-full">
                    {savedHunts.length}
                  </span>
                )}
              </h2>
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setSavedSidebarOpen(false)} className="text-falcon-muted hover:text-falcon-text">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {savedLoading ? (
                Array.from({ length: 3 }).map((_, i) => (
                  <div key={i} className="h-16 bg-falcon-card border border-falcon-border rounded-lg animate-pulse" />
                ))
              ) : savedHunts.length === 0 ? (
                <div className="text-center py-10">
                  <Save className="w-8 h-8 text-falcon-border mx-auto mb-2" />
                  <p className="text-xs text-falcon-muted">保存済みハントはありません</p>
                </div>
              ) : (
                savedHunts.map((hunt) => (
                  <div
                    key={hunt.id}
                    className={`bg-falcon-card border rounded-lg p-3 transition-colors ${
                      loadedHuntId === hunt.id ? 'border-blue-500/60' : 'border-falcon-border hover:border-[#2d4a6e]'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <h3 className="text-xs font-semibold text-falcon-text leading-tight">{hunt.name}</h3>
                        ...
                        {canWrite && (confirmDeleteId === hunt.id ? (
                          <div className="flex gap-0.5">
                            ...
                            <button
                              ...
                              className="px-1.5 py-0.5 bg-falcon-border hover:bg-[#2a3a52] rounded-sm text-xs text-falcon-muted"
                            >
                              取消
                            </button>
                          </div>
                    ...
                    {hunt.description && (
                      <p className="text-xs text-falcon-muted mb-1.5 line-clamp-2">{hunt.description}</p>
                    )}
                    {hunt.params.query && (
                      <div className="font-mono text-xs text-[#4a5568] bg-falcon-bg rounded-sm px-2 py-1 truncate mb-1.5">
                        {hunt.params.query}
                      </div>
                    )}
      ...
      {saveDialogOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-falcon-card border border-falcon-border rounded-xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-semibold text-falcon-text">ハントを保存</h2>
              <button onClick={() => setSaveDialogOpen(false)} className="text-falcon-muted hover:text-falcon-text transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs text-falcon-muted mb-1.5">説明（任意）</label>
                <textarea
                  value={saveDesc}
                  onChange={(e) => setSaveDesc(e.target.value)}
                  placeholder="このハントの目的や検出対象を記述"
                  rows={3}
                  className="w-full px-3 py-2 bg-falcon-bg border border-falcon-border rounded-lg text-sm text-falcon-text placeholder-falcon-muted focus:outline-hidden focus:border-blue-500 resize-none"
                />
              </div>
              <div className="text-xs text-falcon-muted bg-falcon-bg rounded-sm p-2 font-mono">

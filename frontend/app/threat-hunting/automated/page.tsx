        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input value={form.name} onChange={e => set('name', e.target.value)}
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50" placeholder="例: Lateral Movement Detection" />
          </div>
          {/* Description */}
          <div>
            <label className="block text-xs text-[#7d92b0] mb-1">説明</label>
            <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={2}
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50 resize-none" />
          </div>
          {/* Type & Source */}
          ...
          <div>
            ...
            <textarea value={form.query} onChange={e => set('query', e.target.value)} rows={5}
              ...
              className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-xs text-[#e2e8f4] font-mono focus:outline-none focus:border-[#e8002d]/50 resize-none" />
          </div>
          {/* Schedule */}
          <div>
            ...
            {scheduleMode === 'custom' && (
              <input value={form.schedule} onChange={e => set('schedule', e.target.value)}
                className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white font-mono focus:outline-none" placeholder="0 */6 * * *" />
            )}
          </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                <button onClick={() => set('auto_escalate', !form.auto_escalate)}
                  ...
                  <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-[#e2e8f4] transition-transform shadow ${form.auto_escalate ? 'translate-x-5' : 'translate-x-0.5'}`} />
                </button>
                {form.auto_escalate && (
                  <div className="flex items-center gap-1 text-xs text-[#7d92b0]">
                    ...
                    <input type="number" min={1} max={100} value={form.auto_escalate_threshold}
                      ...
                      className="w-12 bg-[#070d19] border border-[#1e2d42] rounded px-2 py-0.5 text-white text-xs text-center focus:outline-none" />
                    <span>件以上で作成</span>
                  </div>
                )}
              </div>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              <input value={mitreInput} onChange={e => setMitreInput(e.target.value)}
                ...
                className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-sm text-white focus:outline-none" placeholder="T1059.001" />
            ...
            <div className="flex flex-wrap gap-1.5">
              {(form.mitre_techniques ?? []).map(t => (
                <span key={t} className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-900/30 border border-purple-700/40 rounded text-xs text-purple-300">
                  {t}
...
function FindingRow({ f }: { f: Finding }) {
  return (
    <div className="flex items-center gap-3 px-3 py-2 bg-[#070d19] rounded-lg border border-[#1e2d42] text-xs">
      <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${f.severity === 'critical' ? 'bg-red-500' : f.severity === 'high' ? 'bg-orange-400' : 'bg-yellow-400'}`} />
      <span className="text-[#7d92b0] w-28 truncate flex-shrink-0">{f.endpoint}</span>
      ...
      <button className="ml-auto px-2 py-0.5 bg-[#1e2d42] hover:bg-[#253650] rounded text-[#7d92b0] whitespace-nowrap">アラート作成</button>
    </div>
  )
}
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex-1 max-w-sm">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="ルール名・説明を検索..."
                className="w-full bg-[#0d1220] border border-[#1e2d42] rounded-lg pl-9 pr-3 py-2 text-sm text-white focus:outline-none focus:border-[#e8002d]/50" />
            </div>
              ...
              <tbody>
                ...
                {filteredRules.map(r => (
                  <tr key={r.id} className="border-b border-[#1e2d42] hover:bg-[#0a1320] transition-colors">
                        ...
                        {r.mitre_techniques.length > 0 && (
                          <div className="flex gap-1 mt-1 flex-wrap">
                            {r.mitre_techniques.slice(0, 3).map(t => (
                              <span key={t} className="text-[10px] px-1.5 py-0.5 bg-purple-900/30 text-purple-400 rounded">{t}</span>
                            ))}
                          </div>
                        )}
                      </div>
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded border text-xs font-medium ${HUNT_TYPE_COLORS[r.hunt_type]}`}>{r.hunt_type}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs ${DS_COLORS[r.data_source]}`}>
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => {/* toggle */ }}
                        ...
                        <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-[#e2e8f4] transition-transform shadow ${r.enabled ? 'translate-x-4' : 'translate-x-0.5'}`} />
                      </button>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => runMutation.mutate(r.id)} title="今すぐ実行"
                          className="p-1.5 hover:bg-green-900/30 rounded text-green-400 transition-colors">
                          <Play className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => setEditRule(r)} title="編集"
                          className="p-1.5 hover:bg-[#1e2d42] rounded text-[#7d92b0] transition-colors">
                          <Edit2 className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => deleteMutation.mutate(r.id)} title="削除"
                          className="p-1.5 hover:bg-red-900/30 rounded text-red-400/70 transition-colors">
              ...
              <tbody>
                {effectivenessTop5.map((r, i) => (
                  <tr key={r.id} className="border-b border-[#1e2d42]/50">
                    ...
                    <td className="px-3 py-2"><span className={`px-2 py-0.5 rounded border text-xs ${HUNT_TYPE_COLORS[r.hunt_type]}`}>{r.hunt_type}</span></td>
                ...
                {filteredExecs.map(e => (
                  <>
                    <tr key={e.id} className="border-b border-[#1e2d42] hover:bg-[#0a1320] transition-colors cursor-pointer"
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${EXEC_STATUS_BADGE[e.status]}`}>{e.status}</span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        {e.escalated && (
                          <span className="px-2 py-0.5 rounded text-xs bg-orange-900/40 text-orange-400 border border-orange-700/40">エスカレーション</span>
                        )}
                      </td>
                    </tr>

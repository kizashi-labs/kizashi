...
const STATUS_STYLES: Record<NotebookStatus, string> = {
  ...
  archived: 'bg-[#1e2d42] text-[#7d92b0] border-[#2e4060]',
}
...
const CELL_TYPE_STYLES: Record<CellType, string> = {
  ...
  note: 'bg-[#1e2d42] text-[#7d92b0] border-[#2e4060]',
    ...
    <div className="flex items-end gap-1 h-24 pt-2">
      {data.map((d, i) => (
        <div key={i} className="flex flex-col items-center gap-1 flex-1">
          <span className="text-[10px] text-[#7d92b0]">{d.value}</span>
          <div
            className="w-full rounded-t bg-[#e8002d]/60 hover:bg-[#e8002d] transition-colors"
            style={{ height: `${(d.value / max) * 64}px` }}
          />
          <span className="text-[9px] text-[#3d5068] truncate w-full text-center">{d.label}</span>
        </div>
      ))}
    </div>
  )
      ...
      <div className="space-y-1">
        {data.map((d, i) => (
          <div key={i} className="flex items-center gap-2 text-xs">
            ...
            <span className="text-[#7d92b0]">{d.label}</span>
    ...
    <div className="space-y-3">
      <div className="flex gap-2">
        <textarea
          ...
          rows={4}
          className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-[#c8d6e8] text-xs font-mono focus:outline-hidden focus:border-[#e8002d]/50 resize-y"
        ...
        <div className="flex flex-col gap-2 shrink-0">
          ...
          <button
            onClick={onRun}
            className="flex items-center gap-1 px-3 py-1.5 rounded-sm bg-[#e8002d] text-white text-xs hover:bg-[#c00025] transition-colors"
          >
      ...
      {cell.results && (
        <div className="overflow-x-auto rounded-sm border border-[#1e2d42]">
          <table className="w-full text-xs">
            <thead className="bg-[#1e2d42]">
              <tr>
                {cell.results.columns.map(col => (
                  <th key={col} className="px-3 py-2 text-left text-[#7d92b0] font-semibold">{col}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {cell.results.rows.length === 0 ? (
                <tr><td colSpan={cell.results.columns.length} className="px-3 py-4 text-center text-[#3d5068]">結果なし</td></tr>
              ) : (
                cell.results.rows.map((row, i) => (
                  <tr key={i} className="border-t border-[#1e2d42] hover:bg-[#0d1220]">
                    {row.map((val, j) => (
...
function VisualizationCell({ cell, prevResults }: { cell: NotebookCell; prevResults?: QueryResult }) {
  ...
  return (
    <div>
      <p className="text-xs text-[#7d92b0] mb-3">直前のクエリ結果を可視化</p>
      {cell.chart_type === 'pie' ? (
...
function NoteCell({ cell, onUpdate }: { cell: NotebookCell; onUpdate: (u: Partial<NotebookCell>) => void }) {
  return (
    <textarea
      ...
      rows={5}
      className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-[#c8d6e8] text-sm focus:outline-hidden focus:border-[#e8002d]/50 resize-y"
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-[#7d92b0] mb-1 block">ファイル名 / IOC</label>
          <input
            value={cell.artifact_filename ?? ''}
            onChange={e => onUpdate({ artifact_filename: e.target.value })}
            className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1.5 text-sm text-white focus:outline-hidden focus:border-[#e8002d]/50"
            placeholder="filename.exe or IP/domain"
          />
        </div>
        <div>
          <label className="text-xs text-[#7d92b0] mb-1 block">ハッシュ (MD5/SHA256)</label>
          <input
            value={cell.artifact_hash ?? ''}
            onChange={e => onUpdate({ artifact_hash: e.target.value })}
            className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1.5 text-xs font-mono text-white focus:outline-hidden focus:border-[#e8002d]/50"
            placeholder="a1b2c3..."
          />
        </div>
      </div>
      <div>
        <label className="text-xs text-[#7d92b0] mb-1 block">エンリッチメント結果</label>
        <div className="bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-xs text-[#c8d6e8]">
...
function FindingCell({ cell, onUpdate }: { cell: NotebookCell; onUpdate: (u: Partial<NotebookCell>) => void }) {
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-[#7d92b0] mb-1 block">発見事項タイトル</label>
          <input
        ...
        <div>
          <label className="text-xs text-[#7d92b0] mb-1 block">深刻度</label>
          <select
      ...
      <div>
        <label className="text-xs text-[#7d92b0] mb-1 block">証拠 (セル参照)</label>
        <input
      ...
      <div>
        <label className="text-xs text-[#7d92b0] mb-1 block">推奨アクション</label>
        <input
...
function AddCellBar({ onAdd }: { onAdd: (type: CellType) => void }) {
  ...
  return (
    <div className="flex items-center gap-1 py-1 group">
      <div className="flex-1 h-px bg-[#1e2d42] group-hover:bg-[#2e4060] transition-colors" />
      <div className="flex gap-1">
        {types.map(({ type, label, icon: Icon }) => (
          <button
            key={type}
            onClick={() => onAdd(type)}
            className={`flex items-center gap-1 px-2 py-1 rounded-sm text-[10px] border transition-all hover:scale-105
...
function NewNotebookModal({ onClose, onCreate }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-6 w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-bold text-white">新規ノートブック</h2>
          <button onClick={onClose} className="p-1.5 rounded-sm text-[#7d92b0] hover:text-white hover:bg-[#1e2d42] transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        <div className="space-y-4">
          ...
          <div>
            <label className="text-xs text-[#7d92b0] mb-1 block">仮説テンプレート</label>
            <select
              value={template}
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            ...
            disabled={!title}
            className="flex-1 py-2 rounded-sm bg-[#e8002d] text-white text-sm font-medium hover:bg-[#c00025] transition-colors disabled:opacity-40"
          >
            作成
          </button>
        </div>
...
function ShareModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-6 w-full max-w-sm shadow-2xl">
        <div className="flex items-center justify-between mb-5">
        ...
        <div className="space-y-2 mb-4">
          {users.map(u => (
            <label key={u} className="flex items-center gap-3 p-2 rounded-sm hover:bg-[#1e2d42] cursor-pointer">
              <input
                type="checkbox"
              ...
              />
              <div className="w-6 h-6 rounded-full bg-linear-to-br from-[#1a6bff] to-[#0044cc] flex items-center justify-center text-[9px] font-bold text-white">
                {u[0]}
              </div>
        ...
        <button
          onClick={onClose}
          className="w-full py-2 rounded-sm bg-[#e8002d] text-white text-sm font-medium hover:bg-[#c00025] transition-colors"
        >
...
export default function ThreatHuntingNotebookPage() {
  ...
  const deleteNotebook = async (id: string) => {
    if (!(await persist('ノートブックの削除', `/api/v1/threat-hunting/notebooks/${id}`, { method: 'DELETE' }))) return
  ...
  if (!selected) return (
    <div className="min-h-screen bg-[#070d19] text-white flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-16 flex flex-col items-center gap-4 text-center">
          <FileText className="w-12 h-12 text-[#3d5068]" />
          <p className="text-[#e2e8f4] font-semibold">ノートブックがありません</p>
          <p className="text-[#7d92b0] text-sm">「新規ノートブック」ボタンから作成してください。</p>
          <button
            onClick={() => setShowNewModal(true)}
            className="mt-2 flex items-center gap-2 px-4 py-2 rounded-sm bg-[#e8002d] text-white hover:bg-[#c00025] text-sm font-medium transition-colors"
          >
            <Plus className="w-4 h-4" /> 新規ノートブック
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#070d19] text-white flex flex-col">
      ...
      <div className="border-b border-[#1e2d42] px-6 py-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#7d92b0]">Threat Hunting Notebook — 調査ワークスペース</p>
          </div>
        </div>
        <button
          onClick={() => setShowNewModal(true)}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 shrink-0 border-r border-[#1e2d42] overflow-y-auto">
              ...
              <div key={nb.id} className="relative group">
                <button
                  ...
                  className={`w-full text-left p-3 rounded-sm border transition-colors
                    ${selectedId === nb.id ? 'bg-[#1d2f4a] border-[#2e4060]' : 'bg-[#0d1220] border-[#1e2d42] hover:border-[#2e4060]'}`}
                >
                  ...
                  <p className="text-[10px] text-[#3d5068]">{nb.created_at} · {displayUser(nb.assigned_to)}</p>
                </button>
                <button
                  onClick={e => {
                    e.stopPropagation()
                  ...
                  className="absolute top-2 right-2 p-1 rounded-sm text-[#3d5068] hover:text-[#e8002d] hover:bg-[#e8002d]/10 opacity-0 group-hover:opacity-100 transition-all"
                  title="削除"
                >
          ...
          <div className="max-w-4xl mx-auto p-6 space-y-6">
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
              <div className="flex items-start justify-between gap-4 mb-3">
                {editingTitle ? (
                  <input
                    ...
                    autoFocus
                    className="flex-1 bg-transparent border-b border-[#e8002d]/50 text-white text-lg font-bold focus:outline-hidden"
                  />
                ) : (
                  <h2
                    className="flex-1 text-lg font-bold text-white cursor-text hover:text-[#e2e8f4] transition-colors"
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                {selected.tags.map(tag => (
                  <span key={tag} className="px-1.5 py-0.5 bg-[#1e2d42] text-[#7d92b0] rounded-sm text-[10px]">#{tag}</span>
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5 space-y-4">
              <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                <span className="w-1 h-4 bg-[#e8002d] rounded-full" /> 仮説 / Hypothesis
              </h3>
              {[
                ...
                <div key={field}>
                  <label className="text-xs text-[#7d92b0] mb-1 block">{label}</label>
                  <textarea
                    value={(selected as any)[field] ?? ''}
                    onChange={e => updateNotebook({ [field]: e.target.value })}
                    rows={3}
                    className="w-full bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 text-sm text-[#c8d6e8] focus:outline-hidden focus:border-[#e8002d]/50 resize-y"
                  />
                </div>
              ))}
            </div>
            ...
            {selected.cells.map((cell, idx) => (
              <div key={cell.id}>
                <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg overflow-hidden">
                  {/* Cell header */}
                  <div className="flex items-center gap-2 px-4 py-2 border-b border-[#1e2d42] bg-[#0a1018]">
                    <button onClick={() => toggleCollapse(cell.id)} className="text-[#3d5068] hover:text-[#7d92b0]">
                    ...
                    <span className="text-[10px] text-[#3d5068] font-mono">#{idx + 1}</span>
                    <div className="flex-1" />
                    <button onClick={() => moveCell(cell.id, 'up')} className="p-1 text-[#3d5068] hover:text-[#7d92b0]"><ArrowUp className="w-3.5 h-3.5" /></button>
                    <button onClick={() => moveCell(cell.id, 'down')} className="p-1 text-[#3d5068] hover:text-[#7d92b0]"><ArrowDown className="w-3.5 h-3.5" /></button>
                    <button onClick={() => removeCell(cell.id)} className="p-1 text-[#3d5068] hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                  {/* Cell body */}
                  {!cell.collapsed && (
                    <div className="p-4">
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5 space-y-4">
              <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                <span className="w-1 h-4 bg-[#e8002d] rounded-full" /> 結論 / Conclusion
              </h3>
              <div>
                <label className="text-xs text-[#7d92b0] mb-1 block">サマリー</label>
                <textarea
              ...
              <div>
                <label className="text-xs text-[#7d92b0] mb-1 block">判定 (Verdict)</label>
                <div className="flex gap-2">
                  {(['positive_finding', 'negative', 'inconclusive'] as Verdict[]).map(v => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-sm border text-xs font-medium transition-colors
                        ${selected.verdict === v ? VERDICT_STYLES[v] : 'border-[#1e2d42] text-[#3d5068] hover:text-[#7d92b0]'}`}
                    >
                      {VERDICT_LABELS[v]}
                    </button>
                  ))}
              ...
              <div>
                <label className="text-xs text-[#7d92b0] mb-2 block">推奨事項</label>
                <div className="space-y-2">
                  {selected.recommendations.map((rec, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <span className="text-xs text-[#e8002d] font-bold">{i + 1}.</span>
                      <input
                        value={rec}
                        onChange={e => {
                        ...
                        className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-sm px-2 py-1 text-xs text-[#c8d6e8] focus:outline-hidden focus:border-[#e8002d]/50"
                      />
                      <button
                        onClick={() => updateNotebook({ recommendations: selected.recommendations.filter((_, j) => j !== i) })}
                        className="p-1 text-[#3d5068] hover:text-red-400"
                      >

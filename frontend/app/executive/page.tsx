  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-6 mt-2 text-xs text-[#7d92b0]">
        <span className="flex items-center gap-1.5"><span className="w-4 h-0.5 bg-[#e8002d] inline-block rounded" />インシデント件数</span>
        <span className="flex items-center gap-1.5"><span className="w-4 h-0.5 bg-[#ffd740] inline-block rounded" />アラート件数</span>
      </div>
    </div>
  )
}
        ...
        <div className="bg-[#0d1220] border border-[#1e2d42] rounded-xl p-5">
          ...
          <div className="flex flex-wrap gap-2">
            {compliance.frameworks.map(f => (
              <span key={f.name} className="px-2 py-1 bg-[#1e2d42] text-[#e2e8f4] text-[10px] font-medium rounded border border-[#2d3f58]">
                {f.name}
              </span>
            ))}
          </div>
            ...
            {topRisks.map((risk, i) => (
              <div key={i} className="p-3 bg-[#070d19] rounded-lg border border-[#1e2d42]">
                <div className="flex items-start gap-2 mb-2">
                  <span className="w-5 h-5 rounded-full bg-[#1e2d42] flex items-center justify-center text-[10px] font-bold text-[#7d92b0] flex-shrink-0 mt-0.5">
                    {i + 1}
                  </span>
                ...
                <div className="flex items-center gap-2 pl-7">
                  ...
                  <span className={`px-1.5 py-0.5 rounded border text-[9px] font-medium ${RISK_COLOR[risk.impact]}`}>{RISK_LABEL[risk.impact]}</span>
                  <span className="text-[10px] text-[#7d92b0]">発生可能性:</span>
                  <span className={`px-1.5 py-0.5 rounded border text-[9px] font-medium ${RISK_COLOR[risk.likelihood]}`}>{RISK_LABEL[risk.likelihood]}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <ul className="space-y-1.5">
            {investments.slice(0, 3).map((inv, i) => (
              <li key={i} className="flex items-start gap-2 text-xs text-[#e2e8f4]">
                <ChevronRight className="w-3.5 h-3.5 text-[#e8002d] flex-shrink-0 mt-0.5" />
                <span>{inv.item}</span>
              </li>
            ))}
          </ul>
              ...
              <tbody>
                {compliance.frameworks.map(f => (
                  <tr key={f.name} className="border-b border-[#1e2d42]/40 hover:bg-[#1e2d42]/20">
                    ...
                    <td className="py-2.5 px-2">
                      <span className={`px-2 py-0.5 rounded border text-[10px] font-medium ${scoreStatusColor(f.score)}`}>{f.status}</span>
                    </td>

          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <button onClick={() => setWeekOffset(w => w-1)} className="text-[#7d92b0] hover:text-white px-3 py-1 border border-[#1e2d42] rounded">◀</button>
              ...
              <div className="flex gap-3 text-xs">
                {(Object.entries(ROLE_COLORS) as [Role, string][]).map(([r, cls]) => (
                  <span key={r} className={`px-2 py-0.5 rounded ${cls}`}>{r}</span>
                ))}
              </div>
            </div>
                      ...
                      {DAYS.map(day => {
                        ...
                        return (
                          <td key={day} className="px-2 py-3 text-center">
                            {name ? <span className={`px-2 py-0.5 rounded text-xs font-medium ${roleColor(name)}`}>{name}</span>
            ...
            {showEndConfirm && (
              <div className="bg-red-900/30 border border-red-700 rounded-lg p-4 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setShowEndConfirm(false)} className="px-3 py-1 border border-[#1e2d42] rounded text-sm text-[#7d92b0]">キャンセル</button>
                  <button className="px-3 py-1 bg-red-700 rounded text-sm text-white">確認して終了</button>
                </div>
              </div>
            )}
                ...
                <tbody>
                  {analysts.filter(a => a.status !== 'offline').map(a => (
                    <tr key={a.id} className="border-b border-[#1e2d42] hover:bg-[#1e2d42]/20">
                      ...
                      <td className="px-3 py-3"><span className={`px-2 py-0.5 rounded text-xs ${ROLE_COLORS[a.role]}`}>{a.role}</span></td>
                      ...
                      <td className="px-3 py-3">
                        {assignTarget === a.id ? (
                          <select className="bg-[#1e2d42] text-white text-xs rounded px-2 py-1 border border-[#1e2d42]" onChange={() => setAssignTarget(null)}>
                            <option>-- アラート選択 --</option>
                            <option>ALERT-2891 (Critical)</option>
                            <option>ALERT-2888 (High)</option>
                          </select>
                        ) : (
                          <button onClick={() => setAssignTarget(a.id)} className="text-xs bg-blue-900 text-blue-300 hover:bg-blue-800 px-2 py-1 rounded">割り当て</button>
                        )}
                      </td>
                    </tr>
                  ))}
              ...
              <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5 space-y-4">
                ...
                <div>
                  ...
                  <textarea value={reportNotes} onChange={e => setReportNotes(e.target.value)}
                    className="w-full bg-[#070d19] border border-[#1e2d42] rounded-lg p-3 text-sm text-white placeholder-[#7d92b0] focus:outline-none focus:border-[#e8002d] h-28 resize-none"
                ...
                {reports.map(r => (
                  <div key={r.id} className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="bg-orange-900 text-orange-300 text-xs px-2 py-0.5 rounded">{r.open_items}件未解決</span>
                    </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-5">
              ...
              <div className="flex gap-4 mt-2 text-xs text-[#7d92b0]">
                <span><span className="inline-block w-3 h-3 bg-blue-600 rounded mr-1" />実際の人数</span>
                <span><span className="inline-block w-3 h-3 bg-[#1e2d42] border border-dashed border-[#7d92b0] rounded mr-1" />必要人数</span>
              </div>
            </div>
                    ...
                    return (
                      <tr key={a.id} className="border-b border-[#1e2d42] hover:bg-[#1e2d42]/20">
                        <td className="px-4 py-2">
                          ...
                          <span className={`text-xs px-1.5 py-0.5 rounded ${ROLE_COLORS[a.role]}`}>{a.role}</span>
                        </td>
                        {SKILL_KEYS.map(k => (
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={pos.title} className={`bg-[#0d1220] border ${pos.border} rounded-lg p-4`}>
                  ...
                  <button className="mt-3 text-xs text-blue-400 hover:text-blue-300 border border-blue-900 px-3 py-1 rounded hover:bg-blue-900/30">詳細を見る</button>
                </div>
              ))}
            </div>
          </div>

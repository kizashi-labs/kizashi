    ...
    <div className="min-h-screen bg-[#070d19] text-white">
      ...
      <div className="border-b border-falcon-border px-6 py-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="flex items-center gap-2 px-4 py-2 bg-falcon-red hover:bg-[#c0001f] rounded-lg text-sm font-medium transition-colors">
        ...
        <div className="grid grid-cols-4 gap-4">
          {stats.map(s => (
            <div key={s.label} className="bg-falcon-surface border border-falcon-border rounded-lg p-3 flex items-center gap-3">
          ...
          {FEATURED.map(f => {
            ...
            return (
              <button key={f.id} onClick={() => art && setSelectedArticle(art)}
                className="bg-falcon-surface border border-falcon-border hover:border-falcon-red/50 rounded-xl p-4 text-left transition-colors group">
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${f.badgeCls}`}>{f.badge}</span>
                  <f.icon className="w-4 h-4 text-falcon-muted group-hover:text-falcon-red transition-colors" />
                </div>
      ...
      <div className="flex" style={{ minHeight: 'calc(100vh - 260px)' }}>
        {/* サイドバー (30%) */}
        <aside className="w-[30%] border-r border-falcon-border p-4 shrink-0">
          {/* 検索 */}
          <div className="relative mb-4">
            ...
            <input
              ...
              placeholder="記事を検索..."
              className="w-full bg-falcon-surface border border-falcon-border rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder-falcon-muted focus:outline-hidden focus:border-falcon-red/60"
            />
          </div>
            ...
            {CATEGORIES.map(cat => (
              <div key={cat.name}>
                <button onClick={() => toggleCat(cat.name)}
                  className="w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-falcon-border/40 transition-colors text-sm">
                  <span className="flex items-center gap-2">
                    <span>{cat.icon}</span>
                    <span className="font-medium">{cat.name}</span>
                    <span className="text-xs text-falcon-muted">({cat.count})</span>
                  </span>
                  {cat.children && cat.children.length > 0 && (
                    expandedCats.includes(cat.name)
                      ? <ChevronRight className="w-3.5 h-3.5 text-falcon-muted rotate-90 transition-transform" />
                      : <ChevronRight className="w-3.5 h-3.5 text-falcon-muted transition-transform" />
                  )}
                </button>
                {expandedCats.includes(cat.name) && cat.children && cat.children.length > 0 && (
                  <div className="ml-4 mt-1 space-y-0.5 pl-3 border-l border-falcon-border">
                    {cat.children.map(child => (
                      <button key={child.name}
                        className="w-full flex items-center justify-between px-2 py-1.5 rounded-sm hover:bg-falcon-border/40 transition-colors text-xs text-falcon-muted hover:text-white">
        ...
        <main className="flex-1 p-6 overflow-auto">
          {!selectedArticle ? (
            /* 記事グリッド表示 */
            <div>
              <h2 className="text-sm font-semibold text-falcon-muted uppercase tracking-wide mb-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                {filteredArticles.map(article => (
                  <button key={article.id} onClick={() => setSelectedArticle(article)}
                    className="bg-falcon-surface border border-falcon-border hover:border-falcon-red/40 rounded-xl p-4 text-left transition-colors group">
                    ...
                    <h3 className="text-sm font-semibold mb-1.5 leading-snug group-hover:text-falcon-red transition-colors">{article.title}</h3>
                    <p className="text-xs text-falcon-muted mb-3 line-clamp-2">{article.summary}</p>
                    <div className="flex items-center justify-between">
                      <div className="flex flex-wrap gap-1">
                        {article.tags.slice(0, 2).map(t => (
                          <span key={t} className="text-xs px-1.5 py-0.5 bg-falcon-border text-falcon-muted rounded-sm">{t}</span>
                        ))}
                      </div>
                      <div className="flex items-center gap-1 text-xs text-falcon-muted">
            ...
            <div>
              <button onClick={() => setSelectedArticle(null)}
                className="flex items-center gap-1.5 text-sm text-falcon-muted hover:text-white mb-5 transition-colors">
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                  ...
                  {selectedArticle.tags.map(t => (
                    <span key={t} className="text-xs px-2 py-0.5 bg-falcon-border text-falcon-muted rounded-full">{t}</span>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-4 text-xs text-falcon-muted">
                  ...
                  <div className="ml-auto flex items-center gap-2">
                    <button className="flex items-center gap-1.5 px-3 py-1 bg-falcon-border hover:bg-[#2a3f5f] rounded-lg transition-colors">
                      <Edit className="w-3.5 h-3.5" /> 編集
                    </button>
                    <button onClick={() => toggleFav(selectedArticle.id)}
                      className={`flex items-center gap-1.5 px-3 py-1 rounded-lg transition-colors ${favorited.includes(selectedArticle.id) ? 'bg-yellow-900/40 text-yellow-300 border border-yellow-700/40' : 'bg-falcon-border hover:bg-[#2a3f5f]'}`}>
              ...
              <div className="bg-falcon-surface border border-falcon-border rounded-xl p-6 mb-6">
                <p className="text-sm text-falcon-muted mb-5 leading-relaxed border-l-2 border-falcon-red pl-4">{selectedArticle.summary}</p>
                {selectedArticle.content ? (
                  <div className="space-y-5">
                    {selectedArticle.content.map((section, i) => (
                      <div key={i}>
                        <h3 className="text-base font-semibold mb-2 text-white">{section.heading}</h3>
                        <p className="text-sm text-falcon-muted leading-relaxed">{section.body}</p>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-sm text-falcon-muted italic">コンテンツは準備中です。</p>
                )}
              </div>
              ...
              {selectedArticle.related && selectedArticle.related.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold text-falcon-muted uppercase tracking-wide mb-3">関連記事</h3>
                    ...
                    {selectedArticle.related.map(rel => {
                      ...
                      return (
                        <button key={rel} onClick={() => relArt && setSelectedArticle(relArt)}
                          className="w-full flex items-center gap-3 p-3 bg-falcon-surface border border-falcon-border hover:border-falcon-red/40 rounded-lg text-left transition-colors group">
                          <FileText className="w-4 h-4 text-falcon-muted group-hover:text-falcon-red transition-colors shrink-0" />
                          <span className="text-sm">{rel}</span>
                          <ChevronRight className="w-4 h-4 text-falcon-muted ml-auto" />
                        </button>
                      )
                    })}
                  </div>

...
export default function SocTicketsPage() {
  ...
  const { data: stats } = useQuery<TicketStats | null>({
    ...
    queryFn: () => apiFetch<TicketStats>('/api/v1/soc/tickets/stats').catch(() => null),
    staleTime: 30000,
    retry: false,
  })
  ...
  const { data: tickets, isLoading } = useQuery<SocTicket[]>({
    ...
    queryFn: () => apiFetchList<SocTicket>('/api/v1/soc/tickets').catch(() => []),
    staleTime: 30000,
    retry: false,
  })
  ...
  const { data: users } = useQuery<UserOption[]>({
    queryKey: ['soc-users'],
    queryFn: () => apiFetchList<UserOption>('/api/v1/users').catch(() => []),
    staleTime: 60000,
    retry: false,
  })
  ...
  const { data: comments } = useQuery<TicketComment[]>({
    ...
    queryFn: () => apiFetchList<TicketComment>(`/api/v1/soc/tickets/${selectedTicket?.id}/comments`).catch(() => []),

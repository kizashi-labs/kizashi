...
export default function CloudAssetsPage() {
  ...
  const { data: providerConfigs = [] } = useQuery<ProviderConfig[], Error, ProviderConfig[]>({
    ...
    queryFn: (): Promise<ProviderConfig[]> => apiFetch('/api/v1/cloud-assets/providers').catch(() => []) as Promise<ProviderConfig[]>,
    staleTime: 60_000,
  })
  ...
  const riskCalcMutation = useMutation({
    mutationFn: (id: string) =>
      apiFetch(`/api/v1/cloud-assets/${id}/risk`, { method: 'POST' }).catch(() => ({})),
  })

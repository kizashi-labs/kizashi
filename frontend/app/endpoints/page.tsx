        ...
        {(['critical', 'high', 'medium', 'low'] as const).map(level => {
          ...
          return (
            <div key={level} className="bg-[#111827] rounded-xl border border-[#1e2d42] p-4 flex items-center gap-3">
              <div className="w-3 h-8 rounded-full flex-shrink-0" style={{ background: style.bar }} />
              <div>
          ...
          {ranked.map((a, idx) => {
            ...
            return (
              <div key={a.id} className="flex items-center gap-3">
                <span className="text-xs text-[#5a6a7a] w-5 text-right flex-shrink-0">{idx + 1}</span>
                <Link href={`/endpoints/${a.id}`}
                  className="text-sm text-[#c9d6e8] hover:text-white transition-colors w-40 truncate flex-shrink-0">
                  {a.hostname}
                </Link>
                ...
                <span className="text-xs font-bold w-8 text-right flex-shrink-0" style={{ color: style.bar }}>
                  {a.risk?.score}
                </span>
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded border flex-shrink-0 ${style.badge}`}>
              ...
              >
                <div className="w-[22%] min-w-0">
                  ...
                  {agent.status === 'isolated' && (
                    <span className="text-xs bg-red-900/40 text-red-300 border border-red-700/50 px-1.5 py-0.5 rounded">隔離中</span>
                  )}
                </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-[#8899aa]
                       bg-[#161f33] border border-[#1e2d42] rounded-lg hover:bg-[#1d2f4a]
      ...
      <div className="flex gap-3 mb-4 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-sm border border-[#1e2d42] rounded-xl
                       bg-[#111827] text-white placeholder-[#5a6a7a]
        ...
        <select
          ...
          className="text-sm border border-[#1e2d42] rounded-xl px-3 py-2
                     bg-[#111827] text-[#8899aa] focus:outline-none focus:border-[#1a6bff]"
        >
              ...
              {agents.map(agent => {
                ...
                <tr key={agent.id}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {isOffline && (
                        <span className="flex-shrink-0 w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" title="オフライン" />
                      )}
                      <Link href={`/endpoints/${agent.id}`}
                    ...
                    {agent.status === 'isolated' && (
                      <span className="ml-2 text-xs bg-red-900/40 text-red-300 border border-red-700/50 px-1.5 py-0.5 rounded">
                        隔離中
                      </span>
                    )}
                  </td>

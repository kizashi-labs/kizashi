    ...
    <div className="min-h-screen bg-[#070d19] p-6 space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-gradient-to-br from-[#e8002d] to-[#a80020] flex-shrink-0">
        ...
        <button
          onClick={() => refetch()}
          disabled={isLoading}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#0d1220] border border-[#1e2d42]
                     text-[#7d92b0] hover:text-white hover:border-[#7d92b0]/40 text-sm transition-colors
                     disabled:opacity-40"
        >
      ...
      <div className="flex flex-wrap gap-2">
        {['all', ...CONTINENT_ORDER].map(cont => {
          ...
          return (
            ...
            >
              {label}
              <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${active ? 'bg-[#e8002d]/30 text-[#e8002d]' : 'bg-[#1e2d42] text-[#7d92b0]'}`}>
                {count}
              </span>
            </button>
          )
            ...
            <>
              <div className="px-4 py-3 border-b border-[#1e2d42] flex items-center justify-between">
                ...
                <button
                  ...
                  className="p-1 rounded hover:bg-[#1e2d42] text-[#7d92b0] hover:text-white transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="overflow-y-auto max-h-[480px]">
                {selectedCountryData.agents.map(agent => (
                  <div key={agent.id} className="flex items-center gap-3 px-4 py-3 border-b border-[#1e2d42]/50 hover:bg-[#111928] transition-colors">
                    <span className={`w-2 h-2 rounded-full flex-shrink-0 ${agent.status === 'online' ? 'bg-[#00c853]' : 'bg-[#7d92b0]'}`} />
                    ...
                    <Link
                      ...
                      className="flex-shrink-0 flex items-center gap-1 text-[10px] text-[#1a6bff] hover:text-blue-300 transition-colors"
                    >
                      詳細

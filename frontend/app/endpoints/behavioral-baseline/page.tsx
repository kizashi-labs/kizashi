        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1e2d42]">
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className={`inline-flex items-center px-2 py-0.5 rounded border text-xs font-medium ${statusConfig[ep.baseline_status].color}`}>
                ...
                <div key={p.name} className={`flex items-center gap-3 p-3 rounded-lg border ${p.is_rare ? 'border-[#e8002d]/40 bg-[#e8002d]/5' : 'border-[#1e2d42] bg-[#070d19]'}`}>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {p.is_rare && (
                        <span className="px-1.5 py-0.5 bg-[#e8002d]/20 text-[#e8002d] text-xs rounded border border-[#e8002d]/30">レア</span>
                      )}
                    </div>
                ...
                <div key={i} className="flex items-center justify-between p-3 bg-[#070d19] border border-[#1e2d42] rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="ml-2 px-1.5 py-0.5 bg-blue-500/20 text-blue-300 text-xs rounded">{d.protocol}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    <div className="flex gap-0.5">
                      {[10, 20, 40, 60, 80, 100].map(v => (
                        <div key={v} className="w-5 h-3 rounded-sm" style={{ backgroundColor: `rgba(232, 0, 45, ${v / 100 * 0.8 + 0.1})` }} />
                      ))}
                    </div>
              ...
              {ep.recent_deviations.length === 0 ? (
                ...
                <div key={dev.id} className={`p-4 rounded-lg border ${dev.severity === 'critical' ? 'border-[#e8002d]/40 bg-[#e8002d]/5' : 'border-[#1e2d42] bg-[#070d19]'}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`px-2 py-0.5 rounded border text-xs font-medium ${severityColor[dev.severity]}`}>
                          {dev.severity.toUpperCase()}
                        </span>
                        <span className="text-[#7d92b0] text-xs bg-[#1e2d42] px-2 py-0.5 rounded">{dev.category}</span>
                      </div>
          ...
          {activeTab === 'exclusions' && (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <input
                  className="flex-1 bg-[#070d19] border border-[#1e2d42] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#e8002d]/60 font-mono"
                  value={newExclusion}
              ...
              {filtered.map(ep => {
                ...
                return (
                  <tr key={ep.id} className="hover:bg-[#0a1020] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded border text-xs font-medium ${sc.color}`}>

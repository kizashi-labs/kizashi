      ...
      <div className="flex flex-wrap items-center gap-2">
        {(['ALL', 'create', 'terminate', 'inject', 'hollowing'] as const).map(t => (
          <button
            ...
            className={`px-2.5 py-1 text-xs rounded transition-colors ${filterType === t ? 'bg-[#1d2f4a] text-white' : 'bg-[#161f33] text-[#7d92b0] hover:text-white border border-[#1e2d42]'}`}
          >
            {t}
          </button>
        ))}
        <input
          ...
          className="bg-[#161f33] border border-[#1e2d42] rounded px-2.5 py-1 text-xs text-white placeholder-[#3d5068] focus:outline-none focus:border-[#e8002d]/40"
        />
        <input
          value={filterUser}
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2.5 py-1 text-xs rounded border transition-colors ${treeView ? 'bg-[#1d2f4a] text-white border-[#2a3f5c]' : 'border-[#1e2d42] text-[#7d92b0] hover:text-white'}`}
        >
          ...
          {roots.map((e, idx) => (
            <div key={e.id} className={`py-1 ${idx > 0 ? 'ml-4 border-l border-[#1e2d42] pl-3' : ''}`}>
              <div className="flex items-center gap-2">
                <span className={`px-1.5 py-0.5 rounded text-[10px] ${PROCESS_TYPE_STYLES[e.event_type]}`}>{e.event_type}</span>
              ...
              {idx % 3 === 0 && (
                <div className="ml-4 border-l border-[#1e2d42] pl-3 mt-1">
                  <div className="flex items-center gap-2 opacity-70">
                    <span className={`px-1.5 py-0.5 rounded text-[10px] ${PROCESS_TYPE_STYLES['create']}`}>create</span>
                    ...
                    <tr key={`${e.id}-exp`} className="border-b border-[#1e2d42]/50 bg-[#161f33]">
                      <td colSpan={9} className="px-4 py-3">
                        <div className="space-y-1">
                          ...
                          <p className="font-mono text-xs text-white bg-[#0d1220] border border-[#1e2d42] px-3 py-2 rounded break-all">{e.cmdline}</p>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          placeholder="パスでフィルタ..."
          className="bg-[#161f33] border border-[#1e2d42] rounded px-2.5 py-1 text-xs text-white placeholder-[#3d5068] focus:outline-none focus:border-[#e8002d]/40 min-w-[180px]"
        />
          ...
          <div className="space-y-1">
            {suspicious.slice(0, 3).map(e => (
              <div key={e.id} className="flex items-center gap-2 text-xs">
                <span className={`px-1.5 py-0.5 rounded text-[10px] ${FILE_TYPE_STYLES[e.event_type]}`}>{e.event_type}</span>
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex gap-1">
          <button onClick={() => setDnsView(false)} className={`px-2.5 py-1 text-xs rounded transition-colors ${!dnsView ? 'bg-[#1d2f4a] text-white' : 'bg-[#161f33] text-[#7d92b0] hover:text-white border border-[#1e2d42]'}`}>
            通信ログ
          </button>
          <button onClick={() => setDnsView(true)} className={`px-2.5 py-1 text-xs rounded transition-colors ${dnsView ? 'bg-[#1d2f4a] text-white' : 'bg-[#161f33] text-[#7d92b0] hover:text-white border border-[#1e2d42]'}`}>
            DNS クエリ
          </button>
        </div>
        {!dnsView && (['ALL', 'clean', 'suspicious', 'malicious'] as const).map(r => (
          <button
            ...
            className={`px-2.5 py-1 text-xs rounded transition-colors ${filterRep === r ? 'bg-[#1d2f4a] text-white' : 'bg-[#161f33] text-[#7d92b0] hover:text-white border border-[#1e2d42]'}`}
          >
            {r}
          </button>
        ))}
            ...
            <tbody>
              {filtered.slice(0, 50).map(e => (
                <tr key={e.id} className={`border-b border-[#1e2d42]/50 hover:bg-[#161f33] transition-colors ${e.reputation === 'malicious' ? 'bg-[#e8002d]/5' : ''}`}>
                  <td className="py-2 px-3 font-mono text-[#7d92b0] whitespace-nowrap">{fmt(e.timestamp)}</td>
                  <td className="py-2 px-3">
                    <span className={`px-1.5 py-0.5 rounded text-[10px] ${e.direction === 'inbound' ? 'bg-blue-900/30 text-blue-400 border border-blue-700/40' : 'bg-orange-900/30 text-orange-400 border border-orange-700/40'}`}>
                  ...
                  <td className="py-2 px-3 whitespace-nowrap">
                    <span className={`px-1.5 py-0.5 rounded text-[10px] ${REP_STYLES[e.reputation]}`}>{e.reputation}</span>
                  </td>
          ...
          <tbody>
            {filtered.slice(0, 50).map(e => (
              <tr key={e.id} className={`border-b border-[#1e2d42]/50 hover:bg-[#161f33] transition-colors ${e.is_persistence ? 'bg-orange-900/10' : ''}`}>
                <td className="py-2 px-3 font-mono text-[#7d92b0] whitespace-nowrap">{fmt(e.timestamp)}</td>
                <td className="py-2 px-3 whitespace-nowrap">
                  <span className={`px-1.5 py-0.5 rounded text-[10px] ${REG_TYPE_STYLES[e.event_type]}`}>{e.event_type}</span>
                </td>
      ...
      <div className="bg-[#0d1220] border border-[#1e2d42] rounded-lg p-4">
        ...
        <div className="relative">
          <div className="flex items-center gap-0 overflow-x-auto pb-2">
            {hours.map(h => (
              <div key={h} className="flex-shrink-0 w-8 text-center">
                <div className="text-[9px] text-[#3d5068] mb-1">{h.toString().padStart(2, '0')}</div>
                <div className={`h-6 w-7 rounded-sm mx-0.5 transition-colors ${
          ...
          <div className="flex gap-3 mt-2">
            <span className="flex items-center gap-1 text-[10px] text-green-400"><span className="w-2 h-2 rounded-sm bg-green-900/50 border border-green-700/50" />ログオン</span>
            <span className="flex items-center gap-1 text-[10px] text-[#ff4d6d]"><span className="w-2 h-2 rounded-sm bg-[#e8002d]/30 border border-[#e8002d]/30" />認証失敗</span>
            <span className="flex items-center gap-1 text-[10px] text-[#3d5068]"><span className="w-2 h-2 rounded-sm bg-[#161f33] border border-[#1e2d42]" />非アクティブ</span>
          </div>
        </div>
      </div>
          ...
          <tbody>
            {events.slice(0, 50).map(e => (
              <tr key={e.id} className={`border-b border-[#1e2d42]/50 hover:bg-[#161f33] transition-colors ${e.event_type === 'failed_logon' ? 'bg-[#e8002d]/5' : ''}`}>
                <td className="py-2 px-3 font-mono text-[#7d92b0] whitespace-nowrap">{fmt(e.timestamp)}</td>
                <td className="py-2 px-3 whitespace-nowrap">
                  <span className={`px-1.5 py-0.5 rounded text-[10px] ${USER_TYPE_STYLES[e.event_type]}`}>{e.event_type}</span>
                </td>
  ...
  return (
    <div className="min-h-screen bg-[#070d19] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          {iocMsg && (
            <span className="text-green-400 text-xs bg-green-900/30 border border-green-700/40 px-3 py-1.5 rounded">
              {iocMsg}
            </span>
          )}
          <button onClick={handleAddIOC} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0d1220] border border-[#1e2d42] text-[#7d92b0] text-xs rounded hover:border-[#2a3f5c] hover:text-white transition-colors">
            <Plus className="w-3.5 h-3.5" />
            IOCに追加
          </button>
          <button onClick={handleExport} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0d1220] border border-[#1e2d42] text-[#7d92b0] text-xs rounded hover:border-[#2a3f5c] hover:text-white transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="md:col-span-2 relative">
          <div className="flex items-center gap-2 bg-[#0d1220] border border-[#1e2d42] rounded-lg p-3">
            <Search className="w-4 h-4 text-[#7d92b0] flex-shrink-0" />
            <input
              value={search}
              onChange={e => { setSearch(e.target.value); setShowDropdown(true) }}
              onFocus={() => setShowDropdown(true)}
              placeholder="ホスト名または IP でエンドポイントを検索..."
              className="flex-1 bg-transparent text-sm text-white placeholder-[#3d5068] focus:outline-none"
            />
            {selectedEndpoint && (
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            disabled={live}
            className="flex-1 bg-[#0d1220] border border-[#1e2d42] rounded-lg px-3 py-2.5 text-white text-sm focus:outline-none focus:border-[#e8002d]/50 disabled:opacity-50"
          >

    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xs" onClick={onClose}>
      <div className="bg-falcon-surface border border-falcon-border rounded-xl w-full max-w-xl p-6 shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4 text-sm">
          <div className="flex flex-wrap gap-2">
            <span className={`px-2 py-0.5 rounded-sm text-xs border ${SEVERITY_COLORS[alert.severity]}`}>{alert.severity.toUpperCase()}</span>
            <span className="px-2 py-0.5 rounded-sm text-xs border bg-blue-500/20 text-blue-300 border-blue-500/30">{ANOMALY_TYPE_LABELS[alert.anomaly_type]}</span>
                  ...
                  <tbody className="divide-y divide-falcon-border/50">
                    {filteredDevices.map(d => (
                      <tr key={d.id} className={`hover:bg-[#070d19]/50 transition-colors ${d.known_vulns.length > 0 ? 'border-l-2 border-l-red-500/40' : ''}`}>
                        ...
                        <td className="px-3 py-3">
                          <span className={`px-1.5 py-0.5 rounded-sm text-[10px] border ${DEVICE_TYPE_COLORS[d.device_type]}`}>{d.device_type}</span>
                        </td>
                        <td className="px-3 py-3">
                        ...
                        <td className="px-3 py-3">
                          <span className={`px-1.5 py-0.5 rounded-sm text-[10px] border ${ZONE_COLORS[d.network_zone]}`}>{d.network_zone}</span>
                        </td>
                        <td className="px-3 py-3">
                  ...
                  {crossZone.map(d => (
                    <div key={d.id} className="bg-[#070d19] border border-yellow-500/20 rounded-lg px-3 py-2.5">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={`text-xs px-1.5 py-0.5 rounded-sm border ${ZONE_COLORS[d.network_zone]}`}>{d.network_zone}</span>
                      </div>

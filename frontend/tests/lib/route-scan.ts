 ...
 * 上限との突き合わせ。
 *
 * **`constName` は文言のためだけにあります。** 下げる指摘に定数の名前が
 * 入っていないと、`scripts/recalibrate_ratchets.py` は「自動では下げられ
 * ません（定数名が文言に無い）」として人に回します —— 公開版は有償機能の
 * ファイルを落とすぶん実測が必ず下がるので、**生成のたびにここの 6 本を
 * 手で測り直す**ことになっていました。名前を書けば、道具がそのまま下げます。
 *
 * 「上限を」と書くだけでは足りません。道具が探すのは
 * 「<英数字の名前> を <数> に下げてください」という形です。
 */
export function ceilingProblem(
  ...
  if (actual < ceiling) {
    return `${what}が ${actual} まで減りました。${constName} を ${actual} に下げてください`
  }
  return null
}

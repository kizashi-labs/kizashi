...
const RANDOM_NOT_A_MEASUREMENT: Record<string, RandomExcuse> = {
  'app/admin/agent-deployment/page.tsx': { why: 'UUID の生成です', guard: null },
  'app/insider-threat/page.tsx': { why: '起票する調査の整理番号です', guard: null },
  'app/threat-modeling/page.tsx': { why: 'キャンバス上に置く座標です', guard: null },
  'app/threat-map/page.tsx': {
    why: '線を流すアニメーションの進捗と、演出用の選択です',
    guard: null,
  },
  'app/admin/data-viz/page.tsx': {
    why:
      'ウィジェットの中身です。WidgetRenderer が USE_MOCK でないときに' +
      '「データ源が接続されていません」を出して打ち切ります',
    guard: 'function WidgetRenderer({ widget }: { widget: Widget }) {\n  if (!USE_MOCK) {',
  },
  'app/forensics/network/page.tsx': {
    why: 'randIp は MOCK_FLOWS と MOCK_PACKETS の中でだけ呼ばれます',
    guard: null,
  },
  'app/endpoints/behavioral-baseline/page.tsx': {
    why: 'generateHeatmap は MOCK_ENDPOINTS の中でだけ呼ばれます',
    guard: 'active_hours: generateHeatmap(),',
  },
  'app/soc/analytics/page.tsx': {
    why: 'MOCK_DATA を組み立てる関数です',
    guard: 'const MOCK_DATA: SOCAnalytics = {',
  },
  'app/reports/benchmark/page.tsx': {
    why: 'buildBenchmarkData は USE_MOCK のときだけ呼ばれます',
    guard: 'USE_MOCK ? buildBenchmarkData(comparison, industry) : EMPTY_BENCHMARK',
  },
  'app/rules/metrics/page.tsx': {
    why: 'buildDailyData は USE_MOCK のときだけ呼ばれます',
    guard: 'USE_MOCK ? buildDailyData(rules, days) : []',
  },
}

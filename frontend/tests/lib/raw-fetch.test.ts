...
// 先頭で静的に読んでおり、そちらに揃える。
import { frontendCalls, toPosix } from './route-scan'
...
function sourceFiles(dir: string, out: string[] = []): string[] {
  for (const name of readdirSync(dir)) {
    ...
    else if (name.endsWith('.tsx') || name.endsWith('.ts')) out.push(toPosix(p))
  }
  return out
}
...
describe('素の fetch', () => {
  ...
  it('走査が届いている', () => {
    expect(sites.length, '素の fetch が1つも見つかりません').toBeGreaterThan(5)
  ...
  it('経路の判定が素の fetch を見ている', () => {
    // 見本はどの版にも入っているページにする。/settings/cloud は公開版
    // （Free）が同梱しないので、パスワードリセット要求の素の fetch を使う。
    const calls = frontendCalls(readFileSync('app/auth/forgot-password/page.tsx', 'utf8'))
    expect(
      calls.some(c => c.path.includes('/auth/password-reset/request')),

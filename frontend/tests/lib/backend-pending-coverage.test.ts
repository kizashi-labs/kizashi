...
describe('バックエンドの無い画面の告知', () => {
  ...
  it('走査が両側に届いている', () => {
    ...
    expect(routes.size).toBeGreaterThan(250)
  ...
  const ANNOUNCED_BUT_ALIVE_CEILING = 0
  ...
  it('サイドバーの「準備中」が、同じ一覧から来ている', () => {
    ...
    expect(
      ceilingProblem('サイドバーに出ている準備中', pending.length, NAV_PENDING, 'NAV_PENDING'),

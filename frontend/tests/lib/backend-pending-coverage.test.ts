...
export function bannerProblems(
  ...
  graceful: Record<string, string> = {}
): string[] {
  const problems: string[] = []
  for (const route of deadScreens) {
    if (route in graceful) continue
  ...
  // **理由が古くなる側も見ます。** 宛先が実装されたのに理由が残っていると、
  // 次にその画面が死んだとき黙って通ります。
  for (const route of Object.keys(graceful)) {
    if (!deadScreens.includes(route)) {
      problems.push(
        `${route}: DEAD_BUT_GRACEFUL に載っていますが、もう死んでいません。` +
          '項目を消してください'
      )
    }
  }
...
/**
 * 呼び出しは届かないが、画面がそれを前提に degrade しているもの。
 *
 * **「準備中」の告知とは別物です。** 告知はバナーを出しますが、ここは
 * 出しません —— 画面自体は動いていて、その一部が静かに空になるだけだから
 * です。バナーを出すと、**動いているものを利用者が避けます。**
 *
 * 理由を書かないと登録できません。宛先が実装されたら上の突き合わせで
 * 落ちるので、古い理由が残り続けることはありません。
 */
export const DEAD_BUT_GRACEFUL: Record<string, string> = {
  '/login':
    'SSO 提供者の一覧 (`GET /api/v1/auth/sso/providers`) だけを叩きます。' +
    'この版のサーバに SSO のルートはありません（router.go に sso が 0 件）。' +
    '404 を受けると一覧は空のままで、SSO ボタンは1つも描かれません。' +
    '**ログイン自体は lib/auth.tsx の POST /api/v1/auth/login で、そちらは' +
    '実在します。** 画面に「準備中」と出すと、動いているログインを' +
    '利用者が避けます。',
}
...
describe('バックエンドの無い画面の告知', () => {
  ...
  it('API がすべて届かない画面は「準備中」と告知している', () => {
    const problems = bannerProblems(deadScreens, announced, DEAD_BUT_GRACEFUL)
  ...
  it.each([
    ...
    // degrade するものとして理由つきで外してある側。
    { name: 'degrade 済み', dead: ['/a'], full: [], partial: [], graceful: { '/a': 'r' }, want: 0 },
    // **理由が古くなった側。** これが無いと、宛先が実装されたあとも
    // 除外が効き続け、次にその画面が死んだとき黙って通ります。
    { name: 'degrade の理由が古い', dead: [], full: [], partial: [], graceful: { '/a': 'r' }, want: 1 },
  ])('判定: $name', ({ dead, full, partial, graceful, want }) => {
    expect(

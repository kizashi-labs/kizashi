  ...
  return (
    <header className="topbar-blur sticky top-0 z-40 flex items-center gap-4 px-5 h-11 border-b border-[#1e2d42]">
      ...
      <div className="flex items-center gap-1.5 text-xs flex-1 min-w-0">
        ...
        {/* 前置きの除外一覧は先頭スラッシュ無しで持つ — 公開版生成の trim-frontend は
            クォート内の「/route」形しか消さないので、この書き方なら行ごと巻き添えにならない */}
        {pathname.split('/').length > 2 && pathname.split('/')[2] &&
          !['admin', 'reports/schedules', 'agents/deploy'].some(p => pathname.startsWith('/' + p)) && (
          <>
            <ChevronRight className="w-3 h-3 text-[#3d5068]" />

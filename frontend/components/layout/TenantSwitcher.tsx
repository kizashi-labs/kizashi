...
export function TenantSwitcher() {
  ...
  return (
    <div className="px-3 pt-3 pb-1" ref={ref}>
      <button
        onClick={() => setOpen(v => !v)}
        className="w-full flex items-center gap-2 bg-[#070d19] border border-[#1e2d42] rounded-sm px-3 py-2 hover:border-[#7d92b0]/40 hover:bg-[#0d1220] transition-all duration-150 group"
        title="テナントを切り替え"
        disabled={isLoading}
      >
        <Building2 className="w-3.5 h-3.5 shrink-0 text-[#3d5068] group-hover:text-[#7d92b0] transition-colors" />
        ...
        <ChevronDown
          className={`w-3 h-3 shrink-0 text-[#3d5068] transition-transform duration-150 ${open ? 'rotate-180' : ''}`}
        />
      </button>

      {open && tenants.length > 0 && (
        <div className="mt-1 bg-[#0d1220] border border-[#1e2d42] rounded-sm shadow-lg overflow-hidden z-50">
          {tenants.map(t => {
            ...
            return (
              ...
              >
                <Building2 className={`w-3.5 h-3.5 shrink-0 ${isSelected ? 'text-[#e8002d]' : 'text-[#3d5068]'}`} />
                ...
                {isSelected && (
                  <Check className="w-3 h-3 text-[#e8002d] shrink-0" />
                )}
              </button>
            )
          })}

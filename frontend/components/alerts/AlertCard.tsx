...
function AlertAge({ createdAt, status }: { createdAt: string; status: string }) {
  ...
  return (
    <span className={`flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 rounded font-mono border ${cls}`}>
...
export function AlertCard({ alert, compact = false }: AlertCardProps) {
    ...
    return (
      <Link
        ...
        className="flex items-center gap-3 px-3 py-2.5 rounded transition-colors
                   hover:bg-[#19253d] group border-l-2"
  ...
  return (
    <Link
      href={`/alerts/${alert.id}`}
      className="block fc-card p-4 hover:border-[#1a6bff]/30 hover:bg-[#161f33] transition-all group
                 border-l-2"
      style={{ borderLeftColor: severityColor }}
    >
      {/* Header */}

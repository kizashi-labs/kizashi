...
const QUICK_NAV: Array<{ href: string; label: string; icon: React.ElementType; color: string }> = [
  ...
  { href: '/incidents', label: 'インシデント',   icon: Siren,           color: '#ff6b35' },
  { href: '/endpoints', label: 'エンドポイント', icon: Monitor,         color: '#00c853' },
  { href: '/rules',     label: '検知ルール',     icon: BookOpen,        color: '#7c3aed' },
]
...
const NAV_ITEMS: NavItem[] = [
  ...
  { href: '/alerts/triage',    label: 'トリアージ',          group: '検知',           keywords: ['triage', '振り分け', '仕分け'], icon: ShieldAlert },
  { href: '/incidents',        label: 'インシデント',        group: '検知',           keywords: ['incident', '事案', '事故'], icon: Siren },
  { href: '/rules',            label: '検知ルール',          group: '検知',           keywords: ['rule', 'sigma', 'ルール', 'detection rules'], icon: BookOpen },
  { href: '/suppressions',     label: 'アラート抑制',        group: '検知',           keywords: ['suppression', '抑制', '除外', 'ホワイトリスト', 'alert suppression'], icon: ShieldAlert },
  { href: '/endpoints',        label: 'エンドポイント',      group: 'エンドポイント', keywords: ['endpoint', 'agent', '端末', 'ホスト', 'agents'], icon: Monitor },
  { href: '/live-response',    label: 'ライブレスポンス',    group: 'エンドポイント', keywords: ['live response', 'remote shell', 'リモート', '端末操作'], icon: Monitor },
  { href: '/forensics',        label: 'フォレンジクス',      group: 'エンドポイント', keywords: ['forensics', '証拠', '調査', 'digital forensics'], icon: Monitor },
  { href: '/quarantine',       label: '検疫ファイル',        group: 'エンドポイント', keywords: ['quarantine', '検疫', '隔離ファイル'], icon: Archive },
  { href: '/software',         label: 'ソフトウェア管理',    group: 'エンドポイント', keywords: ['software', 'package', 'インベントリ'], icon: Monitor },
  { href: '/events',           label: 'イベントログ',        group: 'エンドポイント', keywords: ['event', 'log', 'イベント', 'ログ', 'events'], icon: FileText },
  { href: '/ioc',              label: 'IOC管理',             group: 'インテリジェンス', keywords: ['ioc', 'indicator', '侵害指標', 'hash', 'ip', 'ioc management'], icon: ShieldAlert },
  { href: '/threat-intel',     label: '脅威インテリジェンス', group: 'インテリジェンス', keywords: ['threat intel', 'ti', 'feed', 'フィード', 'threat intelligence'], icon: ShieldAlert },
  { href: '/threat-hunting',   label: 'スレットハンティング', group: 'インテリジェンス', keywords: ['hunt', 'hunting', 'ハンティング', '脅威探索', 'threat hunting'], icon: Crosshair },
  { href: '/network',          label: 'ネットワーク分析',    group: '監視',           keywords: ['network', 'ネットワーク', 'nta', 'network analysis'], icon: Monitor },
  { href: '/playbooks',        label: 'プレイブック',        group: '対応',           keywords: ['playbook', 'soar', '自動対応', 'auto-remediation'], icon: ShieldAlert },
  { href: '/vulnerabilities',  label: '脆弱性管理',          group: '対応',           keywords: ['vulnerability', 'cve', '脆弱性', 'パッチ', 'vulnerability management'], icon: ShieldAlert },
  { href: '/admin/yara-rules', label: 'YARAルール管理',      group: '管理',           keywords: ['yara', 'ヤラ', 'マルウェア検知', 'yara rules'], icon: BookOpen },
  { href: '/settings',         label: '設定',                group: '管理',           keywords: ['settings', 'config', '設定', 'コンフィグ'], icon: LayoutDashboard },
  { href: '/reports',          label: 'レポート',            group: '分析',           keywords: ['report', 'レポート', '帳票'], icon: LayoutDashboard },
  { href: '/admin/audit',      label: '監査ログ',            group: '管理',           keywords: ['audit', 'compliance', '監査', 'audit logs'], icon: FileText },
]
...
export function CommandPalette({ isOpen, onClose }: CommandPaletteProps) {
  ...
  return (
    ...
    >
      ...
      <div
        ref={containerRef}
        className="relative w-full max-w-2xl bg-[#0d1220] border border-[#1e2d42]
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-[#1e2d42]">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <kbd className="inline-flex items-center px-1.5 py-0.5 bg-[#161f33]
                            border border-[#1e2d42] rounded text-[10px] text-[#3d5068] font-mono">
              ESC
            </kbd>
          </div>
        </div>
          ...
          {query.length < 2 && !loading && (
            <div className="py-2">
              ...
              {recentAlerts.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 px-4 py-1.5 bg-[#161f33]/40
                                  border-b border-[#1e2d42]/50">
                    ...
                    return (
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-4 py-2.5 text-left
                                   hover:bg-[#19253d] transition-colors border-b
                                   border-[#1e2d42]/20 last:border-0 group"
                      >
                        <ShieldAlert
                        ...
                        <CornerDownLeft
                          className="w-3 h-3 text-[#3d5068] opacity-0 group-hover:opacity-100
              ...
              {recentSearches.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 px-4 py-1.5 bg-[#161f33]/40
                                  border-b border-[#1e2d42]/50 mt-1">
                    ...
                    {recentSearches.map(s => (
                      <div key={s} className="flex items-center group/pill">
                        <button
                          ...
                          className="text-[11px] px-2.5 py-1 rounded-l bg-[#161f33] border border-[#1e2d42]
                                     border-r-0 text-[#7d92b0] hover:text-[#e2e8f4] hover:border-[#3d5068]
                        ...
                        <button
                          ...
                          className="text-[11px] px-1.5 py-1 rounded-r bg-[#161f33] border border-[#1e2d42]
                                     text-[#3d5068] hover:text-[#e8002d] hover:border-[#3d5068] transition-colors"
                          title="削除"
                        >
                    ...
                    return (
                      <button
                        ...
                        className="flex flex-col items-center gap-1.5 px-3 py-3
                                   hover:bg-[#19253d] transition-colors group"
                      >
                        <Icon
                        ...
                        />
                        <span className="text-[10px] text-[#7d92b0] group-hover:text-[#e2e8f4]
                ...
                return (
                  <div key={kind}>
                    ...
                    <div className="flex items-center gap-2 px-4 py-1.5 bg-[#161f33]/50
                                    border-b border-[#1e2d42]/50 sticky top-0 z-10">
                      <GroupIcon
        ...
        {results.length > 0 && (
          <div className="flex items-center gap-4 px-4 py-2 border-t border-[#1e2d42]
            ...
            <span className="flex items-center gap-1">
              <kbd className="font-mono bg-[#1e2d42] px-1 rounded">↑↓</kbd>
              移動
            </span>
            <span className="flex items-center gap-1">
              <kbd className="font-mono bg-[#1e2d42] px-1 rounded">↵</kbd>
            ...
            <span className="flex items-center gap-1">
              <kbd className="font-mono bg-[#1e2d42] px-1 rounded">Esc</kbd>
              閉じる
            </span>
...
function ResultRow({ result, kind, meta, isSelected, onHover, onClick }: ResultRowProps) {
  ...
  return (
    ...
    >
      {/* Icon */}
      <GroupIcon className="w-4 h-4 flex-shrink-0" style={{ color: iconColor }} />
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        {result.secondary && kind === 'rule' && (
          <span className="text-[10px] text-[#7c3aed] bg-[#7c3aed]/10 px-1.5 py-0.5 rounded font-mono">
            {result.secondary}
          </span>
        )}
        {result.secondary && kind === 'ioc' && (
          <span className="text-[10px] text-[#f59e0b] bg-[#f59e0b]/10 px-1.5 py-0.5 rounded font-mono uppercase">
            {result.secondary}
          </span>
        )}
        {/* Enter hint */}

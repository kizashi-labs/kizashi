...
> **状態**: **4 表のうち 2 表 (agents / alerts) を fail-closed 化済み。**
> 方式はロール分割から「名乗り」に変えた（下の「7. 方式の変更」）。
> 残るのは incidents / users の 2 表。
...
### agents (451) と alerts (453) を落としました

4 表のうち 2 表が済みました。残る 2 表 (`incidents` / `users`) の
抜け道はそのままです。

`agents` を先にしたのは、**検知パイプラインが最も強く依存する表**だから
です。壊れるならいちばん早く、いちばん大きく出ます。`alerts` を 2 番目に
したのは、**系が絶えず書き込む表**だからです —— 読みが通ることを
`agents` で確かめたあと、書きがいちばん重いところで確かめます。
...
落とす前に木全体を `-count=1` で走らせた結果、**落ちたのは台帳 2 件だけ**
でした。本番の経路は 1 つも落ちていません。
...
### 残る 2 表 (incidents / users) を落とすときの手順
...
追えていません。1 表ずつ落とすのは、そこを実運転で埋めるためです。

### 分かっている挙動の変化

`agents` と `alerts` について、**テナントを持たない APIキー**は 0 行を見るように

...
> **状態**: **4 表のうち 3 表 (agents / alerts / incidents) を
> fail-closed 化済み。** 方式はロール分割から「名乗り」に変えた
> （下の「7. 方式の変更」）。残るのは users だけ。
...
### agents (451) / alerts (453) / incidents (454) を落としました

4 表のうち 3 表が済みました。残るのは `users` だけです。
...
台帳 2 件、alerts と incidents で 1 件ずつ**でした。本番の経路は 1 つも
落ちていません。
...
落とした 3 表について、**テナントを持たない APIキー**は 0 行を見るように
